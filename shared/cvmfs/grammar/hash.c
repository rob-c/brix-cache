...
/* Shared emitter for the two public hex renderings: `slash` inserts the CAS
 * fan-out '/' after the first byte (object paths); plain hex omits it. */
static int hash_render(const cvmfs_hash_t *h, char suffix, int slash,
                       char *buf, size_t buflen) {
    static const char hx[] = "0123456789abcdef";
    size_t need = h->len * 2 + 1 + (slash ? 1u : 0u) + (suffix ? 1u : 0u);
...
int cvmfs_hash_to_object_path(const cvmfs_hash_t *h, char suffix, char *buf, size_t buflen) {
    return hash_render(h, suffix, 1, buf, buflen);
}

int cvmfs_hash_to_hex(const cvmfs_hash_t *h, char suffix, char *buf, size_t buflen) {
    return hash_render(h, suffix, 0, buf, buflen);
}
