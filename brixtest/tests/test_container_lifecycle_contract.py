...
def test_declared_shutdown_runs_inside_the_owned_container(tmp_path, runtime):
...
def test_container_shutdown_rejects_unresolved_template_values(tmp_path):
    old_which = launchers_module.shutil.which
    launchers_module.shutil.which = lambda name: "/usr/bin/" + name
    try:
...
def test_oci_group_member_executes_inside_one_owned_container(tmp_path):
