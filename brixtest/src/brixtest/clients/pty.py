# ruff: noqa: E402
"""Pipe and PTY command runners — promoted to core (TS-5, §7.2).

Verbatim from the grown ``tests/cli_pty.py``.  It promotes because it is
generic by construction: pure stdlib, no settings, no fleet, no XRootD.  Any
project driving a CLI that changes behaviour under ``isatty(2)`` needs exactly
this pair.

The module is deliberately NOT named ``cli_pty`` here — core names its modules
for what they are — but note the consequence and why it is safe: this file
does ``import pty``, and under Python 3's absolute imports that is the stdlib
``pty``, never its sibling.  It would only resolve here if this *directory*
were placed on ``sys.path`` directly, which nothing does; the one script this
package spawns lives in another package entirely.  A test pins it.
"""
