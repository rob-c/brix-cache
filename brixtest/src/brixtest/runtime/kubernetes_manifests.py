...
def _kubernetes_volumes(
    ...
    volume_mounts = [{"name": "config", "mountPath": "/brixtest/config", "readOnly": True}]
...
def _kubernetes_resources(server: Server, container: dict) -> None:
    ...
    _apply_pid_limit(container, limits.pids)
...
def _apply_pid_limit(container: dict, value: object) -> None:
    if value is not None:
        container.setdefault("env", []).append({
            "name": "BRIXTEST_PIDS_LIMIT", "value": str(value),
        })
...
def _kubernetes_probe(server: Server, ports: Mapping[str, int]) -> dict:
    probe = server.probe
    timing = {
    ...
    if probe.kind == "exec":
        return {"exec": {"command": [str(part) for part in probe.command]}, **timing}
    return {}
...
def server_resources(
    ...
    peers: Mapping[str, tuple[Server, Mapping[str, int]]] | None = None,
    ...
    container = _server_container(server, command, env, container_ports, volume_mounts, ports)
    pod_spec = _pod_spec(server, container, volumes, host_aliases)
    apply_identity(pod_spec, container, identity)
    deployment = _deployment(
        resource_name, namespace, labels, pod_spec, replicas=server.replicas,
    )
    service = {
    ...
    policies = network_policy_resources(
        server, namespace, ports, {} if peers is None else peers,
    ) if render_network_policy else ()
    return config_map, *claims, *identities, *policies, deployment, service
...
def _server_container(
    ...
    container = {
        "name": "server", "image": _image(server), "imagePullPolicy": "IfNotPresent",
    ...
    readiness = _kubernetes_probe(server, ports)
    if readiness:
...
def _pod_spec(
    server: Server, container: dict, volumes: Sequence[dict],
    ...
    if host_aliases:
        pod_spec["hostAliases"] = [
            {"ip": item.address, "hostnames": list(item.hostnames)} for item in host_aliases
        ]
    return pod_spec
