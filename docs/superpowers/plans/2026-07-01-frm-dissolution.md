...
> - **NOT blockers:** `sd_frm.{c,h}`/`vfs_backend_registry.c`/`tier_build.c` match only `xrootd_sd_frm_create` (the NEW backend); `vfs.h` `frm_residency_probe` is a comment; the `FRM_METRIC` defs live in `src/observability/metrics/` (rename later, don't block deleting `src/frm/`).
>
