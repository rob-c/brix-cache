...
**Files:** `src/types/config.h` (auth-mode enum value if needed), `src/proxy/directives.c` (`xrootd_conf_set_proxy_auth` accepts `gsi`), `src/stream/module.c` (already has `xrootd_tap_proxy_auth`).
