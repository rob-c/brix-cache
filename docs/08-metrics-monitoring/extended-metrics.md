...
```text
  ...
  A fixed SHM table with LRU eviction keeps the series count flat.
   ...
   └────────────┴──────────┘  +1 overflow └────────────┴──────────┘
                              brix_vo_overflow_total ▲ alert: >32 active VOs
...
rate(brix_vo_bytes_tx_total[1m])

# Per-VO request rate
rate(brix_vo_requests_total[1m])
...
rate(brix_unique_users_total[1h]) - rate(brix_unique_users_current[1h])

# Most active users by session count
sort_desc(brix_user_sessions_total[0]) limit 20

# Ratio of unique users to concurrent connections (engagement metric)
brix_unique_users_current / brix_connections_active
```
...
|---|---|---|
| Per-VO counters | `BRIX_VO_MAX_TRACKED` (32 slots) | LRU — oldest slot evicted when full |
| Unique users | `BRIX_USERS_MAX_TRACKED` (1024 slots) | LRU — oldest slot evicted when full |
