...
rate(brix_connections_total[5m])

# Live connections (should be stable, not monotonically growing)
brix_connections_active

# Auth failures as a fraction of logins
rate(brix_requests_total{op="auth",status="error"}[5m])
  / rate(brix_requests_total{op="login",status="ok"}[5m])
```

**What to watch:**

- `brix_connections_active` growing without bound means connections are not being closed. Caused by client-side `xrdcp` hangs, long-running jobs holding file handles, or upstream redirect loops that keep secondaries alive.
- A spike in `brix_connections_total` rate with no corresponding increase in `brix_requests_total{op="open_rd"}` rate means clients are connecting and immediately disconnecting — usually an auth failure or a misconfigured client that cannot finish the login sequence.
- `brix_requests_total{op="auth",status="error"}` increasing is the first signal of an expired CA bundle, a rotated CRL that invalidates a VO's credentials, or a JWKS fetch failure for token auth.
...
rate(brix_requests_total{op="readv"}[5m])
  / rate(brix_requests_total{op=~"read|readv|pgread"}[5m])

# Paged read fraction (data-integrity path)
rate(brix_requests_total{op="pgread"}[5m])
  / rate(brix_requests_total{op=~"read|readv|pgread"}[5m])
```

**Interpretation:**

- A high `readv` fraction (>50%) is normal for ROOT TTree analysis jobs, which use vector reads to fetch multiple branches per event in a single round trip. Each `kXR_readv` request counts as one entry in `brix_requests_total` but transfers the same data as several `kXR_read` calls.
...
rate(brix_requests_total{op="open_rd",status="error"}[5m])

# Read errors (I/O errors on already-open handles)
rate(brix_requests_total{op="read",status="error"}[5m])

# Write errors
rate(brix_requests_total{op="write",status="error"}[5m])
```
...
- `write` errors combined with low `sync` counts can indicate a disk-full condition: writes appear to succeed (buffered), but `fsync` fails. Check `brix_cache_occupancy_ratio` and filesystem free space on the data volume.
...
rate(brix_cache_evictions_total[5m])

# Rate of bytes being reclaimed
rate(brix_cache_evicted_bytes_total[5m])

# Eviction failures (scan/unlink errors)
rate(brix_cache_eviction_errors_total[5m])
```

**Normal behavior:** `brix_cache_occupancy_ratio` should oscillate below `brix_cache_eviction_threshold_ratio`. Evictions fire when occupancy crosses the threshold; afterwards occupancy drops and evictions stop. Continuous evictions mean the cache is too small for the working set or the fill rate exceeds the eviction rate.
...
- `brix_cache_occupancy_ratio` at or above the threshold with `brix_cache_evictions_total` not increasing means eviction is failing. Check `brix_cache_eviction_errors_total` — filesystem permission errors, full inodes, or a stale mount point can stall eviction silently.
- `brix_cache_eviction_errors_total` non-zero: the eviction scanner failed to stat or unlink a candidate file. One or two errors on startup are normal. A continuously growing count means the cache directory has entries the server cannot remove.
...
rate(brix_webdav_responses_total{status_class="4xx"}[5m])
rate(brix_webdav_responses_total{status_class="5xx"}[5m])

# Auth outcome distribution
brix_webdav_auth_total{result="cert_ok"}
brix_webdav_auth_total{result="token_ok"}
brix_webdav_auth_total{result="anonymous_fallback"}
brix_webdav_auth_total{result="rejected"}

# PUT body handling — are uploads being spooled to disk?
brix_webdav_put_bodies_total{mode="spooled"}
brix_webdav_put_bodies_total{mode="threaded"}
```
...
rate(brix_webdav_tpc_total{event="pull_success"}[5m])
  / rate(brix_webdav_tpc_total{event="pull_started"}[5m])

# curl error rate
rate(brix_webdav_tpc_total{event="curl_error"}[5m])
```
...
brix_s3_auth_total{result="signature_mismatch"}
brix_s3_auth_total{result="bad_access_key"}
brix_s3_auth_total{result="bad_date"}

# S3 4xx and 5xx rates
rate(brix_s3_responses_total{status_class="4xx"}[5m])
rate(brix_s3_responses_total{status_class="5xx"}[5m])

# ListObjects pagination
rate(brix_s3_list_truncated_total[5m])
```
...
- `brix_s3_events_total{event="invalid_uri"}` non-zero means S3 clients are sending requests the module cannot route: unknown bucket, malformed path, or a path that resolves outside the configured export root.
- High `brix_s3_list_truncated_total` rate means clients are paginating through large directories. This is not an error, but it indicates a directory listing workload that may be expensive; each continuation is a separate directory scan.
...
rate(brix_stream_response_write_stalls_total[1m])

# Send error rate
rate(brix_stream_response_write_errors_total[1m])

# Oversized payload rejections
rate(brix_stream_oversized_payloads_total[1m])
```
  ...
  - name: xrootd
    rules:
      - alert: XRootDHighReadErrors
        expr: |
          rate(brix_requests_total{op="read",status="error"}[5m]) > 0.1
        for: 5m
        annotations:
      ...
      - alert: XRootDAuthFailures
        expr: |
          rate(brix_requests_total{op="auth",status="error"}[5m]) > 0.05
        for: 5m
        annotations:
      ...
      - alert: XRootDCacheNearFull
        expr: |
          brix_cache_occupancy_ratio
            > brix_cache_eviction_threshold_ratio * 1.05
        for: 5m
        annotations:
      ...
      - alert: XRootDEvictionErrors
        expr: |
          rate(brix_cache_eviction_errors_total[5m]) > 0.01
        for: 5m
        annotations:
      ...
      - alert: XRootDWebDAVRejected
        expr: |
          rate(brix_webdav_auth_total{result="rejected"}[5m]) > 0.1
        for: 5m
        annotations:
