...
Current BriX-Cache advertisement (lines 112–114 of `src/protocols/root/session/protocol.c`):
...
`kXR_ecRedir = 0x00004000` indicates the server can redirect to erasure-coded storage segments. This requires an erasure-coding storage backend (Reed-Solomon stripe layout, parity servers, reconstruction protocol). BriX-Cache is POSIX-backed only. This flag must not be set and is explicitly out of scope.
