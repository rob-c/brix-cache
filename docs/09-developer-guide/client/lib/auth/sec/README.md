...
The files described here remain in [`client/lib/auth/sec/`](../../../../../../client/lib/auth/sec/).
...
| `sec_krb5.c` | `krb5` (Kerberos 5) | 1–2 | AP-REQ from the default credential cache (a `kinit` TGT), libXrdSec-free. Round 2 (only when the server runs `brix_krb5_delegate on` and answers `"fwdtgt"`): forwards the TGT via `krb5_fwd_tgt_creds()` for inbound delegation — needs a forwardable ticket (`kinit -f`), else fails closed. |
...
- `docs/09-developer-guide/client/lib/README.md` — the `libbrix` library overview.
