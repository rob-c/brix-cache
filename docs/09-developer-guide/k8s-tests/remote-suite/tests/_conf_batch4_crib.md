...
- OpenFlags: `XrdCl/XrdClFileSystem.hh:74` (New=kXR_new, Delete=kXR_delete,
...
- Wire enums/error codes: `XProtocol/XProtocol.hh` (kXR_* opcodes,
...
- StatInfo `id` (chunks[0]): ours emits a small number (inode only?), stock emits a
  ...
  `XrdXrootd/XrdXrootdXeq.cc` for the exact id formula. gfal mostly
