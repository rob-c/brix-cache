...
| Wire-length validation | Request parsers validate `dlen`, use divisibility checks (`clone.c`, `readv.c`) and running-total caps (`readv.c` → `XROOTD_MAX_READV_TOTAL`) |
| Overflow-checked size math | Helper `src/core/compat/safe_size.h` already existed (`xrootd_size_mul`/`_add`, `xrootd_*_array`) |
...
- `src/protocols/root/zip/zip_dir.c` — central-directory (`cd_size`) and member (`comp_size`) buffers from an untrusted archive; `+1` sentinel doubles as a walk-past-end guard. Returns the function's existing `XROOTD_ZIP_ECORRUPT` / `-1`.
- `src/auth/token/jwks.c` — JWKS file-load `fsize + 1` (defense-in-depth; see finding 6.4).
- `src/auth/gsi/proxy_req.c` — the single wire-driven site in `xrootd_gsi_assemble_proxy` (`proxy_pem_len` from the client-sent `kXGC_sigpxy` PEM). Enumerated all 5 GSI allocations; the other 4 are OpenSSL `i2d_*`/`BIO_get_mem_data` egress lengths and were correctly left unconverted.
...
- `tests/build_sanitizer.sh` + `tests/test_sanitizer_smoke.py` — ASan+UBSan build (module **and** client) and a **genuine** MD5-verified `xrdcp` round-trip through a `SANITIZE=1` fleet, asserting zero `asan.*` reports. Skip-gated behind `XROOTD_SANITIZER_LANE=1`.
...
- **Phase A:** `readelf` on `client/bin/xrdcp` and `.../ngx_stream_xrootd_module.so` (PIE/RELRO/NOW/noexec-stack confirmed on the real binaries by the final reviewer).
...
XROOTD_SANITIZER_LANE=1 PYTHONPATH=tests pytest tests/test_sanitizer_smoke.py -v
...
*Adding a new fuzz target: write `tests/fuzz/fuzz_<x>.c`, add a `build_fuzz_<x>()` function + a `TARGETS` entry in `run_all.sh`. Adopting `safe_size.h` at a new site: `#include "../shared/safe_size.h"`, guard the wire-driven size with `xrootd_size_add`/`xrootd_size_mul`, return the enclosing function's existing error code. Convert ingress sites only.*
