...
All three protocol config structs embed `ngx_http_xrootd_shared_conf_t` at the top:
...
`xrootd_webdav_voms_cert_dir /etc/grid-security/certificates;` to a WebDAV
...
`xrootd_staged_open()`, body write, then `xrootd_staged_commit()` /
`xrootd_staged_abort()` for atomic rename with cleanup on failure. Both
...
Configuration: add `xrootd_webdav_cache_root /srv/cache;` or
`xrootd_s3_cache_root /srv/cache;` to the respective location block.
...
`http_access.log` using `proto=$xrootd_protocol` while preserving the older
...
**Current state:** Each protocol has its own config struct embedding `ngx_http_xrootd_shared_conf_t`:
- Stream: `ngx_stream_xrootd_srv_conf_t` (server-level) — includes VO rules, group rules, CMS map, proxy config
...
- Stream calls `ngx_http_xrootd_shared_init()` in server-conf creation and
  `ngx_http_xrootd_shared_merge_with_root()` with root default `/`.
- WebDAV/XrdHttp calls the same shared init/merge path with root default `/`.
- S3 calls `ngx_http_xrootd_shared_init()` and `ngx_http_xrootd_shared_merge()`
  with the empty-root default expected by `xrootd_s3_root`.
