...
The v2 harness adds a fifth prong: it scans the TSan log directory but fails **only** on data races whose stack frames name module/AIO code (`src/core/aio|read|write|cache|session|connection`, `_aio_thread`, `read_scratch`, `payload_to_free`, `ctx->destroyed`, `brix_*`), so suppressed benign atomic races in nginx core do not fail the run.
...
|---|---|---|---|
| `fhandle[0]` ∈ {15,16,17,64,200,255} on read/pgread/close/stat/truncate/write/pgwrite | OOB index past the 16-slot `files[]` table | `idx = (int)(unsigned char)fhandle[0]` cast blocks sign-extension; validators reject `idx<0 || idx>=BRIX_MAX_FILES(16)` and `fd<0` → `kXR_FileNotOpen` | `src/protocols/root/read/read.c:101`; `src/protocols/root/connection/fd_table.c:347-352`, `:361-389` |
| read `off=-1`, `off=0x7FFFFFFFFFFFFFFF`, `rlen=0x7FFFFFFF` | Negative/overflow offset into `pread`/`sendfile` | `if (offset < 0)` → `kXR_IOError "negative read offset"`; `rlen` capped at `BRIX_READ_REQUEST_MAX` | `src/protocols/root/read/read.c:121-125` |
| pgread unaligned `off=1 rlen=4097`, `off=-7` | Page-math overflow / negative offset | Handle validated, negative offset → `kXR_IOError`; page boundaries re-derived from file offset | `src/protocols/root/read/pgread.c:115-140` |
| readv `dlen=17` (not mult 16), zero-length, 1025 segs, OOB-handle seg, per-seg `rlen=0x7FFFFFFF`, neg offset | OOB read of payload buffer; segment-count overflow | `dlen%16==0` and `1..MAXSEGS(1024)`; overflow-checked `brix_size_mul`; per-seg negative-offset and `offset+len` overflow rejected | `src/protocols/root/read/readv.c:211-228`, `:86-93`, `:107-121` |
| pgwrite CRC `0xDEADBEEF`, `dlen=4`, unaligned `off=4095` + 10-byte body | Bad-CRC write; short-payload over-read | Reject `payload_len <= CKSZ`; per-page CRC32c verify → `NGX_DECLINED`; `INT64_MAX` offset-overflow guard | `src/protocols/root/write/pgwrite.c:115-117`, `:148-156` |
| writev declared `wlen 0x7FFFFFFF` but short payload | OOB read when declared wlen ≠ real payload | Recover N by scanning until `n*16+Σwlen==dlen`; reject overshoot/undershoot before reading data → `kXR_ArgInvalid` | `src/protocols/root/write/writev.c:79-104` |
| clone neg / 2^63 offsets | OOB / overflow in copy | Validate dst+payload-presence+length-divisibility+item-count; bad offsets fail `copy_file_range`/`pread` as `EINVAL` | `src/protocols/root/read/clone.c:50-74` |
| fattr `numattr=255`, `subcode=99`, truncated nvec | CPU/memory blowup; over-read into fixed `attrs[16]` | `subcode<=kXR_fattrMaxSC`; `numattr` ∈ `1..kXR_faMaxVars(16)` (list must be 0); `fattr_parse_nvec` stops at buffer end → `-1` → `kXR_ArgInvalid` | `src/protocols/root/fattr/dispatch.c:48-62`; `src/protocols/root/fattr/helpers.c:73-103` |
| query bad subcode 999 + `/../`×64 path | Mis-dispatch on unknown infotype | Unknown infotype → `kXR_Unsupported` | `src/protocols/root/query/dispatch.c:80-84` |
| open `/../`×80`etc/passwd`, `/big\x00.bin`, 9000-char path | Path traversal; embedded-NUL truncation | `brix_extract_path` rejects `>MAX_PATH`, embedded-NUL-not-at-end; `openat2(RESOLVE_BENEATH\|RESOLVE_NO_MAGICLINKS)` makes `..`/symlink escape a kernel `EXDEV` → `kXR_NotAuthorized` | `src/protocols/root/path/extract.c:19-55`; `src/fs/path/beneath.c:73-93` |
| lying `dlen` — read `0x40000000`, write `0x7FFFFFFF`, stat `0x10000000` | Huge-allocation DoS / integer overflow | Per-opcode cap via `brix_max_payload_for_request` checked **before** any allocation; over-cap closes the connection; `dlen+1` overflow guard | `src/protocols/root/connection/recv.c:21-46`, `:360-368`, `:62-65` |
| login NUL/non-printable username | NUL-truncation impersonation (`a\x00evil` → user `a`) | Reject any byte `< 0x20 \|\| > 0x7e` → `kXR_ArgInvalid` | `src/protocols/root/session/login.c:85-92` |
| reserved/unknown opcodes 2999/3005/3099/4099/65535/0 | Unknown-opcode mis-dispatch | Unknown opcode → `kXR_Unsupported` | `src/fs/path/beneath.c` funnel + dispatcher |
...
1. **Worker halves touch only the task struct.** `brix_read_aio_thread` (and the pgread/readv/write worker halves) snapshot `fd`/`offset`/`len`/`databuf` into the task and never dereference `ctx`/`c`/`pool`. The contract is documented at `src/core/aio/reads.c:228-241` and the write worker at `src/core/aio/write.c:11-25`.
...
3. **The `ctx->destroyed` guard defers teardown.** `brix_on_disconnect` sets `ctx->destroyed = 1` first (`src/protocols/root/connection/disconnect.c:281`), then frees the raw-heap scratch/payload buffers (`:22-81`). Every AIO done callback first calls `brix_aio_restore_stream`/`_request` (`src/core/aio/resume.c:23-31`, `:41-52`), which returns 0 when `destroyed` and aborts before touching `ctx`/`c`. The pgread completion guard is `src/core/aio/reads.c:407`; the read completion guard is `:266-268`; `brix_aio_resume` re-checks `ctx == NULL || ctx->destroyed` at `:100-117`.
4. **The detached write payload is freed exactly once.** `brix_write_aio_done` frees the detached heap copy **unconditionally before** the guard (`src/core/aio/write.c:40-61`; writev at `:228-244`), and `brix_handle_write` nulls `ctx->payload`/`payload_buf` after posting so disconnect's free cannot race it (`src/protocols/root/write/write.c:116-122`).
5. **The recv loop never reads/dispatches/disconnects while an AIO is in flight.** The disconnect-detection branch routes `n==0`/`NGX_ERROR` through `brix_on_disconnect` before finalize (`src/protocols/root/connection/recv.c:286-293`), but the `XRD_ST_AIO` recv guard (below) ensures that branch is not reached while a worker is running.
...
**Exploit hypothesis:** the pipelined pgread executes against a handle/buffers already freed by endsess teardown; or `brix_on_disconnect` runs twice (once from endsess, once from the RST) and double-frees.

**Defense.** `brix_handle_endsess` does full teardown synchronously: it calls `brix_on_disconnect` (sets `destroyed=1`, frees buffers, releases budget, unregisters the session), then `brix_close_all_files` (each `brix_free_fhandle` `close()`s the fd and resets the slot to `fd=-1`), then clears `ctx->logged_in=0` and `ctx->auth_done=0` (`src/protocols/root/session/lifecycle.c:132-145`). The pipelined pgread then hits two independent gates: `brix_dispatch_require_auth` rejects it with `kXR_NotAuthorized` because the auth flags are now 0 (`src/protocols/root/handshake/policy.c:54-62`), and even if reached, `brix_validate_read_handle` finds `fd<0` → `kXR_FileNotOpen` (`src/protocols/root/connection/fd_table.c:361-376`). The disconnect path is idempotent (buffer frees null their pointers; `brix_free_fhandle` is bounds-guarded and resets state — `src/protocols/root/connection/fd_table.c:233-323`). The Phase-29 drain barrier defers any non-read opcode while the output queue is non-empty (`src/protocols/root/connection/recv.c:391-396`), and the deferred path is itself re-dispatched only when quiescent (`:150-180`).

### Phase D — Resource exhaustion (shed, don't crash)

Three sub-attacks: (1) one session issues 64 `kXR_open /big.bin` calls (far past the 16-slot table); (2) 24 concurrent threads each fire 20 `kXR_pgread` of 48MiB against the 8m `brix_memory_budget`, then RST; (3) 300 rapid connect+RST cycles to churn the session registry.

| Sub-attack | Hypothesis | Defense | Location |
|---|---|---|---|
| 64-open flood | OOB write past `files[16]` | `brix_alloc_fhandle` scans the fixed 16-slot table and returns `-1` when full; open handler maps `-1` to an error reply (no 17th slot written) | `src/protocols/root/connection/fd_table.c:12-28` |
| 24×20× 48MiB pgread | Unbounded heap growth → OOM | `brix_budget_admit` returns 0 when others' in-use heap + request would exceed the budget → `kXR_wait` + `budget_waits_total`, no allocation; `rlen` capped at `BRIX_READ_REQUEST_MAX(64MiB)`; readv total capped at 256MiB | `src/protocols/root/connection/budget.h:100-120`; `src/protocols/root/read/pgread.c:153-184`; `src/protocols/root/read/readv.c:263-290`; `src/protocols/root/read/read.c:301-303` |
| connect/RST storm | Registry slot leak / churn corruption | `brix_on_disconnect` releases budget, concurrency slot, dashboard slots, and (for authed non-bound sessions) the registry slot exactly once; scratch/payload freed and nulled | `src/protocols/root/connection/disconnect.c:295-338`, `:43-57` |
...
**Defense.** Scratch buffers are per-session and reset/trimmed between requests (`out_count==0` trim gate at `src/protocols/root/connection/recv.c` top-of-request); the windowed/sendfile builders clamp to the cached file size (`src/protocols/root/read/read.c:148-233`, `:457-470`); the AIO thread/done split keeps protocol-state mutation on the event loop (`src/core/aio/reads.c:228-241`, `:266`); pgread re-derives page boundaries and recomputes CRC32c per page via `brix_pgread_encode_pages` (`src/protocols/root/read/pgread.c:51-94`, `:96-152`). Because earlier phases were all rejected/serialized without corrupting `ctx`, this read sees a clean session.
...
**Defense.** A bound secondary owns no independent handle: on every read it re-derives the handle from the primary's SHM-published table. `brix_ensure_read_handle` re-checks the published slot under the handle mutex on each request via `brix_session_handle_lookup_hint` (`src/protocols/root/connection/fd_table.c:184`, `:203`, `:198-226`). If the primary closed/reused the slot, the lookup misses and the secondary's local fd is freed and the read is `NGX_DECLINED` (revocation). If still published but stale, the handle is reopened through the same confined opener and the fresh `fstat` `st_dev`/`st_ino` is compared against the published device/inode — `brix_local_file_matches_shared_handle` (`src/protocols/root/connection/fd_table.c:66`, `:76`) and the reopen guard `if (st.st_dev != shared->device || st.st_ino != shared->inode) { close(fd); return NGX_DECLINED; }` (`:144`). An inode swap fails this check and never serves swapped content. Read buffers are per-connection scratch owned by that connection's `ctx` (freed only at its own disconnect/AIO-done), so the primary's close cannot free the secondary worker's in-flight buffer; the secondary's pgread done callback re-validates via the `ctx->destroyed`/streamid guard (`src/core/aio/reads.c:407`).
...
**Defense.** Each secondary is a separate connection with its own `ctx`, slot hint, and AIO task; all five independently re-validate the published slot under the single handle mutex on their next read. The unpublish on primary close (`brix_session_handle_unpublish`, `src/protocols/root/session/handles.c:284`) takes the same mutex, so the clear is atomic w.r.t. each lookup; the full key (`in_use` + sessid + `handle_index`) is re-checked every read via `brix_shared_handle_same_key` (`src/protocols/root/session/handles.c:71`). A secondary that already passed lookup reads from its own confined fd/buffer (no shared mutable state) and its done callback is gated by streamid/`ctx->destroyed`. There is no shared per-handle refcount to corrupt — revocation is purely SHM-slot visibility under a mutex, so N concurrent revocations serialize correctly. The local-fd revocation logic is `src/protocols/root/connection/fd_table.c:198-226`.
...
**Finding (honest statement of the trust model).** The 16-byte sessid is a **bearer token**, not a capability secret. It is built at connection setup from `{time, pid, (uintptr_t)c, ngx_random()}` (`src/protocols/root/connection/handler.c:82`) and is explicitly **not** a CSPRNG value — its purpose is uniqueness within a process, not unguessability. Consequences:
...
- **Blind online forgery is infeasible.** `kXR_bind` is hard-gated on an exact-match registry lookup: `brix_handle_bind` calls `brix_session_lookup(req->sessid)` and on miss returns `kXR_NotAuthorized "bind sessid not recognised"` **before** inheriting any state (`src/protocols/root/session/bind.c:78`, `:86`). The lookup is a 16-byte `ngx_memcmp` against in-use registry slots (`src/protocols/root/session/registry.c:293`, `:315`). A sessid is only inserted into the registry at the point a real session is permitted file I/O (anonymous login or successful `kXR_auth`), so an attacker must possess a sessid that a real session published. The asserted `0/64` random-sessid acceptances confirm the gate; the test treats this as the documented trust boundary, not as a cryptographic guarantee.
...
This is the deterministic version of v1 Phase B. Same defenses, now exercised with the worker reliably parked inside the syscall: the two-phase task split (`src/core/aio/readv.c:93`, `src/core/aio/write.c:47`, `:59`), the `ctx->destroyed` guard at completion (`src/core/aio/resume.c:23`, `:42`, `:115`), the unconditional detached-payload free before the guard, and the recv loop's `XRD_ST_AIO` branch that re-arms the read event and returns without reading/dispatching (`src/protocols/root/connection/recv.c:186`). `ctx->destroyed` is set under the event loop before the scratch/payload buffers are freed (`src/protocols/root/connection/disconnect.c:281`, `:43`), so the in-flight task's done callback no-ops via the guard, making the buffers safe to release.
...
**Defense.** Pipelining is restricted to `kXR_read` only, and only when the response is a single self-contained file-backed sendfile span (`resp_pipelinable && !rd_win_active && out_count < PIPELINE_MAX` — `src/protocols/root/connection/recv.c:436`). Any non-read opcode arriving while reads still drain sets `recv_deferred` and parks in `XRD_ST_SENDING` until the output queue fully drains (`src/protocols/root/connection/recv.c:233`, `:391`), so it never runs while a prior chain references the scratch. The scratch slots are **raw `ngx_alloc` heap** owned by `ctx` (not pool allocations) precisely so the earlier pool-churn UAF cannot recur (`src/core/aio/buffers.c:33-59`, alloc/grow at `:48`); `brix_release_read_buffer` is a no-op for the reusable scratch slots (`:82`); trim happens only at top-of-request with `out_count==0` (`src/protocols/root/connection/recv.c:252`). Each completion is still `ctx->destroyed`/streamid-gated, so the RST is handled like P3.
...
**Defense.** `dlen` is bounded per-opcode before any allocation (`src/protocols/root/connection/recv.c:360`) and the payload buffer always carries a NUL terminator (`:55`). Handles are 0-255 table slots validated in `fd_table` helpers, so a mismatched/embedded chkpoint fhandle resolves to an empty slot (`fd<0` → declined) — bounds check at `src/protocols/root/connection/fd_table.c:190`. `kXR_endsess` **ignores the wire sessid entirely** — it tears down via `brix_on_disconnect` + `brix_close_all_files` then clears `ctx->logged_in=0`/`auth_done=0` (`src/protocols/root/session/lifecycle.c:132`, `:142`), so the pipelined `kXR_read` is rejected by the auth gate; a client cannot keep operating after endsess, even with a forged id. The drain barrier defers the pipelined non-read after a parked response. Username validation remains in force at `src/protocols/root/session/login.c:85`.
...
**Defense.** Because each phase's defenses keep buffers connection-owned, handles inode-validated, and completions `ctx->destroyed`-gated, no cross-request or cross-connection state survives to corrupt a later clean read; the read path returns the file's true bytes (`src/core/aio/reads.c:317` data delivery + `:266` survival guard; framing decode at `src/protocols/root/connection/recv.c:343`; open/stat at `src/protocols/root/read/open_resolved_file.c:220`). The crash harness confirms liveness; this phase confirms correctness end-to-end.
...
|---|---|---|
| `XRD_ST_AIO` recv-loop teardown deferral | While an AIO task is in flight, recv re-arms the read event and returns without reading/dispatching/disconnecting — the connection is frozen until the done callback transitions state back | `src/protocols/root/connection/recv.c:165-170`, `:186-191`, `:413-418`, `:483-488` |
| `read_scratch`/`write_scratch` lifetime + `ctx->destroyed` guard | Scratch is raw `ngx_alloc` (not pool-backed), reused across requests, freed exactly once on disconnect after `destroyed=1`; resume helpers abort when destroyed | `src/core/aio/buffers.c:23-60`, `:75-89`; `src/protocols/root/connection/disconnect.c:43-75`, `:281`; `src/core/aio/resume.c:23-31`, `:115-117` |
| AIO offload policy | write/readv/pgread offload unconditionally; `kXR_read` probes the cache with `preadv2(RWF_NOWAIT)` and completes inline only on an exact warm hit, else falls through to the pool | `src/protocols/root/read/read.c:50`, `:344-423`; `src/core/aio/reads.c:365-387`; `src/protocols/root/write/write.c:105-123`; `src/core/aio/resume.c:67-87` |
| Kernel-enforced path confinement | `openat2(RESOLVE_BENEATH\|RESOLVE_NO_MAGICLINKS)` per-worker rootfd is the security boundary; `#error` guards require kernel ≥ 5.6; mutating `*at()` ops pre-resolve the parent | `src/fs/path/beneath.c:43-49`, `:73-93`, `:129-220`; `src/fs/path/resolve_confined_helpers.c:221-233`, `:286-288`; `src/fs/path/resolve_confined_ops.c:56-115` |
| Bounded file-handle table | Fixed `files[BRIX_MAX_FILES(16)]`; wire fhandle is one untrusted byte; three validators bound-check range + open + capability | `src/protocols/root/connection/fd_table.c:12-28`, `:343-355`, `:361-389`, `:395-409`, `:184-227` |
| `kXR_bind` sessid contract | Bearer-token sessid, registry-validated, identity inherited only from a registered entry; secondaries confined to read-only of published handles | `src/protocols/root/connection/handler.c:77-87`; `src/protocols/root/session/login.c:116-133`; `src/protocols/root/session/bind.c:78-88`, `:89-122`; `src/protocols/root/session/registry.c:203-290` |
| Wire-framing caps + overflow guards | Per-opcode `dlen` cap before allocation; readv segment caps (`SEGSIZE 16`, `MAXSEGS 1024`, total 256MiB); overflow-checked multiplies; pgwrite `INT64` offset guard | `src/protocols/root/connection/recv.c:21-46`, `:62-65`, `:360-368`; `src/protocols/root/read/readv.c:56-57`, `:84-95`, `:106-129`, `:211-272`; `src/protocols/root/write/pgwrite.c:153-154` |
| SHM-global memory budget + windowed reads | Cross-worker atomic caps aggregate scratch; over-budget reads defer with `kXR_wait`; large memory-backed reads stream in ~2MiB windows | `src/protocols/root/connection/budget.h:25-120`; `src/protocols/root/read/read.c:301-312`; `src/protocols/root/read/readv.c:288-299`; `src/core/aio/reads.c:52-103`, `:112-215`; `src/core/aio/buffers.c:111-142`; `src/protocols/root/connection/recv.c:252-258`; `src/core/types/tunables.h:55-57` |
...
```bash
# ASAN nginx in a separate build tree (BRIX_OPTIMIZE=none keeps the module's
# own -O3/LTO profile from fighting the sanitizer flags)
cp -a /path/to/nginx-src /path/to/nginx-asan && cd /path/to/nginx-asan
BRIX_OPTIMIZE=none ./configure --with-stream --with-stream_ssl_module \
