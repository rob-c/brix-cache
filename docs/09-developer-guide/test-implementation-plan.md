...
| 7c | GSI cert-ok path → stream auth_total | M — 1 h | 45 | `gsi_port`, valid proxy | `xrdcp` with valid proxy to gsi_port; assert `brix_auth_total{proto="stream",method="gsi",status="ok"}` delta ≥ 1. |
