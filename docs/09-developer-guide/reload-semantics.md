...
| **(b) Hot-reloaded by a timer** (no reload needed at all) | A per-worker mtime-polling timer reloads the file in place and atomically swaps it; a reload also re-reads it. | CRL (`brix_crl` + `brix_crl_reload`), JWKS (`brix_token_jwks` + refresh interval), XrdAcc authdb (`brix_authdb_engine xrdacc`) |
