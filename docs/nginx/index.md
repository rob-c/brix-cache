...
This document explains the nginx architecture decisions that shape how this
module is structured, and — critically — what nginx provides for free that any
standalone server (including the official XRootD daemon) must implement from
scratch.
...
- [Stream module: why root:// lives here](stream-module.md)
- [HTTP module: why davs:// lives here](http-module.md)
- [nginx internals: event loop, buffers, TLS, shared memory, config, hot reload](internals.md)
...
- [architecture.md](../architecture.md) — end-to-end request lifecycle for both modules
- [types.md](../types.md) — `xrootd_ctx_t` and config struct field reference
- [development.md](../development.md) — AIO pattern, build workflow, configure vs make
- [contributing.md](../contributing.md) — step-by-step guides for adding opcodes and WebDAV methods
- [comparison-with-xrootd.md](../comparison-with-xrootd.md) — developer-hours comparison and deployment decision matrix
- [optimizations.md](../optimizations.md) — sendfile, zero-copy, and buffer chain design decisions
