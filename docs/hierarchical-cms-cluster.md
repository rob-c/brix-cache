...
This document covers the XRootD CMS cluster protocol from first principles:
what it is, where it is used in HEP, what nginx-xrootd already implements,
what the remaining gap is, and a detailed implementation plan for completing
full hierarchical support.

See [cluster-mode.md](cluster-mode.md) for the existing two-tier configuration
...
All five phases documented in [cluster-mode.md](cluster-mode.md) are complete
and tested.
