...
| `brix_frm_max_inflight <N>` | `server` | `128` | Capacity of the stage request registry |
| `brix_frm_stage_ttl <time>` | `server` | `300s` | Expiry of a parked nearline open / prepare record |
...
| `brix_cache_lock_timeout <time>` | `server` | `600s` | No |
...
| `brix_cache_evict_at <pct>` | `location` | `90` | ⚠️ **DEPRECATED (phase-115)** — Removed; use `brix_cache_high_watermark` / `brix_cache_low_watermark` instead. Was: Eviction trigger for stream read cache (not wired on cvmfs plane). |
| `brix_cache_evict_to <pct>` | `location` | `80` | ⚠️ **DEPRECATED (phase-115)** — Removed; use `brix_cache_high_watermark` / `brix_cache_low_watermark` instead. Was: Eviction target (hysteresis partner). |
