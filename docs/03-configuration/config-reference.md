...
| `$brix_checksum` | `crc32c:<hex>`, or `-` | The page CRC brix computed for the served bytes, always algorithm-tagged so it is never misread as adler32/md5. Populated only when page-CRC was active (the xrootd `pgread` / integrity path); a plain WebDAV/S3 GET computes none, so the value is `-`. |
