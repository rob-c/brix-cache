...
| Goal | Where to look |
|---|---|
| TLS-encrypted `root://` (protect file data in transit) | [docs/03-configuration/tls-config.md](tls-config.md) — `brix_tls on` or `roots://` |
| WebDAV (`davs://`) over HTTPS | [docs/04-protocols/](../04-protocols/) |
| Token (JWT/WLCG bearer) authentication | [docs/06-authentication/auth-overview.md](../06-authentication/auth-overview.md) §Token |
| VO / FQAN ACLs with VOMS | [docs/06-authentication/auth-overview.md](../06-authentication/auth-overview.md) §VOMS, `brix_require_vo` |
| S3-compatible endpoint | [docs/03-configuration/directives.md](directives.md) `brix_s3` |
| Prometheus metrics | [docs/08-metrics-monitoring/monitoring-guide.md](../08-metrics-monitoring/monitoring-guide.md) |
| CRL checking | [docs/03-configuration/directives.md](directives.md) `brix_crl` |
| Production PKI (real IGTF/grid CA) | [docs/06-authentication/](../06-authentication/) |
