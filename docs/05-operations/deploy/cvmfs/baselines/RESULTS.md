...
the historical harness JSON. The former `run_matrix.sh` driver (T15) is now
`tests/cmdscripts/cvmfs_matrix.py`; manual edits belong only in the Notes column.
...
| stock-nginx  | site    | | | | | | | 2026-07-02 | SKIP: netem needs sudo; comparison-driver rerun pending |
...
the comparison driver owns the loss/reorder/jitter/site rows and the
squid/varnish baselines where installed). The corrupt-profile contrast is already decisive:
