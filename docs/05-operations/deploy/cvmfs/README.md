...
The files described here remain in [`deploy/cvmfs/`](../../../../deploy/cvmfs/).
...
The historical shell acceptance tests have been ported to Python. The
`minimal` scenario in `tests/cmdscripts/cvmfs_live.py` and the `evict`
scenario in `tests/cmdscripts/cvmfs_live_ext.py` use the mock Stratum-1 in
`tests/cvmfs/mock_stratum1.py`. The first checks read-through cache behavior;
the second checks hot/cold cache placement and occupancy-driven reclamation.
These drivers own their temporary server lifecycle. See the
[test-suite guide](../../../09-developer-guide/testing/README.md) for execution
and environment requirements.

The minimal three-directive configuration they exercise is:
