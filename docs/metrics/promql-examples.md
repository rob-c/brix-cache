...
This document provides concrete PromQL queries for common monitoring scenarios. Replace `[1m]`, `[5m]`, or `[3600]` with the desired aggregation window as needed (1-minute, 5-minute, and hourly windows shown respectively).
...
(
  sum(rate(xrootd_bytes_tx_ipv4_total[5m]))       # native XRootD IPv4 tx
+ sum(rate(xrootd_webdav_bytes_tx_ipv4_total[5m]))  # WebDAV IPv4 tx
) / 1024 / 1024

# Combined IPv6 throughput in MB/s — all protocols except S3
(
  sum(rate(xrootd_bytes_tx_ipv6_total[5m]))       # native XRootD IPv6 tx
+ sum(rate(xrootd_webdav_bytes_tx_ipv6_total[5m]))  # WebDAV IPv6 tx
) / 1024 / 1024
```
...
(
  sum(rate(xrootd_bytes_rx_ipv4_total[5m])) + sum(rate(xrootd_bytes_rx_ipv6_total[5m]))
+ sum(rate(xrootd_webdav_bytes_rx_ipv4_total[5m])) + sum(rate(xrootd_webdav_bytes_rx_ipv6_total[5m])))
) / 1024 / 1024
...
rate(xrootd_vo_bytes_tx_total{vo="cms"}[5m]) / 1024 / 1024

# Compare with aggregate native XRootD tx to see VO share of total
rate(xrootd_vo_bytes_tx_total{vo="cms"}[5m]) 
/ sum(rate(xrootd_bytes_root_tx_total[5m])) * 100
```

### VO request rate:
```promql
# Requests per second from CMS users (native XRootD only)
rate(xrootd_vo_requests_total{vo="cms"}[5m])
```

### Per-VO upload vs download ratio (native XRootD):
```promql
# For a single VO: upload (bytes_rx) vs download (bytes_tx) ratio
sum(rate(xrootd_bytes_rx_ipv4_total{port="1094",auth="gsi"}[5m])) + sum(rate(xrootd_bytes_rx_ipv6_total{port="1094",auth="gsi"}[5m])))
/ (
  sum(rate(xrootd_bytes_tx_ipv4_total{port="1094",auth="gsi"}[5m])) + sum(rate(xrootd_bytes_tx_ipv6_total{port="1094",auth="gsi"}[5m])))
)
```
...
xrootd_unique_users_current

# Total unique users ever seen since process start (never decreases)
xrootd_unique_users_total

# Ratio — what fraction of lifetime users are still active?
xrootd_unique_users_current / xrootd_unique_users_total * 100
```

### New users per hour:
```promql
# Approximate new unique users per hour (accounting for evictions)
increase(xrootd_unique_users_total[3600]) 
- increase(xrootd_user_evictions_total[3600])
```

### Most active users (top 20 by session count):
```promql
sort_desc(xrootd_user_sessions_total{hash!=""}) limit 20
```
...
rate(xrootd_bytes_tx_ipv4_total{port="1094",auth="gsi"}[5m]) / 1024 / 1024    # MB/s IPv4
rate(xrootd_bytes_tx_ipv6_total{port="1094",auth="gsi"}[5m]) / 1024 / 1024    # MB/s IPv6

# Sum both for total VO throughput on this port
(rate(xrootd_bytes_tx_ipv4_total{port="1094",auth="gsi"}[5m]) + rate(xrootd_bytes_tx_ipv6_total{port="1094",auth="gsi"}[5m])) / 1024 / 1024
```
...
```promql
sort_desc(
  rate(xrootd_vo_bytes_tx_total[5m]) 
/ vector(1) * 1024 / 1024
) limit 10
```
