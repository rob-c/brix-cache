...
This document provides guidance on interpreting key metric groups, what healthy values look like, and what changes signal a problem. Prometheus `rate()` and `irate()` over a scrape interval of 15s or 30s are assumed throughout.
