...
This page describes what the nginx-xrootd module currently implements compared
to the data-server surface of the official `xrootd` daemon and XRootD protocol
...
See [tls.md](tls.md) for configuration details.
...
| `kXR_QPrep` | 2 | ✅ Per-file availability status; `A <path>` (on disk) or `M <path>` (missing) per line. `kXR_prepare` with `kXR_stage` returns request ID `"0"` and stores the path list for subsequent polls. |
| `kXR_Qcksum` | 3 | ✅ adler32, md5, sha1, sha256 |
| `kXR_Qxattr` | 4 | ✅ Extended attributes (legacy path; prefer `kXR_fattr`) |
| `kXR_Qspace` | 5 | ✅ Filesystem free/total via `statvfs` |
| `kXR_Qckscan` | 6 | ✅ Checksum-cancel query; validates path and returns success because checksums are computed synchronously |
...
| `kYR_select` / `kYR_try` / `kYR_redirect` | ❌ | Manager-directed client redirect. Only relevant when this node acts as a sub-manager. Not planned for pure leaf deployments. |
...
See [cluster-mode.md](cluster-mode.md) for configuration examples and architecture.
...
See [webdav.md](webdav.md) for details.
...
lifecycle, part-number validation (1–10 000), and atomic assembly via
temp-file + rename.  Not implemented: virtual-hosted-style buckets, and full
AWS S3 policy/IAM semantics.
...
[comparison-with-xrootd.md](comparison-with-xrootd.md) for deployment-type
analysis.
...
| **Full hierarchical CMS cluster** — `kYR_select` / `kYR_try` / `kYR_redirect` (sub-manager-directed redirect) not implemented | Simple two-tier (redirector → leaf data servers) works; multi-tier sub-manager topologies do not |
