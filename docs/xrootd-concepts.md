...
This document explains the core ideas that underpin XRootD's design — the
protocol model, the data-transfer patterns, the security architecture, and the
cluster topology — at a level that is useful for anyone implementing or
operating an XRootD-compatible server.
