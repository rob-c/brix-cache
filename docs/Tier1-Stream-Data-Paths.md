...
|--------|------|-----|-------------|
| 3013 | kXR_read | 0x0BB5 | Read data from file |
| 3019 | kXR_write | 0x0BBD | Write data to file |
| 3017 | kXR_stat | 0x0BB9 | Get file status |
| 3003 | kXR_close | 0x0BA3 | Close file handle |
| 3011 | kXR_ping | 0x0BB3 | Keep-alive ping |
| 3004 | kXR_dirlist | 0x0BA4 | List directory contents |
| 3015 | kXR_open | 0x0BB7 | Open file (boundary case) |
| 3018 | kXR_statx | 0x0BBA | Batch stat multiple files (boundary case) |
...
#### Entry Point
`dispatch_read()` in `src/handshake/dispatch_read.c` handles opcode dispatch for read operations. The switch case for `kXR_read` calls the read handler with parsed parameters.
...
#### Error Handling
- ENOENT → kXR_NotFound (4007) → HTTP 404
- EACCES/EPERM → kXR_NotAuthorized (4007) → HTTP 403  
- EINVAL → kXR_ArgInvalid (4007) → HTTP 400
- EIO → kXR_IOError (4007) → HTTP 500
- ENOMEM → kXR_NoMemory (4007) → HTTP 507
...
#### Entry Point
`dispatch_read()` in `src/handshake/dispatch_read.c` handles opcode dispatch for write operations via the dispatch_write() function called from the switch case for `kXR_write`.
...
- ENOSPC → kXR_NoSpace (4007) → HTTP 507
- EROFS → kXR_IOError (4007) → HTTP 500
...
#### Entry Point
`dispatch_read()` in `src/handshake/dispatch_read.c` handles opcode dispatch for stat operations via the dispatch_read() function called from the switch case for `kXR_stat`.
...
#### Entry Point
`dispatch_read()` in `src/handshake/dispatch_read.c` handles opcode dispatch for close operations via the dispatch_read() function called from the switch case for `kXR_close`.
...
- EBADF → kXR_ArgInvalid (4007) → HTTP 400
- EIO → kXR_IOError (4007) → HTTP 500
...
#### Entry Point
`dispatch_read()` in `src/handshake/dispatch_read.c` handles opcode dispatch for ping operations via the dispatch_read() function called from the switch case for `kXR_ping`.
...
- ECONNRESET → kXR_IOError (4007) → HTTP 500
- ETIMEDOUT → kXR_IOError (4007) → HTTP 500
...
#### Entry Point
`dispatch_read()` in `src/handshake/dispatch_read.c` handles opcode dispatch for directory listing operations via the dispatch_read() function called from the switch case for `kXR_dirlist`.
...
### 7. kXR_open (3015) — Boundary Case

#### Entry Point
`dispatch_read()` in `src/handshake/dispatch_read.c` handles opcode dispatch for open operations via the dispatch_read() function called from the switch case for `kXR_open`. The handler is implemented in `src/read/open.c`.
...
Wire layout (big-endian):
- Bytes 0-1: Opcode (kXR_open = 3015)
- Byte 2: Handle (single byte, 0-255)
...
### 8. kXR_statx (3018) — Boundary Case

#### Entry Point
`dispatch_read()` in `src/handshake/dispatch_read.c` handles opcode dispatch for batch stat operations via the dispatch_read() function called from the switch case for `kXR_statx`. The handler is implemented in `src/read/statx.c`.
...
Wire layout (big-endian):
- Bytes 0-1: Opcode (kXR_statx = 3018)
- Byte 2: Handle (single byte, 0-255)
...
|-------|----------------|-------------|-------------|
| ENOENT | kXR_NotFound (4007) | 404 Not Found | File/directory not found |
| EACCES/EPERM | kXR_NotAuthorized (4007) | 403 Forbidden | Permission denied |
| EINVAL | kXR_ArgInvalid (4007) | 400 Bad Request | Invalid argument |
| EIO | kXR_IOError (4007) | 500 Internal Server Error | I/O error |
| ENOMEM | kXR_NoMemory (4007) | 507 Insufficient Storage | Out of memory |
| ENOSPC | kXR_NoSpace (4007) | 507 Insufficient Storage | No space left on device |
| EROFS | kXR_IOError (4007) | 500 Internal Server Error | Read-only file system |
| EBADF | kXR_ArgInvalid (4007) | 400 Bad Request | Bad file descriptor |
...
- `src/handshake/dispatch_read.c` — opcode dispatch switch cases for Tier 1 operations (dispatch_read, dispatch_write)
