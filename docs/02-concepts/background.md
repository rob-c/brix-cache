...
The LHC produces roughly 15 petabytes of raw collision data per year. Moving that data from detector buffers to storage nodes to analysis farms is a solved problem in High Energy Physics — the solution is XRootD. This page explains what XRootD is, why nginx-xrootd builds on it, and the one naming trap that trips everyone up.

## What is XRootD?

XRootD is the file transfer protocol of High Energy Physics. It moves data between storage systems and analysis jobs at CERN, SLAC, and Fermilab — ROOT ntuples and collision datasets that routinely reach tens of gigabytes each.

The protocol is conceptually similar to FTP or HTTP, but purpose-built for physics workflows:
...
This module serves bytes. It doesn't parse ROOT files, inspect TTrees, or know anything about event data. A ROOT application reads through it because ROOT's I/O layer knows how to open `root://` URLs via the XRootD client library.
...
[cluster-mode.md](../05-operations/cluster-management.md). It does not implement remote storage
backends (HDFS, EOS, Ceph). It also does not implement XrdMon UDP monitoring
and never will — see [Monitoring](../08-metrics-monitoring/monitoring-guide.md).
