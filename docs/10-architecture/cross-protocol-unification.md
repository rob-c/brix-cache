...
│  src/fs/path/           xrootd_resolve_path_*()     [stream]      │
...
Source client sends kXR_open (TPC pull)
    │
    ▼
Worker A: xrootd_tpc_key_register()    src/tpc/engine/key_registry.c
    creates key entry in SHM with a 128-bit random key

Remote server connects to Worker B
    │
    ▼
Worker B: xrootd_tpc_key_lookup()      src/tpc/engine/key_registry.c
