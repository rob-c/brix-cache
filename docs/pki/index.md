...
This document explains the complete X.509 certificate stack used by
nginx-xrootd, and how that stack is consumed during authentication over both
`root://` (XRootD native) and `https://` (WebDAV).  If you want the
step-by-step commands for creating test certificates, see
[test-pki.md](test-pki.md).  If you want the quick-reference config
directives, see [authentication.md](authentication.md).
