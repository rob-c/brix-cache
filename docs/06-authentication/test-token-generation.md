...
Create a JWT/WLCG token signing authority, mint tokens, and fire them at BriX-Cache — all with local tools, no external identity provider needed. This is the token equivalent of [test-pki.md](test-pki-setup.md), and a useful reference for understanding how WLCG bearer-token auth works across both the native XRootD stream protocol and WebDAV.
