...
>
> **Verification Needed:** Check against `src/fs/cache/` for actual implementation.
>
...
### Existing Cache Subsystem (`src/fs/cache/`)
...
src/protocols/root/read/open_cache.c
  → xrootd_cache_open_or_fill()      -- in src/fs/cache/open_or_fill.c
    ...
    → if miss: origin fetch          -- src/fs/cache/fetch.c
    → open cached fd                 -- src/fs/cache/io.c
```
...
### `src/fs/cache/open.c` — Unified Entry Point
 ...
 * HOW:
 *   1. Map logical path → cache path (src/fs/cache/paths.c)
 *   2. Acquire per-file lock (src/fs/cache/lock.c)
 *   3. If cache file exists and is valid → open it, set fh->from_cache=1
 *   4. If miss → origin fetch (src/fs/cache/fetch.c), then re-open
...
`xrootd_cache_should_writethrough()` is a thin wrapper over `writethrough_decision.c`, now exposed via `src/fs/cache/writethrough.h`.
...
`src/fs/cache/paths.c` gains `xrootd_cache_meta_path()`. `open.c` reads the sidecar to validate freshness before opening the cached data file.

---

## Eviction Policy Integration

The existing eviction policy (`src/fs/cache/evict_policy.c`) tracks access via a background thread. After Phase 4 it is the single source of eviction signals — receiving access events from all three protocols:
...
`src/fs/cache/origin_protocol.c` currently speaks the XRootD protocol to fetch missing content from a redirector or storage node. For WebDAV and S3 protocols, the origin may also speak HTTP or S3. This requires a thin dispatch:

```c
// src/fs/cache/origin_protocol.c (extended):
typedef enum {
...
| `src/fs/cache/meta.h` | Public header for cache metadata |

### Modified files
| File | Change |
|:---|:---|
| `src/fs/cache/open_or_fill.c` | Rename/refactor into `open.c`; remove stream-specific context |
| `src/fs/cache/paths.c` | Add `xrootd_cache_meta_path()` |
