...
| Full proxy file cache (PFC) | Yes | Partial/No | BriX-Cache has open/cache and local data-plane helpers, not the full upstream PFC subsystem. |
...
| Checksum plugin ecosystem | Yes | Partial | BriX-Cache supports checksum query paths plus CRC-64/XZ and CRC-64/NVME, but not the full upstream checksum plugin matrix. |
| XrdFrm/MSS/tape staging ecosystem | Yes | Partial | BriX-Cache has FRM queue/Tape REST integration; full upstream migration, purge, space, and MSS driver behavior needs site review. |
