...
> | **BriX-Cache** | C | this project — nginx stream module (server) + native client |
...
* **Reuse** — **EOS does not implement the protocol at all.** It loads into the
  ...
  mapping).
* **Reimplement** — **dCache (Java/Netty)**, **go-hep (Go)**, and **BriX-Cache
...
go-hep vs. BriX-Cache**, with EOS standing in for "stock C++ + storage
semantics."
...
* **BriX-Cache** — the only event-loop reimplementation; full GSI/token/SSS +
...
> **Architectural consequence unique to BriX-Cache.** Because the protocol runs
...
* **Client request header (24 bytes):** `streamid[2] | requestid[2] | body[16] |
  ...
  prefix and extracts dlen from bytes `[20:24]`; BriX-Cache
...
`Marshaler`/`Unmarshaler` interface pair; BriX-Cache uses a cascade of
...
compares against a fixed array; go-hep does `reflect.DeepEqual`), and BriX-Cache
...
| BriX-Cache | 5.x | full: `haveTLS\|gotoTLS\|tlsLogin`, upgrade via `ngx_ssl_create_connection` |
...
capability bits). BriX-Cache, when the client sets `kXR_secreqs`, appends a
...
* **Session id:** XRootD derives it from FD/Inst/PID/SID then masks
  (`XrdXrootdXeq.cc:1105`); go-hep and BriX-Cache generate 16 random-ish bytes
  (BriX-Cache explicitly notes its sessid is *not* crypto-grade,
...
* **Auth trigger:** when security is configured, the login response carries the
  ...
  `XRD_LOGGEDIN|XRD_NEED_AUTH`; BriX-Cache leaves `auth_done=0`).
...
| Mechanism | XRootD C++ / EOS | dCache xrootd4j | go-hep | BriX-Cache |
|---|---|---|---|---|
...
| GSI detail | XRootD C++ (reference) | dCache xrootd4j | BriX-Cache |
|---|---|---|---|
...
mode that surfaced during BriX-Cache interop testing — see §5.4.
...
The following broke BriX-Cache against **real dCache** and **real EOS** in turn;
...
| Feature | XRootD C++ / EOS | dCache xrootd4j | go-hep | BriX-Cache |
|---|---|---|---|---|
...
> only two of the reimplementations: XRootD C++ and BriX-Cache.** dCache and
...
BriX-Cache's pgread uses a **gapped in-place layout**: `preadv` reads page data
...
* **Feature completeness for production WLCG `root://`:**
  **XRootD C++ ≈ EOS ≈ BriX-Cache** (full GSI/token/SSS + TLS + readv + pgread)
...
* **Architecturally**, the five span the full reuse↔reimplement spectrum: EOS
  ...
  contract. BriX-Cache is unique in being event-loop-based and in sharing a
...
| Concern | XRootD C++ | dCache xrootd4j | go-hep | BriX-Cache |
|---|---|---|---|---|
...
GSI interoperability claims (§5.4) were established by running the BriX-Cache
