# Client Tooling — Official XRootD vs. BriX-Cache Native Suite
...
`XrdCl/` (the C++ library), `XrdApps/` (the `xrdcp` /
...
- **`XrdCl` core library** (`XrdCl/`). The public API is the
...
- **`xrdfs`** — `XrdCl/XrdClFS.cc` plus the command registry
  ...
  `FileSystem`.
- **`XrdClHttp` plugin** (`XrdClHttp/`) — a *separate*
...
- **`pyxrootd` bindings** (upstream `python/` bindings) — a C extension module
  ...
  handlers.
- **`xrootdfs` FUSE driver** (`XrdFfs/`) — notably does
...
## In BriX-Cache (native pure-C suite)
