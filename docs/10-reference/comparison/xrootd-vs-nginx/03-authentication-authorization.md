# Authentication & Authorization: official XRootD vs BriX-Cache
...
- **`root://` handshake:** `src/protocols/root/session/login.c` drives `kXR_protocol →
  kXR_login → kXR_auth` from a single `brix_auth` enum
  (`tunables.h:221` `BRIX_AUTH_{NONE,GSI,TOKEN,BOTH,SSS,UNIX,KRB5,HOST,PWD}`).
  `brix_handle_login()` (`login.c:61`) builds the `&P=` advertisement; the
...
- **The decision pipeline** for `root://` runs through
  `src/auth/authz/auth_gate.c` `brix_auth_gate_op()` (authdb → VO ACL → token scope,
...
- *Module:* `src/auth/gsi/auth.c` routes on the 4-byte `credtype` and the GSI step
  byte; round-1 → `brix_gsi_send_cert()` (`cert_response.c:128`), round-2 →
  `brix_gsi_parse_x509()` (`parse_x509.c:251`) or the signed variant
  (`parse_x509.c:123`). DH primitives are in the **shared** `src/auth/gsi/gsi_core.c`
  (`brix_gsi_dh_keygen`, `_dh_derive`) used by both this server and the native
...
- *Module:* table-driven over
  ...
  (`gsi_core.h:76`). Operators can filter/reorder with `brix_gsi_ciphers`
...
- *Module:* a tri-state operator control `brix_gsi_signed_dh` →
  `BRIX_GSI_SDH_{OFF,AUTO,REQUIRE}` (`tunables.h:231`), with
...
- *Official:* proxy certs handled as `XrdCryptogsiX509Chain`; CA defaults
  ...
  download).
- *Module:* `src/auth/gsi/config.c` `brix_rebuild_gsi_store()` sets
  ...
  in `src/core/config/process.c` `brix_crl_reload_handler()` (a CApath directory is
...
**Module.** `src/auth/token/validate.c` `brix_token_validate()` is a single
...
- **Caches:** an **always-on, per-worker, lockless L1** validation cache
  ...
  cross-worker SHM L2** (`token_cache.c`, `brix_token_cache zone=...`, key =
...
- **Macaroons:** `src/auth/token/macaroon.c` validates the HMAC chain in-process
  ...
  **issuer-pinning** (`validate.c:201`: `location`/`iss` must match the
  ...
  configured). Discharge bundles are supported (`brix_macaroon_validate_bundle()`,
...
- *Module:* `src/auth/krb5/auth.c` — also **native `krb5_rd_req()` AP-REQ, not
  GSSAPI**; gated on build flag `BRIX_HAVE_KRB5` (absent ⇒ unconditional
  ...
  `brix_krb5_principal`, `brix_krb5_keytab`, `brix_krb5_ip_check`. The two
...
- *Module:* `src/auth/unix/auth.c` — also takes the username from the wire
  ...
  (`brix_unix_peer_is_loopback()` accepts 127/8, ::1, AF_UNIX) and rejects
  remote peers unless `brix_unix_trust_remote on`. It also allow-lists the
...
- *Module:* `src/auth/host/auth.c` — **a real implementation** (added 2026-06,
  ...
  (`brix_host_pattern_match()`: leading `.` = domain suffix, else exact,
  case-insensitive). **Hostname/suffix only — no CIDR/IP matching**; empty
  allowlist = deny-all. Directive: `brix_host_allow`. (The reference gap docs
  have been reconciled to show `host` as implemented, same as `pwd`.)

### Support matrix

| Method | Official XRootD | BriX-Cache | Notes |
|---|---|---|---|
| Anonymous | core flow | `brix_auth none` | both |
...
- **`xrdacc`** (`src/auth/authz/acc/`): a **faithful XrdAcc port** with a generational
  ...
  `brix_acc_cap_subcomp()`). Reverse-DNS via `brix_acc_resolve_peer()`
  ...
  `brix_acc_access()` (`access.c:86`) ports `XrdAccAccess::Access` (exclusive →
...
- *Module:* a dedicated VO-ACL tier (`src/auth/authz/acl.c`,
  `brix_check_vo_acl_identity()`; rule lookup in `find_rule.c`, longest-prefix
  over `vo_rules`), plus the `brix_require_vo` directive. Empty required-VO ⇒
...
- *Module:* `src/auth/token/scopes.c` checks the requested path against the token's
  ...
  `brix_token_check_read`/`_check_write` (write accepts write|create|modify),
  enforced as the **third** tier of `auth_gate.c` and at the WebDAV verifier
  (`webdav/auth_token.c`, `webdav_check_token_write_scope()`).

> The CLAUDE.md HELPERS list names `brix_token_check_scope` — that exact symbol
> does not exist; the real helpers are `brix_token_check_read`/`_check_write`
> and `webdav_check_token_write_scope`/`brix_identity_check_token_scope`.

### The decision pipeline

Module `root://` authorization is a single fail-closed pipeline,
`src/auth/authz/auth_gate.c` `brix_auth_gate_op()` (`:212`): **authdb → VO ACL →
...
(`brix_auth_cache zone=...`, `auth_cache.c`, default TTL 30s) is fronted by a
...
1. **Gate on the *completed* auth verdict only.** Access is gated on
   ...
   `ctx->logged_in`. The gate is `brix_dispatch_require_auth()` in
...
5. **Path confinement** — all wire paths resolve through
   `resolve_path()`/`brix_open_confined_canon()` before any syscall
...
6. **OCSP REVOKED is always fatal** regardless of soft-fail
   ...
   (`brix_gsi_max_inflight_handshakes`) are all conservative defaults.
...
sec.protocol sss -s /etc/brix/sss.keytab
...
`/etc/brix/authdb`, SSS keytab via `xrdsssadmin`.

### BriX-Cache (`nginx.conf`)
...
stream {
  server {
    ...
    brix_trusted_ca      /etc/grid-security/certificates;   # dir -> CApath
    brix_gsi_signed_dh   auto;            # off|auto|require
    ...
    # tokens
    brix_token_jwks      /etc/brix/jwks.json;   # local file
    brix_token_issuer    https://issuer.example/;
    brix_token_audience  https://se.example/;
    brix_macaroon_secret /etc/brix/macaroon.key;
    brix_token_cache     zone=tok:8m;     # opt-in SHM L2

    # other methods
    brix_sss_keytab        /etc/brix/sss.keytab;
    brix_krb5_principal    host/se.example@REALM;
    brix_krb5_keytab       /etc/krb5.keytab;
    brix_unix_trust_remote off;           # loopback-only by default
    brix_pwd_file          /etc/brix/passwd;
    brix_host_allow        .example.org;
