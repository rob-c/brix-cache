...
| Storage abstraction | `XrdSfs`/`XrdOss` virtual plugins | confined `openat2(RESOLVE_BENEATH)` POSIX helpers (`src/fs/path/`) behind one `brix_vfs_*` seam, with in-tree storage drivers (POSIX, S3, HTTP, Ceph, CephFS, `forward://` proxying, cache and stage decorators) selected by store URL | drivers are compiled in and chosen by configuration, not dlopened: there is no `XrdOss`/`XrdPss` plugin ABI, and an upstream plugin binary cannot be loaded |
