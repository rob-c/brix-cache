# `root://` Binary Wire Protocol — Official XRootD vs. BriX-Cache
...
| BriX-Cache | `kXR_PROTOCOLVERSION` (`0x00000520`) | always `kXR_DataServer`=1 |

 BriX-Cache source: `client_hello.c:84-87` writes `htonl(kXR_PROTOCOLVERSION)`
...
**BriX-Cache** (`src/protocols/root/session/protocol.c`):
...
**BriX-Cache** (`src/protocols/root/session/login.c`):
...
| # | Opcode | Official handler | BriX-Cache handler | Impl? | Framing / semantics parity |
|---|--------|------------------|----------------------|-------|----------------------------|
...
**Coverage summary:** of the 33 official request opcodes (3000-3032), BriX-Cache
...
| User command | Opcodes exercised | Works against BriX-Cache? |
|---|---|---|
...
**BriX-Cache** (`/home/rcurrie/HEP-x/nginx-xrootd/src/`):
