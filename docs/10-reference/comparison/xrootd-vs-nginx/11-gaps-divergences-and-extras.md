...
- **Alternative:** also the HTTP dashboard/admin API (`src/observability/dashboard/`) and nginx
  logs.
...
| **Prometheus pull metrics** | `src/observability/metrics/`, `/metrics` endpoint | Low-cardinality counters and latency histograms in one process-wide zone, `{proto}`-labeled across all five planes (stream/WebDAV/S3/cvmfs/GridFTP), plus rate-limit/cache/FRM/mirror/cluster families — the intended replacement for UDP XrdMon. |
