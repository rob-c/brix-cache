# Security Posture & Hardening — Official XRootD vs. BriX-Cache

> Part of the [XRootD vs BriX-Cache comparison set](./README.md).

This document compares the **official XRootD C++** server with the
**BriX-Cache** module on **security posture and hardening**: path confinement,
...
- **BriX-Cache is an nginx module**: a master + unprivileged workers, a
...
### BriX-Cache: kernel-enforced `RESOLVE_BENEATH`
...
**Net.** Both servers reject `..` lexically. BriX-Cache additionally makes
...
(`XrdXrootdProtocol.cc:404-426`). BriX-Cache: the path extractor enforces the
...
| Control | Official XRootD | BriX-Cache |
|---|---|---|
...
| Area | Official XRootD | BriX-Cache |
|---|---|---|
...
an Acc plugin). BriX-Cache pushes more guarantees into code that an operator
...
| Dimension | Official | BriX-Cache | Posture |
|---|---|---|---|
...
fail-closed auth, framing robustness, revocation — BriX-Cache reaches parity
