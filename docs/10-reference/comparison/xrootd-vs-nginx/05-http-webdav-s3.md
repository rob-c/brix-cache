...
| SSRF / DNS-pinning | not in reviewed source | `tpc_curl_secure()` forces `SSL_VERIFYPEER=1`/`VERIFYHOST=2`, resolves+pins the target IP via `CURLOPT_RESOLVE` (`brix_net_target_check_dns_pin()`) to close the TOCTOU window; policy gated by `brix_tpc_allow_local`/`_allow_private` |
...
| `brix_tpc_allow_local` / `_allow_private` | 0 / 1 | SSRF policy for loopback / RFC 1918 targets |
...
| X.509 / GSI client cert | chain extraction in `XrdHttpSecurity.cc` (`EECname()`/`EEChash()` → `SecEntity`) | `src/protocols/webdav/auth_cert.c`, `pki.c`; `brix_trusted_ca_dir`/`cafile`/`crl`/`verify_depth` |
