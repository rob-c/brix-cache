# HTTP Family — XrdHttp / WebDAV / HTTP-TPC / S3: Official XRootD vs. nginx-xrootd
...
*client-side* `XrdClS3/` plugin). The nginx-xrootd side cites this repository's
`src/webdav/` and `src/s3/` trees. Where a fact was already established by the
...
The WebDAV/XrdHttp plane is an nginx HTTP module under `src/webdav/`. The master
request router is `ngx_http_xrootd_webdav_handler()` in `src/webdav/dispatch.c`, which
...
`xrootd_webdav_operations[]` descriptor array in `src/webdav/operation_table.c`. The
handler is installed in `NGX_HTTP_CONTENT_PHASE` by
`src/webdav/postconfig.c`. The XrdHttp dialect (the `X-Xrootd-*` headers, `?xrd.stats`,
`Want-Digest`, HTTP↔`kXR_*` status mapping) is layered on top in `src/webdav/xrdhttp.c`,
...
`DAV: "1, 2"` (`src/webdav/methods_basic.c`), and it ships real `LOCK`/`UNLOCK`
handlers (`src/webdav/lock.c`), `PROPPATCH` and dead properties
(`src/webdav/methods_basic.c`, `dead_props.c`, `prop_xattr.c`), plus `SEARCH`
(`src/webdav/search.c`, advertised via `DASL: <DAV:basicsearch>`) and a read-only
`ACL` discovery method (`src/webdav/acl.c`). All of these are beyond the upstream
XrdHttp method enum.

The S3 gateway is a **second, independent** nginx HTTP module under `src/s3/`, with its
own dispatcher (`ngx_http_s3_handler` in `src/s3/handler.c`), operation descriptor
table (`src/s3/operation_table.c`), and SigV4 verification stack
...
nginx-xrootd file and entry function.

| Method | RFC class | Official XrdHttp | Our handler | Notes |
|---|---|---|---|---|
| `GET` | class 1 | `rtGET`; ranges via `XrdHttpReadRangeHandler`; ETag (`addETagHeader`, `inode-dev`); chunked TE; no compression | `src/webdav/get.c` → `webdav_handle_get()` | Single + multi-range (`multipart/byteranges`); `If-Modified-Since` → 304; optional response compression via `xrootd_webdav_compress`; TLS=memory-backed buffers, cleartext=sendfile |
| `HEAD` | class 1 | `rtHEAD` | `src/webdav/methods_basic.c` → `webdav_handle_head()` | Stat via `webdav_resolve_stat()`; `Content-Length`, `Last-Modified`, optional `ETag`; `Content-Type` `httpd/unix-directory` for dirs |
| `PUT` | class 1 | `rtPUT` | `src/webdav/put.c` → `webdav_handle_put_body()` | Async body read; lock check **before** body read; staged temp file + atomic rename; thread-pool offload |
| `DELETE` | class 1 | `rtDELETE` | `src/webdav/namespace.c` → `webdav_handle_delete()` | Recursive delete; on collections checks child locks via `webdav_check_locks_tree()` |
| `MKCOL` | class 1 | `rtMKCOL` | `src/webdav/namespace.c` → `webdav_handle_mkcol()` | Collection create (mkdir, confined) |
| `OPTIONS` | class 1/2 | `rtOPTIONS`; `Allow` header | `src/webdav/methods_basic.c` → `webdav_handle_options()` | Emits `DAV: "1, 2"`, `DASL: <DAV:basicsearch>`, `MS-Author-Via: DAV`; `Allow` built from `allow_write` (read-only vs. full set) |
| `PROPFIND` | class 1 | `rtPROPFIND` | `src/webdav/propfind.c` → `webdav_handle_propfind()` | `Depth: 0/1/infinity`; live + dead properties; libxml2 hardened (`NONET`, no-XXE, no `HUGE`); `infinity` capped at `XROOTD_WEBDAV_PROPFIND_MAX_ENTRIES` (DoS ceiling) |
| `PROPPATCH` | class 1 | **not present** (no `rtPROPPATCH`) | `src/webdav/methods_basic.c` → `webdav_handle_proppatch()` (dead-prop storage in `dead_props.c`/`prop_xattr.c`) | Returns 207 Multi-Status; dead props stored as hex-encoded xattrs — needed by clients that treat `501` as fatal |
| `MOVE` | class 1 | `rtMOVE` | `src/webdav/move.c` → `webdav_handle_move()` | Lock check on **both** source and destination; thread-pool offload; rename within confined root |
| `COPY` | class 1 | `rtCOPY` (also the HTTP-TPC verb) | `src/webdav/copy.c` → `webdav_handle_copy()`; TPC path → `src/webdav/tpc.c` | Local server-side copy (recursive tree → temp → rename); when `Source`/`Credential` header present, dispatched to HTTP-TPC instead (see below) |
| `LOCK` | **class 2** | **not present** | `src/webdav/lock.c` → `webdav_handle_lock()` | xattr-backed; exclusive + shared; `Depth: 0`/`infinity`; locks persist across restart unless `xrootd_webdav_lock_startup_sweep on` |
| `UNLOCK` | **class 2** | **not present** | `src/webdav/lock.c` → `webdav_handle_unlock()` | Removes lock xattr; `If:`/lock-token matching |
| `SEARCH` | RFC 5323 (DASL) | **not present** | `src/webdav/search.c` → `webdav_handle_search()` | Basic `<DAV:basicsearch>` over the confined namespace; `Depth: 0/1/infinity` |
| `ACL` | RFC 3744 | **not present** | `src/webdav/acl.c` → `webdav_handle_acl()` | Read-only discovery; refuses client-side ACL mutation (403) |
| `POST` | — | `rtPOST` | `src/webdav/macaroon_endpoint.c` → `webdav_handle_macaroon_token()` (also S3 browser-form POST in `src/s3/`) | Our WebDAV `POST` is the macaroon/token issuance endpoint (`/.oauth2/token`) |
...
(`src/webdav/prop_xattr.c`), created with `XATTR_CREATE` so the kernel serialises lock
creation (race losers get `EEXIST` → 423 Locked). `webdav_check_locks()`
(`src/webdav/lock.c`) walks from the target path up to the export root checking each
...
- **Checksum / digest.** Upstream `XrdHttpChecksumHandler` answers `Want-Digest` (RFC 3230)
  ...
  `UNIXcksum`, `crc32c` (per `XrdHttp/README-CKSUM.md`). Our `src/webdav/xrdhttp.c`
...
- **CORS.** Upstream CORS is the optional `XrdHttpCors` plugin (`cors.origin` config,
  ...
  in `src/webdav/cors.c`, driven by `xrootd_webdav_cors_origin` (allowlist, repeatable)
...
| Aspect | Official `XrdHttpTpc` | nginx-xrootd `src/webdav/tpc*.c` |
|---|---|---|
| Dispatch | `rtCOPY` handler in `XrdHttpTpcTPC.cc`; `Source`/`Destination` headers select pull/push | `ngx_http_xrootd_webdav_tpc_handle_copy()` in `src/webdav/tpc.c`; detects `Source`/`Destination` (rejects both-or-neither with 400) |
| Outbound client | libcurl (`#include <curl/curl.h>`, `curl_multi` for concurrency) | libcurl via `src/webdav/tpc_curl.c` (`curl_multi`); curl path configurable (`xrootd_webdav_tpc_curl`, default `/usr/bin/curl`) |
...
| Performance markers | `XrdHttpTpc::PMarkManager`; periodic chunked `Perf Marker` blocks | `src/webdav/tpc_marker.c`; same `Perf Marker` wire block (`Timestamp:`/`Stripe Index:`/`Stripe Bytes Transferred:`/`Total Stripe Count:`/`End`), 202 + chunked body, 200 ms poll, interval set by `xrootd_webdav_tpc_marker_interval` (default 0 = off) |
| Credential / delegation | `Credential:` header (`none` only in reviewed source); `TransferHeaderN` forwarding (e.g. `TransferHeaderAuthorization`); `tpcForwardCreds` for redirects | `Credential:` modes `none`, `oidc-agent`, `token-exchange` (`src/webdav/tpc_cred.c`, `tpc_cred_parse.c`); injects `Authorization: Bearer <tok>` into the transfer |
| OAuth2 token exchange | not in reviewed source | RFC 8693 exchange against `xrootd_webdav_tpc_token_endpoint` using the request's bearer as subject token (`src/webdav/tpc_cred.c`) |
...
performance markers), and nginx-xrootd **adds** operational hardening: OAuth2 token
...
The full HTTP-TPC tuning surface is in `src/webdav/tpc_config.c`:
...
(`src/webdav/tape_rest.c`, gated by `xrootd_webdav_tape_rest`), is an nginx-forward
...
plugin for talking *to* S3 stores, whereas nginx-xrootd ships a **server-side S3 REST
endpoint** under `src/s3/`. There is no upstream feature to compare it against, so every
...
**Dispatch** (`src/s3/handler.c`, `src/s3/operation_table.c`): `s3_parse_uri()` splits
...
|---|---|---|
| GetObject | `GET /<bucket>/<key>` | `src/s3/object.c` |
| HeadObject | `HEAD /<bucket>/<key>` | `src/s3/object.c` |
| HeadBucket | `HEAD /<bucket>` | `src/s3/handler.c` |
| PutObject | `PUT /<bucket>/<key>` | `src/s3/put.c` |
| CopyObject | `PUT /<bucket>/<key>` + `x-amz-copy-source` | `src/s3/copy.c` |
| DeleteObject | `DELETE /<bucket>/<key>` | `src/s3/object.c` |
| DeleteObjects (batch) | `POST /<bucket>/?delete` | `src/s3/delete_objects.c` |
| ListObjectsV2 | `GET /<bucket>/?list-type=2` | `src/s3/list_objects_v2.c` |
| ListObjectsV1 | `GET /<bucket>/` (no `list-type`) | `src/s3/list_objects_v1.c` |
| InitiateMultipartUpload | `POST /<bucket>/<key>?uploads` | `src/s3/multipart_initiate.c` |
| UploadPart | `PUT …?uploadId=&partNumber=` | `src/s3/multipart_*.c` |
| UploadPartCopy | `PUT …?uploadId=&partNumber=` + copy-source | `src/s3/multipart_complete_upload_part_copy.c` |
| CompleteMultipartUpload | `POST …?uploadId=` | `src/s3/multipart_complete.c` |
| AbortMultipartUpload | `DELETE …?uploadId=` | `src/s3/multipart_abort.c` |
| ListParts | `GET …?uploadId=` | `src/s3/multipart_complete_list_parts.c` |
| Get/Put/DeleteObjectTagging | `…?tagging` | `src/s3/tagging.c` |
| POST Object (browser form) | `POST /<bucket>/` multipart/form-data | `src/s3/post_object.c` |
| GetBucketVersioning / ACL / CORS | `GET …?versioning` / `?acl` / `?cors` | `src/s3/handler.c` (canned/disabled responses; unsupported mutations return `501 NotImplemented`) |

### SigV4 authentication

`src/s3/auth_sigv4_*` verifies **AWS Signature Version 4** server-side in three forms:
...
- **Streaming `aws-chunked`** — `src/s3/aws_chunked.c` runs a chunk-framing state machine
...
- **ListObjects** supports both V1 (`list_objects_v1.c`) and V2 (`list_objects_v2.c`),
  ...
  (`src/s3/list_cache.c`/`.h`, enabled by `xrootd_s3_list_cache`, TTL
...
- **Conditional requests** (`src/s3/conditional.c`) implement RFC 9110 §13.2.2 precedence:
...
- **Checksums** (`src/s3/checksum.c`) cover `crc32`, `crc32c`, `crc64nvme`, `sha1`,
...
because nginx-xrootd reuses nginx's TLS engine and certificate model rather than XrdTls.

| Concern | Official XrdHttp | nginx-xrootd |
|---|---|---|
...
| X.509 / GSI client cert | chain extraction in `XrdHttpSecurity.cc` (`EECname()`/`EEChash()` → `SecEntity`) | `src/webdav/auth_cert.c`, `pki.c`; `xrootd_webdav_cadir`/`cafile`/`crl`/`verify_depth` |
...
| Area | Official XrdHttp | nginx-xrootd | Verdict |
|---|---|---|---|
...
| Server-side S3 REST endpoint | **no** (only `XrdClS3` client plugin) | **yes** (`src/s3/`) | nginx-only |
...
with nginx-xrootd adding full **WebDAV class 2** (locking, dead props, SEARCH, ACL) and
