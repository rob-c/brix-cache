...
| Unified (proto-labeled — `proto="stream\|webdav\|s3\|cvmfs\|gridftp"`, i.e. every plane, generated from `src/core/types/proto_list.h`) | `brix_io_ops_total`, `brix_io_bytes_read`/`_written`, `brix_io_latency_seconds` (histogram), `brix_auth_total`, `brix_cache_requests_total{cache_status}`, `brix_cred_select_*_total`, `brix_tpc_transfers_total`, `brix_tpc_gsi_delegated_total` (`result="ok\|expired\|absent"` — outbound native-TPC GSI proxy-delegation outcome) |
