...
| Rate limiting / bandwidth / concurrency | Upstream `XrdThrottle` and `XrdBwm`. | `src/net/ratelimit/`, `brix_rate_limit_zone`, `brix_rate_limit_rule`, `brix_bandwidth_limit`, `brix_concurrency_limit`, `brix_throttle_max_open_files`. | Partial / nginx+ | The nginx shaping engine is cross-protocol and identity-aware, and is more capable for the common case. Of the `throttle.*` contract only the per-user open-files cap is enforced: phase-95 deleted the `max_active_connections` directive and the IO-load / `userconfig` engines, which were implemented but never had an admission point. `throttle.data`/`throttle.iops` pacing, loadshed and fairness remain unimplemented. |
