...
| 13100–13619 | `tests/cvmfs/conformance_common.py` `PORT_BLOCKS` | cvmfs conformance corpus: one 20-port block per test file (mock origins base+0..9, nginx base+10..19). Canonical bases only — the whole map is tiled by a per-session offset (lock-port claim) so concurrent sessions never collide. Newest blocks: `srv_scvmfs_x509` 13520, `srv_scvmfs_voms` 13540, `srv_stratum0` 13560, `srv_s0_scvmfs` 13580 (phase-96 S13/S14), `srv_s0_quickstart` 13600 (the shipped-binary runbook lane behind `docs/05-operations/cvmfs-stratum0.md`). Adding a block entry grows the tile span — safe for new sessions, never rebases a running one |
