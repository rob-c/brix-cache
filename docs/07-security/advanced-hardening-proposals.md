...
### Proposal:
*   **Model Checking**: Use a formal tool (like TLA+ or a bounded model checker) to verify the path-resolution and ACL-intersection logic in `src/auth/authz/acl.c` and `src/protocols/root/handshake/policy.c`.
