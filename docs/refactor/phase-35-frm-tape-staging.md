...
B2 turns the existing fire-and-forget staging hook (`src/query/prepare_cmd.c` — a double-fork `execv` that captures no state, reaps nothing, and hard-codes `reqid="0"` at `prepare.c:338`) into a **durable, async, de-duplicating tape-staging subsystem** that survives client disconnect and worker restart. Concretely:
...
| File | Responsibility |
|---|---|
| `src/frm/frm.h` | Public API: `frm_queue_t`, `frm_req_view_t`, `frm_request_*`, `frm_reqid_generate`, `frm_reap_expired`, and the `xrootd_frm_conf_t` directive struct. The only header `src/query/prepare.c`, the HTTP Tape REST, and residency code include. |
...
| `src/frm/index.c` | **SHM hot index**: zone configure/init (`ngx_shared_memory_add`, `(void*)1` sentinel, flexible-array `slots[]`, `ngx_shmtx` lock-first), `frm_index_insert/lookup/remove/scan`, generation check, LRU reap-on-full + `reconcile_full_total` metric (the `src/session/registry.c:249-289` pattern). |
...
| `src/frm/residency_xattr.c` | `user.frm.residency` xattr encode/decode/read/write (mirrors `src/webdav/prop_xattr.c:23-118`). |
...
| `src/webdav/tape_rest.c` / `.h` | WLCG Tape REST API router + body handlers (Phase 2). |
...
- **config script** — add all `src/frm/*.c` to `ngx_module_srcs` (after the `src/query/prepare_cmd.c` line at **L441**); add `src/frm/*.h` to `ngx_xrootd_stream_deps`.
...
**CREATE:**
- `src/frm/residency.h`, `src/frm/residency_xattr.c` — `user.frm.residency` xattr (mirror `src/webdav/prop_xattr.c:23-118`; absent ⇒ ONLINE, `NGX_DECLINED` on `ENODATA`/`ENOATTR`).
...
**CREATE:**
- `src/webdav/tape_rest.h` / `src/webdav/tape_rest.c` — router `webdav_tape_handle(r)` (parses `/api/v1/{stage,release,unpin,archiveinfo,fileinfo}[/{id}[/cancel]]`), method gating, per-verb auth/scope, jansson parse/build, reqid minting (reuse `RAND_bytes` per `macaroon_endpoint.c:388`). Endpoint→M1 mapping:
...
- `src/webdav/module.c` (~L321, next to `xrootd_webdav_tpc`) — register `xrootd_webdav_tape_rest` flag directive.
- `src/webdav/webdav.h` (~L139) — `ngx_flag_t tape_rest;` in loc_conf; init `NGX_CONF_UNSET` in `config.c:create_loc_conf`; `ngx_conf_merge_value(...,0)` (off by default).
- `src/webdav/dispatch.c` — at top of `ngx_http_xrootd_webdav_handler`, **before** proxy-mode delegation (L70) and method routing, insert the `/api/v1/` prefix check → `webdav_tape_handle(r)` (POST bodies via `xrootd_http_read_body` → `NGX_DONE`; GET/DELETE inline). Same structural slot as the macaroon block (42–67).
...
- **config** — add `src/webdav/tape_rest.c` to `ngx_module_srcs` (~L572 webdav block), `tape_rest.h` to `ngx_xrootd_webdav_deps` (~L208).
...
- `src/read/open_request.c` — replace the Phase-1 `xrootd_send_wait` stall (for stageable misses) with: `xrootd_stage_admit`/look-up-by-lfn dedup → `xrootd_stage_add_waiter{ngx_pid, c->number, hash(sessid), cur_streamid, waitresp_sent=0}` → `xrootd_send_waitresp` now → latch `waitresp_sent=1` → return to `XRD_ST_REQ_HEADER`. (Keep a config/feature flag so Phase-1 `kXR_wait` behavior remains selectable.)
...
- `src/query/prepare.c` notify block (393–399) — now able to send `asyncms` on real completion for `missing>0` (the deferred "completion pipe" future-work comment is resolved).
- `src/connection/disconnect.c` — **no required teardown**: waiters are identified by `conn_number`, so a stale waiter is harmless (reaped at delivery or by TTL). The transfer is **not** cancelled by disconnect. Optional best-effort clear of `(ngx_pid, c->number)` waiters for tidiness.
