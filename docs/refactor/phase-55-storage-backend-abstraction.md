...
| Kernel confinement = `RESOLVE_BENEATH` | `src/path/beneath.h`, `xrootd_ns_*` in `src/compat/namespace_ops.c` | `openat2(RESOLVE_BENEATH)` is a filesystem concept; object stores confine by **key prefix** |
...
| Namespace | `src/fs/vfs_unlink.c`, `vfs_rename.c`, `vfs_mkdir.c`, `vfs_copy.c`, `src/compat/namespace_ops.c` | delete/rmdir/rename/mkdir/copy implementation and escape mapping |
...
`src/compat/namespace_ops.c` / `src/path/beneath.*` bodies, moved behind the vtable:
...
```bash
rg -n '\b(open|openat|pread|pwrite|fsync|ftruncate|stat|fstat|lstat|rename|unlink|mkdir|opendir|fdopendir|readdir|copy_file_range)\s*\(' src/fs src/compat/namespace_ops.c src/path/beneath.c
```
...
- Two directives, per server/location, declared alongside the existing storage
  ...
  `ngx_stream_xrootd_srv_conf_t` struct in `src/types/config.h` and merged in
  `src/config/server_conf.c` (exact hunks in §17.17–§17.19):
...
rg -n '\b(open|openat|pread|pwrite|fsync|ftruncate|stat|fstat|lstat|rename|unlink|mkdir|opendir|fdopendir|readdir|copy_file_range|getxattr|setxattr|listxattr|removexattr)\s*\(' \
  src/fs src/compat/namespace_ops.c src/path/beneath.c \
...
today's `src/fs/`, `src/compat/namespace_ops.c`, and `src/path/beneath.*` code, moved
...
- `xrootd_sigv4_signing_key()` (`src/compat/sigv4.h`) — the 4-round HMAC chain;
- `xrootd_hmac_sha256()`, `xrootd_sha256()`, the streaming SHA256
  (`xrootd_sha256_stream_*`) (`src/compat/crypto.h`);
- `xrootd_hex_encode()` (`src/compat/hex.h`).
...
1. A **canonical-request builder for signing** (the inbound `build_canonical_qs` in
   ...
   `xrootd_s3_canonical_request()` into a new `src/compat/s3_canon.c` that *both* the
...
2. A **blocking, worker-thread HTTP transport**. Two candidates exist:
   ...
   factor its core into `src/compat/httpc.c` (`xrootd_httpc_exchange()`), a blocking
...
### 17.18 `src/types/config.h` (srv-conf struct)

**Intent:** add the paired store-config objects and the built instance pointers to
`ngx_stream_xrootd_srv_conf_t` (§6.6). The struct lives in `src/types/config.h`, not
`src/config/config.h`; it ends at line 740 (`} ngx_stream_xrootd_srv_conf_t;`).
...
`src/types/config.h` must `#include "../fs/backend/sd.h"` (or forward-declare
...
`module_storage_directives.c`) + `src/config/server_conf.c` (merge)
...
Merge logic — in `src/config/server_conf.c`, alongside the `cache_root` merge (line 366):
...
- **Files:** `src/config/config.h`, `src/config/directives.c`, merge in `merge_*_conf()`.
- **Depends-on:** PR-02
- **Review checklist:**
...
- **Files:** `src/fs/vfs_unlink.c`, `vfs_rename.c`, `vfs_mkdir.c`, `src/compat/namespace_ops.c`, `src/fs/backend/sd_posix.c`.
- **Depends-on:** PR-08
- **Review checklist:**
...
- **Files:** `src/fs/vfs_staged.c`, `src/compat/staged_file.c`, `src/fs/backend/sd_posix.c`.
- **Depends-on:** PR-09
- **Review checklist:**
...
- **Scope:** Add the §9.2 ripgrep allowlist check as a fast CI gate: raw storage syscalls
  in `src/fs`, `src/compat/namespace_ops.c`, `src/path/beneath.c` must be **only** in
...
- **Files:** `src/fs/vfs_staged.c`, `src/fs/backend/sd_object.c`, `src/config/directives.c` (staging live).
- **Depends-on:** PR-19
- **Review checklist:**
...
- **What:** the §9.2 ripgrep allowlist check — raw storage syscalls in
  `src/fs`, `src/compat/namespace_ops.c`, `src/path/beneath.c` only in
