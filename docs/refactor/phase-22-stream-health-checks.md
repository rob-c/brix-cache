...
| **C** | Health-check timer | ✅ **Done — different start hook** | `xrootd_hc_mgr_t`, `xrootd_hc_timer_handler` (`:487`), `xrootd_hc_manager_start` (`:504`), `xrootd_hc_start` (`:368`). Started from **`src/config/process.c:163`** (the module's `init_process`), **not** `src/stream/module.c` as the file map said. Scan interval = `interval / slots`, min 100 ms. |
| **D** | Config directives | ✅ **Done** | All six `xrootd_health_check[_interval/_timeout/_threshold/_blacklist/_type]` directives are registered in **`src/stream/module.c`** (not `src/config/directives.c`); conf fields `hc_enabled/hc_interval_ms/hc_timeout_ms/hc_threshold/hc_blacklist_ms/hc_type` in `src/types/config.h:357-362`. Off by default. |
...
2. **Directives live in `src/stream/module.c`**, the start hook in
   `src/config/process.c`, and the ctx struct stays private to `health_check.c` —
...
**File:** `src/config/directives.c` (register), `src/stream/module.c` (merge)
...
| `src/config/directives.c` | Modify | Register 6 new `xrootd_health_check*` directives |
| `src/types/config.h` | Modify | Add HC fields to `ngx_stream_xrootd_srv_conf_t` |
...
| `src/config/config.h` | Modify | Add `health_check.c` to `NGX_ADDON_SRCS` |
...
`src/config/config.h` before running `./configure`. All other changes are in existing
