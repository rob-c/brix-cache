...
- [x] `tests/test_cms_tier_topology.py` (2026-08-04) — **the last migratable direct launcher, and the last inline config heredoc in a `test_*.py`**. The six-node cmsd tree (mgr → sub1 → {leafA, leafB, sub2 → leafC}) is now the committed `configs/nginx_cms_tier.conf` driven by a module-scoped `LifecycleHarness` on the new fixed `lc-cms-tier` ledger block (primary listen = root manager; the other five listens are that entry's `extra` ports, addressed by the `port_key` field each `NODES` row now carries — so the template and the topology declaration are one contract). Deleted the module's private `_module_prelude`/`_probe_feature`/`_resolve_nginx` binary hunt, `_free_ports`, `_render_conf`, and the hand-rolled `-s stop` + pidfile-kill teardown (~145 lines). The feature-capability check the probe existed for is preserved without a second launch: a settle window that closes with **zero** `cmsd role:` lines means the binary predates the auto-role feature, and the fixture skips. `worker_processes 3` is kept in the template — the single-connection gate assertion depends on it. 7/7 green (6 role lines, 5 register edges, on ledger ports 31200-31205). Removed from both `LAUNCH_BACKLOG` and `INLINE_CONFIG_BACKLOG` — **the inline backlog is now empty**.
- [~] `tests/_perf_netem_helpers.py` (2026-08-04) — DEFERRED (documented lint backlog entry, third of the standalone-lab class alongside `userns/e2e_redteam.py` and `cmdscripts/system_live_ports.py`). This is the one direct launcher the registry structurally *cannot* own: the phase-33 high-BDP A/B harness starts its nginx **inside a private network namespace** (`unshare -n`) at the far end of a synthesized `veth` + `tc netem` link, so the listen is unreachable from the host the registry probes for readiness, its port cannot collide with a fleet port, and it is reaped inside the namespace that created it. It is a helper module (never collected by pytest) driven by an operator-invoked harness. It surfaced only when the guard learned to see `str(nginx_bin)` argv0 — it was an unflagged direct launcher long before.
