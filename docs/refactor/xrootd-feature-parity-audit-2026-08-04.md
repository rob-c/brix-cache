...
parameter); **`sitename` now answers the configured name (wave 46):** the new
`brix_sitename <name>` directive populates the site-identity slot and a Qconfig
`sitename` emitter reports it, matching stock's `all.sitename`-set behaviour,
while still echoing the key when unset — so the stock-parity default is
preserved AND a configured site now labels the node for monitoring/federation.
The same `brix_sitename` also fills the kXR_QStats `<statistics site="…">`
attribute (was hard-coded empty) that summary-monitoring/federation dashboards
read (`tests/test_qconfig_sitename.py`, 5).

(e) **Configurable default checksum (wave 48)** — stock's `xrootd.chksum` sets the server's default/preferred checksum algorithm; BriX hard-coded adler32. `brix_checksum_default <algo>` now drives the algorithm a kXR_Qcksum with no explicit selection computes AND the one advertised first in the Qconfig `chksum` preference cslist (WLCG crc32c / cloud sha256). Applied at every default site — the Qcksum path and handle handlers and the Qckscan scan — with an explicit per-request algo still winning and an unset/invalid value degrading to adler32 (Qconfig list then byte-identical to before). `tests/test_checksum_default.py` (5: default drives Qcksum+Qconfig order / explicit override / unset regression / bad-value degrade / `nginx -t`). The remaining named keys
(sysid/wan_port/wan_window/cid) were verified LIVE against the fleet's stock
5.6.9 reference server to ECHO under a default config — BriX's echo fallback is
therefore already byte-faithful, and no BriX directive can set them, so echo is
the permanent answer (`tests/test_query_extended.py::TestQconfigStockKeyParity`).
(NOTE, found this wave, CLOSED IN 2.0: the whole `brix_cache_advertise`
Pelican-federation family — enable/key/data_url/web_url/sitename/issuer_url/
interval/namespace — was declared, merged, and read by
`fs/cache/origin/pelican_register.c` while NO directive registered any of it, so
`brix_cache_advertise` was an "unknown directive" and the federation
registration was unreachable. 2.0 registers the seven missing names on the
http+stream planes; the eighth field is still written by `brix_sitename`, as
predicted here. See `docs/10-reference/release-2.0-readiness.md` §(c.3)
"Registered nowhere", the `brix_cache_advertise*` section of
`docs/03-configuration/directives.md`, and
`tests/test_release20_registered_nowhere.py`.) brix-xrdcp now
...
(f) **Site checksum plugins (2.0 F8, 2026-09-07)** — stock's `xrootd.chksum <name> <path> [parms]` loads an XrdCks plugin; BriX had a fixed nine-algorithm table. `brix_checksum_plugin <name> <path.so> [parms]` (stream or http main context, one process-wide registry keyed on the cycle, at most 8) now dlopens a shared object exporting `brix_cks_plugin` against a plain-C99 ABI (`src/core/compat/checksum_plugin_abi.h`: abi 1, name ≤15 lowercase alnum, digest ≤64 bytes, state ≤4096 bytes, init/update/final), verifies it (absolute regular non-writable file, symbol, ABI, name, bounds, self-test with the parms, no collision with a built-in or an earlier plugin) at `nginx -t`, and hands the name to the same algorithm lookup every consumer already uses, so Qcksum `<name>:`/`?cks.type=`, `brix_checksum_default`, the Qconfig `chksum` list (default, built-ins, plugins) and WebDAV `Want-Digest` work with no per-protocol change; the host walks the object and hex-encodes (INVARIANT 9). `contrib/checksum-plugins/` (FNV-1a 64 example + README); `tests/test_release20_checksum_plugin.py` (26).
...
per-subtree attrs), deliberate/N-A (ztn expiry modes; `pfc.urlcgi` landed as 2.0 F5, `cms.fsxeq` as 2.0 F17 and the `ofs.tpc` identity matrix as 2.0 F18, all three reversing a refusal recorded here),
...
(af) **2026-08-10/11 session (waves 24–48)** — a run of bounded server-plane
items, several of which the (ae) candour note had written off as too risky. The
biggest was the full **§1.1 response-offloading arc**: read/readv/pgread
responses now route over bound secondary data channels (kXR_bind pathids), with a
complete pipelining-reservation gate and an `io_offload_total{proto}` metric —
the server half of the do_Offload parity the client's multi-stream fan-out pairs
with. Also landed: **§1.16 runtime admin socket** (per-worker unix sockets;
list/disc/msg/pause/cont/abort; 0600 privilege boundary); **§1.3 login abilities
kXR_fullurl + kXR_readrdok** — the (ae) note called these "wire-format-risky
without the XProtocol oracle", but the *installed* `/usr/include/xrootd/.../
XProtocol.hh` carries both the bit values and the redirect body layout, so they
landed (fullurl → self-contained `root://` redirect targets; readrdok gates the
§1.10 fsoverload read-redirect so a client that can't follow one gets kXR_wait);
**§2.18 locate kXR_prefname** (hostname vs IP in the location token); **§3.1/§3.3
oss.quota** advertisement **plus** `brix_oss_quota_enforce` write-plane
enforcement priced off the same probe Qspace reports; **§3.14 mirage** synthetic
sizes-only backend; **§3 tape dirlist + rcreate** (FRM adapter list/mkpath
verbs); **§1.14 brix_sitename** (feeds the Qconfig `sitename` key AND the QStats
`<statistics site=…>` attribute — both were empty/echo before) and
**brix_checksum_default** (the `xrootd.chksum`-default analog; drives the Qcksum
default algo and the Qconfig `chksum` preference order). Two findings worth
carrying forward: the whole `brix_cache_advertise` Pelican-federation family was
**declared, merged, and read by `pelican_register.c` but registered by NO
directive** — federation registration was unreachable, a self-contained
config-wiring task, **done in 2.0** (register §(c.3)); and the data-plane
request-flag surface (open mkpath/retstat,
stat offline/vfs, dirlist dstat/dcksm/online, Qcksum `?cks.type=`) was
re-verified **complete** — that vein is now exhausted. **Net: the (ae) note holds
— what remains is the large architectural bodies below, not more bounded fixes.**
...
| 3 | Login `ability`/`ability2` honored: ~~kXR_fullurl~~ (landed, wave 42); kXR_redirflags, hasipv64/onlyprv4/6 addr-family variants, lclfile open | PARTIAL (**fullurl landed 2026-08-10** — UNBLOCKED: the redirect body layout + XLoginAbility values are in the INSTALLED `XProtocol.hh`, no need for the gone client tree) | The advertised bitmasks now persist on the session (`ctx->login.ability`/`ability2`, stored at `session/login.c` from the codec's already-decoded fields — the audit's "decoded, ignored" was literal), and `kXR_fullurl` (bit 0) is honored at the redirect emission choke point (`response/control.c brix_send_redirect`): a fullurl-capable client gets a SELF-CONTAINED `root://host:port` URL in the host field (the unambiguous cross-port form; the numeric port field keeps its value — harmless duplication, the URL is authoritative for such clients), while clients that did not advertise it keep the classic host-only form byte-identical. Unknown ability bits are stored untouched and change nothing. `tests/test_login_fullurl.py` (3, driven through the §1.10 fsoverload redirect as the deterministic trigger: fullurl ⇒ `root://sibling.example:2094`; plain ⇒ `sibling.example`; garbage bits 0xFE with fullurl clear ⇒ host-only, no misbehavior). **`kXR_readrdok` also landed (wave 44):** a redirect issued in RESPONSE to a kXR_read/readv (not at open time) is only followable by a client that advertised readrdok. The §1.10 fsoverload read-redirect choke point (`read.c brix_fsoverload_backoff`) now gates on it: a non-readrdok client's overloaded read degrades to the safe kXR_wait backoff instead of a mid-read redirect it would mishandle — correct parity, and more correct for BriX's own client (its async engine can't follow read redirects, rows §7.3/§7.11, and it advertises ability=0). `tests/test_fsoverload_stall.py::test_overload_redirect_needs_readrdok` (readrdok-clear ⇒ kXR_wait even with a redirect host configured) + the existing redirect test now logs in with readrdok. STILL OPEN: `kXR_redirflags` (its flag vocabulary is NOT in the installed header — genuinely oracle-limited), the hasipv64/onlyprv4/6 address-family redirect selection (needs dual-stack redirect-target config to matter), `lclfile`, and `ability2` `kXR_ecredir` (stored, unhonored) |
...
| 18 | Locate: ~~`kXR_prefname` ignored (IP only)~~ **honored 2026-08-10 (wave 45)**; M/m manager entry types still absent | PARTIAL | **`kXR_prefname`** (locate option 0x0100): the client can now ask for the server's DNS hostname in the `S<access><host>:<port>` token instead of the IP literal — honored at the `locate_format_local` choke point via `gethostname(2)` (process-cached; the same node identity `cms/send.c` registers upward), option-clear stays the IP byte-identical. Fixes a decoded-and-ignored option whose comment wrongly claimed the hostname was already the default. `tests/test_locate_prefname.py` (default⇒IP regression / prefname⇒gethostname / one-server discriminator). STILL: only 'S' (server) entry types — 'M'/'m' (manager) typing needs BriX to answer locate as a cluster manager that lists sub-managers (it redirects instead), a CMS-topology feature |
| 19 | cms.fsxeq external program per namespace op | SHIPPED 2026-09-09 as 2.0 **F17** — the refusal below was withdrawn by operator decision | `brix_cms_fsxeq`; the fork/exec objection is answered by bounding it (`brix_subprocess_run`, own session, `closefrom(3)`, process-group kill at `brix_cms_fsxeq_timeout`) and running it off a thread pool, so a wedged program costs one pool slot, not the event loop |
...
| # | Gap | Status | Notes |
|---|-----|--------|-------|
| 1 | **Space groups**: ~~`oss.space` named groups~~ (LANDED 2026-09-06, phase-115 W3.3: `brix_oss_space <group> <prefix> [quota=…]`, `space_group_conf.c` + `write_space_group.c` — prefix-owned groups, longest wins, per-group usage/quota in `kXR_Qspace`, per-group `kXR_overQuota`, `?oss.cgroup=` placement check at write open); multi-partition alloc minfree/headroom/fuzz, `.anew` protocol, reloc between spaces | PARTIAL | groups are prefixes of the single export tree, never separate partitions (`tier.h` stays single-location) |
...
| 3 | Persistent usage ledger + quota outside pblock (pblock has full SQLite uid/gid rollup + EDQUOT; posix/S3/HTTP have statvfs only) | PARTIAL (**enforcement slice landed 2026-08-10, wave 41**) | **`brix_oss_quota_enforce on`** makes wave-29's advertised `brix_oss_quota` REAL: a kXR_write/writev/pgwrite whose length would push the export's usage past the quota is refused `kXR_overQuota` at the shared write-admission chokepoint (`write.c brix_write_within_maxsize`, the §3.9 oss.maxsize gate — zero call-site changes; the quota check runs independently of maxsize). Usage comes from the SAME probe the Qspace report advertises (`brix_query_space_probe`, exported from `query/space.c`): a driver `space` slot answers exactly (pblock catalog), plain POSIX falls back to statvfs of the export's filesystem — CONSERVATIVE on a shared mount, documented (a lab quota on a dedicated volume is the exact statvfs case). TTL-cached 5 s/worker (statvfs is cheap; the pblock catalog is SQLite — never per-write); probe FAILURE fails OPEN (quota is policy, not integrity). Default off = advertisement-only, byte-identical. `tests/test_oss_quota_enforce.py` (enforce-on admits under quota; enforce-on refuses over quota — xrdcp surfaces the `overQuota` wire error verbatim; default stays advertisement-only). **Exact-usage pblock leg (landed, wave 43):** the wave-41 deferral was a GRAMMAR trap, not lazy-init — `pblock:<dir>` (single colon) matches NO parse branch and silently ran default posix; the supported form is `pblock://<dir>?opts` (export-root machinery + `pblock.opts` sidecar). With `pblock://<ns>?quota=1g` arming the catalog space slot, the enforce gate proves EXACT usage: 48K lands, second 48K refused over the 64K `brix_oss_quota`, and after `xrdfs rm` + TTL the same upload is ADMITTED again — the discriminator that usage is the catalog, not statvfs (whole-FS used would still refuse). **Misconfig guard (landed, same wave):** an unrecognized `brix_storage_backend` scheme now FAILS `nginx -t` ("unrecognized backend scheme" + the known list) instead of silently running the default posix backend — exactly the trap that cost the wave-41 leg; the legacy single-colon `posix:<dir>`/`pblock:<dir>` spellings stay accepted. `tests/test_oss_quota_enforce.py` now 5. REMAINING: the durable per-uid/gid usage LEDGER itself (accurate per-identity deltas at every size-changing chokepoint — the true XrdOssSpace file analog) |
| 4 | ~~**Tape online-buffer purge policy** (frm_purged F1/F2): monitor logs only, never acts; MSS adapter has `purge` verb, nothing drives it~~ | **LANDED 2026-09-05 (phase-115 W3.2)** — `src/fs/backend/frm/sd_frm_purge.c` LRU pass over the `tape://` online buffer, driven by `brix_frm_purge_watermark <hi> <lo>` (filesystem arm) and/or `brix_frm_purge_max_bytes <size>` (owned-bytes cap), every `brix_frm_purge_interval`, worker-0 timer in `process_frm_purge.c`; releases only adapter-confirmed on-tape copies, skips young (<30 s) / pinned / symlinks; still no external polprog | `tests/test_phase115_tape_purge.py`; history-storage-and-caching.md §6 |
| 5 | ~~**OssArc**: dataset→zip aggregation to tape, member-indexed recall~~, backup queue | **LANDED 2026-09-06 (phase-115 W3.1)** — `tape://<adapter>/<base>?arc=<1..8>` wraps the MSS adapter in `src/fs/backend/frm/sd_frm_arc.c` (+ `sd_frm_arc_store.c`, ZIP container `frm_zip.c`): dataset = first `<depth>` path components, sealed by the `.brix-dataset-complete` marker into one stored ZIP `<ds>.brixarc.zip` on tape with sidecar `<base>/.arcidx/<ds>.idx`; member reads recall and extract one member, stat/dirlist answer from the sidecar; sealed datasets immutable (EPERM/EEXIST/EINVAL). **The backup-queue half is not implemented**: sealing is synchronous inside the marker's staged commit | `tests/test_phase115_tape_arc.py` (15) |
...
| 12 | stagemsg/StageEvents external notification file | LANDED (2.0 F2, 2026-09-08) | `brix_frm_stagemsg <file>`: one `%`-escaped line per stage-engine / prepare-registry / MSS transition (`src/fs/xfer/stage_events.c`); the in-band waiter + ledger stay as before |
...
| 8 | Per-directory stats/quota tree (dirstats/DirState/ResourceMonitor/purge-pin) | PARTIAL (**dirstats landed 2026-08-11**) | **`brix_dirstats on`**: a kXR_stat of a DIRECTORY now reports its recursive subtree byte-size (on-demand du through the seam-clean `brix_fs_walk`; `stat.c`) instead of the inode size. Off = inode size, byte-identical. Residual: the persistent incremental DirState/ResourceMonitor + per-directory QUOTA enforcement + purge-pin engine (BriX computes on demand vs stock's incremental catalog — documented divergence). |
| 9 | Direct cache access (`pss.dca` redirect-to-local-path) | MISSING (**row corrected 2026-09-05**) | The 2026-08-11 entry over-claimed: commit 3ba9890d6 added only the `brix_pss_dca` flag plumbing (registration, field, init, merge) and no consumer — `open_resolved_file.c` never issued a kXR_lclfile redirect and the string `lclfile` does not occur in `src/`. The inert directive was removed for 2.0 (phase-95 precedent for `brix_throttle_max_active_connections`); a config carrying it is now refused as an unknown directive. See the 2.0 readiness register. |
| 10 | Forwarding-proxy mode (client-named origin URL + protocol allowlist) | **LANDED 2026-09-08** (2.0 F5) | `brix_storage_backend forward://root[,roots] permit=<host\|.suffix>…` — the `xroot_fwd` driver (`src/fs/backend/xroot/sd_xroot_fwd*.c`) resolves `/root://host:port//file` keys to one sd_xroot child per admitted origin; the host verdict IS the TPC egress guard's `brix_tpc_host_pattern_match`; a missing permit list is a config error (no open relay); `tests/test_release20_forward_proxy.py` + `tests/test_sd_xroot_fwd_key.py` |
...
| 13 | Per-open CGI blocksize/prefetch override (`pfc.urlcgi`) | **LANDED 2026-09-08** (2.0 F5) | `brix_cache_urlcgi [blocksize {ignore\|<min> <max>}] [prefetch {ignore\|<min> <max>}]` arms and clamps `pfc.blocksize=` / `pfc.prefetch=` per open; `pfc.` is a typed namespace under `brix_opaque_strict`; `tests/test_release20_cache_urlcgi.py` |
| 14 | pss.permit host ACL, pss.persona mapped identity (BriX forwards REAL identity instead — arguably better), pss.reproxy, root:// origin connection pool (per-fill bootstrap today) | **pss.permit LANDED 2026-09-08** as `permit=` on the forward:// line (row 10); persona/reproxy/pool stay MISSING/low | pool matters only if origin-open latency shows up |
...
| 4 | VOMS: ~~FQAN-pattern→user mapfile (XrdVomsMapfile)~~ **LANDED 2026-08-11**; vomsfun certfmt/grpopt/vos/grps filters; global "AC required" independent of path rules | PARTIAL | A VOMS FQAN now maps to a local account with PRECEDENCE over the raw DN, resolved against the (now wildcard-capable) `brix_idmap` grid-mapfile — an FQAN key (`"/atlas/Role=production"`, glob-capable) or a DN key both match; the first FQAN in credential order that maps wins (`auth_gate.c brix_acc_gate_map_name`, iterating `id->vo_list`). BriX unifies the two stock mapfiles (gridmap+vomsmapfile) into one quoted-key file since both keys are `/`-delimited. Residual: the vomsfun certfmt/grpopt/vos/grps FILTER matrix + global AC-required. Test: `tests/c/idmap_test.c` case 5b (FQAN key → account). |
...
| 6 | sss v2 entity breadth: vorg/role/caps/**endorsements**/attr pairs/proxied creds not parsed; XrdSecsssID per-connection registry (multiplexing proxies); --getcreds | **CLOSED 2026-09-09 (release-2.0 F9)** | `src/protocols/root/protocol/sss.h`, `src/core/compat/sss_entity.c`, `client/lib/auth/sss/sss_id.h`; `brix_sss_getcreds` is the `--getcreds` equivalent. Attribute *pairs* remain out: xrootd's own sss does not put them on the wire either |
| 7 | pwd extras: auto-registration `-a:`, syspwd, per-user $HOME files, cryptfile, client-verification `-vc`, maxfail, cred export, **xrdpwdadmin tool** | PARTIAL (**maxfail landed 2026-08-11**) | **`brix_auth_maxfail <n>`**: the per-connection failed-auth cap (pwd/gsi/…) is now configurable (`gsi/auth.c`; <=0/unset keeps the built-in 10). Residual (legacy pwd relaxations): auto-registration `-a:`, syspwd/`/etc/shadow`, per-user \$HOME files, cryptfile, `-vc`, cred export, xrdpwdadmin. |
| 8 | GSI options: ~~`-gmapfun` DN-regex mapping (exact-DN file only)~~ **DN pattern mapping LANDED 2026-08-11**, gmapopt mode matrix, `-exppxy` file-export templating, `-authzpxy`, ca `noverify/verifyss`, `-crlext`, md list directive, trustdns/showdn/moninfo | PARTIAL | The grid-mapfile now matches DN **patterns** — a quoted key carrying a glob metacharacter (`*`/`?`/`[`) is `fnmatch(3)`-matched against the DN, closing the "exact-DN file only" gap so a whole CA/OU subtree maps to one account. Exact keys win first, then patterns in file order (`idmap_gridmap.c idmap_gridmap_lookup`). Residual (mostly relaxations BriX chose not to have): `gmapopt` mode matrix, `-exppxy` proxy-export templating, `-authzpxy`, the CA-relaxation knobs. |
...
| 11 | authdb residual grammar: `v`/`l` selectors in compound ids, `x` privilege | **CLOSED 2026-09-10 (release-2.0 F20)** | Field 1 is a SET of up to six distinct, AND-ed selectors from `u g p a v l`; a compound id splits on `\|` into one component per selector (a single-selector rule keeps its id verbatim, because DNs carry punctuation), and `v`+`l` are paired POSITIONALLY over the index-aligned VOMS attribute CSVs, so a cross-tuple credential cannot satisfy a paired rule. `x` = `BRIX_AUTH_STAGE`, demanded by `kXR_prepare -s`/`-e` and by the VFS `STAGE`/`EVICT` mutations (before 2.0 both sat in the UPDATE range, so `w` implied staging). Stricter than xrootd on purpose: an unknown selector or privilege letter, a repeat, a bad arity or an empty id component refuses the LINE at `nginx -t` instead of dropping the character — deferred to merge time so `brix_authdb_engine xrdacc` files are untouched. `src/auth/authz/authdb_grammar.c`, `authdb.c`; `tests/test_release20_authdb_residuals.py` (21). Two defects found on the way: the VOMS role never reached the identity at all (fixed with a raw-FQAN CSV channel), and `v`+`l` were crossing (fixed by `adb_pair_matches`) |
...
| 5 | `http.header2cgi` arbitrary header→CGI bridge | PARTIAL (**landed 2026-08-11**) | **`brix_webdav_header2cgi <HTTP-header> <cgikey>`** (repeatable) maps an incoming request header into the xrd opaque the authz gate + backend read, as `&<cgikey>=<value>` (`xrdhttp.c xrdhttp_apply_header2cgi`, after the ?xrd.* parse, bounded to XRDHTTP_OPAQUE_MAX). |
...
| 7 | ofs.tpc identity matrix: `allow dn/group/host/vo`, `require {all|client|dest} <auth>`, `restrict <path>`, `oids` | **SHIPPED 2026-09-09 as 2.0 F18** | `brix_tpc_allow_identity <dn|group|host|vo> <pattern>` (repeatable, a rule's selectors AND together, rules OR), `brix_tpc_require <all|client|dest> <auth>[,<auth>…]`, `brix_tpc_restrict <path>` and `brix_tpc_oids on|off`. One pure core (`src/tpc/common/identity_matrix.c`) behind one choke point per plane — `tpc_matrix_gate()` in `protocols/root/read/open_tpc.c` (all four native roles, before any dial) and `webdav_tpc_matrix_gate()` in `protocols/webdav/tpc.c`. Stage order is the security order: oids → allow → require → restrict; every stage default-PERMIT when unconfigured (a no-op for non-adopters) and fail-CLOSED once configured, `oids` being the one unconditional default-DENY. A leg carrying `tpc.org` was opened by the peer SERVER (`dest` party); one without it by the initiating CLIENT — so `require dest` is not satisfiable by a client credential. `restrict` is NARROWER than stock: component-boundary match, so `/data` never admits `/database`. This is a policy layer INSIDE the host-plane `brix_tpc_source_guard`/`_allow` confinement — it can only narrow, never widen. `tests/test_tpc_identity_matrix_unit.py` (C core, 20 cases) + `tests/test_release20_tpc_identity_matrix.py` (33 live/grammar cases) |
| 8 | Native TPC multi-stream (`ofs.tpc streams`) — `brix_tpc_streams` caps the client's `tpc.str`, `kXR_bind` sub-streams, rounds of 1MiB reads (2.0 F7); push multi-stream N/A (no native push) | PRESENT | HTTP pull has it too |
...
| 11 | Redirect collapse / VirtualRedirector registry client-side (also the metalink hook); async engine doesn't follow redirects itself | PARTIAL (**registry landed 2026-08-11**) | A client-side redirect-collapse / VirtualRedirector registry (`client/lib/net/redir_registry.c`): records URL→target with transitive COLLAPSE (A→B→C answers A→C), cycle-safe (refuses a mapping that would loop) + LRU-bounded, feeding repeated opens straight to the final endpoint. `client/tests/c/ops_dsl_redir_test.c` (5). Residual: wiring the async engine to FOLLOW a mid-op redirect through this registry. |
...
| 16 | Declarative ops DSL (XrdClOperations) | PARTIAL (**core landed 2026-08-11**) | A declarative operation-pipeline core (`client/lib/ops/opdsl.c`) — the XrdClOperations Then/Else model in C: compose steps as data, execute in order, SHORT-CIRCUIT on the first failure into an optional `otherwise` handler. `client/tests/c/ops_dsl_redir_test.c` (3, incl. short-circuit + otherwise). Residual: the full typed operation set (Open|Read|Write|…) as first-class builders over this engine. |
...
15. Long tail: VOMS mapfile, SciTokens rule mapfile, sss v2 entity/ID registry,
    ...
    **CLOSED 2026-09-09 as 2.0 F18** (§6 row 7),
