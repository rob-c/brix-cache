...
(NOTE, found this wave: the whole `brix_cache_advertise` Pelican-federation
family — enable/key/data_url/web_url/sitename/issuer_url/interval/namespace — is
declared, merged, and read by `fs/cache/origin/pelican_register.c` but NO
directive registers any of it: `brix_cache_advertise` is an "unknown directive".
`brix_sitename` reuses the orphaned `advertise.sitename` slot, so wiring the rest
of that family later reuses the same field. Wiring the full advertiser is its own
task — the federation registration is currently unreachable.) brix-xrdcp now
...
carrying forward: the whole `brix_cache_advertise` Pelican-federation family is
**declared, merged, and read by `pelican_register.c` but registered by NO
directive** — federation registration is currently unreachable, a self-contained
config-wiring task; and the data-plane request-flag surface (open mkpath/retstat,
...
| 9 | Direct cache access (`pss.dca` redirect-to-local-path) | PARTIAL (**landed 2026-08-11**) | **`brix_pss_dca on`**: a cache-resident file opened by a kXR_lclfile-capable client is redirected to its local path for direct off-FS reads (`open_resolved_file.c`); strictly gated on the client's advertised lclfile ability (0x40) + opt-in, read-opens only ⇒ zero effect on other clients. lclfile redirect body-encoding is oracle-limited (documented). |
