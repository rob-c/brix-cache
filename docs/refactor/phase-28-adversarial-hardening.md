...
| # | File:line | Issue | Sev | Guard |
|---|-----------|-------|-----|-------|
| C1 | `src/net/manager/registry.c`, `src/net/cms/server_recv.c:306` | A data node self-reports `host:port:paths` at CMS login; the manager later **redirects clients to that host** (`src/protocols/root/read/locate.c:60,78,116`). A rogue/spoofed node registers an attacker host → clients (and their credentials) are redirected to the attacker. | High | Authenticate CMS registration (shared secret / mTLS on the cms port); validate the advertised host against an allowlist / the peer's connecting IP; rate-limit + alert on registration churn |
| C2 | `src/protocols/root/read/locate.c:116` | Redirect host string is emitted from registry data onto the wire — ensure it cannot contain control bytes / be a different scheme. | Low | Validate host syntax before emit (reuse `xrootd_sanitize_log_string`-style validation for wire host) |
...
| D2 | cross-cutting | **Deny-by-default audit:** every handler must reach a default-deny if no auth/ACL rule matches. The global `allow_write` gate is correctly checked before serving writes (`src/protocols/root/read/open_request.c:253`) — extend the same "explicit allow required" audit to *every* op (stat, locate, dirlist, fattr, query). | Med | Matrix test: each op × (no-auth, wrong-scope, wrong-VO) must 403/NotAuthorized |
