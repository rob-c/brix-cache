...
**Status:** HISTORICAL DESIGN / IMPLEMENTED (reconciled 2026-09-03). Later work
wired Bwm reservation and removed the inert `charge_io`, userconfig and
max-active engines rather than exposing dead configuration. The pgwrite
zero-recompute proposal is closed as incompatible with the later xmeta-P3
unification: wire CRCs cover 4 KiB pgwrite fragments, while CSI records cover
configurable cinfo blocks (normally 1 MiB). The transport CRC is still verified
during decode; CSI correctly computes the checksum for its different storage
extent. The §0.IMPL table is preserved as the chronological record.
**Date:** 2026-06-26
...
| **PR-3 W2a** | CSI page-checksum tagstore | ✅ **COMPLETE — engine + full data plane, functionally verified end-to-end.** `fs/backend/csi_tagstore.{c,h}` + `csi_verify.{c,h}` (unit-tested 11/11), `CAP_FSCS`, directives/fields/merge. **Handle lifecycle:** `open_resolved_file.c` opens/attaches the tagstore, `fd_table.c` closes+frees it. **Data plane wired on ALL stream read/write paths** via `brix_vfs_job_t.csi`: warm-cache fast path, sync inline, AND AIO thread-pool read-verify + write/pgwrite tag-update; **sendfile is disabled for CSI handles (ADR-6)** so cleartext reads verify. The later xmeta-P3 record uses configurable block granules rather than the wire's fixed 4 KiB page, so pgwrite CRCs are verified in the decoder and CSI folds/recomputes its own correctly-sized block CRCs. **Live e2e:** clean read matches and a corrupted on-disk block fails instead of serving corruption. HTTP/S3 sendfile reads do not verify by design (ADR-6). |
| **PR-4 W2b** | hole/require/loose options + scrub | ✅ **COMPLETE.** Options wired (`csi_fill`/`csi_require`/`csi_loose` honored: fill flag in the header, require refuses untagged reads at open, strict/loose drives the RMW path). **Paced background scrub LANDED:** pure engine `fs/backend/csi_scrub.{c,h}` (libc + xmeta + crc32c, no nginx) re-verifies every recorded per-block CRC32c of every tagged file under the export root against disk — surfacing at-rest rot the hot read path never sees (it only verifies blocks a read fully spans). Fail-open on coverage gaps (unset slot / short-on-disk block skipped), fail-closed only on an actual CRC difference. Unit-tested standalone **12/12** (`tests/c/test_csi_scrub.c`: clean pass, on-disk block-1 poke → MISMATCH reported once, untagged → NOTAGS, absent path no-crash, recursive walk surfaces subdir rot, budget cap, missing root no-op). Wired as a per-server maintenance timer (`brix_csi_scrub_handler` in `process_timers.c`, armed by `brix_init_server_csi_scrub_timer` in `process_server_init.c`) driven by directive **`brix_csi_scrub_interval <time>`** (default 0 = off); one full walk per interval (the interval IS the pacing — never a hot self-rearm), mismatches folded into new metric `brix_csi_scrub_mismatch_total` + an `error.log` DIAG per corrupt block. |
...
> **IMPLEMENTED (W2b, 2026-07-29).** Shipped directive is **`brix_csi_scrub_interval
> <time>`** (default 0 = off). The pure engine lives in `fs/backend/csi_scrub.{c,h}`
> (libc + `fs/meta/xmeta` + crc32c, no nginx — unit-tested standalone in
> `tests/c/test_csi_scrub.c`, 12/12); it re-verifies the per-block CRC32c recorded
> in each file's unified **xmeta** record (the `BLOCKCRC` section — the checksums
> the data plane already writes), NOT a separate `.xrdt` sidecar. The nginx caller
> is a per-server maintenance timer (`brix_csi_scrub_handler` in
> `src/core/config/process_timers.c`, armed by `brix_init_server_csi_scrub_timer`
> in `process_server_init.c`): one full same-device walk of `common.root_canon`
> per interval, re-armed at the operator's cadence — the interval is the pace.
> Fail-open on coverage gaps (an unset block slot or a block short on disk is
> skipped, never failed); fail-closed only on an actual CRC difference. Mismatches
> increment the new per-server metric **`brix_csi_scrub_mismatch_total`** and emit
> one `BRIX_DIAG_CRIT` line per corrupt block (path + block index + recorded vs
> on-disk CRC).
