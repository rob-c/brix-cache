...
|---|---|---|---|
| ENOENT | kXR_NotFound (3004) | 404 | File not found |
| EACCES/EPERM | kXR_NotAuthorized (3009) | 403 | Permission denied |
| EINVAL | kXR_ArgInvalid (3010) | 400 | Invalid argument |
| EIO | kXR_IOError (3012) | 500 | I/O error |
| ENOMEM | kXR_NoMemory (3013) | 507 | Out of memory |
...
|---|---|---|---|---|
| 3000 | kXR_protocol | dispatch.c | ServerResponseHdr { status, dlen } | — |
| 3001 | kXR_open | read/open.c | ServerOpenBody + statbuf? | errno → kXR_* |
| 3002 | kXR_read | read/read.c | ServerReadBody { data_len, data } | errno → kXR_* |
| 3005 | kXR_write | write/write.c | ServerWriteBody { status, dlen } | errno → kXR_* |
| 3006 | kXR_close | close.c | ServerCloseBody { status, dlen } | errno → kXR_* |
| 3009 | kXR_login | session/login.c | ServerLoginBody { status, dlen } | errno → kXR_* |
| 3012 | kXR_readv | read/readv.c | ServerReadvBody { per-segment results } | errno → kXR_* |
| 3015 | kXR_sync | write/sync.c | ServerSyncBody { status, dlen } | errno → kXR_* |
| 3016 | kXR_stat | stat.c | ServerStatBody { ino, size, flags, mtime } | errno → kXR_* |
| 3017 | kXR_dirlist | dirlist/handler.c | ServerDirListBody { count, entries[] } | errno → kXR_* |
| 4007 | kXR_pgread | pgread.c | ServerPgReadBody { per-page CRC32c } | errno → kXR_* |
| 4008 | kXR_pgwrite | pgwrite.c | ServerPgWriteBody { status, dlen } | errno → kXR_* |
