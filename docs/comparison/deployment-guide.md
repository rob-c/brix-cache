...
| Sub-manager / three-tier hierarchy | **Partially** — the module can be a sub-manager (running CMS client + CMS server), but `kYR_select` / `kYR_try` / `kYR_redirect` (manager-directed lateral redirect) is not implemented |
...
> **Verdict:** Two-tier topologies work. Multi-tier sub-manager topologies need
