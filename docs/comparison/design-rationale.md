...
Upstream XRootD is active and still the reference implementation. The official
...
XRootD's legacy monitoring ecosystem is historically important, especially for
WLCG accounting. But UDP monitoring and site-specific collectors are not the
most convenient primitive for current operations teams.
...
| low-to-mid parallel `davs://` | nginx-xrootd is very competitive and wins several measured concurrency points |
| high parallel native `root://` | nginx-xrootd overtakes in the measured table as concurrency rises |
| high parallel WebDAV | nginx-xrootd wins the measured 32/64 concurrency points and keeps p95 lower in the README table |
