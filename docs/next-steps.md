...
The current XRootD data-server opcode set is implemented and the legacy
`kXR_gpfile` opcode is explicitly rejected as unsupported. The module is
functionally complete for the WLCG-style POSIX data-server deployment it targets.
This document describes the next wave of work: making the codebase easier for
new contributors to read, navigate, extend, and debug.
...
- [x] **`docs/types.md`** — Core type reference: `xrootd_ctx_t` field groups,
...
- [x] **`docs/handler-reference.md`** — Added WebDAV handler building blocks
...
- [x] **`docs/quirks.md`** — Added WebDAV COPY routing disambiguation (§15),
