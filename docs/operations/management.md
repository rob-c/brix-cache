...
#### Checksum cancel (`QueryCode.CHECKSUMCANCEL`)

Reference XRootD uses `kXR_Qckscan` to cancel an asynchronous checksum scan.
nginx-xrootd computes checksums synchronously, so this query validates the path
and returns success as a no-op cancellation.
...
no-ops; they do not trigger origin fills or evict read-through cache entries.
