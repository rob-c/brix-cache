...
int main(void) {
     ...
     *          ATOMIC_TEMP and TRUNCATE caps are absent. */
    {
        ...
        assert((brix_vfs_get_caps(w) & XRDC_VFS_CAP_FADVISE)      != 0);  /* fadvise ok */
        assert(brix_vfs_pwrite(w, 512, "BLK", 3, &st) == 0);
        assert(brix_vfs_commit(w, &st) == 0);
        brix_vfs_close(w);
    ...
    /* Test 2 (error): open of a non-existent device path for WRITE fails cleanly. */
    {
        ...
        brix_vfs_open_opts o = { .io_uring = 0, .expected_size = -1, .cred = NULL };
        int rc = brix_vfs_open("block:///tmp/vfs_blk_no_such_device_xrdc",
