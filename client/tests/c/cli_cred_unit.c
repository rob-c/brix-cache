 ...
 *
 * WHAT: Unit tests for brix_cli_cred_store_build (cli_cred.c): the builder
 *       that maps per-tool CLI values into an brix_cred_store without requiring
 *       callers to know brix_cred_config field names.
 ...
 * HOW:  Calls brix_cli_cred_store_build directly (no server), isolates the
 *       environment before each test, and inspects via brix_cred_available +
 *       brix_cred_acquire.  Pattern matches client/tests/c/cred_unit.c.
 *
...
#include "cred.h"     /* brix_cred_store, brix_cred_available, brix_cred_acquire,
 ...
 *       brix_cred_store_new picks them up at the highest-precedence level.
