...
static struct sockaddr_storage g_bind_ss;
static socklen_t               g_bind_len = 0;
...
static __thread unsigned long t_conn_id = 0;
...
apply_command(char *line, char *reply, size_t rsz)
{
     ...
     * grammar and re-run, so no command is defined twice. */
    {
        const char *j = line;
        while (*j == ' ' || *j == '\t') j++;
        if (*j == '{') {
            char verbline[2000];
            if (fp_json_to_verb(j, verbline, sizeof verbline) != 0) {
                if (reply && rsz) {
                    snprintf(reply, rsz, "err: bad json command\n");
                }
                return;
            }
    ...
    if (strcmp(verb, "event-log") == 0) {
        if (args[0] == '\0') {
            if (reply && rsz) {
                snprintf(reply, rsz, "err: usage: event-log <path>\n");
            }
        } else if (fp_event_open(args) != 0) {
            if (reply && rsz) {
                snprintf(reply, rsz, "err: cannot open '%s'\n", args);
            }
        } else if (reply && rsz) {
            snprintf(reply, rsz, "ok: event-log %s\n", args);
        }
        return;
    }
    if (strcmp(verb, "toxic") == 0) {
        fp_toxic_cmd(args, reply, rsz);
        return;
    }
    if (strcmp(verb, "route") == 0) {
