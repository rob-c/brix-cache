 ...
 *       brix_kxr_to_errno. Open handles live in a malloc'd lg_xfs_handle in fi->fh.
 *
...
brix_url        lg_url;
brix_opts       lg_opts;
int             lg_max_conns = 8;   /* metadata pool size; --max-conns */

/* Kernel-side caching policy (set on the FUSE connection in lg_xfs_init). The kernel
...
double          lg_attr_timeout  = 1.0;   /* --attr-timeout  */
double          lg_entry_timeout = 1.0;   /* --entry-timeout */
int             lg_kernel_cache  = 0;     /* --kernel-cache  */
int             lg_xattr         = 0;     /* --xattr: enable getxattr/etc. */
...
size_t          lg_readahead = 1024 * 1024;   /* --readahead */
size_t          lg_writeback = 1024 * 1024;   /* --writeback */
...
static ssize_t
lg_xfs_io_pread(void *be, int64_t off, void *buf, size_t n, brix_status *st)
{
    lg_xfs_handle *h = be;
    return brix_rfile_pread(&h->f, off, buf, n, st);
}
static int
lg_xfs_io_pwrite(void *be, int64_t off, const void *buf, size_t n, brix_status *st)
{
    lg_xfs_handle *h = be;
...
static int
lg_xfs_flush_wbuf(lg_xfs_handle *h, brix_status *st)
{
    return brix_iobuf_flush(&h->io, st);
}

/* The error mapping, the conn-health test, and the pooled metadata-op runner are
 * shared with the async driver in lib/fuse_ops.c.  lg_xfs_err/lg_xfs_conn_healthy stay
 * as thin local wrappers (used throughout the read/write/open paths); the
 * metadata ops below call brix_fuse_run(lg_pool, 0, 0, 0,…) — max_retries 0 is exactly
 * this simple driver's single checkout → op → checkin → map behaviour. */
int
lg_xfs_conn_healthy(const brix_status *st)
{
    return brix_fuse_conn_healthy(st);
}

int
lg_xfs_err(const brix_status *st)
{
...
static void
lg_xfs_fill_stat(const brix_statinfo *si, struct stat *stbuf)
{
...
static int
lg_xfs_getattr(const char *path, struct stat *stbuf, struct fuse_file_info *fi)
{
    brix_statinfo si;
    brix_status   st;
    int           rc;
    ...
    rc = brix_fuse_run(lg_pool, 0, 0, 0,brix_fuse_op_stat, &a, &st);
    if (rc != 0) {
        return rc;
    }
    lg_xfs_fill_stat(&si, stbuf);
    return 0;
}

static int
lg_xfs_readdir(const char *path, void *buf, fuse_fill_dir_t filler, off_t offset,
            struct fuse_file_info *fi, enum fuse_readdir_flags flags)
{
    ...
    rc = brix_fuse_run(lg_pool, 0, 0, 0,brix_fuse_op_dirlist, &a, &st);
    if (rc != 0) {
        return rc;
    }
    filler(buf, ".", NULL, 0, 0);
...
static lg_xfs_handle *
lg_xfs_handle_new(int *err)
{
    lg_xfs_handle *h = calloc(1, sizeof(*h));
    brix_status st;
    ...
    brix_status_clear(&st);
    if (brix_connect(&h->conn, &lg_url, &lg_opts, &st) != 0) {
...
static void
lg_xfs_handle_free(lg_xfs_handle *h)
{
    brix_close(&h->conn);
    pthread_mutex_destroy(&h->lock);
...
static int
lg_xfs_open(const char *path, struct fuse_file_info *fi)
{
    lg_xfs_handle *h;
    brix_status st;
    ...
    if (rc != 0) {
        lg_xfs_handle_free(h);
        return lg_xfs_err(&st);
    }
    brix_iobuf_init(&h->io, h, lg_xfs_io_pread, lg_xfs_io_pwrite, h->writable,
                    lg_readahead, lg_writeback);
    fi->fh = (uint64_t) (uintptr_t) h;
    return 0;
}

static int
lg_xfs_create(const char *path, mode_t mode, struct fuse_file_info *fi)
{
    lg_xfs_handle *h;
    brix_status st;
    int         rc, err = 0;
...
static int
lg_xfs_read(const char *path, char *buf, size_t size, off_t offset,
         struct fuse_file_info *fi)
{
    lg_xfs_handle *h = (lg_xfs_handle *) (uintptr_t) fi->fh;
    brix_status st;
    ssize_t     r;
    ...
    return r < 0 ? lg_xfs_err(&st) : (int) r;
}

static int
lg_xfs_write(const char *path, const char *buf, size_t size, off_t offset,
          struct fuse_file_info *fi)
{
    lg_xfs_handle *h = (lg_xfs_handle *) (uintptr_t) fi->fh;
    brix_status st;
    ...
    pthread_mutex_unlock(&h->lock);
    return rc != 0 ? lg_xfs_err(&st) : (int) size;
}

static int
lg_xfs_release(const char *path, struct fuse_file_info *fi)
{
    lg_xfs_handle *h = (lg_xfs_handle *) (uintptr_t) fi->fh;
    brix_status st;
    ...
    brix_status_clear(&st);
    pthread_mutex_lock(&h->lock);
    (void) lg_xfs_flush_wbuf(h, &st);   /* persist buffered writes before close */
    (void) brix_rfile_close(&h->f, &st);
    pthread_mutex_unlock(&h->lock);
    lg_xfs_handle_free(h);   /* closes the dedicated conn + destroys the lock */
    fi->fh = 0;
    return 0;
}
...
static int
lg_xfs_flush(const char *path, struct fuse_file_info *fi)
{
    lg_xfs_handle *h = (lg_xfs_handle *) (uintptr_t) fi->fh;
    brix_status st;
    int         rc;
    ...
    brix_status_clear(&st);
    pthread_mutex_lock(&h->lock);
    rc = lg_xfs_flush_wbuf(h, &st);
    pthread_mutex_unlock(&h->lock);
    return rc != 0 ? lg_xfs_err(&st) : 0;
}

static int
lg_xfs_fsync(const char *path, int datasync, struct fuse_file_info *fi)
{
    lg_xfs_handle *h = (lg_xfs_handle *) (uintptr_t) fi->fh;
    brix_status st;
    int         rc;
    ...
    brix_status_clear(&st);
    pthread_mutex_lock(&h->lock);
    rc = lg_xfs_flush_wbuf(h, &st);                       /* flush buffered writes */
    if (rc == 0) {
...
static int
lg_xfs_mkdir(const char *path, mode_t mode)
{
    brix_status st;
    brix_status_clear(&st);
    struct brix_fuse_ctx_mkdir a = { path, (int) (mode & 0777) };
    return brix_fuse_run(lg_pool, 0, 0, 0,brix_fuse_op_mkdir, &a, &st);
}

static int
lg_xfs_unlink(const char *path)
{
    brix_status st;
    brix_status_clear(&st);
    return brix_fuse_run(lg_pool, 0, 0, 0,brix_fuse_op_rm, (void *) path, &st);
}

static int
lg_xfs_rmdir(const char *path)
{
    brix_status st;
    brix_status_clear(&st);
    return brix_fuse_run(lg_pool, 0, 0, 0,brix_fuse_op_rmdir, (void *) path, &st);
}

static int
lg_xfs_rename(const char *from, const char *to, unsigned int flags)
{
    brix_status st;
    if (flags != 0) {
    ...
    return brix_fuse_run(lg_pool, 0, 0, 0,brix_fuse_op_mv, &a, &st);
}

static int
lg_xfs_chmod(const char *path, mode_t mode, struct fuse_file_info *fi)
{
    brix_status st;
    (void) fi;
    brix_status_clear(&st);
    struct brix_fuse_ctx_chmod a = { path, (int) (mode & 0777) };
    return brix_fuse_run(lg_pool, 0, 0, 0,brix_fuse_op_chmod, &a, &st);
}

static int
lg_xfs_truncate(const char *path, off_t size, struct fuse_file_info *fi)
{
    brix_status st;
    (void) fi;
    brix_status_clear(&st);
    struct brix_fuse_ctx_trunc a = { path, (int64_t) size };
    return brix_fuse_run(lg_pool, 0, 0, 0,brix_fuse_op_trunc, &a, &st);
}

/* chown/utimens succeed as no-ops so tools like `cp -p` don't abort the copy. */
static int
lg_xfs_chown(const char *path, uid_t uid, gid_t gid, struct fuse_file_info *fi)
{
...
static int
lg_xfs_utimens(const char *path, const struct timespec tv[2],
            struct fuse_file_info *fi)
{
...
static int
lg_xfs_statfs(const char *path, struct statvfs *stbuf)
{
    char               text[1024];
    brix_status        st;
    brix_conn         *c;
...
static int
lg_xfs_access(const char *path, int mask)
{
    brix_statinfo si;
    brix_status   st;
    brix_conn    *c;
...
static void *
lg_xfs_init(struct fuse_conn_info *conn, struct fuse_config *cfg)
{
    (void) conn;
    cfg->attr_timeout     = lg_attr_timeout;
    cfg->entry_timeout    = lg_entry_timeout;
    cfg->negative_timeout = lg_attr_timeout;
    cfg->kernel_cache     = lg_kernel_cache;
    cfg->use_ino          = 1;
    return NULL;
}

const struct fuse_operations lg_xfs_ops = {
    .init     = lg_xfs_init,
    .getattr  = lg_xfs_getattr,
    .readdir  = lg_xfs_readdir,
    .open     = lg_xfs_open,
    .create   = lg_xfs_create,
    .read     = lg_xfs_read,
    .write    = lg_xfs_write,
    .flush    = lg_xfs_flush,
    .release  = lg_xfs_release,
    .fsync    = lg_xfs_fsync,
    .mkdir    = lg_xfs_mkdir,
    .unlink   = lg_xfs_unlink,
    .rmdir    = lg_xfs_rmdir,
    .rename   = lg_xfs_rename,
    .chmod    = lg_xfs_chmod,
    .truncate = lg_xfs_truncate,
    .chown    = lg_xfs_chown,
    .utimens  = lg_xfs_utimens,
    .statfs   = lg_xfs_statfs,
    .access   = lg_xfs_access,
    .getxattr    = lg_xfs_getxattr,
    .setxattr    = lg_xfs_setxattr,
    .listxattr   = lg_xfs_listxattr,
    .removexattr = lg_xfs_removexattr,
};
