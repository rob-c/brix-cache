 ...
 *       Implements brix_cred_bearer() returning the XRDC_CRED_BEARER handler with
 ...
 *         > cfg->bearer_path   (read that file, symlink-safe via brix_open_credfile)
 *         > brix_token_discover() (env/default: $BEARER_TOKEN → $BEARER_TOKEN_FILE
 ...
 *       task C2 where brix_cred_autorefresh is adapted to the (cfg, st) contract.
 *
 ...
 *
 * WHAT: open `path` via brix_open_credfile (O_NOFOLLOW, owned-by-euid, non-secret)
 ...
 * HOW:  brix_open_credfile returns an fd (caller closes); fdopen + fread;
 ...
 *
 * WHAT: cfg->bearer_literal > cfg->bearer_path > brix_token_discover().
 ...
 */
static char *
obtain_token(const brix_cred_config *cfg)
{
 ...
 * WHY:  used by auth pre-flight diagnostics and brix_cred_available(); must agree
 *       with what acquire() would do.
 * HOW:  obtain_token then free; 1 if non-NULL.
 */
static int
bearer_available(const brix_cred_config *cfg)
{
 ...
 *       3) fill *out;
 *       4) brix_token_meta_get for not_after (0 if not a JWT or exp absent);
 *       5) return 0 on success, -1 + XRDC_EAUTH on no token.
 */
static int
bearer_acquire(const brix_cred_config *cfg, brix_cred_view *out,
               int64_t *not_after, brix_status *st)
{
    char            *tok;
    brix_token_meta  meta;
    ...
    brix_token_meta_get(g_tok, &meta);
...
/* handler accessor */
static const brix_cred_handler s_bearer_handler = {
...
/*
 * brix_cred_bearer — return the static bearer-token handler.
 *
