...
#include "auth/gsi/proxy_req.h"    /* brix_gsi_sign_pxyreq (X.509 delegation)   */
#include "protocols/root/protocol/gsi.h"     /* kXGS_pxyreq / kXGC_sigpxy / kXRS_* constants */
#include "core/compat/crypto.h"    /* brix_sha256 (libxrdproto) */
...
gsi_have(brix_conn *c)
{
    ...
    if (c != NULL && c->opts.cred != NULL
        && brix_cred_available(c->opts.cred, XRDC_CRED_X509_PROXY)) {
        return 1;
    }
 ...
 * opened through brix_credfile_bio (secret=1: no symlink, owned by euid, 0600) —
 * the shared safe reader in proxy.c. Probe-quiet: NULL just means "no usable
 * proxy" and the auth driver moves on. */
#define proxy_bio(path) brix_credfile_bio((path), 1)
          ...
          brix_status *st)
{
    ...
    if (!brix_gsi_rand(client_rtag, sizeof(client_rtag))) {
        brix_status_set(st, XRDC_EAUTH, 0, "gsi: RNG failed");
        return -1;
    }
...
static void
gsi_resolve_proxy(brix_conn *c, char *proxy, size_t sz, brix_status *st)
{
    if (c != NULL && c->opts.cred != NULL) {
        brix_cred_view v;
        if (brix_cred_acquire(c->opts.cred, XRDC_CRED_X509_PROXY, 0, &v, st) == 0
            && v.path != NULL) {
 ...
 * brix_gsi_sign_pxyreq consumes.  0 / -1. */
static int
 ...
 */
static int
gsi_sigpxy(brix_conn *c, const uint8_t *sbody, uint32_t slen, uint8_t **payload,
           uint32_t *plen, brix_status *st)
{
    ...
    if (brix_gsi_find_bucket(sbody, slen, (uint32_t) kXRS_main, &enc, &enclen)
        != 0) {
    ...
    if (brix_gsi_find_bucket(plain, plainlen, (uint32_t) kXRS_x509_req,
                              &reqpem, &reqpemlen) != 0
        || gsi_req_pem_to_der(reqpem, reqpemlen, &reqder, &reqderlen) != 0) {
        brix_status_set(st, XRDC_EAUTH, 0, "gsi: pxyreq x509_req missing/bad");
        free(plain);
        return -1;
    }
    free(plain);
    ...
    brix_gbuf_start(&inner, (uint32_t) kXGC_sigpxy);
    brix_gbuf_bucket(&inner, (uint32_t) kXRS_x509, signed_pem, siglen);
    brix_gbuf_end(&inner);
    free(signed_pem);
    if (!inner.err) {
    ...
    if (enc2 == NULL) {
        brix_status_set(st, XRDC_EAUTH, 0, "gsi: sigpxy main encrypt failed");
        return -1;
    }

    /* 4. Outer kXGC_sigpxy = { kXRS_main(enc) + kXRS_cipher_alg }. */
    brix_gbuf_start(&outer, (uint32_t) kXGC_sigpxy);
    brix_gbuf_bucket(&outer, (uint32_t) kXRS_main, enc2, enc2len);
    brix_gbuf_bucket(&outer, (uint32_t) kXRS_cipher_alg, c->gsi_deleg_cipher,
                       strlen(c->gsi_deleg_cipher));
    brix_gbuf_end(&outer);
    free(enc2);
    if (outer.err) {
        brix_gbuf_free(&outer);
        brix_status_set(st, XRDC_EAUTH, 0, "gsi: sigpxy assembly failed");
        return -1;
    }
 ...
 * gsi_core kernel (brix_gsi_build_cert_response) so the native client and the
 ...
 * configured proxy file) and maps the kernel's error string onto brix_status.
 */
static int
gsi_more(brix_conn *c, const uint8_t *sbody, uint32_t slen, uint8_t **payload,
         uint32_t *plen, brix_status *st)
{
    char      proxy[512];
    uint8_t  *proxy_pem;
    ...
    if (proxy_key == NULL || proxy_pem == NULL) {
        free(proxy_pem);
        EVP_PKEY_free(proxy_key);
        brix_status_set(st, XRDC_EAUTH, 0, "gsi: cannot load proxy credential");
        return -1;
    }
    ...
    err[0] = '\0';
    rc = brix_gsi_build_cert_response_ex(sbody, slen, proxy_pem, proxy_pem_len,
