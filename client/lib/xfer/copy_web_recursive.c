/*
 * copy_web_recursive.c - recursive WebDAV/S3 download and web placement.
 * The copy engine owns transport recursion; xrdcp retains only source-list
 * expansion and process-level batch orchestration.
 */
#include "copy_internal.h"

const char *
copy_web_scheme(brix_web_proto proto)
{
    switch (proto) {
...
static void
web_path_basename(const char *path, char *out, size_t capacity)
{
...
static void
web_mkdirs_for(const char *filepath)
{
...
static int
web_mkcol_parents(const web_mkcol_ctx_t *m, const char *rel,
                  brix_status *st)
{
...
static int
place_to_web(const char *dstroot, const web_place_ctx_t *p)
{
    brix_weburl du;
    ...
    if (brix_weburl_parse(dstroot, &du) != 0) {
        brix_status_set(p->status, XRDC_EUSAGE, 0,
                        "bad web dst URL %s", dstroot);
        return -1;
    }
    snprintf(dbase, sizeof(dbase), "%s", du.path);
    blen = strlen(dbase);
    while (blen > 0 && dbase[blen - 1] == '/') { dbase[--blen] = '\0'; }
    dbearer = (p->opts != NULL && p->opts->bearer != NULL)
              ? p->opts->bearer : getenv("BEARER_TOKEN");
    {
        web_mkcol_ctx_t m = { &du, dbase, dbearer, p->conn };
        if (web_mkcol_parents(&m, p->rel, p->status) != 0) {
            return -1;
        }
    }
    if ((size_t) snprintf(dsturl, sizeof(dsturl), "%s://%s:%d%s/%s",
                          copy_web_scheme(du.proto), du.host, du.port, dbase,
                          p->rel) >= sizeof(dsturl)) {
        brix_status_set(p->status, XRDC_EUSAGE, 0,
                        "web->web dst path too long");
        return -1;
    }
    return copy_web_leaf_retry(p->srcurl, dsturl, p->opts, p->conn,
                               p->status);
}
...
static int
place_to_local(const char *dstroot, const web_place_ctx_t *p)
{
    char dstfile[XRDC_PATH_MAX];

    if ((size_t) snprintf(dstfile, sizeof(dstfile), "%s/%s", dstroot, p->rel)
            >= sizeof(dstfile)) {
        brix_status_set(p->status, XRDC_EUSAGE, 0, "dst path too long");
        return -1;
    }
    web_mkdirs_for(dstfile);
    return copy_web_leaf_retry(p->srcurl, dstfile, p->opts, p->conn,
                               p->status);
}
...
static int
recursive_place(const char *dstroot, const web_place_ctx_t *p)
{
    if (p->opts != NULL && !brix_copy_filter_match(p->opts, p->rel)) {
        return 0;   /* filtered — not an error, skip silently */
    }
    if (p->opts != NULL && p->opts->dry_run) {
                 ...
                 char *relbuf, size_t relbufsz)
{
    ...
    web_path_basename(path, relbuf, relbufsz);   /* fallback: flatten */
    return relbuf;
}
...
{
    const char *scheme = copy_web_scheme(u->proto);
...
recursive_s3_download(const brix_weburl *u, const char *dstdir,
                      const brix_copy_opts *fo, const brix_opts *co,
                      brix_status *status)
{
    brix_status st;
    char      **keys = NULL;
    ...
    size_t      n = 0;
    recdl_ctx_t c = { dstdir, fo, co, 0, 0, 0 };
    ...
    if (c.fail != 0) {
        brix_status_set(status, XRDC_ESOCK, 0,
                        "%zu recursive S3 transfer(s) failed", c.fail);
        return -1;
    }
    return 0;
}
...
int
copy_web_recursive_download(const char *src, const char *dstdir,
                            ...
                            brix_status *status)
{
    ...
    if (brix_weburl_parse(src, &u) != 0) {
        ...
        brix_status_set(status, XRDC_EUSAGE, 0,
                        "bad web source URL: %s", src);
        return -1;
    }
    if (u.is_s3) {
        return recursive_s3_download(&u, dstdir, &fo, co, status);
    }
    ...
    if (c.fail != 0) {
        brix_status_set(status, XRDC_ESOCK, 0,
                        "%zu recursive WebDAV transfer(s) failed", c.fail);
        return -1;
    }
    return 0;
}
