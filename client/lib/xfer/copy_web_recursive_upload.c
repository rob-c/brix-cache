/*
 * copy_web_recursive_upload.c - recursive local-to-web directory upload.
 * Owned by the copy engine: applications select a transfer, while libbrix
 * owns its directory walk, filtering, credentials and HTTP operations.
 */
...
static void
walk_handle_dir(web_upload_ctx_t *c, const walk_child_t *ch)
{
    if (!c->endpoint->is_s3
        && (c->opts == NULL || !c->opts->dry_run)) {
        ...
        if (web_join(c->base, ch->rel, rpath, sizeof(rpath)) != 0
            || brix_webdav_mkcol(c->endpoint, rpath, c->bearer,
                                 c->conn ? c->conn->verify_host : 1,
                                 c->conn ? c->conn->ca_dir : NULL,
                                 pcert, &mst) != 0) {
    ...
    copy_web_upload_walk(c, ch->local, ch->rel);
}
...
static void
walk_handle_file(web_upload_ctx_t *c, const walk_child_t *ch)
{
    char        rurl[XRDC_PATH_MAX * 2 + 320];
    char        rpath[XRDC_PATH_MAX * 2];
    brix_status cst;

    if (c->opts != NULL && !brix_copy_filter_match(c->opts, ch->rel)) {
        return;   /* filtered — skip, not a failure */
    }
    if (web_join(c->base, ch->rel, rpath, sizeof(rpath)) != 0
        || (size_t) snprintf(rurl, sizeof(rurl), "%s://%s:%d%s",
                             c->scheme, c->endpoint->host,
                             c->endpoint->port, rpath)
               >= sizeof(rurl)) {
        fprintf(stderr, "xrdcp: remote path too long for %s\n", ch->rel);
        c->failed++;
        return;
    }
    if (c->opts != NULL && c->opts->dry_run) {
        printf("[dry-run] copy %s -> %s\n", ch->local, rurl);
        c->copied++;
        return;
    }
    brix_status_clear(&cst);
    if (copy_web_leaf_retry(ch->local, rurl, c->opts, c->conn, &cst) == 0) {
        c->copied++;
        if (c->opts == NULL || !c->opts->silent) {
...
void
copy_web_upload_walk(web_upload_ctx_t *c, const char *localdir,
                     const char *rel)
{
...
static void
web_upload_create_base(const brix_weburl *endpoint, const char *base,
                       const web_upload_ctx_t *ctx)
{
    brix_status  status;
    char         proxybuf[512];
    const char  *proxy;

    if (endpoint->is_s3 || base[0] == '\0'
        || (ctx->opts != NULL && ctx->opts->dry_run)) {
        return;
    }
    proxy = brix_web_proxy_pem(proxybuf, sizeof(proxybuf));
    brix_status_clear(&status);
    (void) brix_webdav_mkcol(endpoint, base, ctx->bearer,
                            ctx->conn ? ctx->conn->verify_host : 1,
                            ctx->conn ? ctx->conn->ca_dir : NULL,
                            proxy, &status);
}
...
int
copy_web_recursive_upload(const char *localdir, const char *dst,
                          ...
                          brix_status *status)
{
    ...
    c.endpoint = &u;
    c.base    = base;
    c.scheme  = copy_web_scheme(u.proto);
    ...
    web_upload_create_base(&u, base, &c);

    copy_web_upload_walk(&c, localdir, "");
    if (o == NULL || !o->silent) {
    ...
    if (c.failed != 0) {
        brix_status_set(status, XRDC_ESOCK, 0,
                        "%zu recursive web upload(s) failed", c.failed);
        return -1;
    }
    return 0;
}
