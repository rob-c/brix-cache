 ...
 *       account, auto-refresh flag) into an brix_cred_config and returns a
 *       live brix_cred_store.
 ...
 *       builder: it allocates nothing itself — brix_cred_store_new does — and
 ...
 *
 * brix_cred_store_new() discovers handlers through WEAK accessors (cred.c). A
 * weak *undefined* reference does NOT pull the defining object out of the static
 * libbrix.a, so a statically-linked tool would otherwise build a store with NO
 * handlers — brix_cred_available() then always returns 0 and GSI/token/sss are
 ...
 * TU every cred-store tool already links (brix_cli_cred_store_build) — forces
 ...
 */
extern const brix_cred_handler *brix_cred_x509(void);
extern const brix_cred_handler *brix_cred_bearer(void);
extern const brix_cred_handler *brix_cred_sss(void);
extern const brix_cred_handler *brix_cred_krb5(void);
extern const brix_cred_handler *brix_cred_s3keys(void);

const brix_cred_handler *(*const brix_cli_cred_handler_anchor[])(void)
    __attribute__((used)) = {
    brix_cred_x509, brix_cred_bearer, brix_cred_sss,
    brix_cred_krb5, brix_cred_s3keys,
};

/*
 * brix_cli_cred_store_build — build a credential store from CLI/env values.
 *
 * WHAT: constructs an brix_cred_config from the caller's parsed CLI values and
 *       delegates to brix_cred_store_new.
 ...
 *
 * Returns a new store on success, or NULL on OOM (brix_cred_store_new failure).
 * The caller frees with brix_cred_store_free.
 */
struct brix_cred_store *
brix_cli_cred_store_build(const char *proxy,
                           const char *bearer,
                           ...
                           int         auto_refresh)
{
    ...
    return brix_cred_store_new(&cfg);
}
