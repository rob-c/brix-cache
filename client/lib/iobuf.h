 ...
 *
 * WHAT: an xrdc_iobuf coalesces sequential reads into one larger server read and
 ...
 *         - legacy: xrdc_file_read/_write over a pinned xrdc_conn+xrdc_file;
 *         - async : afh_pread (mfile/webfile) + xrdc_mfile_pwrite.
...
typedef ssize_t (*xrdc_iobuf_pread_fn)(void *be, int64_t off, void *buf,
                                       size_t n, xrdc_status *st);
typedef int     (*xrdc_iobuf_pwrite_fn)(void *be, int64_t off, const void *buf,
                                        size_t n, xrdc_status *st);

typedef struct {
    void                 *be;        /* opaque backend handle                    */
    xrdc_iobuf_pread_fn   pread;
    xrdc_iobuf_pwrite_fn  pwrite;
    int                   writable;
...
void xrdc_iobuf_init(xrdc_iobuf *b, void *be, xrdc_iobuf_pread_fn pr,
