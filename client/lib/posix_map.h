 ...
 *
 * These are pure functions over libxrdc data structures with NO connection model
...
void xrdc_statinfo_to_stat(const xrdc_statinfo *si, int allow_symlink,
                           struct stat *stbuf);
...
void xrdc_parse_qspace(const char *text, unsigned long long *total,
...
int  xrdc_fattr_listxattr_xlate(const char *raw, size_t rawlen,
