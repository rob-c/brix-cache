...
/* P44-C (ii-b): XRDC_IO_URING_LOOP=rxtx additionally swaps cleartext conns to
 * multishot RECV / one-shot SEND with provided buffers (TLS conns stay on the
 * POLL_ADD readiness tier).  Only meaningful when the uring loop itself is on;
 * best-effort — setup degrades back to POLL_ADD-only if the kernel lacks
 * provided-buffer rings. */
int
brix_loop_want_rxtx(void)
{
#if (BRIX_HAVE_LIBURING)
    const char *e = getenv("XRDC_IO_URING_LOOP");
    return e != NULL && strcmp(e, "rxtx") == 0;
#else
    return 0;
#endif
}
