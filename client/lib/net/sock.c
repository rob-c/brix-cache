...
{
    /* Env-proxy tunnelling: if http_proxy/https_proxy/all_proxy names a proxy for
     * this target (and no_proxy does not exempt it), connect to the proxy and open
     * an HTTP CONNECT tunnel to host:port, then hand the tunnelled fd back so TLS /
     * the XRootD handshake run over it unchanged. Env proxy overrides any in-config
     * proxy (agreed policy). No env proxy → the direct path below, untouched. */
    brix_proxy_t px;
    if (brix_proxy_resolve(NULL, host, port, &px)) {
        brix_proxy_report(&px, host, port);
        int pfd = connect_resolved(px.host, px.port, timeout_ms, brix_netpref_family(), st);
        if (pfd < 0)
            pfd = connect_resolved(px.host, px.port, timeout_ms, AF_UNSPEC, st);
        if (pfd < 0) {
            brix_status_set(st, XRDC_ESOCK, 0, "connect proxy %s:%d failed",
                            px.host, px.port);
            return -1;
        }
        char perr[160];
        if (brix_proxy_connect_tunnel(pfd, host, port, timeout_ms, perr, sizeof(perr)) != 0) {
            close(pfd);
            brix_status_set(st, XRDC_ESOCK, 0, "proxy CONNECT %s:%d via %s:%d: %s",
                            host, port, px.host, px.port, perr);
            return -1;
        }
        brix_sock_tune(pfd);
        return pfd;
