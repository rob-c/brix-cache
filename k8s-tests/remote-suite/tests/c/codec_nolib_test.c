 ...
 *       compiled in (-DBRIX_HAVE_ZLIB, -lz) but EVERY optional backend
 *       (zstd/xz/brotli/bzip2/lz4) compiled WITHOUT its -DBRIX_HAVE_* macro —
...
static const struct {
    ...
    { BRIX_CODEC_ZSTD,   "zstd",  "zstd"  },
    { BRIX_CODEC_BROTLI, "brotli", "br"   },
    { BRIX_CODEC_XZ,     "xz",    "xz"    },
    { BRIX_CODEC_BZIP2,  "bzip2", "bzip2" },
    { BRIX_CODEC_LZ4,    "lz4",   "lz4"   },
};
...
static int
roundtrip_ok(brix_codec_id_t id)
{
    ...
    uint8_t                comp[512];
    uint8_t                back[512];
    brix_codec_stream_t *cs, *ds;
    size_t                 in_pos, out_pos;
    brix_codec_rc_t      rc;
    size_t                 comp_len, back_len;

    cs = brix_codec_open(id, BRIX_CODEC_DIR_COMPRESS, -1, NULL);
    if (cs == NULL) {
        return 0;
    }
    in_pos = 0; out_pos = 0;
    do {
        rc = brix_codec_step(cs, (const uint8_t *) plain, sizeof(plain) - 1,
                               &in_pos, comp, sizeof(comp), &out_pos, 1);
    } while (rc == BRIX_CODEC_OK);
    brix_codec_close(cs);
    ...
    ds = brix_codec_open(id, BRIX_CODEC_DIR_DECOMPRESS, -1, NULL);
    if (ds == NULL) {
        return 0;
    }
    in_pos = 0; out_pos = 0;
    do {
        rc = brix_codec_step(ds, comp, comp_len, &in_pos,
...
main(void)
{
    ...
    CHECK(brix_codec_available(BRIX_CODEC_IDENTITY),
          "IDENTITY must always be available");

    /* zlib is mandatory — gzip + deflate must be live and round-trip. */
    CHECK(brix_codec_available(BRIX_CODEC_GZIP), "gzip must be available (zlib)");
    CHECK(brix_codec_available(BRIX_CODEC_DEFLATE), "deflate must be available (zlib)");
    CHECK(roundtrip_ok(BRIX_CODEC_GZIP), "gzip round-trip must work");
    CHECK(roundtrip_ok(BRIX_CODEC_DEFLATE), "deflate round-trip must work");
    ...
    for (i = 0; i < sizeof(optional_codecs) / sizeof(optional_codecs[0]); i++) {
        brix_codec_id_t          id = optional_codecs[i].id;
        ...
        snprintf(buf, sizeof(buf), "%s brix_codec_available() must be 0", nm);
        CHECK(brix_codec_available(id) == 0, buf);
        ...
        CHECK(brix_codec_by_name(nm, strlen(nm)) == d, buf);
        snprintf(buf, sizeof(buf), "%s must resolve by_http_token", nm);
        CHECK(brix_codec_by_http_token(optional_codecs[i].token,
        ...
        CHECK(brix_codec_open(id, BRIX_CODEC_DIR_COMPRESS, -1, NULL) == NULL, buf);
        snprintf(buf, sizeof(buf), "%s decompress open must return NULL", nm);
        CHECK(brix_codec_open(id, BRIX_CODEC_DIR_DECOMPRESS, -1, NULL) == NULL, buf);
    }
