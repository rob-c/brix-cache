...
import klib  # remote: files/dirs/symlinks created on the SERVER via kubectl exec
...
def _make_file(name, content=b"x"):
    ...
    klib.svc_write(SERVER_SVC, path, content)


def _make_dir(name):
    klib.svc_mkdir(SERVER_SVC, os.path.join(DATA_DIR, name.lstrip("/")))


@pytest.fixture(scope="module", autouse=True)
def _seed_standard_test_file():
    """remote: the local fleet ships a /test.txt; seed it on the SERVER."""
    klib.svc_write(SERVER_SVC, os.path.join(DATA_DIR, "test.txt"),
                   b"hello from nginx-xrootd\n")
    yield
    klib.svc_rm(SERVER_SVC, os.path.join(DATA_DIR, "test.txt"))
...
class TestQckscan:
    ...
    def test_qckscan_symlink_escape_is_not_followed(self, tmp_path):
        # remote: "outside" lives on the SERVER, outside the export root, and the
        # escaping symlink is created server-side.
        outside = "/tmp/qckscan_outside"
        klib.svc_mkdir(SERVER_SVC, outside)
        klib.svc_write(SERVER_SVC, outside + "/hidden.txt", b"hidden")

        scan_dir = os.path.join(DATA_DIR, "qckscan_symlink")
        klib.svc_mkdir(SERVER_SVC, scan_dir)
