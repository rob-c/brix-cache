...
import klib  # remote: read/clean server-side files via kubectl exec

ANON_URL  = f"root://{SERVER_HOST}:{NGINX_ANON_PORT}"
GSI_URL   = f"root://{SERVER_HOST}:{NGINX_GSI_PORT}"
# remote: DATA_DIR now names the mega SERVER's data root, inspected via klib.
...
def cleanup_uploads():
    ...
    yield
    try:  # remote: clean the SERVER's data dir via kubectl exec (best-effort)
        for f in klib.svc_listdir(SERVER_SVC, DATA_DIR):
            if f.startswith(PREFIX) or f.startswith(GSI_PREFIX):
                klib.svc_rm(SERVER_SVC, os.path.join(DATA_DIR, f))
    except Exception:
        pass
...
class TestWriteAnon:
    ...
    def test_upload_small_file(self):
        ...
        assert klib.svc_exists(SERVER_SVC, dest), "uploaded file not found on disk"
        assert klib.svc_read(SERVER_SVC, dest) == content, "file content mismatch"
        os.unlink(local)
    ...
    def test_upload_medium_file(self):
        ...
        dest = os.path.join(DATA_DIR, remote)
        blob = klib.svc_read(SERVER_SVC, dest)  # remote: read the server's copy
        assert len(blob) == size, f"size mismatch: {len(blob)} != {size}"
        assert hashlib.md5(blob).hexdigest() == expected_md5, "MD5 mismatch after upload"
        os.unlink(local)
    ...
    def test_upload_overwrite(self):
        ...
        assert klib.svc_read(SERVER_SVC, dest) == b"replaced content\n", "overwrite failed"
        os.unlink(local)
    ...
    def test_upload_rejected_without_credentials_on_gsi_port(self):
        ...
        assert not klib.svc_exists(
            SERVER_SVC, os.path.join(DATA_DIR, f"{PREFIX}nocreds.txt")
