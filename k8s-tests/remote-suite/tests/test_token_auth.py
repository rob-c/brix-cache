...
import urllib3
import pytest
import klib  # remote: verify writes on the SERVER
...
class TestWebDavBearerToken:
    ...
    def test_put_with_write_scope(self, issuer):
        ...
        try:
            ...
            local_path = os.path.join(SERVER_DATA, "token_test_write.txt")
            assert klib.svc_exists(SERVER_SVC, local_path)
            assert klib.svc_read(SERVER_SVC, local_path) == test_data
        finally:
            # Clean up
            try:
                klib.svc_rm(SERVER_SVC, os.path.join(SERVER_DATA, "token_test_write.txt"))
...
class TestScopeEnforcement:
    ...
    def test_write_scope_root(self, issuer):
        ...
        try:
            ...
            try:
                klib.svc_rm(SERVER_SVC, os.path.join(SERVER_DATA, "scope_test_write.txt"))
            except FileNotFoundError:
                pass
    ...
    def test_write_scope_subpath(self, issuer):
        ...
        subdir = os.path.join(SERVER_DATA, "token_subdir")
        klib.svc_mkdir(SERVER_SVC, subdir)
