...
def _data_path(rel: str) -> str:
    ...
    return os.path.join(SERVER_DATA, rel.lstrip("/"))
...
def scratch_file(tmp_path):
    ...
    dst = _data_path(name)
    if klib.svc_exists(SERVER_SVC, dst):
        klib.svc_rm(SERVER_SVC, dst)
...
class TestPut:
    ...
    def test_put_overwrite_returns_200_or_204(self):
        ...
        try:
            ...
            assert klib.svc_read(SERVER_SVC, dst) == b"overwritten\n"
        finally:
            if klib.svc_exists(SERVER_SVC, dst):
                klib.svc_rm(SERVER_SVC, dst)
    ...
    def test_put_content_reaches_disk(self):
        ...
        try:
            ...
            assert klib.svc_read(SERVER_SVC, dst) == content
        finally:
            if klib.svc_exists(SERVER_SVC, dst):
                klib.svc_rm(SERVER_SVC, dst)
...
class TestDelete:
    ...
    def test_delete_existing_file(self):
        ...
        assert code == 204
        assert not klib.svc_exists(SERVER_SVC, dst), "File should be gone after DELETE"
...
class TestMkcol:
    ...
    def test_mkcol_creates_directory(self):
        ...
        try:
            ...
            assert code == 201
            assert klib.svc_isdir(SERVER_SVC, dst), "MKCOL should have created a directory"
        finally:
            if klib.svc_exists(SERVER_SVC, dst):
                klib.svc_rmtree(SERVER_SVC, dst)
    ...
    def test_mkcol_nested_missing_parent_returns_409(self):
        ...
        if klib.svc_exists(SERVER_SVC, parent):
...
class TestPathHardening:
    ...
    def test_delete_rejects_double_encoded_nul_path(self):
        ...
        dst = _data_path(name)
        klib.svc_write(SERVER_SVC, dst, b"webdav nul hardening\n")
        ...
        try:
            ...
            assert klib.svc_exists(SERVER_SVC, dst), "double-encoded NUL unexpectedly deleted the file"
        finally:
            if klib.svc_exists(SERVER_SVC, dst):
                klib.svc_rm(SERVER_SVC, dst)
...
class TestPropfind:
    ...
    def test_propfind_depth1_escapes_xml_metacharacters_in_href(self):
        ...
        dst = _data_path(name)
        klib.svc_write(SERVER_SVC, dst, b"xml escape\n")
...
class TestRoundTrip:
    ...
    def test_mkcol_put_get_in_subdirectory(self):
        ...
        try:
            ...
            if klib.svc_exists(SERVER_SVC, dst_dir):
                klib.svc_rmtree(SERVER_SVC, dst_dir)
...
class TestWebdavTraversal:
    ...
    def test_put_dot_dot_escape(self):
        ...
        escaped = "/webdav_escaped_probe.txt"  # server path outside export via ../
        try:
            code = _http_code(
                "--path-as-is",
                "-X", "PUT",
            ...
            assert code in (400, 403, 404), f"expected rejection, got {code}"
            assert not klib.svc_exists(SERVER_SVC, escaped), "file created outside root"
        finally:
            if klib.svc_exists(SERVER_SVC, escaped):
                klib.svc_rm(SERVER_SVC, escaped)
    ...
    def test_symlink_traversal_via_get(self):
        ...
        outside = "/tmp/webdav_escape_outside"
        klib.svc_mkdir(SERVER_SVC, outside)
        secret_file = os.path.join(outside, "secret.txt")
        klib.svc_write(SERVER_SVC, secret_file, "leaked data\n")
        ...
        if True:
            try:
                ...
                if os.path.islink(link_path):
                    klib.svc_rm(SERVER_SVC, link_path)
