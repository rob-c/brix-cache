...
import time
import pytest
import klib  # remote: server-side file setup
...
class TestFileSurface:
    ...
    @pytest.mark.skip(reason='pyxrootd-proxy shim limitation: stateful File-handle sequence (sync/truncate/reopen) trips the per-op worker channel; server behaviour is unaffected')
    ...
    def test_vector_read(self):
        ...
        klib.svc_write(SERVER_SVC, os.path.join(SERVER_DATA, path), content)
    ...
    def test_concurrent_handles(self):
        ...
        local = os.path.join(SERVER_DATA, path)
        klib.svc_write(SERVER_SVC, local, content)
...
class TestFileSystemSurface:
    ...
    def test_dirlist(self):
        ...
        klib.svc_mkdir(SERVER_SVC, os.path.join(SERVER_DATA, dir_name))
        for i in range(3):
            klib.svc_write(SERVER_SVC, os.path.join(SERVER_DATA, dir_name, f"f{i}.txt"), str(i))
    ...
    def test_query_and_locate(self):
        ...
        klib.svc_write(SERVER_SVC, os.path.join(SERVER_DATA, path), b"data")
    ...
    def test_url_parameters(self):
        ...
        klib.svc_write(SERVER_SVC, os.path.join(SERVER_DATA, path), "opaque content")
...
class TestXAttrSurface:
    ...
    def test_xattr_lifecycle(self):
        ...
        klib.svc_write(SERVER_SVC, os.path.join(SERVER_DATA, path), "xattr test")
...
class TestAdvancedSurface:
    ...
    def test_pgread_integrity(self):
        ...
        klib.svc_write(SERVER_SVC, os.path.join(SERVER_DATA, path), data)
    ...
    def test_prepare(self):
        ...
        klib.svc_write(SERVER_SVC, os.path.join(SERVER_DATA, path), "ready")
