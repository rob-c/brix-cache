...
def _start_local_minio():
    if shutil.which("docker") is None:
        pytest.skip("docker not available for the MinIO backend")
    result = subprocess.run(
        ["bash", os.path.join(HERE, "minio_harness.sh"), "start"],
...
class TestMinioS3Forward:
    ...
    def test_wrong_credentials_are_rejected_upstream(self, minio):
        ...
        _require_wrong_credential_node(base, p)
        try:
            _assert_wrong_credential_requests(port)
        finally:
            _stop_nginx(base)


def _require_wrong_credential_node(base, result):
    if result.returncode == 0:
        return
    _stop_nginx(base)
    pytest.fail(f"[brix-machinery] wrong-cred config rejected at parse time: "
                f"{result.stderr[-300:]}")


def _assert_wrong_credential_requests(port):
    assert _wait_port("127.0.0.1", port), \
        "[brix-machinery] wrong-cred node never listened"
    result = requests.put(f"http://127.0.0.1:{port}/forged.bin",
                          data=os.urandom(4096), timeout=30)
    assert result.status_code >= 400, (
        f"SECURITY: PUT with a wrong backend secret succeeded "
        f"({result.status_code}) — credential is not being forwarded")
    stored = minio_request("GET", f"/{BUCKET}/forged.bin")
    assert stored.status_code == 404, \
        "SECURITY: object written to MinIO despite a bad credential"
    result = requests.get(f"http://127.0.0.1:{port}/direct.bin", timeout=30)
    assert result.status_code >= 400, (
...
_load_continuations(
    globals(),
    __file__,
    "_minio_s3_forward_part2.py",
    "_minio_s3_forward_part3.py",
)
