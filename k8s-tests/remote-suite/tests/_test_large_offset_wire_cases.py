...
class TestNearInt64Max:
    ...
    def test_read_just_below_int64_max(self, huge_sparse_near_max):
        ...
        sock = _session()
        try:
            fh = _open_read_or_skip(sock, name)
            _assert_sparse_hole(sock, fh, size - 1024)
            _assert_past_eof(sock, fh)
            _close(sock, fh)
            assert _ping(sock)[1] == kXR_ok
        finally:
            sock.close()
...
class TestStatSizeAbove4GiB:
    ...
    def test_stat_reports_full_size(self, big_sparse_4g):
        ...
        sock = _session()
        try:
            reported = _reported_size(sock, name)
...
class TestLargeTruncate:
    ...
    def test_truncate_to_above_4gib(self):
        ...
        sock = _session()
        try:
            fh = _open_write_or_skip(sock, name)
            ...
            _assert_truncate_report(_stat_size(sbody), target)
            assert _ping(sock)[1] == kXR_ok
        finally:
            sock.close()
            _unlink(full)
...
def _open_read_or_skip(sock, name):
    _, status, body = _open(sock, name, kXR_open_read)
    if status != kXR_ok:
        pytest.skip(f"server refused near-max sparse file: {_error_code(body)}")
    return body[:4]


def _assert_sparse_hole(sock, handle, offset):
    _, status, body = _read(sock, handle, offset, 256)
    assert status == kXR_ok, _error_code(body)
    assert len(body) <= 256
    assert body == b"\x00" * len(body)


def _assert_past_eof(sock, handle):
    _, status, body = _read(sock, handle, INT64_MAX - 1, 16)
...
def _page_payload(data):
    checksum = crc32c(data) if _CRC32C_OK else 0
    return struct.pack("!I", checksum) + data


def _reported_size(sock, name):
...
def _assert_full_stat_size(reported, expected):
    ...
    assert reported == expected, f"stat size {reported} != actual {expected}"
    assert reported > 0xFFFFFFFF, "test file must exceed 4 GiB"


def _open_write_or_skip(sock, name):
    _, status, body = _open(sock, name, kXR_open_updt)
    if status != kXR_ok:
        code = _error_code(body)
        pytest.skip(f"anon server read-only (open updt -> {code}); need write on")
    return body[:4]


def _assert_truncate_report(reported, target):
    if reported == target:
        return
