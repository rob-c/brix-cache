...
import klib  # remote: in-root mkdir verify happens on the SERVER
SERVER_SVC = "mega"
...
class TestAuthorizedWriterIsStillConfined:
    ...
    def test_legit_in_root_mkdir_succeeds_then_escape_is_blocked(self):
        ...
        if in_root_write_allowed:
            created = os.path.join("/data/xrootd", ok_dir.lstrip("/"))
            assert klib.svc_isdir(SERVER_SVC, created), "in-root mkdir did not create the dir"
            klib.svc_rmtree(SERVER_SVC, created)
