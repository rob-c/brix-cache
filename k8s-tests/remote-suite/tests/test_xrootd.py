...
import klib  # remote: the fleet's standard test files are seeded onto the SERVER
...
# remote: the local fleet seeds these standard files; here we deterministically
# generate random.bin's bytes and seed both onto the SERVER for the module.
TEST_FILES["random.bin"]["content"] = _random.Random(20260705).randbytes(
    TEST_FILES["random.bin"]["size"])


@pytest.fixture(scope="module", autouse=True)
def _seed_server_test_files():
    for name, spec in TEST_FILES.items():
        klib.svc_write(SERVER_SVC, f"{SERVER_DATA}/{name}", spec["content"])
    yield
    for name in TEST_FILES:
        klib.svc_rm(SERVER_SVC, f"{SERVER_DATA}/{name}")
...
class TestAnonymous:
    ...
    def test_read_large_file_integrity(self, anon_fs):
        ...
        expected_md5 = hashlib.md5(TEST_FILES["random.bin"]["content"]).hexdigest()
