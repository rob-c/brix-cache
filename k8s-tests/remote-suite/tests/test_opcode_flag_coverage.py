...
SERVER_SVC = "mega"
import klib  # remote: dirs/files created on the SERVER
...
def test_kXR_dstat_dirlist_emits_leadin_and_entry_stat():
    ...
    klib.svc_mkdir(SERVER_SVC, root)
    klib.svc_write(SERVER_SVC, os.path.join(root, "alpha.txt"), b"alpha")
...
def test_kXR_dcksm_dirlist_emits_extended_stat_and_checksum_token():
    ...
    klib.svc_write(SERVER_SVC, os.path.join(root, "checksum.txt"), payload)
...
def test_kXR_dcksm_dirlist_crc32():
    ...
    klib.svc_write(SERVER_SVC, os.path.join(root, "f.txt"), payload)
...
def test_kXR_dcksm_xattr_cache_populated_and_reused(algo, hashfn):
    ...
    klib.svc_write(SERVER_SVC, filepath, payload)
...
def test_kXR_mkdirpath_creates_missing_parent_directories():
    ...
    with _login_session() as sock:
        ...
        assert not klib.svc_exists(SERVER_SVC, _disk(target))

        status, _ = _mkdir(sock, target, kXR_mkdirpath, streamid=b"\x00\x22")

    assert status == kXR_ok
    assert klib.svc_isdir(SERVER_SVC, _disk(target))


def test_kXR_posc_write_persists_after_successful_close():
    target = f"/{PREFIX}posc/persisted.bin"
    klib.svc_mkdir(SERVER_SVC, os.path.dirname(_disk(target)))
    ...
    assert klib.svc_read(SERVER_SVC, _disk(target)) == payload
