...
def data_file():
    ...
    klib.svc_mkdir(SERVER_SVC, DATA_ROOT)
    full = os.path.join(SERVER_DATA, DATA_NAME.lstrip("/"))
    klib.svc_write(SERVER_SVC, full, PATTERN)
    return DATA_NAME
...
class TestPgwriteSecurity:
    ...
    def wr_handle(self):
        ...
        full = os.path.join(SERVER_DATA, path.lstrip("/"))
        klib.svc_write(SERVER_SVC, full, b"\x00" * PG_PAGESZ)
