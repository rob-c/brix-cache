def _rt58_segment_01(key, data, port):
    T = "rdl_"
    ta = mint(key, "alice")
    adir = os.path.join(data, "alice")
    host = "%s:%d" % (HOST, port)
    bearer = "Bearer %s" % ta
    return T, ta, adir, host, bearer


def _rt58_recovery_anchor_write_a_known_body(port, ta):
...
def _rt58_segment_03(port, ta, ANCHOR):
    ...
    def recover(label, n=[0]):
        ...
        dt = time.time() - t0
        ok(all((st == 200, b == ANCHOR, dt < 5.0)),
           "recovery after %s: alice GET /alice/hello.txt -> 200 exact bytes, fast "
           "(HTTP %s, %.2fs)" % (label, st, dt))
    return recover


def _rt58_1_single_path_max_scale_segment(recover, T, port, ta, adir):
    ...
    return seg, st, pmax_created


def _rt58_a_root_stat_on_the_same(st, pmax_created, seg, recover, T):
    ok(all((st in (400, 403, 404, 414, 500, 507, -1), not pmax_created)),
...
def _rt58_the_deepest_directory_the_chain_reached(DEPTH, cur, port, ta, adir, T):
    made = 0
    ...
    ok(all((made >= 20, os.path.isdir(top), os.stat(top).st_uid == UID_ALICE)),
    ...
    return made, deepest_dir


def _rt58_a_put_at_the_bottom_broker(data, deepest_dir, made, port, ta, recover):
    ...
    if made >= 1:
        ...
        ok(any((all((st in (200, 201, 204), leaf_owned)), st in (403, 404, 409, 414, 500, 507))),
           "PUT at bottom of 40-deep tree: alice-owned if created, else clean reject "
           "(HTTP %s)" % st)
    recover("40-deep MKCOL+PUT")


def _rt58_segment_01_2(T, port, ta):
    for i in range(5):
...
def _rt58_one_xrd_fs_process_doing_several(T, results):
...
def _rt58_content_correctness_for_one_of_them(results, elapsed, recover):
    good = sum(1 for rc, _o in results.values() if rc == 0)
    ok(all((good >= 5, elapsed < 25.0)),
    ...
    ok(all((rc0 == 0, 'H0' in out0)),
       "concurrent root:// open returned the OWN file's body (no handle bleed)")
    recover("concurrent root:// handles")


def _rt58_when_xrd_avail(port, ta, T, recover):
    results = _rt58_segment_01_2(T, port, ta)

    opener = _rt58_one_xrd_fs_process_doing_several(T, results)

    elapsed = _rt58_segment_03_2(opener)

    _rt58_content_correctness_for_one_of_them(results, elapsed, recover)



def _rt58_3_concurrent_root_handle_pressure_a(T, port, ta, recover, host, bearer):
    ...
    if xrd_avail():
        ...
        i = _rt58_when_xrd_avail(port, ta, T, recover)
    else:
...
def _rt58_4b_one_near_64kib_single_header(m, resp, recover):
    st = int(m.group(1)) if m else -1
    ok(all((b'BOB-PRIVATE-SECRET' not in any((resp, b'')), b'root:x:0:0' not in any((resp, b'')))),
       "200-header request: no tenant/passwd leak (HTTP %s)" % st)
    ok(any((st in (200, 400, 414, 431, 494, -1), st >= 400)),
...
def _rt58_segment_10(host, bearer, big_val, port, resp, st):
    ...
    m = re.match(rb"HTTP/1\.[01] (\d{3})", resp or b"")
    st = int(m.group(1)) if m else -1
    ok(any((st in (400, 431, 414, 494, -1), st >= 400)),
       "near-64KiB single header value: rejected at the buffer ceiling (HTTP %s)" % st)
    return resp, st


def _rt58_5_proppatch_with_60_dead_properties(resp, st, recover, T, port, ta, adir):
    ok(b'root:x:0:0' not in any((resp, b'')),
...
def _rt58_segment_12(sets, T, port, ta, pp_fp):
    ...
    ok(all((os.path.exists(pp_fp), os.stat(pp_fp).st_uid == UID_ALICE)),
       "60-dead-prop target still owned by alice after the prop flood")
    pp_st = os.stat(pp_fp)
    return pp_st


def _rt58_segment_01_3(T, s3port, data):
    ...
    return st, lk_created, lk_bad


def _rt58_very_long_s3_query_string_on(st, lk_bad, lk_created, s3port, T, recover):
    ok(any((st in (400, 403, 404, 414, 500, 501, -1), all((st in (200, 201), not lk_bad)))),
    ...
    ok(b'BOB-PRIVATE-SECRET' not in any((b, b'')),
       "very long S3 query string leaked no tenant secret (HTTP %s)" % st)
    recover("long S3 key/query")


def _rt58_when_s3port(T, s3port, data, recover):
    st, lk_created, lk_bad = _rt58_segment_01_3(T, s3port, data)

    _rt58_very_long_s3_query_string_on(st, lk_bad, lk_created, s3port, T, recover)



def _rt58_xattr_count_is_bounded_the_inode(pp_st, pp_fp, recover, s3port, T, data):
    ...
    # =====================================================================
    if s3port:
        _rt58_when_s3port(T, s3port, data, recover)
    else:
        ok(True, "long S3 key/query stressor skipped (no s3port configured)")


def _rt58_7_bounded_slow_partial_sends_slowloris(host, bearer, port, resp, m):
...
def _rt58_the_crucial_proof_right_after_the(m, dt, resp, ANCHOR, recover, host, bearer):
    st = int(m.group(1)) if m else -1
    ok(all((dt < 12.0, any((st in (-1, 400, 408, 414, 431), st >= 400, st == -1)))),
       "slow partial send (never-completed headers): client-timeout / clean reject, "
       "no worker wedge (HTTP %s, %.1fs)" % (st, dt))
    ok(all((b'hi-from-alice' not in any((resp, b'')), ANCHOR not in any((resp, b'')))),
...
def _rt58_segment_01_4(adir, T, scanned, drift):
    for nm in os.listdir(adir):
        ...
        if u == UID_SVC or u == 0:
            drift += 1
    return scanned, drift


def _rt58_try_body(adir, T, scanned, drift):
    scanned, drift = _rt58_segment_01_4(adir, T, scanned, drift)

    return scanned, drift


def _rt58_8_final_invariant_sweep_every_file(recover, adir, T):
    ...
    scanned = 0
    try:
        scanned, drift = _rt58_try_body(adir, T, scanned, drift)
    except OSError:
        pass
    ok(all((scanned > 0, drift == 0)),
       "ownership invariant: of %s rdl_ artifacts, NONE owned by svc(1500)/root(0) "
       "(drift=%s)" % (scanned, drift))


def _rt58_nothing_escaped_the_export_root_via(data, T, recover):
...
def run_resource_dos_limits(key, data, port, s3port):
    ...
    T, ta, adir, host, bearer = _rt58_segment_01(key, data, port)

    ANCHOR = _rt58_recovery_anchor_write_a_known_body(port, ta)

    recover = _rt58_segment_03(port, ta, ANCHOR)

    seg, st, pmax_created = _rt58_1_single_path_max_scale_segment(recover, T, port, ta, adir)

    DEPTH, cur = _rt58_a_root_stat_on_the_same(st, pmax_created, seg, recover, T)

    made, deepest_dir = _rt58_the_deepest_directory_the_chain_reached(DEPTH, cur, port, ta, adir, T)

    _rt58_a_put_at_the_bottom_broker(data, deepest_dir, made, port, ta, recover)

    resp, m = _rt58_3_concurrent_root_handle_pressure_a(T, port, ta, recover, host, bearer)

    big_val = _rt58_4b_one_near_64kib_single_header(m, resp, recover)

    resp, st = _rt58_segment_10(host, bearer, big_val, port, resp, st)

    pp_fp, sets = _rt58_5_proppatch_with_60_dead_properties(resp, st, recover, T, port, ta, adir)

    pp_st = _rt58_segment_12(sets, T, port, ta, pp_fp)

    _rt58_xattr_count_is_bounded_the_inode(pp_st, pp_fp, recover, s3port, T, data)

    resp, dt, m = _rt58_7_bounded_slow_partial_sends_slowloris(host, bearer, port, resp, m)

    ok_steps = _rt58_the_crucial_proof_right_after_the(m, dt, resp, ANCHOR, recover, host, bearer)

    _rt58_segment_16(ok_steps, port, ANCHOR)

    _rt58_8_final_invariant_sweep_every_file(recover, adir, T)

    _rt58_nothing_escaped_the_export_root_via(data, T, recover)
