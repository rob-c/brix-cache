...
import klib  # remote: seed on the SERVER
SERVER_SVC="mega"
...
def _write_data_file(name, content):
    klib.svc_write(SERVER_SVC, os.path.join("/data/xrootd", name), content)
...
class TestBindHandleSlotCache:
    ...
    @pytest.mark.skip(reason="source-contract test: repo src/ tree not present in the remote client")
