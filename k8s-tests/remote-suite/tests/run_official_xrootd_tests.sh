#!/usr/bin/env bash
#
# Run the stock-client interop scenarios against our nginx+xrootd-module proxy
# instead of a native xrootd server.
#
# The scenarios follow the shape the stock integration tests use:
#   setup    — start a server
#   run      — exercise it via xrdfs/xrdcp
#   teardown — stop the server
#
# We skip setup/teardown (nginx is already running), point HOST at our nginx
# listener, and run each scenario.  The stock CLIENTS are the oracle; no XRootD
# source tree is read — this is a clean-room implementation and the suite must
# run from this repository alone.
#
# Usage:
...
# The stock host.sh round-trip scenario.
test_host_adapted() {
    local HOST="root://localhost:${NGINX_ANON_PORT}/"
    ...
    # Upload a known file, download it, diff.  The payload is written here
    # rather than copied out of an upstream checkout: the assertion is
    # byte-equality after the round trip, which holds for any payload, and the
    # scenario can no longer skip itself for a missing XRootD source tree.
    local srcfile="${LOCAL}/host.cfg.src"
    cat > "${srcfile}" <<'PAYLOAD'
# host.cfg - round-trip payload for the host interop scenario.
# Generated by this script; no upstream source tree is consulted.
all.export /
xrootd.async off
xrd.port any
...
echo "═══════════════════════════════════════════════════════════════"
echo "  Stock-client interop scenarios → nginx+xrootd module"
