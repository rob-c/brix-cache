...
SERVER_SVC = "mega"
import klib  # remote: files created on the SERVER
...
def _seed_test_txt():
    """remote: seed the standard fleet /test.txt on the SERVER."""
    klib.svc_write(SERVER_SVC, DATA_DIR + "/test.txt", b"hello from nginx-xrootd\n")
    yield
    klib.svc_rm(SERVER_SVC, DATA_DIR + "/test.txt")
...
class TestMalformedDlen:
    ...
    def test_dlen_zero_write_opcode(self):
        ...
        klib.svc_write(SERVER_SVC, fullpath, b"init")
        sock = _full_session()
    ...
    def test_dlen_large_write_payload_allowed(self):
        ...
        if open_status != kXR_ok:
            klib.svc_write(SERVER_SVC, fullpath, b"")
            _, open_status, open_body = _open_file(sock, path, kXR_open_updt)
        assert open_status == kXR_ok
        fhandle = open_body[:4]
        req = struct.pack("!2sH4sqiI", b"\x00\x03", kXR_write,
...
class TestFastPathAttacks:
    ...
    def test_write_zero_bytes(self):
        ...
        fullpath = os.path.join(DATA_DIR, path.lstrip("/"))
        klib.svc_write(SERVER_SVC, fullpath, b"hello")
        sock = _full_session()
        _, open_status, open_body = _open_file(sock, path, kXR_open_updt)
        assert open_status == kXR_ok
        fhandle = open_body[:4]
