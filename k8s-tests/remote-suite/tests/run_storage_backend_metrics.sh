...
# composed storage stack per export as `brix_storage_backend_info{...}` (backend,
...
echo "== /metrics exposes brix_storage_backend_info for the composed export =="
out=$(curl -s "http://127.0.0.1:${WPORT}/metrics")
line=$(printf '%s\n' "$out" | grep '^brix_storage_backend_info' | head -1)
[ -n "$line" ] && ok "info gauge present" || { bad "no brix_storage_backend_info line"; printf '%s\n' "$out" | grep -i storage | head; }
