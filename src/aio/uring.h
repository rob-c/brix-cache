 ...
 *       single dispatch seam `xrootd_aio_post_task()` (src/aio/resume.c).  It is
 ...
 *
 * HOW:  Everything liburing-specific is guarded by XROOTD_HAVE_LIBURING.  When
 ...
 *       validator hard-fails any `xrootd_io_uring on` block at config time with
 *       no kernel needed (so `nginx -t` flags a stub build anywhere).  Assumes
 *       ngx_xrootd_module.h (nginx + config + tunables) is already included. */
...
typedef struct {
    ...
    uint8_t               op_kind;    /* xrootd_uring_op_e; selects OUT xlation */
 ...
 * accessor xrootd_uring_worker() — no new exported global.  Only the event
...
typedef struct {
    ...
    xrootd_uring_slot_t *slots;         /* completion-slot table, queue_depth    */
...
typedef enum {
    ...
    XRD_URING_OP_WRITEV,     /* IORING_OP_WRITEV -> xrootd_writev_aio_t (+FSYNC)*/
 ...
 * postconfiguration.  If any enabled server block requests `xrootd_io_uring on`
 ...
 */
ngx_int_t xrootd_uring_validate_conf(ngx_conf_t *cf);
...
ngx_int_t     xrootd_uring_killswitch_configure(ngx_conf_t *cf);
/* The flag pointer for this process (NULL when not configured). */
ngx_atomic_t *xrootd_uring_killswitch_ptr(void);
...
ngx_int_t     xrootd_uring_panicfile_arm(ngx_cycle_t *cycle, ngx_str_t *path);
/* Whether `xrootd_io_uring_admin on` exposed the admin endpoint (set at config
 * time; read by the dashboard handler). */
void          xrootd_uring_admin_set_enabled(ngx_uint_t on);
...
#define XROOTD_URING_FSYNC_COOKIE  0xfffffffffffffffeULL
 ...
 * its use with XROOTD_HAVE_LIBURING.
 */
...
/*
 * xrootd_uring_disabled — lock-free read of the SHM hot kill-switch (SB-W5b).
...
/*
 * xrootd_uring_op_for — map a bound thread task to its io_uring op-kind by its
...
/*
 * xrootd_uring_submit — build + submit one SQE (or a linked chain) for a mapped
