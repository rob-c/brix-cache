...
| `api.c` | **(http)** All read JSON endpoints (`/transfers` compat + `/api/v1/{transfers,transfers/{id},snapshot,events,history,cluster,cache,ratelimit}`) built with **jansson**; SHM memory-barrier read pattern; auth + GET/HEAD gate; stale-GC of dead slots during scan. |
| `api_admin.c` | **(http)** Phase 23 write API: auth (CIDR/bearer/require-both), strict whitelist input validation, async JSON body reader, cluster registry writes, dynamic proxy-pool writes, structured audit log, and the directive setters. |
