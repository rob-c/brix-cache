...
".table-wrap{padding:0 1rem 1rem;overflow:auto}table{width:100%;border-collapse:collapse;font-size:12px}th,td{padding:.5rem .6rem;border-bottom:1px solid #252d36;text-align:left;vertical-align:top}th{position:sticky;top:50px;background:#151b23;color:#98a6b3;z-index:1}tbody tr{cursor:pointer}tbody tr:hover,tbody tr:focus{background:#17202a;outline:none}.path,.identity{max-width:360px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.pill{display:inline-block;border-radius:999px;padding:.1rem .45rem;background:#263241}.state-active{color:#7ee787}.state-idle{color:#d29922}.state-stalled,.state-error{color:#ff7b72}.dir-read{color:#7ee787}.dir-write{color:#ffa657}.dir-tpc{color:#d2a8ff}\n"
...
"<label for=\"state-filter\">State<select id=\"state-filter\"><option value=\"\">All</option><option>active</option><option>idle</option><option>stalled</option><option>closing</option><option>error</option></select></label>\n"
...
"function poll(){fetch(API+'/snapshot',{cache:'no-store'}).then(function(r){if(r.status===401){location.href='/xrootd/login';return null}if(!r.ok)throw new Error(r.status);return r.json()}).then(function(data){if(!data)return;lastSnapshot=data;el('status').className='status live';el('status').textContent='live '+new Date(data.server_ms).toLocaleTimeString();render();if(selectedDetail)openDetail(selectedDetail,true)}).catch(function(){el('status').className='status bad';el('status').textContent='disconnected'})}\n"
...
"loadControls();el('detail-close').onclick=closeDetail;el('export-snapshot').onclick=exportSnapshot;document.addEventListener('keydown',function(e){if(e.key==='Escape')closeDetail()});poll();setInterval(poll,2000);\n"
"</script>\n"
"</body></html>\n";
