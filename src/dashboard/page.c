...
"function poll(){fetch(API+'/snapshot',{cache:'no-store'}).then(function(r){if(r.status===401){location.href='/xrootd/login';return null}if(!r.ok)throw new Error(r.status);return r.json()}).then(function(data){if(!data)return;lastSnapshot=data;applyAnon(!!data.anonymous);el('status').className='status live';el('status').textContent='live '+new Date(data.server_ms).toLocaleTimeString();render();if(selectedDetail)openDetail(selectedDetail,true)}).catch(function(){el('status').className='status bad';el('status').textContent='disconnected'})}\n"
...
"function applyAnon(a){var x=el('anon-banner');if(x)x.hidden=!a;var c=el('download-config');if(c)c.style.display=a?'none':'';var e=el('export-snapshot');if(e)e.style.display=a?'none':''}\n"
