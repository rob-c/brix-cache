 ...
 *
 * WHAT: Registers the xrootd_dashboard shared-memory zone during stream
 *       postconfiguration.  Called after xrootd_configure_metrics() in
 ...
 *       stream module source list because it references ngx_stream_xrootd_module.
 */
...
{
    ngx_str_t  zone_name = ngx_string("xrootd_dashboard_v2");
    ngx_str_t  events_name = ngx_string("xrootd_dashboard_events");
    ...
    ngx_xrootd_dashboard_shm_zone->init = ngx_xrootd_dashboard_shm_init;
    ...
    ngx_xrootd_dashboard_events_shm_zone->init =
    ...
    ngx_xrootd_dashboard_history_shm_zone->init =
        ngx_xrootd_dashboard_history_shm_init;
