# src/types — Core type definitions, tunables, and the canonical identity object

## Overview

`src/types/` is the **type vocabulary of the entire module**. It holds the per-connection
...
`src/ngx_xrootd_module.h` so a contributor can read exactly one concept without wading
...
`src/ngx_xrootd_module.h:105`), each after the nginx, OpenSSL, `protocol/`, `metrics/`,
...
Nothing "runs" in `src/types/` except the `identity.c` helpers — these headers are
...
- **Add an auth method →** add an `XROOTD_AUTHN_*` bit (`identity.h`), teach
  ...
  lives (`config:268`), not `src/config/config.h`.
...
- `src/ngx_xrootd_module.h` — the umbrella header that includes these in dependency order
