...
| `read_compress.c` | Phase-42 W4 inline read compression: `xrootd_read_compressed()` — the opt-in `kXR_read` path for a handle opened with `?xrootd.compress=<codec>` (gated by `xrootd_read_compress`). Synchronously reads a `XROOTD_READ_CHUNK_MAX`-bounded plaintext window, compresses it as one self-contained codec frame (`src/core/compat/codec_core.c`) into the `cmp_scratch` keep-slot, and queues it as a single response (the native client inflates). Strictly isolated so the default plaintext path in `read.c` is byte-identical; pgread/readv never reach it, preserving the pgread CRC32c invariant. |
