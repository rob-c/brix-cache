 ...
 * HOW: Delegate to xrootd_crc32c_value() from the compat layer — pure wrapper, no local computation. */
{
    return xrootd_crc32c_value(buf, len);
}
 ...
 * HOW: Delegate to xrootd_crc32c_copy_value() from the compat layer — pure wrapper, no local computation. */
{
    return xrootd_crc32c_copy_value(src, dst, len);
}
 ...
 * HOW: Delegates to xrootd_checksum_u32_fd(CRC32C) — the shared pread+accumulate loop in
 *      compat/checksum.c; returns (uint32_t)-1 sentinel on I/O error, matching prior API. */
uint32_t
xrootd_crc32c_file(int fd, const char *path, ngx_log_t *log)
{
    uint32_t out;

    if (xrootd_checksum_u32_fd(XROOTD_CHECKSUM_CRC32C, fd, path, log, &out)
        != NGX_OK)
    {
        return (uint32_t) -1;
    }
