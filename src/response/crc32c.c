...
/*
 * CRC32c (Castagnoli) used by paged read/write status framing.
 *
 * Fast path: SSE4.2 crc32q (8 bytes/instruction, ~1 cycle/8 bytes).
 * Slow path: byte-at-a-time table lookup, used on non-SSE4.2 or non-x86.
 */

static const uint32_t crc32c_tbl[256] = {
...
static uint32_t
crc32c_sw(const uint8_t *p, size_t len)
{
    uint32_t crc = 0xFFFFFFFFu;
    while (len--) {
        crc = (crc >> 8) ^ crc32c_tbl[(crc ^ *p++) & 0xFF];
    }
    return crc ^ 0xFFFFFFFFu;
}

static uint32_t
crc32c_copy_sw(const u_char *src, u_char *dst, size_t len)
{
    uint32_t crc = 0xFFFFFFFFu;
    while (len--) {
        uint8_t b = *src++;
        *dst++ = b;
        crc = (crc >> 8) ^ crc32c_tbl[(crc ^ b) & 0xFF];
    }
    return crc ^ 0xFFFFFFFFu;
}


/* ── SSE4.2 hardware path (x86-64 only) ────────────────────────────────── */

#ifdef __x86_64__

static int g_has_sse42 = -1;

static int
has_sse42(void)
{
    if (g_has_sse42 < 0) {
        g_has_sse42 = __builtin_cpu_supports("sse4.2") ? 1 : 0;
    }
    return g_has_sse42;
}
...
static uint32_t
crc32c_hw(const uint8_t *p, size_t len)
{
...
/* Fused copy + CRC using hardware crc32q; reads each byte once. */
__attribute__((target("sse4.2")))
static uint32_t
crc32c_copy_hw(const u_char *src, u_char *dst, size_t len)
{
    uint64_t crc = 0xFFFFFFFFu;
...
#ifdef __x86_64__
    if (has_sse42()) {
        return crc32c_hw((const uint8_t *) buf, len);
    }
#endif
    return crc32c_sw((const uint8_t *) buf, len);
}


/*
 * Fused copy + CRC32c: copies len bytes from src to dst in a single pass,
 * returning the CRC32c of the copied data.  Eliminates the double-read of
 * data that would occur when calling xrootd_crc32c() and ngx_memcpy()
 * separately on the same buffer.
 */
uint32_t
xrootd_crc32c_copy(const u_char *src, u_char *dst, size_t len)
{
#ifdef __x86_64__
    if (has_sse42()) {
        return crc32c_copy_hw(src, dst, len);
    }
#endif
    return crc32c_copy_sw(src, dst, len);
}
