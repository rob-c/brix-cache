...
static size_t
pmark_build(xrootd_pmark_flow_t *f, const char *state, int with_end,
    u_char *buf, size_t buflen)
{
    xrootd_pmark_sockstats_t  st;
    char       now[48], endt[80];
    uint64_t   recv, sent;
    ngx_str_t  host;
    int        n;
    ...
    xrootd_pmark_sockstats((int) f->fd, &st);
    ...
    xrootd_pmark_iso8601_now(now, sizeof(now));

    endt[0] = '\0';
    if (with_end) {
        char e[48];
        xrootd_pmark_iso8601_now(e, sizeof(e));
...
static void
pmark_emit(xrootd_pmark_flow_t *f, const char *state, int with_end,
    ngx_log_t *log)
{
    u_char               buf[1280];
    size_t               n;
    xrootd_pmark_conf_t *pm = f->pm;
    ngx_uint_t           i;
    ...
    if (pm->dest_sa) {
        xrootd_pmark_dest_t *d = pm->dest_sa->elts;
        for (i = 0; i < pm->dest_sa->nelts; i++) {
            ...
            if (sendto(fd, buf, n, 0, (struct sockaddr *) &d[i].ss, d[i].len)
                < 0)
            {
                XROOTD_PMARK_METRIC_INC(pmark_firefly_dropped_total);
                ngx_log_debug1(NGX_LOG_DEBUG_CORE, log, ngx_errno,
                    "pmark: firefly sendto failed (state=%s)", state);
            } else {
                XROOTD_PMARK_METRIC_INC(pmark_firefly_sent_total);
            }
        }
    }
    ...
    if (f->want_origin && f->peer_len) {
        ...
        if (o.ss_family == AF_INET) {
            ((struct sockaddr_in *) &o)->sin_port = htons(XROOTD_PMARK_FF_PORT);
        } else if (o.ss_family == AF_INET6) {
            ((struct sockaddr_in6 *) &o)->sin6_port =
                htons(XROOTD_PMARK_FF_PORT);
        }
    ...
    const char *path, const char *cgi, ngx_log_t *log)
{
    ...
    if (xrootd_pmark_runtime_ensure(pm, ngx_cycle->pool, log) != NGX_OK) {
        return NULL;
    }
    if (xrootd_pmark_map_codes(pm, vo_csv, user, path, cgi, &exp, &act)
        != NGX_OK)
    {
        XROOTD_PMARK_METRIC_INC(pmark_map_unresolved_total);
    ...
    xrootd_pmark_iso8601_now(f->start_iso, sizeof(f->start_iso));

    /* Endpoints: 0 = peer (client), 1 = local (server). */
    xrootd_pmark_endpoint((int) c->fd, 0, pip, sizeof(pip), &pport, &pafi);
    xrootd_pmark_endpoint((int) c->fd, 1, sip, sizeof(sip), &sport, &safi);
    f->afi = pafi;
    ...
    if (pm->flowlabel) {
        (void) xrootd_pmark_flowlabel_apply(c, (int) c->fd, exp, act, log);
    }
    ...
    XROOTD_PMARK_METRIC_INC(pmark_flows_started_total);
    return f;
}


void
xrootd_pmark_flow_end(xrootd_pmark_flow_t *flow, ngx_log_t *log)
{
    ...
    if (flow->firefly_started && flow->pm->firefly) {
        ...
        XROOTD_PMARK_METRIC_INC(pmark_flows_ended_total);
    }
}


void
xrootd_pmark_flow_echo(xrootd_pmark_flow_t *flow, ngx_log_t *log)
{
    if (flow == NULL || flow->pm == NULL) {
        return;
    }
...
void
xrootd_pmark_flow_cleanup(void *data)
{
    xrootd_pmark_flow_end((xrootd_pmark_flow_t *) data, ngx_cycle->log);
}


void
xrootd_pmark_firefly_oneshot(xrootd_pmark_conf_t *pm, int fd, ngx_uint_t exp,
    ngx_uint_t act, int peer_is_src, const char *app, ngx_log_t *log)
{
    ...
    xrootd_pmark_iso8601_now(f.start_iso, sizeof(f.start_iso));
    xrootd_pmark_endpoint(fd, 0, pip, sizeof(pip), &pport, &pafi);
    xrootd_pmark_endpoint(fd, 1, sip, sizeof(sip), &sport, &safi);
    f.afi = pafi;
...
void
xrootd_pmark_http_mark(xrootd_pmark_conf_t *pm, ngx_pool_t *pool,
    ngx_connection_t *c, int is_write, const char *vo_csv, const char *user,
    const char *path, const char *cgi)
{
    xrootd_pmark_flow_t *fl;
    ngx_pool_cleanup_t  *cln;

    fl = xrootd_pmark_flow_begin(pm, pool, c, is_write, vo_csv, user, path, cgi,
                                 c->log);
    if (fl == NULL) {
        return;
    }
    ...
    if (cln != NULL) {
        cln->handler = xrootd_pmark_flow_cleanup;
        cln->data    = fl;
    }
}
