#ifndef XROOTD_CACHE_CSTORE_H
#define XROOTD_CACHE_CSTORE_H
...
#define XROOTD_CMETA_AUTO    0
#define XROOTD_CMETA_LOCAL   1
#define XROOTD_CMETA_XATTR   2
#define XROOTD_CMETA_SIDECAR 3
...
typedef struct {
    ...
    int                   meta_mode;    /* XROOTD_CMETA_* (resolved, never AUTO)   */
...
typedef ngx_int_t (*xrootd_cstore_visit_fn)(const char *key,
    const xrootd_cache_cinfo_t *ci, const xrootd_sd_stat_t *stx, void *ctx);
 ...
 * XROOTD_CMETA_AUTO to resolve from the store's caps, or a forced mode. Builds the
...
ngx_int_t xrootd_cstore_init(xrootd_cstore_t *cs, xrootd_sd_instance_t *store,
...
xrootd_sd_staged_t *xrootd_cstore_fill_open(xrootd_cstore_t *cs, const char *key,
    mode_t mode);

/* Write `len` bytes at `off` into the fill sink. Returns bytes written, or -1. */
ssize_t xrootd_cstore_fill_write(xrootd_sd_staged_t *st, const void *buf,
...
ngx_int_t xrootd_cstore_fill_commit(xrootd_sd_staged_t *st);

/* Drop a partial fill (consumes `st`). Best-effort. */
void xrootd_cstore_fill_abort(xrootd_sd_staged_t *st);
...
xrootd_sd_obj_t *xrootd_cstore_serve_open(xrootd_cstore_t *cs, const char *key,
    int *err);
...
typedef int (*xrootd_cstore_fill_block_fn)(void *ctx, uint64_t blk);
...
ssize_t xrootd_cstore_serve_pread(int cache_fd, const uint8_t *bitmap,
    uint64_t nblocks, uint32_t block_size, off_t size, void *buf, size_t len,
    off_t off, xrootd_cstore_fill_block_fn fill, void *ctx);
 ...
 * cache object). */
int xrootd_cstore_partial_open(xrootd_cstore_t *cs, const char *key, mode_t mode,
...
ngx_int_t xrootd_cstore_evict(xrootd_cstore_t *cs, const char *key);
...
ngx_int_t xrootd_cstore_cinfo_load(xrootd_cstore_t *cs, const char *key,
...
ngx_int_t xrootd_cstore_cinfo_store(xrootd_cstore_t *cs, const char *key,
...
ngx_int_t xrootd_cstore_scan(xrootd_cstore_t *cs, xrootd_cstore_visit_fn visit,
    void *ctx);
...
ngx_int_t xrootd_cstore_freespace(xrootd_cstore_t *cs, uint64_t *total,
    uint64_t *avail);

#endif /* XROOTD_CACHE_CSTORE_H */
