...
operator configures a target: `xrootd_mirror_url http[s]://host:port` on a
WebDAV location, or `xrootd_stream_mirror_url host:port` on a stream server
block. Up to `XROOTD_MIRROR_MAX_TARGETS` (4) shadows may be configured per
...
(`xrootd_mirror_should_sample`) applies the configured sample percentage.

The subsystem covers three surfaces, all sharing one config block
(`xrootd_mirror_conf_t` in `mirror.h`) and one set of low-cardinality metrics:
...
| File | Responsibility |
|---|---|
| `mirror.h` | Protocol-agnostic shared config (`xrootd_mirror_conf_t`, `xrootd_mirror_target_t`), the HTTP method bitmask (`XROOTD_MIRROR_M_*`) and XRootD opcode bitmask (`XROOTD_MIRROR_OP_*`), and the inline `xrootd_mirror_should_sample()` / `xrootd_mirror_status_class()` helpers. Pulls in no HTTP- or stream-specific types so it is includable from either surface. |
| `http_mirror.h` | Public API for the HTTP/WebDAV mirror: the two phase handlers, the merge-time `xrootd_http_mirror_setup()`, and the `xrootd_mirror_url` / `xrootd_mirror_methods` directive setters. |
...
| `stream_mirror.h` | Public API for the stateless stream mirror: `xrootd_stream_mirror_maybe()` (the launch hook) plus the `xrootd_stream_mirror_url` / `xrootd_mirror_opcodes` / `xrootd_mirror_exclude_opcodes` directive setters. |
...
- **`xrootd_mirror_conf_t`** (`mirror.h`) — the shared knobs embedded in both the
...
- **`xrootd_mirror_target_t`** (`mirror.h`) — one resolved shadow: display `url`,
  `host` (Host:/SNI/log label), `port`, `ssl`, `url_base`
  (`scheme://host[:port]`, HTTP only), and the config-time-resolved `sockaddr` /
  `socklen`.
- **Opcode / method bitmasks** (`mirror.h`) — `XROOTD_MIRROR_M_*` and
  `XROOTD_MIRROR_OP_*`. Note `*_DEFAULT` vs `*_OP_ALL`: when mirroring is enabled
  with no explicit `xrootd_mirror_opcodes`, the stream default is `OP_ALL` (mirror
  everything, de-select via `xrootd_mirror_exclude_opcodes`). All write/mutation
  bits (`*_WRITE_ALL`) are deliberately excluded from the default sets and gated
  by a second independent flag, `mirror_writes`.
- **`xrootd_stream_mirror_t`** (`stream_mirror.c`) — the per-replay async client:
  its own `pool`/`conn`/`log`, the `xrootd_mir_phase_t` state, response
...
- **`xrootd_wmirror_conn_t` / `xrootd_wmirror_file_t`** (`stream_wmirror.c`) — the
  per-connection accumulation state (`ctx->wmirror`): a `files[XROOTD_MAX_FILES]`
...
- **`xrootd_wmirror_replay_t`** (`stream_wmirror.c`) — the detached write-replay
  client, structurally parallel to `xrootd_stream_mirror_t` but driven through the
...
**HTTP/WebDAV.** `xrootd_http_mirror_precontent_handler`
...
`xrootd_http_mirror_proxy`, which builds an `ngx_http_upstream_t` against the
...
compares status classes and bumps metrics; `xrootd_http_mirror_log_handler`
...
`xrootd_http_mirror_setup` (called from `../webdav/config.c`).

**Stream reads / metadata mutations.** `../handshake/dispatch.c` calls
`xrootd_stream_mirror_maybe()` immediately after a read- or write-opcode dispatch
...
`xrootd_stream_wmirror_on_open()` (from `../read/open_resolved_file.c` on a write
open) starts accumulation; `xrootd_stream_wmirror_observe()` (from
...
transfers buffer ownership; `xrootd_stream_wmirror_cleanup()` (from
`../connection/disconnect.c`) frees any leftover per-file buffers.

**Calls out to.** `../upstream/README.md` (`xrootd_upstream_build_bootstrap`
wire framing), `../metrics/README.md` (`xrootd_metrics_shared`), `../webdav/`
...
- **Fire-and-forget, never blocking the client.** The primary response is already
  ...
  (`xrootd_mir_finish`/`wmir_finish`) just tears down the shadow and counts a
...
- **Stateless replayability is mandatory for reads.** `stream_mirror.c` replays
  ...
  `XROOTD_MIRROR_OPEN_WRITE_FLAGS` are rejected, `stream_mirror.c:139-171`).
...
- **Benign-difference handling (mirror must "just work").** A shadow that returns
  ...
  NOT counted as divergence (`xrootd_mir_on_response`, `xrootd_mir_dispatch`).
...
- **Bounded buffers.** Shadow response bodies are capped at 64 KiB
  ...
  `XROOTD_MIRROR_MAX_PAYLOAD` (4 KiB); write accumulation at 4 MiB/file and
...
- **Body cloning for PUT (HTTP).** `xrootd_http_mirror_clone_body` duplicates only
...
- **Add a mirrorable HTTP method:** add a bit in `mirror.h` (`XROOTD_MIRROR_M_*`),
  map it in `xrootd_http_mirror_method_bit`, accept its name in
  `xrootd_http_mirror_set_methods`, and (if write-implying) include it in
  `XROOTD_MIRROR_M_WRITE_ALL` so `mirror_writes` gates it. If it carries a body,
  extend `xrootd_http_mirror_method_has_body` and the body-clone path.
- **Add a mirrorable XRootD opcode:** add a bit in `mirror.h`
  (`XROOTD_MIRROR_OP_*`), map `kXR_*` → bit in `xrootd_mirror_opcode_bit`, decide
  replayability in `xrootd_mirror_request_replayable`, accept its name in
  `xrootd_mirror_parse_opcode_args`, and add it to `OP_ALL` (and, if a write,
  `OP_WRITE_ALL`).
- **Add a metric:** declare an `ngx_atomic_t mirror_*` field in
  `../metrics/metrics.h`, export it in `../metrics/`, and increment via the local
  `MIR_HTTP_INC` / `XROOTD_MIR_METRIC_INC` / `XROOTD_WMIR_METRIC_INC` macro.
- **Add a directive:** declare the field in `xrootd_mirror_conf_t`, register the
  `ngx_command_t` (WebDAV in `../webdav/module.c`, stream in
  `../stream/module.c`), and merge it in the surface's `merge_*_conf`. The setters
  here (`xrootd_http_mirror_set_url`, `xrootd_stream_mirror_set_url`, etc.) are the
  template.

## See also

- [`../handshake/README.md`](../handshake/README.md) — stream dispatch hooks that
  invoke `xrootd_stream_mirror_maybe` / `xrootd_stream_wmirror_observe`.
...
- [`../upstream/README.md`](../upstream/README.md) —
  `xrootd_upstream_build_bootstrap` and the health-check probe this mirror's wire
