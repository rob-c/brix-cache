 ...
 * after the configured TTL (default XROOTD_TPC_KEY_TTL_MS; override with
 * xrootd_tpc_key_ttl) to prevent resource leaks if
...
#define XROOTD_TPC_KEY_SLOTS   256
#define XROOTD_TPC_KEY_LEN     128
#define XROOTD_TPC_KEY_TTL_MS  60000   /* 60 seconds */

typedef struct {
    char        key[XROOTD_TPC_KEY_LEN];
...
typedef struct {
    ngx_shmtx_sh_t          lock;
    xrootd_tpc_key_entry_t  slots[XROOTD_TPC_KEY_SLOTS];
} xrootd_tpc_key_table_t;

/* Called once from postconfiguration to allocate the shared memory zone. */
ngx_int_t  xrootd_tpc_key_configure_registry(ngx_conf_t *cf);
 ...
 */
int   xrootd_tpc_key_consume(const char *key);
