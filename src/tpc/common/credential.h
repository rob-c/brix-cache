#ifndef BRIX_TPC_COMMON_CREDENTIAL_H
#define BRIX_TPC_COMMON_CREDENTIAL_H
 ...
 *       brix_tpc_credential_type_t (NONE / PROXY / TOKEN), the
 *       brix_tpc_credential_t value object (proxy_pem, bearer, identity,
 ...
 *
 * HOW: brix_tpc_credential_parse() sniffs the raw credential (honouring an
 *      optional caller hint, the "Bearer " prefix, or a "-----BEGIN" PEM
 *      marker) into a typed brix_tpc_credential_t; brix_tpc_credential_validate()
 *      rejects empty or expired credentials; brix_tpc_credential_type_name()
...
typedef enum {
    BRIX_TPC_CREDENTIAL_NONE  = 0,
    BRIX_TPC_CREDENTIAL_PROXY = 1,
    BRIX_TPC_CREDENTIAL_TOKEN = 2,
} brix_tpc_credential_type_t;
...
ngx_int_t brix_tpc_credential_parse(const ngx_str_t *raw_credential,
...
ngx_int_t brix_tpc_credential_validate(
    const brix_tpc_credential_t *cred, ngx_log_t *log);
