 ...
 *       flattened xrootd_tpc_transfer_snapshot_t used to copy a transfer out of
 ...
 *      snapshot type deliberately mirrors xrootd_tpc_transfer_t but with embedded
 *      char buffers so callers can read a consistent copy without holding the
 *      registry lock or pointing into shared memory.
 *
 * HOW: xrootd_tpc_registry_configure() reserves the shared-memory zone at config
 *      time; xrootd_tpc_registry_add() publishes a transfer and returns its id;
 *      update/remove/find mutate or look up by id; xrootd_tpc_registry_snapshot()
 *      bulk-copies up to max_transfers entries; xrootd_tpc_progress_emit() is the
...
/* Reserve the "xrootd_tpc_transfers" shared-memory zone and register its init
 ...
 * xrootd_tpc_registry_update() for transfer id and returns its result.
