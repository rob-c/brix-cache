...
2. **Overflow-checked size arithmetic** (`safe_size.h`, header-only). The
   primitives `xrootd_size_mul`/`xrootd_size_add` and the array allocators
   `xrootd_palloc_array`/`xrootd_pcalloc_array`/`xrootd_alloc_array` turn
...
| File | Responsibility |
|---|---|
| `file_serve.c` | Implements `xrootd_http_serve_file_ranged()` — the 5-phase shared HTTP body-send pipeline: (1) parse `Range:` via `xrootd_http_parse_range`, short-circuiting to `416` if unsatisfiable; (2) emit `Last-Modified`/`Content-Length`/`Content-Range`/ETag via `xrootd_http_set_file_headers` and fire the optional `pre_header_send` hook; (3) start dashboard transfer tracking via `xrootd_dashboard_http_start_identity`; (4) `dup()` the fd, release the VFS handle, send the range via `xrootd_http_send_file_range`; (5) post-send byte accounting (`xrootd_dashboard_http_add`) + cache-access recording (`xrootd_cache_record_access`) for cache-backed handles. |
| `file_serve.h` | Public interface: `xrootd_http_serve_file_ranged()` prototype, the `xrootd_http_serve_opts_t` input struct, the `xrootd_http_serve_result_t` output struct, the `xrootd_http_pre_header_fn` hook typedef, and the `XROOTD_SERVE_RANGE_FULL/_PARTIAL/_UNSATISFIED` outcome constants. |
| `safe_size.h` | Header-only (all `static ngx_inline`, no new translation unit): `xrootd_size_mul`/`xrootd_size_add` (overflow-detecting via `__builtin_*_overflow`, with a portable fallback for other compilers) and `xrootd_palloc_array`/`xrootd_pcalloc_array` (request-pool) / `xrootd_alloc_array` (heap, for the long-lived stream path) array allocators that return `NULL` on overflow, on a zero-size request, or on OOM. Compiles standalone under `XROOTD_SAFE_SIZE_STANDALONE` for the fuzz target. |

## Key types & data structures

- **`xrootd_http_serve_opts_t`** (`file_serve.h:22`) — caller-supplied options for a
  serve. Carries `xfer_proto` (`XROOTD_XFER_PROTO_WEBDAV`/`_S3`) and `op_name`
  (`"GET"`/`"GetObject"`) for dashboard/metric labelling, the resolved
  `identity` display string, `etag_flags` (`XROOTD_ETAG_WEAK` or 0), and the
  optional `pre_header_send`/`pre_header_ud` hook.
- **`xrootd_http_pre_header_fn`** (`file_serve.h:19`) —
...
- **`xrootd_http_serve_result_t`** (`file_serve.h:31`) — what the caller reads
  back to drive its own metrics: `range_result` (one of `XROOTD_SERVE_RANGE_*`)
...
- **`xrootd_vfs_file_t` / `xrootd_vfs_stat_t`** (from `../fs/vfs.h`) — the
  already-open handle and its stat snapshot that the caller hands in. Accessed
  here only through VFS accessors: `xrootd_vfs_file_fd`, `xrootd_vfs_file_path`,
  `xrootd_vfs_file_from_cache`, `xrootd_vfs_close`.
...
`xrootd_http_serve_file_ranged()`, called from two HTTP read handlers:
...
satisfied from cache). It builds an `xrootd_http_serve_opts_t`, calls in with the
open handle + stat, and reads back `xrootd_http_serve_result_t` to bump its own
range/bytes metrics.

Inside, the module calls out to:
- `../compat/range.h` — `xrootd_http_parse_range` (RFC 7233 `Range:` parsing).
- `../compat/http_file_response.h` — `xrootd_http_set_file_headers` and
  `xrootd_http_send_file_range` (the actual nginx header emission and
  sendfile/range body send).
- `../dashboard/dashboard_tracking.h` — `xrootd_dashboard_http_start_identity` /
  `_add` / `_error` / `_finish` for live transfer monitoring.
- `../cache/open.h` — `xrootd_cache_record_access` to update LRU access stats when
  the bytes came from a cache-backed handle (`xrootd_vfs_file_from_cache`).
- `../fs/vfs.h` — handle accessors and `xrootd_vfs_close`.
...
Wire-driven allocation sites compute the size with `xrootd_size_mul` (or allocate
...
- `../read/readv.c` — the readv segment-descriptor array, guarded both by an
  explicit `xrootd_size_mul(segment_count, sizeof(*ranges), …)` pre-check
  (`readv.c:87`) and by allocation through `xrootd_alloc_array(c->log,
...
- `../cache/evict_candidates.c` — the eviction-candidate list realloc growth, sized
  with `xrootd_size_mul(new_cap, sizeof(list->elts[0]), …)` (`evict_candidates.c:237`)
  before the buffer is grown.

The header also compiles standalone for the libFuzzer target
`tests/fuzz/fuzz_safe_size.c`, which defines `XROOTD_SAFE_SIZE_STANDALONE` to skip
...
- **The handle is closed for you — exactly once.** Every return path in
  `xrootd_http_serve_file_ranged` calls `xrootd_vfs_close(fh, …)` before
...
- **Body is sent over a `dup()`'d fd, not the handle's fd** (`file_serve.c:108`).
  ...
  failure is a `500` (after `xrootd_dashboard_http_error`/`_finish`), not a silent
  partial response.
...
- **Metrics are the caller's job, by design.** This module returns
  ...
  sets. Do not add `XROOTD_*_METRIC_INC` calls here.
- **TLS vs cleartext is handled downstream, not here.** The actual buffer choice
  ...
  `xrootd_http_send_file_range` / `../compat/http_file_response.c`. Do not add a
...
- **Pool vs heap allocator must match the buffer's lifetime.** Use
  `xrootd_palloc_array`/`xrootd_pcalloc_array` for request-pool memory (freed when
  the request pool is destroyed) and `xrootd_alloc_array` (free with `ngx_free`)
...
xrootd_http_serve_opts_t   opts = {0};
xrootd_http_serve_result_t res;

opts.xfer_proto      = XROOTD_XFER_PROTO_WEBDAV;   /* or _S3 */
opts.op_name         = "GET";                       /* or "GetObject" */
opts.identity        = resolved_identity;
opts.etag_flags      = XROOTD_ETAG_WEAK;            /* or 0 */
opts.pre_header_send = my_checksum_header_hook;     /* or NULL */
opts.pre_header_ud   = my_ctx;

ngx_int_t rc = xrootd_http_serve_file_ranged(r, fh, &vst, fs_path, &opts, &res);
...
with `xrootd_palloc_array(pool, n, sizeof(*p))` (or `xrootd_pcalloc_array` for
zeroed, `xrootd_alloc_array(log, …)` for heap/stream-lifetime), and check for
`NULL`. For a bare size computation feeding something else, use
`xrootd_size_mul`/`xrootd_size_add` and check the `NGX_OK`/`NGX_ERROR` return
before reading `*out`.
