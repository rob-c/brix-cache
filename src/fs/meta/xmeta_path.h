#ifndef XROOTD_FS_META_XMETA_PATH_H
#define XROOTD_FS_META_XMETA_PATH_H
...
#define XROOTD_XMETA_PATH_XATTR    "user.xrd.cinfo"
#define XROOTD_XMETA_PATH_SIDECAR  ".cinfo"
#define XROOTD_XMETA_PATH_XATTR_MAX (64 * 1024)

/* Compose "<path>.cinfo" into dst[cap]. 0 on success, -1 if too long. */
int  xrootd_xmeta_sidecar_path(char *dst, size_t cap, const char *path);

/* Load + decode the record for `path` (xattr first, then the sidecar).
 * XROOTD_XMETA_OK (caller frees *xm) / FOREIGN (nothing valid recorded) /
 * ERR (errno). */
int  xrootd_xmeta_path_load(const char *path, xrootd_xmeta_t *xm);

/* Encode + persist the record for `path` (xattr preferred, sidecar fallback;
 * exactly one carrier survives). OK / ERR (errno). */
int  xrootd_xmeta_path_save(const char *path, const xrootd_xmeta_t *xm);

/* Drop both carriers (best-effort, absent is fine). Always OK. */
int  xrootd_xmeta_path_remove(const char *path);
...
int  xrootd_xmeta_path_lock(const char *path);
void xrootd_xmeta_path_unlock(int lockfd);

#endif /* XROOTD_FS_META_XMETA_PATH_H */
