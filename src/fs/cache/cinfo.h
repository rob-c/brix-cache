#ifndef BRIX_CACHE_CINFO_H
#define BRIX_CACHE_CINFO_H
 ...
 *       brix_cache_cinfo_record_block(), which serialises the
...
#include "meta.h"   /* BRIX_CACHE_META_ETAG_MAX + brix_cache_meta_t */
#include "fs/meta/xmeta.h"   /* the unified on-disk record this maps to */

#define BRIX_CACHE_CINFO_MAGIC   0x58434931u   /* "XCI1", little-endian */
#define BRIX_CACHE_CINFO_VERSION 3

/* cinfo.flags */
#define BRIX_CINFO_F_COMPLETE 0x0001u   /* every block present */
#define BRIX_CINFO_F_PARTIAL  0x0002u   /* some, not all, blocks present */
#define BRIX_CINFO_F_VERIFIED 0x0004u   /* contents checked vs the origin digest */
#define BRIX_CINFO_F_DIRTY    0x0008u   /* local writes pending write-back */
#define BRIX_CINFO_F_EXPIRES  0x0010u   /* expires_at present+valid
...
#define BRIX_CACHE_DIRTY_BLOCK  (1024u * 1024u)
 ...
 * The bitmap (brix_cache_cinfo_bitmap_len(nblocks) bytes) follows on disk
 * immediately after sizeof(brix_cache_cinfo_t) bytes of header.
 */
typedef struct {
    uint32_t magic;          /* BRIX_CACHE_CINFO_MAGIC */
    uint16_t version;        /* BRIX_CACHE_CINFO_VERSION */
    uint16_t flags;          /* BRIX_CINFO_F_* */
    ...
    uint8_t  etag_len;
    char     etag[BRIX_CACHE_META_ETAG_MAX];      /* origin etag, not NUL-term */
    uint8_t  cks_alg_len;
...
#define BRIX_CACHE_CINFO_HDR_SIZE (sizeof(brix_cache_cinfo_t))
/* Pre-phase-68 v3 header (no expires_at/filled_at trailer): 16 bytes shorter. */
#define BRIX_CACHE_CINFO_HDR_SIZE_PRE68 (sizeof(brix_cache_cinfo_t) - 16)
 ...
 * brix_cache_cinfo_bitmap_len()+memset(0xff). Returns NGX_OK / NGX_ERROR.
 */
 ...
 * BRIX_CINFO_F_DIRTY and widens [dirty_lo,dirty_hi); dirty_since is stamped
...
/*
 * Clear the dirty extent for cache_path: drop BRIX_CINFO_F_DIRTY, zero the
 ...
 * reaped (see brix_cache_reap_reason_t):
 *   is_dirty   — BRIX_CINFO_F_DIRTY set AND dirty_lo<dirty_hi (un-flushed data)
...
typedef struct {
    ...
    uint64_t flush_gen;
    uint64_t last_flush;
} brix_cache_cinfo_state_t;

ngx_int_t brix_cache_cinfo_state(const char *cache_path,
    brix_cache_cinfo_state_t *out);

#endif /* BRIX_CACHE_CINFO_H */
