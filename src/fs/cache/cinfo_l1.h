#ifndef BRIX_CACHE_CINFO_L1_H
#define BRIX_CACHE_CINFO_L1_H
 ...
 *       record (brix_cache_cinfo_t validity/stats/digest, no bitmap). It is the
...
#include "cinfo.h"   /* brix_cache_cinfo_t (the cached header record) */
...
brix_cinfo_l1_t *brix_cinfo_l1_create(size_t max_entries, ngx_log_t *log);

/* Free an L1 cache and every entry it holds. NULL-safe. */
void brix_cinfo_l1_destroy(brix_cinfo_l1_t *l1);
...
ngx_int_t brix_cinfo_l1_get(brix_cinfo_l1_t *l1, const char *key,
...
void brix_cinfo_l1_put(brix_cinfo_l1_t *l1, const char *key,
    const brix_cache_cinfo_t *hdr);

/* Drop `key` from the cache (on eviction / unlink / a failed fill). Idempotent. */
void brix_cinfo_l1_drop(brix_cinfo_l1_t *l1, const char *key);

#endif /* BRIX_CACHE_CINFO_L1_H */
