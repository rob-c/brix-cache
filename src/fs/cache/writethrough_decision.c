 ...
 * prefixes (whitelist) → default ALLOW_SYNC. A custom brix_wt_decision_fn can
 * replace it via config.
 */

/* brix_wt_default_decide — the policy decision at kXR_open write intent (from
...
brix_wt_decision_t brix_wt_default_decide(const char *path, uint16_t options,
                                               void *user_data)
{
    brix_wt_decision_cfg_t *cfg;
    brix_cache_admit_cfg_t  a;
    off_t                     size = 0;
    int                       is_new;

    if (user_data == NULL || path == NULL) {
        return BRIX_WT_DECISION_DENY;          /* fail-closed */
    }
    cfg = (brix_wt_decision_cfg_t *) user_data;
    ...
    if (brix_cache_admit(&a, path, size, is_new) == BRIX_CACHE_DECLINE) {
        return BRIX_WT_DECISION_DENY;
    }
    return BRIX_WT_DECISION_ALLOW_ASYNC;
}

/* brix_wt_config_init_prefixes — populate a cf->pool-lived prefix array (created
                                          ...
                                          const char *directive_name)
{
    ...
    if (*out_array == NULL) {
        *out_array = ngx_array_create(cf->pool, 4, sizeof(brix_wt_prefix_entry_t));
        if (*out_array == NULL) {
            return NGX_ERROR;
        }
    }
    ...
    for (i = 0; i < prefix_list->nelts; i++) {
        ...
        brix_wt_prefix_entry_t *entry = ngx_array_push(*out_array);
        if (entry == NULL) {
            return NGX_ERROR;
        }
        entry->prefix = src[i];
