 ...
 * Refreshes *usage with brix_cache_fs_usage() so the caller's threshold loop
 ...
 */
static ngx_int_t
brix_cache_evict_one(ngx_stream_brix_srv_conf_t *conf, brix_ctx_t *ctx,
    ngx_connection_t *c, brix_cache_evict_list_t *list, size_t idx,
    ngx_log_t *log, brix_cache_fs_usage_t *usage, ngx_uint_t *evicted_files,
    ...
    uint64_t *evicted_bytes_out)
{
    ...
    if (conf->cache_max_file_size > 0) {
             ...
             && evict_rc == NGX_OK; i++)
        {
            ...
            evict_rc = brix_cache_evict_one(conf, ctx, c, &list, i, log,
