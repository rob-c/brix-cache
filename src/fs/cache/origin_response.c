...
/* brix_cache_read_response — read XRootD server response header + body
...
/* brix_cache_set_origin_error — extract origin error code + message, fallback
...
int
brix_cache_read_response(brix_cache_fill_t *t,
    ...
    uint32_t *dlen, uint32_t max_body)
{
    ...
    *dlen = 0;
    ...
    if (brix_cache_io_recv_exact(oc, &hdr, sizeof(hdr)) != 0) {
    ...
    *body = malloc((size_t) *dlen + 1);
    ...
    if (brix_cache_io_recv_exact(oc, *body, *dlen) != 0) {
        free(*body);
        *body = NULL;
        brix_cache_set_error(t, kXR_ServerError, errno,
...
void
brix_cache_set_origin_error(brix_cache_fill_t *t, u_char *body,
    uint32_t dlen, const char *fallback)
{
    ...
    if (xrd_error_body_decode(body, dlen, &errcode, &wmsg, &wlen) == 0
        ...
        msg[wlen] = '\0';
        brix_cache_set_error(t, errcode, 0, msg);
        return;
    }

    brix_cache_set_error(t, errcode, 0, fallback);
}
