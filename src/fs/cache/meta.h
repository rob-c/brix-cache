#ifndef BRIX_CACHE_META_H
#define BRIX_CACHE_META_H

/*
 * meta.h — on-disk cache-validity metadata sidecar interface.
 *
 * WHAT: Defines brix_cache_meta_t (origin mtime, size, and a bounded etag)
 *       and the read/write/derive API implemented in meta.c, plus the
 *       BRIX_CACHE_META_ETAG_MAX bound on the inline etag field.
 *
 ...
 *       without invalidating existing sidecars. brix_cache_meta_path() (in
...
#define BRIX_CACHE_META_ETAG_MAX 55
 ...
 */
#define BRIX_CACHE_META_VERSION 1
...
typedef struct {
    ...
    uint64_t size;
    uint8_t  etag_len;
    char     etag[BRIX_CACHE_META_ETAG_MAX];
...
#define BRIX_CACHE_META_BASE_SIZE \
    (offsetof(brix_cache_meta_t, version))
 ...
 */
int brix_cache_meta_path(char *dst, size_t dstsz, const char *cache_path);
 ...
 * BRIX_CACHE_META_ETAG_MAX etag bytes, silently truncating longer etags) into
 * the caller-owned *meta, which it first zeroes. Returns NGX_OK; NGX_ERROR (sets
 * errno EINVAL) only if st or meta is NULL.
 */
ngx_int_t brix_cache_meta_from_stat(const struct stat *st, const char *etag,
 ...
 */
ngx_int_t brix_cache_meta_read(ngx_log_t *log, const char *cache_path,
    brix_cache_meta_t *meta);
 ...
 */
ngx_int_t brix_cache_meta_write(ngx_log_t *log, const char *cache_path,
 ...
 */
ngx_int_t brix_cache_meta_touch(ngx_log_t *log, const char *cache_path,
    uint64_t nbytes);

#endif /* BRIX_CACHE_META_H */
