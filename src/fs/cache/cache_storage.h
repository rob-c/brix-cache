#ifndef BRIX_CACHE_STORAGE_H
#define BRIX_CACHE_STORAGE_H

/*
 * cache_storage.h — per-role SD storage instances for the cache.
 *
 * WHAT: The cache performs ALL disk byte-I/O through an brix_sd_instance_t per
 ...
 *
 * HOW:  brix_cache_storage_init() (per-worker, from process.c) opens the O_PATH
 *       rootfds and builds the instances; brix_cache_storage_cleanup() closes
...
#include "cstore.h"                 /* brix_cstore_t (policy-layer adapter) */
...
ngx_int_t brix_cache_storage_init(ngx_stream_brix_srv_conf_t *conf,
    ngx_cycle_t *cycle);

/* Close the per-role rootfds at worker exit. NULL-safe / idempotent. */
void brix_cache_storage_cleanup(ngx_stream_brix_srv_conf_t *conf);
...
brix_cstore_t *brix_cache_storage_cstore(const ngx_stream_brix_srv_conf_t *conf);
/* §14: always NULL now (the legacy cache_origin whole-file source is retired —
 * an `brix_cache on` cache fills from the export's registered backend, resolved
 * per fill by the C-1 spine). Kept for the transitional callers. */
brix_sd_instance_t *brix_cache_source_inst(const ngx_stream_brix_srv_conf_t *conf);
...
brix_sd_instance_t *brix_cache_wt_stage_sd_inst(const ngx_stream_brix_srv_conf_t *conf);
/* The POSIX instance holding the .cinfo/.meta sidecar tree. NULL if no cache. */
brix_sd_instance_t *brix_cache_state_storage(const ngx_stream_brix_srv_conf_t *conf);
...
brix_sd_instance_t *brix_cache_wt_stage(const ngx_stream_brix_srv_conf_t *conf);
...
brix_sd_instance_t *brix_cache_storage_by_root(const char *cache_root_canon);
brix_sd_instance_t *brix_cache_state_by_root(const char *cache_root_canon);
/* The POSIX state (sidecar) root for a cache root (== cache root if co-located). */
const char *brix_cache_state_root_by_root(const char *cache_root_canon);
 ...
 * 0 / -1. */
int brix_cache_sidecar_path(const char *cache_root, const char *state_root,
 ...
 * cache_path); a POSIX cache falls back to brix_cache_file_ready(). */
int brix_cache_ready(const ngx_stream_brix_srv_conf_t *conf,
...
const char *brix_cache_key_under_root(const ngx_stream_brix_srv_conf_t *conf,
                                        const char *cache_path);

/* Export-relative cache key for `resolved` (server-controlled, leading-slash).
 * 0 / -1 (not under the export root, or overflow). */
int brix_cache_key(const ngx_stream_brix_srv_conf_t *conf,
