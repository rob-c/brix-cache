/*
 * s3_transport_setup.c — operator policy, per-thread curl handle lifecycle,
 * per-request curl option configuration and the upstream trace line for the
 * server-side libcurl brix_s3_transport_t. Split verbatim out of s3_transport.c
 * (request execution + response capture + vtable) with no behavior change; see
 * s3_transport_internal.h for the shared seam.
 */
