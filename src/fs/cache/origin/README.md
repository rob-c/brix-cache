...
driver vtable (`brix_cache_transport_t`, declared in `transport.h`) selected from
...
| File | Responsibility |
|---|---|
| `transport.h` | The driver seam: `brix_cache_transport_t` vtable, capability bits (`BRIX_CACHE_CAP_*`), the parsed-URL type, origin-digest type, `brix_cache_origin_url_parse()`, and `brix_cache_transport_for(scheme)`. |
