...
/* brix_cache_lock_is_stale — is the lock file at `lock_path` orphaned?
 *
 ...
 */
static int
brix_cache_lock_is_stale(const char *lock_path, time_t timeout)
{
    int          fd;
    ssize_t      n;
    char         buf[64];
...
/* brix_cache_try_lock — attempt exclusive lock on cache fill path
...
static int
brix_cache_try_lock(brix_cache_fill_t *t)
{
    int  fd, n;
    char body[64];
 ...
 * usleep(BRIX_CACHE_LOCK_POLL_USEC) provides backoff to reduce contention. Every failure
 * path sets cache error via brix_cache_set_syserror() or brix_cache_set_error(). */

int
brix_cache_wait_or_lock(brix_cache_fill_t *t, int *owned)
{
    time_t start;
    ...
    *owned = 0;
    ...
    for (;;) {
        ...
        ready = brix_cache_file_ready(t->cache_path);
        if (ready == 1) {
            return 0;
        }
        if (ready < 0) {
        ...
        if (brix_cache_lock_is_stale(t->lock_path,
        ...
        if (time(NULL) - start >= t->conf->cache_lock_timeout) {
            brix_cache_set_error(t, kXR_FileLocked, 0,
                                   "timed out waiting for cache fill lock");
            return -1;
        }

        usleep(BRIX_CACHE_LOCK_POLL_USEC);
    }
}
