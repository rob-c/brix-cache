...
cstore_make_parents(brix_cstore_t *cs, const char *key)
{
    ...
    for (i = 1; i < n; i++) {                   /* skip the leading '/' */
        if (path[i] == '/') {
            path[i] = '\0';
            /* EEXIST ok; svc-owned cache tree, not world-listable */
            (void) cs->store->driver->mkdir(cs->store, path, BRIX_PERM_RESTRICTED);
            path[i] = '/';
        }
    }
}
    ...
    off_t size, char *path_out, size_t path_cap)
{
    ...
    cstore_make_parents(cs, key);
    /* svc-owned cache file; private default */
    fd = open(path, O_RDWR | O_CREAT | O_CLOEXEC, mode ? mode : BRIX_PERM_PRIVATE);
    if (fd < 0) {
        return -1;
    }
...
/* ---- scan (namespace walk) ------------------------------------------------ */
/* The scan cluster (brix_cstore_scan + cstore_scan_* helpers) lives in the
 * sibling cstore_scan.c to keep this file under the size cap. */
