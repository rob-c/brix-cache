...
| `vfs_backend_config.c` | Per-export storage-backend **directive parsing** (phase-67 split): turns the `xrootd_*_backend` / origin / §14-credential config into `xrootd_vfs_backend_entry_t` registry entries. |
| `vfs_backend_registry.c` / `vfs_backend_internal.h` | Per-export backend **registry**: the `xrootd_vfs_backend_entry_t` table, source build + tier/decorator composition, and `xrootd_vfs_backend_resolve()`. Also exposes `xrootd_vfs_backend_http_endpoint(root_canon, &host, &port, &tls, &base)` — the HTTP origin of an `http`/`https` backend, for protocol-side **uncached passthroughs** (phase-68 cvmfs geo/manifest) that address the same origin the tier fills from. (phase-67 split the old monolithic `vfs_backend_registry.c` into these two.) |
