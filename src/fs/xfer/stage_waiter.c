...
ngx_int_t
brix_stage_waiter_configure(ngx_conf_t *cf, ngx_uint_t slots)
{
    ngx_str_t  zone_name = ngx_string("brix_stage_waiters");
    size_t     zone_size;
    ...
    zone_size = brix_shm_zone_size(sizeof(stage_waiter_table_t)
...
ngx_int_t
brix_stage_waiter_add(const char *reqid, uint16_t options,
...
stage_waiter_deliver_one(const stage_waiter_t *w)
{
    ...
    brix_stage_registry_t       *q = brix_stage_registry_singleton();
    ...
    if (w->code == 0 && q != NULL
        && brix_stage_request_get(q, w->reqid, &rec, c->log) == NGX_OK)
    {
        ...
        body[4 + mlen] = '\0';
        (void) brix_send_attn_asynresp(ctx, c, w->client_streamid,
                                         (uint16_t) kXR_error,
...
brix_stage_waiter_deliver(const char *reqid, int code)
{
    ...
    brix_stage_waiter_poll_local();
}

void
brix_stage_waiter_poll_local(void)
{
    stage_waiter_table_t *tbl = stage_waiter_table();
...
void
brix_stage_waiter_drop_conn(int conn_fd, ngx_atomic_uint_t conn_number)
{
    stage_waiter_table_t *tbl = stage_waiter_table();
    ngx_uint_t            i;
