 ...
 */
#ifndef BRIX_SCAN_THROTTLE_H
#define BRIX_SCAN_THROTTLE_H
...
void brix_scan_tb_init(brix_scan_tb_t *tb, double rate_bps, double capacity,
                         uint64_t now_ns);

/* Accrue tokens for the elapsed interval (caps at capacity). */
void brix_scan_tb_refill(brix_scan_tb_t *tb, uint64_t now_ns);
...
uint64_t brix_scan_tb_wait_ns(brix_scan_tb_t *tb, double need, uint64_t now_ns);

/* Spend `need` tokens (may drive tokens negative only down to -capacity). */
void brix_scan_tb_consume(brix_scan_tb_t *tb, double need);
...
int brix_scan_budget_hit(const brix_scan_budget_t *b,
...
double brix_scan_adapt(int foreground_active, double latency_ewma_ms,
                         double nominal_latency_ms);

#endif /* BRIX_SCAN_THROTTLE_H */
