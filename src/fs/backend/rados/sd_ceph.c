...
/* sd_ceph_pread — one rados_read at off; returns bytes read (>=0, 0 = at/after
...
sd_ceph_init(xrootd_sd_instance_t *inst, void *driver_conf)
{
    ...
    rc = rados_create(&st->cluster, sd_ceph_user_id(st->user));
    if (sd_ceph_set_errno(rc)) {
        return NGX_ERROR;
    }
    if (sd_ceph_set_errno(rados_conf_read_file(st->cluster, st->conf_file))) {
        rados_shutdown(st->cluster);
        return NGX_ERROR;
    }
    if (st->keyring != NULL) {
        rados_conf_set(st->cluster, "keyring", st->keyring);
    }
    if (sd_ceph_set_errno(rados_connect(st->cluster))) {
        rados_shutdown(st->cluster);
        return NGX_ERROR;
    }
    if (sd_ceph_set_errno(rados_ioctx_create(st->cluster, st->pool,
                                             &st->ioctx)))
    {
        rados_shutdown(st->cluster);
        return NGX_ERROR;
