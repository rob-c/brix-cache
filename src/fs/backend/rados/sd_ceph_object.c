 ...
 *       carry the cinfo/checksum-at-rest records. The rename slot (copy+delete
 *       on the flat namespace) is split out into sd_ceph_object_rename.c to keep
 *       both files under the source-size guard.
 *
 ...
 * callback over the catalog enumeration, aborting on the first hit. The
 * sd_ceph_child_probe_t it fills is declared in sd_ceph_internal.h (shared with
 * the rename TU). NON-static: sd_ceph_object_rename.c reuses it. */
int
