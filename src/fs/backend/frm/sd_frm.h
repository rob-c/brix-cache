#ifndef XROOTD_SD_FRM_H
#define XROOTD_SD_FRM_H
 ...
 *       (xrootd_mss_adapter_t): residency (online/offline/absent), recall
...
#define XROOTD_RESIDENCY_NEARLINE  1   /* recallable (a stage is in flight/queued) */
#define XROOTD_RESIDENCY_OFFLINE   2   /* on tape; a recall will fault it in       */
#define XROOTD_RESIDENCY_ABSENT   (-1) /* unknown to the MSS                       */
 ...
 * malloc-owned instance, or NULL (errno set). Destroy with xrootd_sd_frm_destroy. */
xrootd_sd_instance_t *xrootd_sd_frm_create(const char *adapter,
    const char *location, ngx_log_t *log);

/* Free an instance built by xrootd_sd_frm_create. NULL-safe. */
void xrootd_sd_frm_destroy(xrootd_sd_instance_t *inst);

#endif /* XROOTD_SD_FRM_H */
