 ...
 * void* (cast to xrootd_cstore_t* by the caller) to keep sd_cache.h off cstore.h. */
void *
xrootd_sd_cache_cstore(const xrootd_sd_instance_t *inst)
{
    return xrootd_sd_cache_instance_is(inst) ? &SD_CACHE_ST(inst)->cstore : NULL;
}
