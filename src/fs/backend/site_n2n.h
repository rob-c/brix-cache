#ifndef BRIX_SITE_N2N_H
#define BRIX_SITE_N2N_H
...
typedef enum {
    BRIX_N2N_IDENTITY = 0,   /* pfn = lfn (no translation) */
    BRIX_N2N_RAL,            /* RAL/Glasgow RADOS: "<pool>:<prefix><lfn>" */
    BRIX_N2N_CEPHFS_PATH     /* CephFS POSIX path: "<localroot><lfn>" */
} brix_n2n_scheme_t;
...
int brix_n2n_pfn2lfn(const brix_n2n_cfg_t *cfg, const char *pfn,
                       char *lfn, size_t cap);
...
int brix_n2n_extract_pool(const char *objname, char *pool, size_t cap,
                            const char **rest);

#endif /* BRIX_SITE_N2N_H */
