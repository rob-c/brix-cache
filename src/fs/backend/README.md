...
— the 63-slot × 13-driver matrix, with a verdict for every empty cell (protocol
cannot express it · the generic seam is exact · real gap) and the open-gap list.
Check it with `python3 tools/diag/sd_slot_matrix.py --check`, which fails if
...
fs/backend/
  ...
  xroot/   sd_xroot_fwd*.{c,h}                   ← forwarding proxy: client-named root:// origins (forward://, 2.0 F5)
  gsiftp/  sd_gsiftp*.{c,h} gftp_*.{c,h}         ← FTP/GridFTP remote-origin driver
```
...
| `xroot/sd_xroot_fwd.c` / `_obj.c` / `_ns.c` / `_key.{c,h}` | The **forwarding proxy driver** `brix_sd_xroot_fwd_driver` (`xroot_fwd`, 2.0 F5, XrdPss forwarding mode), selected by `brix_storage_backend forward://root[,roots] permit=<host\|.suffix>…`. A client names the origin INSIDE the key it opens (`/root://host:port//file`); `sd_xroot_fwd_key.c` — ngx-free, unit-tested standalone by `tests/test_sd_xroot_fwd_key.c` — parses it and answers the permit verdict with the TPC egress guard's `brix_tpc_host_pattern_match`, so one host-pattern rule serves both. `sd_xroot_fwd.c` admits the target (scheme outside the list → `ENOTSUP`, host outside the list → `EACCES` **before any dial**) and keeps one `sd_xroot` child per distinct admitted `host:port` (created on first use from the line's origin template, so `verify_pages`/`nearline`/`credential=` apply per origin); `_obj.c` relays the handle-returning slots and `_ns.c` the path-keyed ones through the shared `brix_sd_*_maybe_cred` forwarders (`SD_XROOT_FWD_HOP`, the same gated-relay shape as `PBLOCK_CRED_GATED`); `rename`/`server_copy` refuse `EXDEV` unless both keys land on one origin. Refused on every store tier (`forward://` is an export origin) and refuses a missing permit list at config time. E2E: `tests/test_release20_forward_proxy.py`. |
| `gsiftp/` | The **outbound FTP/GridFTP origin driver** selected by `brix_storage_backend ftp://host[:21]/base` or `gsiftp://host[:2811]/base`. It maps the SD range-read, stat/list, namespace mutation, staged-write and credential-scoped slots onto bounded RFC 959/3659 control and passive MODE-S data sessions. `gsiftp://` performs client-role GSI ADAT authentication with a static or per-user X.509 proxy, retains VOMS attributes in the proxy chain, and protects control commands after authentication; passive data ports are pinned to the connected control peer, ignoring a server-supplied PASV address. Staged PUT uploads a unique temporary object and publishes it with RNFR/RNTO. The v1 data channel is `PROT C`/`DCAU N`; MODE E, PROT P, Kerberos and password auth are not advertised. Live evidence: `tests/test_gsiftp_backend.py` and `tests/test_gsiftp_backend_voms.py`; parser unit: `tests/c/gftp_parse_test.c`. |
...
| `csi_tagstore.c` / `.h` | The **CSI block-checksum state** (phase-59, later unified by xmeta P3): one CRC32C per configurable cinfo block in the file's `user.xrd.cinfo` record or `.cinfo` carrier. Write handles fold complete-block CRCs in memory and merge once at close; partial dirty edges are recomputed within a fixed budget. |
| `csi_verify.c` | Read-side verification and write-side folding over the unified xmeta BLOCKCRC table. A pgwrite's 4 KiB wire CRCs verify transport integrity during decode; CSI deliberately recomputes its normally 1 MiB storage-granule CRCs because the two checksum records describe different byte extents. |
| `csi_unittest.c` | Standalone suite for the CSI/xmeta verify and folding contract, including clean reads, corruption, unaligned updates, geometry and bounded edge recomputation. |
