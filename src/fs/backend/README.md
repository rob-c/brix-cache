...
| File | Responsibility |
|---|---|
| `sd.h` | The capability bitmap (`BRIX_SD_CAP_*`), the opaque handle types (`brix_sd_driver_t`/`instance`/`obj`/`dir`/`staged`), the POD `brix_sd_stat_t`/`brix_sd_dirent_t`, the driver vtable, the capability-gated accessors (`brix_sd_caps`/`fd`/`supports`/`backend_name`), and the registry API. |
