 ...
 *       stat, unlink, mkdir, setattr and rename over the SQLite catalog;
 *       directory iteration (opendir/readdir/closedir); and the object xattr CRUD
 *       (get/list/set/remove). rename is a pure catalog row move (blocks are
 *       id-addressed, so no bytes are touched). The server_copy slots live in the
 *       sibling sd_pblock_namespace_copy.c.
 *
