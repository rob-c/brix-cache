 ...
 * callers must map this to kXR_NotAuthorized (see brix_kxr_from_errno).
 */
 ...
 * brix_*_confined_canon() functions it replaces (which also opened a rootfd
...
int brix_stat_beneath(int rootfd, const char *reqpath, struct stat *st);
...
int brix_lstat_beneath(int rootfd, const char *reqpath, struct stat *st);
...
DIR *brix_opendir_beneath(int rootfd, const char *reqpath);

/* Remove a file (is_dir=0) or empty directory (is_dir=1). */
int brix_unlink_beneath(int rootfd, const char *reqpath, int is_dir);

/* Create a directory. */
int brix_mkdir_beneath(int rootfd, const char *reqpath, mode_t mode);
...
brix_beneath_rel(const char *reqpath)
{
...
/*
 * brix_beneath_strip_root — turn an absolute, already-canonical path that
 * lives under root_canon into the root-relative tail the beneath API expects.
 *
 * This is the bridge for migrating the legacy brix_*_confined_canon(root_canon,
 * abspath) callers to brix_*_beneath(rootfd, rel): those functions already
 ...
 * brix_path_within_root() rejection), and the caller must treat it as an
 ...
 */
static inline const char *
brix_beneath_strip_root(const char *root_canon, const char *abspath)
{
