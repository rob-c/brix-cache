 ...
 *       (brix_log_path_warning), export-root boundary checking
 *       (brix_path_within_root), absolute→root-relative conversion
 *       (brix_resolved_relative_to_root), opening the root anchor fd
 *       (brix_open_root_fd), the openat2(2) confined-open wrapper plus a
 *       runtime capability probe (brix_openat2_runtime_available), the
 *       O_NOFOLLOW segment-by-segment fallback for pre-5.6 kernels
 *       (brix_open_confined_parent_fallback), parent/base splitting
 *       (brix_split_relative_parent), and the combined confined-parent opener
 *       (brix_open_confined_parent_canon).
 *
 ...
 * (control bytes/quotes/non-ASCII → hex via brix_sanitize_log_string) so a
 * hostile client path cannot inject forged entries into the log.
 */
void
brix_log_path_warning(ngx_log_t *log, const char *prefix, const char *path)
{
    char safe_path[512];
...
/*
 * brix_path_within_root — 1 iff canonical path_canon is exactly root_canon or
 ...
 */
int
brix_path_within_root(const char *root_canon, const char *path_canon)
{
...
/*
 * brix_resolved_relative_to_root — strip root_canon from the canonical
 ...
 */
int
brix_resolved_relative_to_root(ngx_log_t *log, const char *root_canon,
...
/*
 * brix_open_root_fd — open the export root as an O_PATH|O_DIRECTORY|O_CLOEXEC
 * anchor fd for subsequent confined openat2/openat calls (caller closes it).
 * Returns the fd, or -1 with a WARN log on failure.
 */
int
brix_open_root_fd(ngx_log_t *log, const char *root_canon)
{
    int fd;
...
/*
 * brix_open_dir_no_symlink — openat one directory component with
 * O_PATH|O_DIRECTORY|O_CLOEXEC|O_NOFOLLOW so the segment-by-segment fallback
 * cannot traverse a symlink at any step.
 */
static int
brix_open_dir_no_symlink(int parentfd, const char *name)
{
...
/*
 * brix_open_confined_parent_fallback — pre-openat2 confinement: walk `parent`
 ...
 */
int
brix_open_confined_parent_fallback(int rootfd, const char *parent)
{
    ...
    while (*seg != '\0') {
        ...
        if (seg[0] == '\0'
            || brix_path_component_forbidden(seg, strlen(seg)))
        {
            close(curfd);
            errno = EACCES;
            return -1;
        ...
        nextfd = brix_open_dir_no_symlink(curfd, seg);
        close(curfd);
        if (nextfd < 0) {
            return -1;
        }
 ...
 *       a relative path produced by brix_resolved_relative_to_root() must be
...
int
brix_split_relative_parent(const char *rel, char *parent, size_t parentsz,
...
/*
 * brix_open_confined_parent_canon — resolve `resolved` to a root-relative
 ...
 */
int
brix_open_confined_parent_canon(ngx_log_t *log, const char *root_canon,
    const char *resolved, char *base, size_t basesz)
{
    ...
    if (!brix_resolved_relative_to_root(log, root_canon, resolved,
                                          rel, sizeof(rel))
        || !brix_split_relative_parent(rel, parent, sizeof(parent),
                                         base, basesz))
    {
        return -1;
    }
