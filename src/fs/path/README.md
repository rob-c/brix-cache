...
| `auth_cache.h` / `auth_cache.c` | Cross-worker SHM cache of the combined gate verdict (`brix_kv_t`-backed, short TTL, default 30 s). `auth_cache.c` only parses the `brix_auth_cache zone=… [ttl=…]` directive and validates the zone is large enough (key≥32, val≥`sizeof(brix_auth_cache_val_t)`); the lookup/store is inline in `auth_gate.c` where every key input is in scope. Stream-only. |
...
| `acl.c` | VO (Virtual Organization) ACL: `brix_finalize_vo_rules` canonicalises rule paths at startup; `brix_vo_list_contains` tests a VO against a comma-separated client VO list (empty `required_vo` ⇒ allow-all, empty `vo_list` ⇒ deny); `brix_check_vo_acl[_identity]` does longest-prefix lookup + membership check, logs sanitized deny. No matching rule ⇒ allow. |
| `find_rule.c` | Boundary-aware longest-prefix matcher (`/data` matches `/data/x` but not `/data-x`) for VO rules, group-policy rules, and the manager-map (`brix_find_manager_map`, cluster routing). All inputs must be canonical before matching; on length ties the later rule wins (`>=`). |
...
| `extract.c` | `brix_extract_path`: pull a NUL-terminated path out of a raw wire payload — reject oversize (> `BRIX_MAX_PATH`) and *embedded* NULs (a single trailing NUL is allowed/trimmed), optionally truncate a `?cgi` suffix. |
| `strip_cgi.c` | `brix_strip_cgi`: standalone (no module include) truncate-at-`?` so the resolver never tries to open `file?checksum=md5` on disk (wire-path invariant #4). |
...
| `merge.c` | `brix_merge_arrays`: concatenate parent+child `ngx_array_t` (parent first) for main→srv→loc config inheritance of rule lists. |
...
| `stat_body.c` | `brix_make_stat_body`: format the XRootD `kXR_stat`/`kXR_statx` ASCII body `"<id> <size> <flags> <mtime>"`, OR-ing in caller `extra_flags` (e.g. `kXR_cachersp`); separate VFS (block-count) vs real-fs (inode + permission flags) modes. |
| `access_log.c` | Per-request access-log line (client IP, authmethod gsi/sss/unix/krb5/anon, DN, Apache-style timestamp, verb/path/detail, OK/ERR, bytes, duration ms), all fields sanitized. Phase 33 per-worker 64 KiB batch buffer flushed on full / fd-switch / 1 s timer / disconnect (`brix_access_log_flush`). |
...
- **`brix_path_mode_t`** (`op_path.h`) — per-operation existence policy: `EXISTING` (target must
...
- **`brix_auth_cache_conf_t` / `brix_auth_cache_val_t`** (`auth_cache.h`) — directive config
...
- **New ACL/policy rule type:** add the rule struct + a `brix_finalize_<type>_rules` wrapper over
  `brix_finalize_path_rules` (see `acl.c`/`authdb.c`/`group_policy.c`), a `brix_find_<type>_rule`
  in `find_rule.c`, and a `brix_check_<type>` returning NGX_OK/deny. Wire it into `auth_gate.c`
