...
| `vfs_backend_config_n2n.c` | Parses, validates and registers each export's logical-to-physical name mapping; rejects invalid RAL/backend combinations and oversized pool/prefix values at config time. |
...
| `vfs_observe_internal.h` | Inline VFS latency, metric, request-monitor and structured-access-log observation shared by per-operation units through `vfs_internal.h`. |
| `vfs_authz.c` / `vfs_authz.h` | Shared identity-only authorization evaluator and the position-1.5 mutation/read gates. |
| `vfs_authz_types.h` | Public per-export authorization-backstop mode, borrowed rules bundle and context binder. |
