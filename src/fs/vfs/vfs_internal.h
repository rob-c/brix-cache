...
#include "fs/vfs/vfs_authz.h"   /* policy->authorization fused gates (C12) */
...
struct brix_vfs_file_s {
    ...
    /* Protocol attribution accompanies the carried policy so a late denial is
     * observable without dereferencing the request-owned context. */
    brix_proto_t       metrics_proto;
    /* Phase-108: late handle mutations must re-run the authorization backstop,
     * but ctx may have been a stack object.  Carry the bounded rule bundle and
     * the pool-lived inputs needed to reconstruct a transient gate context.
     * Rule tables/root are config-lifetime borrows; identity/io_monitor share
     * the handle's pool lifetime. */
    ...
    brix_vfs_mutation_op_t op)
{
    ...
    return brix_vfs_gate_confined(ctx, op);
}
...
#include "vfs_observe_internal.h"
