 ...
 *
 * WHAT: Implements brix_vfs_pwrite_full(), the EINTR-safe / short-write-safe
 ...
 *
 * HOW:  brix_vfs_pwrite_full() wraps the fd in a POSIX storage-driver object and
...
ngx_int_t
brix_vfs_pwrite_full(ngx_fd_t fd, const u_char *buf, size_t len,
    off_t offset)
{
    size_t          done = 0;
    brix_sd_obj_t obj;
    ...
    while (done < len) {
        ssize_t n = brix_sd_posix_driver.pwrite(&obj, buf + done, len - done,
                                                  offset + (off_t) done);
