...
`xrootd_auth sss` (`XROOTD_AUTH_SSS`, value 4) plus an `xrootd_sss_keytab`
directive.
...
through [`../gsi/auth.c`](../gsi/auth.c) (`xrootd_handle_auth`), which dispatches
on credential type to `xrootd_handle_sss_auth()` in `auth_request.c`. On success
the handler populates `ctx->dn` / `ctx->vo_list` / `ctx->primary_vo`, sets the
unified `ctx->identity` (`XROOTD_AUTHN_SSS`), registers the session in
...
`xrootd_sss_build_proxy_credential()` (`auth_proxy_credential.c`) to mint a
...
transport-independent `xrootd_sss_verify_blob()` (`auth_crypto_helpers.c`), and
loads its keytab through the shared `xrootd_sss_load_keytab()` (`config.c`).
...
| File | Responsibility |
|---|---|
| `auth_request.c` | `xrootd_handle_sss_auth()` — the `kXR_auth` `"sss"` handler. Parses the outer packet, looks up the key by id, decrypts the cleartext, verifies CRC32 + timestamp, parses the identity TLV, applies key policy (`anyuser`/`anygroup`/`usrgroup`/...), sets `ctx->dn`/`vo_list`/`primary_vo`/`identity`, registers the session, emits metrics, and replies `kXR_ok` (or `kXR_authmore` for the interactive SNDLID form). |
| `auth_crypto_helpers.c` | Shared crypto + wire primitives: big-endian read/write (`xrootd_sss_read_be32`/`be64`, `write_be32`), software `xrootd_sss_crc32` (poly `0xedb88320`), `xrootd_sss_bf32_crypt` (Blowfish-CFB64 via OpenSSL EVP, zero IV, no padding, OpenSSL-3 legacy-provider load), key lookup (`xrootd_sss_find_key`/`find_key_arr`), and the transport-independent verifier `xrootd_sss_verify_blob` used by CMS. |
| `auth_identity_challenge.c` | Identity TLV parsing and the challenge/failure responses: `xrootd_sss_parse_identity` (bounds-checked TAG-LEN-VALUE stream → name/grps/host/ip), `xrootd_sss_copy_packed_string` (overflow-safe field extraction), `xrootd_sss_auth_failed` (sends `kXR_NotAuthorized`), and `xrootd_sss_send_authmore` (encrypts a fresh `nobody` LGID challenge for the local-id round-trip). |
| `auth_proxy_credential.c` | `xrootd_sss_build_proxy_credential()` — builds an outbound `kXR_auth` `"sss"` credential when *this* server acts as an SSS client to an upstream (proxy mode): random nonce + gen-time + NAME TLV → CRC32 → BF32 encrypt → outer header with 8-byte BE key-id. |
| `config.c` | Startup keytab loader/validator: `xrootd_sss_load_keytab()` (shared by stream + CMS) opens the keytab `O_NOFOLLOW`/`O_CLOEXEC`, checks permissions via the shared `sss_keytab_mode_ok()`, parses each `N:/k:/u:/g:/n:/e:` line via the shared `sss_keytab_parse_line()` into a neutral entry, copies it into `xrootd_sss_key_t`, sets policy `opts`, and skips expired entries; `xrootd_configure_sss_auth()` wires it into the per-server config when `auth == XROOTD_AUTH_SSS`. |
...
| `sss_internal.h` | Cross-file contract: wire-length constants (`XROOTD_SSS_HDR_LEN` 16, `XROOTD_SSS_DATA_HDR_LEN` 40), `XROOTD_SSS_BASE_TIME` epoch, enc/option/TLV-type codes, the `xrootd_sss_identity_t` decoded-identity struct, and all shared prototypes. |
...
- **`xrootd_sss_key_t`** (defined in [`../types/config.h`](../types/config.h)) —
  one parsed keytab entry: `id` (`int64_t` wire key-id), `exp` (expiry `time_t`,
  0 = never), `opts` (policy bitmask), `key[XROOTD_SSS_KEY_MAX]` + `key_len` (raw
  secret, max 128 bytes), and the `name[XROOTD_SSS_NAME_MAX]` /
  `user[XROOTD_SSS_USER_MAX]` / `group[XROOTD_SSS_GROUP_MAX]` strings. Lives in
  `conf->sss_keys` (an `ngx_array_t`).
- **`xrootd_sss_identity_t`** (`sss_internal.h`) — the identity decoded from a
...
- **Wire layout** — outer packet: 4-byte magic `"sss\0"`, 1-byte version, spare,
  ...
  (`XROOTD_SSS_ENC_BF32` = `'0'` at offset 7), 8-byte BE key-id at offset 8,
...
- **Key policy `opts`** ([`../types/tunables.h`](../types/tunables.h),
  `XROOTD_SSS_OPT_*`): `ALLUSR` `0x01` / `ANYUSR` `0x02` (take username from the
  ...
  `XROOTD_SSS_OPT_USEDATA` `0x00` / `SNDLID` `0x01` declared in `sss_internal.h`.
...
1. [`../handshake/`](../handshake/) dispatcher → [`../gsi/auth.c`](../gsi/auth.c)
   `xrootd_handle_auth` matches credential type `"sss"`, checks
   `conf->auth == XROOTD_AUTH_SSS` (`gsi/auth.c:126`), and calls
   `xrootd_handle_sss_auth(ctx, c, conf)` (`gsi/auth.c:130`).
2. `auth_request.c` validates the outer header, `xrootd_sss_find_key()` selects
   the key by id, `xrootd_sss_bf32_crypt(0, ...)` decrypts, CRC32 + `gen_time`
   freshness are checked, and `xrootd_sss_parse_identity()` extracts the TLV.
3. If the credential's option byte is `SNDLID`, control diverts to
   `xrootd_sss_send_authmore()` (a `kXR_authmore` challenge) and the client
   re-submits a `USEDATA` credential.
4. On success the handler sets `ctx->dn`/`vo_list`/`primary_vo`, updates
   `ctx->identity` via `xrootd_identity_set_dn`/`set_vos_csv`
   ([`../types/identity.h`](../types/identity.h)), records metrics through
   [`../metrics/`](../metrics/) (`xrootd_track_vo_activity`,
   `xrootd_track_unique_user`), calls `xrootd_session_register()`
   ([`../session/`](../session/registry.h)), and returns `kXR_ok` via
   `XROOTD_RETURN_OK`.

**Outbound (proxy mode):** [`../proxy/`](../proxy/) (`events_bootstrap.c`) calls
`xrootd_sss_build_proxy_credential()` to encrypt a credential with the local
...
`xrootd_sss_verify_blob(ctx->conf->sss_keys, CMS_SSS_LIFETIME, ...)` to validate a
peer's self-contained credential out of the `kYR_xauth` frame; the keytab itself
is loaded at config time by `xrootd_sss_load_keytab()` from
[`../cms/`](../cms/) `server_module.c`.

**Startup:** [`../config/postconfiguration.c`](../config/postconfiguration.c)
calls `xrootd_configure_sss_auth()` alongside the GSI/token loaders; any failure
...
- **Replay window is mandatory.** `gen_time` (offset 32, in seconds since the
  `XROOTD_SSS_BASE_TIME` 2008 epoch that dodges the 2038 32-bit overflow) is
...
- **Only self-contained (`USEDATA`) credentials authenticate inbound.** The
  ...
  path; in `xrootd_sss_verify_blob()` it is explicitly **`NGX_DECLINED`**, never
  accepted (`auth_crypto_helpers.c:277`).
- **Keytab files must be private — checked at load.** `xrootd_sss_keytab_mode_ok`
...
- **Fail-closed parsing everywhere.** The TLV parser (`auth_identity_challenge.c`)
  ...
  a malformed credential into an out-of-bounds read. `xrootd_sss_copy_packed_string`
  clamps to `dst_len-1`; the keytab line parser rejects odd-length hex, oversized
  fields, and non-positive ids. `xrootd_sss_bf32_crypt` rejects `key_len == 0`
  and `> INT_MAX` lengths. `xrootd_sss_verify_blob` bounds its decrypt scratch to
...
- **`auth == XROOTD_AUTH_SSS` is enforced twice.** The dispatcher rejects `"sss"`
...
- **nginx discipline:** stream-path allocation uses the connection pool
  ...
  `xrootd_build_resp_hdr` + `xrootd_queue_response` ([`../response/`](../response/)),
  never raw writes. Wire strings reaching logs go through
  `xrootd_sanitize_log_string`. The keytab loader runs at config time and is the
  *only* place blocking file I/O (`open`/`fgets`/`fstat`) is permitted — never on
  a live connection.

## Entry points / extending

- **Add a new keytab field:** add a `case` in `xrootd_sss_parse_key_line()`
  (`config.c`) switching on the field prefix char; extend `xrootd_sss_key_t`
...
- **Add a new identity TLV type:** add the constant to `sss_internal.h` and a
  `case` in `xrootd_sss_parse_identity()` (`auth_identity_challenge.c`); remember
  non-`RAND` fields must increment `id_count` to count as a valid identity.
- **Add a new key policy option:** define `XROOTD_SSS_OPT_*` in
  [`../types/tunables.h`](../types/tunables.h), set it from a sentinel
  user/group/name value in `xrootd_sss_parse_key_line()`, and apply it where
  `key->opts` is consulted in `auth_request.c`.
- **Reuse SSS over a new transport:** call `xrootd_sss_verify_blob()` (verify) or
  `xrootd_sss_build_proxy_credential()` (mint) — they are transport-independent
  and allocate nothing; do not re-implement the decrypt/CRC/replay chain.

## See also

- [`../gsi/`](../gsi/) — credential-type dispatch (`xrootd_handle_auth`) and the
  x509/GSI auth path.
