...
| `config.c` | Startup keytab loader/validator: `xrootd_sss_load_keytab()` (shared by stream + CMS) opens the keytab `O_NOFOLLOW`/`O_CLOEXEC`, enforces private permissions, parses each `N:/k:/u:/g:/n:/e:` line into `xrootd_sss_key_t`, decodes the hex key (`xrootd_sss_decode_hex`), sets policy `opts`, and skips expired entries; `xrootd_configure_sss_auth()` wires it into the per-server config when `auth == XROOTD_AUTH_SSS`. |
