...
"<section class=\"panels\" style=\"grid-template-columns:1fr\">\n"
"<div class=\"panel\"><h2>History</h2><svg id=\"history-spark\" class=\"spark\" preserveAspectRatio=\"none\" role=\"img\" aria-label=\"Recent transfer activity\"></svg><div id=\"history-caption\" class=\"sub\"></div></div>\n"
"</section>\n"
 ...
 *       storage, cluster, events, history.buckets, server_ms, avg_bps,
 *       idle_ms, ...) must stay in lockstep with the JSON emitted by
 *       dashboard/api.c.
...
"function renderCards(){var p=(lastSnapshot&&lastSnapshot.protocols)||{};var names=['root','webdav','s3','cvmfs','tpc'];el('protocol-cards').innerHTML=names.map(function(n){var x=p[n]||{};var tot=(x.bytes_rx_total!=null||x.bytes_tx_total!=null)?'<div class=\"sub\">total in '+fmt(x.bytes_rx_total||0)+' / out '+fmt(x.bytes_tx_total||0)+'</div>':'';return '<div class=\"card\"><h2>'+n+'</h2><div class=\"metric\">'+(x.active||0)+' active</div><div class=\"sub\">in '+rate(x.ingress_bps||0)+' / out '+rate(x.egress_bps||0)+'</div>'+tot+'</div>'}).join('')}\n"
...
"function renderPanels(){var c=(lastSnapshot&&lastSnapshot.cache)||{};var listeners=c.listeners||[];el('cache-panel').innerHTML='<div class=\"metric\">'+(c.enabled?'enabled':'disabled')+'</div><div class=\"sub\">listeners '+listeners.length+'</div><div class=\"sub\">WT pending '+(((c.write_through||{}).flush_pending)||0)+'</div>';var st=(lastSnapshot&&lastSnapshot.storage)||{};var exs=st.exports||[];var sio=st.io||{};el('storage-panel').innerHTML='<div class=\"metric\">'+exs.length+' export'+(exs.length===1?'':'s')+'</div>'+exs.slice(0,4).map(function(x){var cap=(x.bytes_total>0)?' '+Math.round((x.occupancy_ratio||0)*100)+'% of '+fmt(x.bytes_total):(x.remote?' remote':'');return '<div class=\"sub\">'+esc(x.backend)+cap+'</div>'}).join('')+Object.keys(sio).map(function(k){return '<div class=\"sub\">'+esc(k)+' r '+fmt(sio[k].bytes_read_total)+' / w '+fmt(sio[k].bytes_written_total)+'</div>'}).join('');var cl=(lastSnapshot&&lastSnapshot.cluster)||{};var servers=cl.servers||[];el('cluster-panel').innerHTML='<div class=\"metric\">'+servers.length+' servers</div>'+servers.slice(0,4).map(function(s){return '<div class=\"sub\">'+esc(s.host)+':'+esc(s.port)+' '+(s.stale?'stale':'ok')+'</div>'}).join('');renderCvmfs();var events=(lastSnapshot&&lastSnapshot.events)||[];el('events-list').innerHTML=events.slice(-20).reverse().map(function(e){return '<div class=\"event\"><span>'+esc(e.class)+'</span><span>'+esc(e.protocol)+'</span><span>'+esc(e.message)+'</span></div>'}).join('')||'<div class=\"sub\">No recent events</div>';drawHistory()}\n"
/* CVMFS panel: hit ratio (bytes served from cache vs total served), LAN-out
 * vs WAN-in split, fill/verify failure counts, and the busiest repositories.
 * Data comes from the snapshot's \"cvmfs\" section (api_cvmfs.c); repo names
 * are wire-supplied, so esc() is mandatory (XSS guard). */
"function renderCvmfs(){var cv=(lastSnapshot&&lastSnapshot.cvmfs)||{};var hit=cv.bytes_served_hit_total||0;var served=hit+(cv.bytes_served_fill_total||0);if(!cv.enabled){el('cvmfs-panel').innerHTML='<div class=\"metric\">idle</div><div class=\"sub\">no cvmfs traffic</div>';return}var ratio=served?Math.round(hit/served*100):0;var repos=(cv.repos||[]).slice();repos.sort(function(a,b){return((b.bytes_served_hit_total||0)+(b.bytes_served_fill_total||0))-((a.bytes_served_hit_total||0)+(a.bytes_served_fill_total||0))});el('cvmfs-panel').innerHTML='<div class=\"metric\">'+ratio+'% hit</div>'+'<div class=\"sub\">out '+fmt(served)+' / in '+fmt(cv.origin_bytes_total||0)+' (origin)</div>'+'<div class=\"sub\">fills '+(cv.fills_total||0)+' &middot; failed '+(cv.fill_failures_total||0)+' &middot; verify '+(cv.verify_failures_total||0)+'</div>'+'<div class=\"sub\">repos '+repos.length+' &middot; origins '+((cv.upstreams||[]).length)+'</div>'+repos.slice(0,3).map(function(rp){return '<div class=\"sub\">'+esc(rp.name)+' '+fmt((rp.bytes_served_hit_total||0)+(rp.bytes_served_fill_total||0))+'</div>'}).join('')}\n"
"function drawHistory(){var h=(lastSnapshot&&lastSnapshot.history&&lastSnapshot.history.buckets)||[];var svg=el('history-spark');var max=1;var pts=h.map(function(b,i){var v=(b.active_root||0)+(b.active_webdav||0)+(b.active_s3||0)+(b.active_cvmfs||0)+(b.active_tpc||0);if(v>max)max=v;return v});var w=300,hgt=42,d=pts.map(function(v,i){var x=pts.length<2?0:i*(w/(pts.length-1));var y=hgt-(v/max)*(hgt-4)-2;return (i?'L':'M')+x.toFixed(1)+' '+y.toFixed(1)}).join(' ');svg.setAttribute('viewBox','0 0 '+w+' '+hgt);svg.innerHTML='<path d=\"'+d+'\" fill=\"none\" stroke=\"#8cc8ff\" stroke-width=\"2\"/><line x1=\"0\" y1=\"40\" x2=\"300\" y2=\"40\" stroke=\"#303842\"/>';el('history-caption').textContent=pts.length+' buckets, peak '+max+' active'}\n"
"function render(){renderCards();renderRows();renderPanels()}\n"
"function poll(){fetch(API+'/snapshot',{cache:'no-store'}).then(function(r){if(r.status===401){location.href='/brix/login';return null}if(!r.ok)throw new Error(r.status);return r.json()}).then(function(data){if(!data)return;lastSnapshot=data;applyAnon(!!data.anonymous);el('status').className='status live';el('status').textContent='live '+new Date(data.server_ms).toLocaleTimeString();render();if(selectedDetail)openDetail(selectedDetail,true);if(!data.anonymous&&!filesTried){filesTried=true;filesHostOk=true;filesBrowse('/');filesLoadExports()}}).catch(function(){el('status').className='status bad';el('status').textContent='disconnected'})}\n"
 ...
 * Two sources share the tab: the host tree (/files, browse_root) and the
 * VFS export browser (/vfs/files — the LOGICAL namespace of any backend,
 * pblock/ceph included). filesExport=-1 selects the host tree. */
"var filesTried=false;var filesExport=-1;var filesExports=null;var filesHostOk=false;\n"
"function filesSrcRender(){var sel=el('files-src');if(!sel)return;var o='';"
"if(filesHostOk)o+='<option value=\"-1\">host: browse root</option>';"
"(filesExports||[]).forEach(function(x){o+='<option value=\"'+x.index+'\">export '+x.index+': '+esc(x.root)+' ['+esc(x.backend)+']</option>'});"
"sel.innerHTML=o;sel.value=String(filesExport);"
"sel.onchange=function(){filesExport=Number(sel.value);filesBrowse('/')}}\n"
"function filesLoadExports(){fetch(API+'/vfs',{cache:'no-store'}).then(function(r){return r.ok?r.json():null}).then(function(d){"
"if(!d||!d.exports||!d.exports.length)return;filesExports=d.exports;"
"if(!filesHostOk&&filesExport<0){filesExport=d.exports[0].index}"
"el('files-section').hidden=false;filesSrcRender();if(!filesHostOk)filesBrowse('/')}).catch(function(){})}\n"
...
"function filesBrowse(p){var u=(filesExport<0)?API+'/files?path='+encodeURIComponent(p||'/'):API+'/vfs/files?export='+filesExport+'&path='+encodeURIComponent(p||'/');"
"fetch(u,{cache:'no-store'}).then(function(r){if(r.status===404){if(filesExport<0){filesHostOk=false;filesSrcRender()}return null}if(r.status===401){location.href='/brix/login';return null}if(!r.ok)throw new Error(r.status);return r.json()}).then(function(d){if(!d)return;if(filesExport<0)filesHostOk=true;el('files-section').hidden=false;filesSrcRender();filesRender(d)}).catch(function(){})}\n"
