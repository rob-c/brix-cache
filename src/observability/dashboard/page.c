...
"function poll(){fetch(API+'/snapshot',{cache:'no-store'}).then(function(r){if(r.status===401){location.href='/xrootd/login';return null}if(!r.ok)throw new Error(r.status);return r.json()}).then(function(data){if(!data)return;lastSnapshot=data;applyAnon(!!data.anonymous);el('status').className='status live';el('status').textContent='live '+new Date(data.server_ms).toLocaleTimeString();render();if(selectedDetail)openDetail(selectedDetail,true);if(!data.anonymous&&!filesTried){filesTried=true;filesHostOk=true;filesBrowse('/');filesLoadExports()}}).catch(function(){el('status').className='status bad';el('status').textContent='disconnected'})}\n"
...
"fetch(u,{cache:'no-store'}).then(function(r){if(r.status===404){if(filesExport<0){filesHostOk=false;filesSrcRender()}return null}if(r.status===401){location.href='/xrootd/login';return null}if(!r.ok)throw new Error(r.status);return r.json()}).then(function(d){if(!d)return;if(filesExport<0)filesHostOk=true;el('files-section').hidden=false;filesSrcRender();filesRender(d)}).catch(function(){})}\n"
