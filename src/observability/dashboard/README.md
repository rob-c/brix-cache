...
| `config.c` | **(stream)** Registers the three SHM zones (`brix_dashboard_v2`, `_events`, `_history`) during stream postconfiguration; sets the `(void*)1` first-startup sentinel; owns the private zone pointers exposed through the `brix_dashboard_get_*shm_zone()` accessors. |
