...
- **root://** (stream): begun in [`src/protocols/root/read/open_request.c`](../../protocols/root/read/README.md) on
  the first local data open; ended in [`src/protocols/root/connection/disconnect.c`](../../protocols/root/connection/README.md).
  Handle stored on `brix_ctx_t.pmark_flow`.
- **WebDAV / S3** (HTTP): begun in [`src/protocols/webdav/dispatch.c`](../../protocols/webdav/README.md) /
  [`src/protocols/s3/handler.c`](../../protocols/s3/README.md) post-auth; ended via an `ngx_pool_cleanup`.
...
- [`src/core/config/shared_conf.h`](../../core/config/README.md) — where the pmark config is embedded.
