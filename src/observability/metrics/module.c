 ...
 * HOW: Create/merge location config for the `brix_metrics` directive (boolean flag);
 *      register handler function ngx_http_brix_metrics_handler; define module context with
...
static char *
ngx_http_brix_metrics_merge_loc_conf(ngx_conf_t *cf,
    void *parent, void *child)
{
    ngx_http_brix_metrics_loc_conf_t *prev = parent;
    ngx_http_brix_metrics_loc_conf_t *conf = child;
...
/*
 * WHAT: Register the `brix_metrics` directive and bind its content handler.
 * WHY: The directive enables the /metrics Prometheus export endpoint at this location.
 *      When enabled, nginx calls ngx_http_brix_metrics_handler for all requests to this path.
...
/*
 * WHAT: Parse the `brix_health` flag and bind the /healthz content handler.
 ...
 *      core handler at ngx_http_brix_health_handler so GET /healthz is served
...
/*
 * WHAT: Define the `brix_metrics` nginx directive for location-level configuration.
...
static ngx_command_t ngx_http_brix_metrics_commands[] = {

    { ngx_string("brix_metrics"),
      NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
      ngx_http_brix_metrics_set,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_brix_metrics_loc_conf_t, enable),
      NULL },

    { ngx_string("brix_health"),
      NGX_HTTP_LOC_CONF | NGX_CONF_FLAG,
      ngx_http_brix_health_set,
      NGX_HTTP_LOC_CONF_OFFSET,
      offsetof(ngx_http_brix_metrics_loc_conf_t, health),
      NULL },
...
static ngx_http_module_t ngx_http_brix_metrics_module_ctx = {
    ...
    NULL,                                      /* merge srv conf      */
    ngx_http_brix_metrics_create_loc_conf,   /* create loc conf     */
    ngx_http_brix_metrics_merge_loc_conf     /* merge loc conf      */
};
...
ngx_module_t ngx_http_brix_metrics_module = {
    NGX_MODULE_V1,
    &ngx_http_brix_metrics_module_ctx,
    ngx_http_brix_metrics_commands,
    NGX_HTTP_MODULE,
