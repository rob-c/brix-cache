 ...
 *      bytes and occupancy_ppm (parts per million). brix_export_stream_cache_metrics() iterates server slots,
    ...
    ngx_brix_metrics_t *shm)
{
    ...
    for (i = 0; i < BRIX_METRICS_MAX_SERVERS; i++) {
        ...
        ngx_snprintf((u_char *) port_str, sizeof(port_str), "%ui%Z", srv->port);
        mw_printf(mw,
            "brix_cache_evictions_total{port=\"%s\",auth=\"%s\"} %lu\n",
            port_str, srv->auth,
    ...
    for (i = 0; i < BRIX_METRICS_MAX_SERVERS; i++) {
        ...
        ngx_snprintf((u_char *) port_str, sizeof(port_str), "%ui%Z", srv->port);
        mw_printf(mw,
            "brix_cache_evicted_bytes_total{port=\"%s\",auth=\"%s\"} %lu\n",
            port_str, srv->auth,
    ...
    for (i = 0; i < BRIX_METRICS_MAX_SERVERS; i++) {
        ...
        ngx_snprintf((u_char *) port_str, sizeof(port_str), "%ui%Z", srv->port);
        mw_printf(mw,
            "brix_cache_eviction_errors_total{port=\"%s\",auth=\"%s\"} %lu\n",
            port_str, srv->auth,
     ...
     * (brix_cache_reap_dirty), split by the `reason` label (see
     * brix_cache_reap_reason_t): "abandoned" (un-flushed dirty discarded — data
    ...
    for (i = 0; i < BRIX_METRICS_MAX_SERVERS; i++) {
        ...
        ngx_snprintf((u_char *) port_str, sizeof(port_str), "%ui%Z", srv->port);
        mw_printf(mw,
            "brix_wt_dirty_handles{port=\"%s\",auth=\"%s\"} %lu\n",
            port_str, srv->auth,
    ...
    for (i = 0; i < BRIX_METRICS_MAX_SERVERS; i++) {
        ...
        ngx_snprintf((u_char *) port_str, sizeof(port_str), "%ui%Z", srv->port);
        mw_printf(mw,
            "brix_wt_flush_pending{port=\"%s\",auth=\"%s\"} %lu\n",
            port_str, srv->auth,
    ...
    for (i = 0; i < BRIX_METRICS_MAX_SERVERS; i++) {
        ...
        ngx_snprintf((u_char *) port_str, sizeof(port_str), "%ui%Z", srv->port);
        mw_printf(mw,
            "brix_wt_flush_bytes_total{port=\"%s\",auth=\"%s\"} %lu\n",
            port_str, srv->auth,
