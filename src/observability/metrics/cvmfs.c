...
static const char *brix_cvmfs_class_names[BRIX_CVMFS_CLASS_COUNT] = {
    "cas",
    "manifest",
    "geo",
    "reject",
...
static int
cvmfs_repo_slot_is(const ngx_brix_cvmfs_repo_metrics_t *r,
    const char *name, size_t len)
{
    return r->state == BRIX_CVMFS_REPO_READY
        && strlen(r->name) == len
        && memcmp(r->name, name, len) == 0;
}

ngx_brix_cvmfs_repo_metrics_t *
brix_cvmfs_repo_slot(const char *name, size_t len)
{
    ngx_brix_metrics_t            *m = brix_metrics_shared();
    ngx_brix_cvmfs_repo_metrics_t *repos;
    ngx_brix_cvmfs_repo_metrics_t *other;
    ngx_uint_t                       i;

    if (m == NULL || name == NULL || len == 0
        || len >= BRIX_CVMFS_REPO_NAME_MAX)
    {
        return NULL;
    }
    repos = m->cvmfs.repos;
    other = &repos[BRIX_CVMFS_REPO_SLOTS - 1];

    /* Fast path: lowest-index READY match (the convergence rule). */
    for (i = 0; i < BRIX_CVMFS_REPO_SLOTS - 1; i++) {
    ...
    for (i = 0; i < BRIX_CVMFS_REPO_SLOTS - 1; i++) {
        if (repos[i].state == BRIX_CVMFS_REPO_EMPTY
            && ngx_atomic_cmp_set(&repos[i].state, BRIX_CVMFS_REPO_EMPTY,
                                  BRIX_CVMFS_REPO_CLAIMED))
        {
             ...
             * name; the lowest-index rule keeps everyone converged. */
            {
                ...
                for (j = 0; j < BRIX_CVMFS_REPO_SLOTS - 1; j++) {
...
static int
cvmfs_repo_slot_is_dup(const ngx_brix_cvmfs_repo_metrics_t *repos,
    ngx_uint_t i)
{
    ngx_uint_t j;
...
static int
cvmfs_up_slot_is(const ngx_brix_cvmfs_upstream_metrics_t *u, const char *host)
{
    return u->state == BRIX_CVMFS_REPO_READY && strcmp(u->name, host) == 0;
}
...
cvmfs_upstream_slot(const char *host)
{
    ...
    len = strlen(host);
    if (len >= BRIX_CVMFS_UPSTREAM_NAME_MAX) {
        return NULL;
    }
    ...
    for (i = 0; i < BRIX_CVMFS_UPSTREAM_SLOTS - 1; i++) {
        if (ups[i].state == BRIX_CVMFS_REPO_EMPTY
            && ngx_atomic_cmp_set(&ups[i].state, BRIX_CVMFS_REPO_EMPTY,
                                  BRIX_CVMFS_REPO_CLAIMED))
        {
            memcpy(ups[i].name, host, len + 1);
            (void) ngx_atomic_cmp_set(&ups[i].state, BRIX_CVMFS_REPO_CLAIMED,
                                      BRIX_CVMFS_REPO_READY);
            {
                ngx_uint_t j;

                for (j = 0; j < BRIX_CVMFS_UPSTREAM_SLOTS - 1; j++) {
...
void
brix_cvmfs_upstream_record(const char *host, int ok, off_t bytes,
    long dur_ms, int failover)
{
    ngx_brix_cvmfs_upstream_metrics_t *u = cvmfs_upstream_slot(host);
    ngx_uint_t                           b;

    if (u == NULL) {
        return;
    }
    BRIX_ATOMIC_INC(&u->requests_total);
    if (failover) {
        BRIX_ATOMIC_INC(&u->failovers_total);
    }
    if (!ok) {
        BRIX_ATOMIC_INC(&u->fill_failures_total);
        return;
    }
    BRIX_ATOMIC_INC(&u->fills_total);
    if (bytes > 0) {
        BRIX_ATOMIC_ADD(&u->origin_bytes_total, (ngx_atomic_uint_t) bytes);
    }
    if (dur_ms < 0) {
        dur_ms = 0;
    }
    for (b = 0; b < BRIX_CVMFS_UP_HBUCKETS; b++) {
        if (dur_ms <= brix_cvmfs_up_bucket_ms[b]) {
            BRIX_ATOMIC_INC(&u->dur_bucket[b]);
            break;                       /* non-cumulative; exporter sums up */
        }
    }
    BRIX_ATOMIC_INC(&u->dur_count);    /* +Inf bucket == dur_count         */
    BRIX_ATOMIC_ADD(&u->dur_sum_ms, (ngx_atomic_uint_t) dur_ms);
}

/* 1 iff an EARLIER READY upstream slot carries the same name (claim-race dup). */
static int
cvmfs_up_slot_is_dup(const ngx_brix_cvmfs_upstream_metrics_t *ups,
    ngx_uint_t i)
{
    ngx_uint_t j;
...
static void
cvmfs_export_upstreams(metrics_writer_t *mw, ngx_brix_cvmfs_metrics_t *c)
{
    static const struct {
        const char *name;
        const char *help;
    ...
    for (f = 0; f < sizeof(fam) / sizeof(fam[0]); f++) {
        ...
        for (i = 0; i < BRIX_CVMFS_UPSTREAM_SLOTS; i++) {
            ngx_brix_cvmfs_upstream_metrics_t *u = &c->upstreams[i];
    ...
    mw_printf(mw,
        "# HELP brix_cvmfs_upstream_fill_duration_seconds origin fill duration per upstream\n"
        "# TYPE brix_cvmfs_upstream_fill_duration_seconds histogram\n");
    for (i = 0; i < BRIX_CVMFS_UPSTREAM_SLOTS; i++) {
        ngx_brix_cvmfs_upstream_metrics_t *u = &c->upstreams[i];
        unsigned long cum = 0;
...
void
brix_export_cvmfs_metrics(metrics_writer_t *mw, ngx_brix_metrics_t *shm)
{
    ngx_brix_cvmfs_metrics_t *c = &shm->cvmfs;

    mw_emit_labeled(mw, "brix_cvmfs_requests_total",
        "CVMFS requests by traffic class", "class",
        brix_cvmfs_class_names, BRIX_CVMFS_CLASS_COUNT,
        c->requests_total);

    mw_emit_scalar(mw, "brix_cvmfs_negative_hits_total",
        "404s absorbed by the per-worker negative cache",
        &c->negative_hits_total);
    mw_emit_scalar(mw, "brix_cvmfs_fills_total",
        "origin fills published to the cache", &c->fills_total);
    mw_emit_scalar(mw, "brix_cvmfs_fill_failures_total",
        "fills that failed definitively", &c->fill_failures_total);
    mw_emit_scalar(mw, "brix_cvmfs_verify_failures_total",
        "CAS verify mismatches (fill quarantined, never admitted)",
        &c->verify_failures_total);
    mw_emit_scalar(mw, "brix_cvmfs_origin_failovers_total",
        "read attempts that failed over to the next-ranked origin",
        &c->origin_failovers_total);
    mw_emit_scalar(mw, "brix_scvmfs_requests_total",
    ...
    mw_printf(mw,
        "# HELP brix_cvmfs_bytes_served_total bytes served to clients by cache disposition\n"
        "# TYPE brix_cvmfs_bytes_served_total counter\n"
        "brix_cvmfs_bytes_served_total{source=\"hit\"} %lu\n"
        "brix_cvmfs_bytes_served_total{source=\"fill\"} %lu\n",
        (unsigned long) c->bytes_served_hit_total,
        (unsigned long) c->bytes_served_fill_total);
    mw_emit_scalar(mw, "brix_cvmfs_origin_bytes_total",
        ...
        for (i = 0; i < BRIX_CVMFS_REPO_SLOTS; i++) {
            ...
            for (cls = 0; cls < BRIX_CVMFS_CLASS_COUNT; cls++) {
                mw_printf(mw,
                    ...
                    rm->name, brix_cvmfs_class_names[cls],
