 ...
 * registry SHM zone (via brix_srv_snapshot) rather than from the stream
 ...
 * HOW: take a point-in-time copy of the registry SHM via brix_srv_snapshot()
 ...
 */
void
brix_export_cluster_metrics(metrics_writer_t *mw)
{
    ...
    brix_srv_snapshot_entry_t  entries[BRIX_SRV_REGISTRY_SLOTS];
    ngx_uint_t                   n, i;
    ngx_msec_t                   now;
    ...
    n   = brix_srv_snapshot(entries, BRIX_SRV_REGISTRY_SLOTS, now);

    mw_printf(mw,
        "# HELP brix_cluster_servers_registered "
            "Number of data servers currently in the cluster registry.\n"
        "# TYPE brix_cluster_servers_registered gauge\n"
...
#define CLUSTER_LABEL(idx) \
    ...
    mw_printf(mw,
        "# HELP brix_cluster_server_free_megabytes "
            "Free disk space (MB) last reported by each registered data server.\n"
        "# TYPE brix_cluster_server_free_megabytes gauge\n");
    for (i = 0; i < n; i++) {
        CLUSTER_LABEL(i);
        mw_printf(mw,
            "brix_cluster_server_free_megabytes{server=\"%s\"} %u\n",
            srv_label, (unsigned int) entries[i].free_mb);
    }

    mw_printf(mw,
        "# HELP brix_cluster_server_utilization_percent "
            "Storage utilisation percent (0-100) last reported by each data server.\n"
        "# TYPE brix_cluster_server_utilization_percent gauge\n");
    for (i = 0; i < n; i++) {
        CLUSTER_LABEL(i);
        mw_printf(mw,
            "brix_cluster_server_utilization_percent{server=\"%s\"} %u\n",
            srv_label, (unsigned int) entries[i].util_pct);
    }

    mw_printf(mw,
        "# HELP brix_cluster_server_last_seen_seconds "
            "Seconds since this data server last sent a CMS heartbeat to the manager.\n"
        "# TYPE brix_cluster_server_last_seen_seconds gauge\n");
    for (i = 0; i < n; i++) {
        ...
        CLUSTER_LABEL(i);
        mw_printf(mw,
            "brix_cluster_server_last_seen_seconds{server=\"%s\"} %.3f\n",
            srv_label, age_s);
    }

    mw_printf(mw,
        "# HELP brix_cluster_server_blacklisted "
            "1 if the server is temporarily blacklisted after a CMS disconnect, "
            "0 if available.\n"
        "# TYPE brix_cluster_server_blacklisted gauge\n");
    for (i = 0; i < n; i++) {
        ...
        CLUSTER_LABEL(i);
        mw_printf(mw,
            "brix_cluster_server_blacklisted{server=\"%s\"} %d\n",
            srv_label, bl);
    }
    ...
    mw_printf(mw,
        "# HELP brix_cluster_server_disconnect_total "
            "Cumulative CMS disconnect count for each registered data server.\n"
        "# TYPE brix_cluster_server_disconnect_total counter\n");
    for (i = 0; i < n; i++) {
        CLUSTER_LABEL(i);
        mw_printf(mw,
            "brix_cluster_server_disconnect_total{server=\"%s\"} %u\n",
