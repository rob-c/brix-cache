 ...
 *      1) brix_vo_bytes_tx/rx_total — per-VO byte counters (counter)
 *      2) brix_vo_requests_total — per-VO request count (counter)
 *      3) brix_vo_overflow_total — evicted VO entries since process start (counter)
 *      4) brix_unique_users_current/gauge + total/counter + evictions/counter
 *         — active users, lifetime users, and LRU eviction count (gauge + counter)
 *      5) brix_user_sessions_total{hash} — session count per hashed user ID (gauge)
 *      VO names truncated to BRIX_VO_NAME_LEN characters; users identified by
