...
/* brix_track_vo_activity — bump the per-VO atomic byte + request counters for
...
ngx_int_t
brix_track_vo_activity(ngx_brix_metrics_t *shm, const char *vo_name,
                          size_t bytes_tx, size_t bytes_rx)
{
    ...
    for (i = 0; i < BRIX_VO_MAX_TRACKED; i++) {
        ngx_brix_vo_slot_t *slot = &shm->vo_global.slots[i];

        if (!slot->name[0]) {
            continue;
        }

        if (ngx_strncmp(slot->name, vo_name, BRIX_VO_NAME_LEN) == 0) {
            match = (int) i;
            break;
        }
    }
    ...
    if (match >= 0) {
        ngx_brix_vo_slot_t *slot = &shm->vo_global.slots[match];
        BRIX_ATOMIC_ADD(&slot->bytes_tx_total, bytes_tx);
        BRIX_ATOMIC_ADD(&slot->bytes_rx_total, bytes_rx);
        BRIX_ATOMIC_INC(&slot->requests_total);
        ...
        if (target < 0) {
            BRIX_ATOMIC_INC(&shm->vo_global.overflow_total);
            target = 0;
            memset(shm->vo_global.slots[0].name, 0, sizeof(shm->vo_global.slots[0].name));
            shm->vo_global.slot_count--;
        }

        ngx_brix_vo_slot_t *slot = &shm->vo_global.slots[target];
        size_t copy_len = ngx_min(strlen(vo_name), BRIX_VO_NAME_LEN - 1);
...
/* brix_track_unique_user — track unique identities by FNV-1a hash (no DNs as
...
ngx_int_t
brix_track_unique_user(ngx_brix_metrics_t *shm, const char *identity,
                          size_t identity_len)
{
    uint32_t        h;
    ngx_uint_t      i;
    ...
    h = brix_fnv1a_hash(identity, identity_len);
    ...
    if (match >= 0) {
        BRIX_ATOMIC_INC(&shm->user_tracking.slots[match].sessions_total);
        ...
        if (target < 0) {
            ...
            BRIX_ATOMIC_DEC(&shm->user_tracking.unique_count);
        }
        ...
        BRIX_ATOMIC_INC(&shm->user_tracking.unique_count);
        BRIX_ATOMIC_INC(&shm->user_tracking.total_unique);
    }
