 ...
 *
 * WHAT: Emits the xrootd_frm_* metric families from the shared metrics block:
 ...
 *   stage.c) via the XROOTD_FRM_METRIC_* macros into shm->frm; this file only
...
xrootd_export_frm_metrics(metrics_writer_t *mw, ngx_xrootd_metrics_t *shm)
{
    ...
    mw_printf(mw,
        "# HELP xrootd_frm_in_flight Stage requests currently QUEUED or STAGING.\n"
        "# TYPE xrootd_frm_in_flight gauge\n"
        "xrootd_frm_in_flight %lu\n",
     ...
     */
    mw_printf(mw,
        "# HELP xrootd_frm_stage_latency_seconds Tape recall latency in seconds.\n"
        "# TYPE xrootd_frm_stage_latency_seconds histogram\n");
    value = 0;
    for (b = 0; b < XROOTD_FRM_LATENCY_BUCKETS - 1; b++) {
        value += (unsigned long long)
            ngx_atomic_fetch_add(&shm->frm.stage_latency_bucket[b], 0);
        mw_printf(mw,
            "xrootd_frm_stage_latency_seconds_bucket{le=\"%lu\"} %llu\n",
            xrootd_frm_latency_bounds[b], value);
    }
    value += (unsigned long long) ngx_atomic_fetch_add(
        &shm->frm.stage_latency_bucket[XROOTD_FRM_LATENCY_BUCKETS - 1], 0);
    mw_printf(mw,
        "xrootd_frm_stage_latency_seconds_bucket{le=\"+Inf\"} %llu\n", value);
    mw_printf(mw,
        "xrootd_frm_stage_latency_seconds_sum %llu\n"
        "xrootd_frm_stage_latency_seconds_count %llu\n",
