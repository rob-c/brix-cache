 ...
 *
 * Include after "../ngx_brix_module.h".
 */
#ifndef BRIX_SSS_SSS_INTERNAL_H
#define BRIX_SSS_SSS_INTERNAL_H
...
uint32_t brix_sss_read_be32(const u_char *p);
uint64_t brix_sss_read_be64(const u_char *p);
void     brix_sss_write_be32(u_char *p, uint32_t v);

/* CRC32 over the cleartext payload (used for integrity check). */
uint32_t brix_sss_crc32(const u_char *p, size_t len);

/* Blowfish-CFB64 encrypt/decrypt.  encrypt=1 → encrypt, 0 → decrypt. */
ngx_int_t brix_sss_bf32_crypt(int encrypt, const u_char *key, size_t key_len,
...
const brix_sss_key_t *brix_sss_find_key(
    ngx_stream_brix_srv_conf_t *conf, int64_t id);
...
const brix_sss_key_t *brix_sss_find_key_arr(ngx_array_t *keys, int64_t id);

/*
 * brix_sss_verify_blob — validate a self-contained SSS credential against a
 ...
 */
ngx_int_t brix_sss_verify_blob(ngx_array_t *keys, time_t lifetime,
...
ngx_int_t brix_sss_parse_identity(const u_char *data, size_t len,
    brix_sss_identity_t *id);

/* Send a kXR_error(kXR_NotAuthorized) and log the failure. */
ngx_int_t brix_sss_auth_failed(brix_ctx_t *ctx, ngx_connection_t *c);

/* Encrypt a challenge with key and send a kXR_authmore response. */
ngx_int_t brix_sss_send_authmore(brix_ctx_t *ctx, ngx_connection_t *c,
    const brix_sss_key_t *key, const u_char *hdr, size_t hdr_len);

#endif /* BRIX_SSS_SSS_INTERNAL_H */
