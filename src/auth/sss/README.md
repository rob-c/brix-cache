...
session identity is set. Kernel path confinement ([`src/fs/path/beneath.c`](../../fs/path/))
...
| File | Responsibility |
|---|---|
| `sss_framing.c` / `.h` | validate the fixed magic ("sss\0" + BF32 encoding marker), the key-name-size field, and the header-length/NUL-termination against the received datagram length. |
| `sss_keytab_kernel.h` | The pure-C keytab grammar shared by the server's keytab loader (src/sss/config.c) and the native client's keytab reader (client/lib/sss_keytab.c): the one-line field parser and the keytab-file permission check. |

## Key types & data structures

- **`brix_sss_key_t`** (defined in [`src/core/types/config.h`](../../core/types/config.h)) —
...
- **Key policy `opts`** ([`src/core/types/tunables.h`](../../core/types/tunables.h),
...
4. On success the handler sets `ctx->dn`/`vo_list`/`primary_vo`, updates
   ...
   ([`src/core/types/identity.h`](../../core/types/identity.h)), records metrics through
...
[`../cms/`](../../net/cms/) `src/net/cms/server_module.c`.

**Startup:** [`src/core/config/postconfiguration.c`](../../core/config/postconfiguration.c)
...
- [`../config/`](../../core/config/) — `src/core/config/postconfiguration.c` startup wiring.
