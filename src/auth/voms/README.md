...
chain (`gsi/auth.c` for `root://`, `src/protocols/webdav/auth_cert.c` for `davs://`), it calls
`brix_extract_voms_info()` here to populate the per-connection identity. Those
VO strings then flow to the ACL layer (`src/auth/authz/acl.c`, `src/auth/authz/auth_gate.c`),
...
(`src/core/config/policy.c`), so an operator who asks for VO ACLs cannot silently get a
...
| `voms_http.h` | Thin re-declaration of `brix_extract_voms_info()` for **HTTP** handlers (`src/protocols/webdav/auth_cert.c`). Uses only `<ngx_core.h>` + `<openssl/x509.h>` so HTTP code can call VOMS extraction *without* including `src/core/ngx_brix_module.h`, which drags in `ngx_stream.h` and conflicts with the HTTP layer. Never include both headers together. |

### Other files

| File | Responsibility |
|---|---|
| `vo_token.h` | brix_vo_token_is_safe() — 1 if a VO/FQAN token is safe to place into the module's comma-separated VO list and later into metric labels / access-log fields, 0 otherwise. |
...
- **Output strings (caller-owned, not a type here)** — `primary_vo` (single VO)
  ...
  (`src/core/types/context.h`) for `root://` and in the request `identity`
  (`src/core/types/identity.c`) for HTTP, with caller-supplied buffer sizes
  (256 / 1024 bytes at the GSI and WebDAV call sites).

## Control & data flow

**Startup.** `src/core/config/postconfiguration.c` calls `brix_voms_init(cf->log)`
once. It `dlopen`s the library and resolves symbols, or logs a notice and
declines. Later, `src/core/config/policy.c` consults `brix_voms_available()` and the
...
- `davs://` HTTP — `src/protocols/webdav/auth_cert.c` (via `voms_http.h`) calls it with the
...
`src/auth/authz/acl.c` (`brix_vo_list_contains()`, `brix_check_vo_acl()`) and
`src/auth/authz/auth_gate.c` to enforce `brix_require_vo`, and by `../metrics`
...
- **Fail-soft on the data path, fail-closed at config.** Missing VOMS extensions
  ...
  *without* a loadable `libvomsapi` is rejected at startup in `src/core/config/policy.c`
...
- **HTTP/stream header split.** HTTP code must include `voms_http.h`, never
  `voms_internal.h`/`src/core/ngx_brix_module.h` — the stream umbrella header redefines
...
- **Add a new caller of VOMS extraction**: call `brix_extract_voms_info()`,
  ...
  code includes the prototype from `src/core/ngx_brix_module.h`; HTTP code includes
...
- **Enforce a new VO policy directive**: that work lives in `src/core/config/policy.c`
  and `src/auth/authz/acl.c`, not here — this subsystem only *produces* the VO strings.

## See also

- [`../gsi/README.md`](../gsi/README.md) — X.509 proxy verification; primary
  stream caller of `brix_extract_voms_info`.
- [`../../protocols/webdav/README.md`](../../protocols/webdav/README.md) — `auth_cert.c`, the HTTP caller
  (via `voms_http.h`).
- [`../../fs/path/README.md`](../../fs/path/README.md) — `acl.c` / `src/auth/authz/auth_gate.c` consume the
  VO strings to enforce `brix_require_vo`.
- [`../../core/config/README.md`](../../core/config/README.md) — `src/core/config/postconfiguration.c` calls
  `brix_voms_init`; `policy.c` validates `brix_require_vo`, `brix_vomsdir`,
  `brix_voms_cert_dir`.
- [`../../core/types/README.md`](../../core/types/README.md) — `brix_ctx_t` and the request
