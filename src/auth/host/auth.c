 ...
 *       BRIX_AUTH_HOST), an allowlist MUST be configured (empty = deny all),
 ...
 */
static ngx_flag_t
brix_host_pattern_match(const char *host, const ngx_str_t *pat)
{
...
static ngx_flag_t
brix_host_allowed(ngx_stream_brix_srv_conf_t *conf, const char *host)
{
    ngx_str_t  *pats;
    ngx_uint_t  i;
    ...
    for (i = 0; i < conf->host_allow->nelts; i++) {
        if (brix_host_pattern_match(host, &pats[i])) {
            return 1;
        }
    }
    return 0;
 ...
 * hostname.  Returns kXR_ok via BRIX_RETURN_OK or kXR_NotAuthorized on any
