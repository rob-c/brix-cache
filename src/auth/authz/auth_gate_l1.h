#ifndef BRIX_PATH_AUTH_GATE_L1_H
#define BRIX_PATH_AUTH_GATE_L1_H
 ...
 *   (auth_cache.c / `brix_kv_t`): an L1 hit returns the verdict WITHOUT taking
 *   the per-zone `ngx_shmtx` spinlock that `brix_kv_get` holds.
 *
 ...
 *
 * SCOPE / CORRECTNESS:
 *   - Used ONLY when the operator enabled `brix_auth_cache` (it is an L1 in
...
#include <ngx_core.h>
#include "auth_cache.h"   /* brix_auth_cache_val_t */

typedef struct brix_auth_l1_s brix_auth_l1_t;
