#ifndef BRIX_CRYPTO_OCSP_H
#define BRIX_CRYPTO_OCSP_H
 ...
 *
 *   brix_ocsp_check_cert() — query the OCSP responder URL embedded in a
 *     client certificate and verify its revocation status.  Used in the GSI
 *     authentication path after X.509 chain verification.
 *
 *   brix_ocsp_staple_fetch() — fetch an OCSP response for the server's own
 ...
 */
ngx_int_t brix_ocsp_staple_fetch(ngx_log_t *log,
    ngx_stream_brix_srv_conf_t *xcf);

#endif /* BRIX_CRYPTO_OCSP_H */
