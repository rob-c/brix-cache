...
| File | Responsibility |
|------|---------------|
| `gsi_verify.c` | `brix_gsi_verify_chain()` — the unified X509_STORE_CTX lifecycle: init with leaf + untrusted chain + CA store, set `X509_V_FLAG_ALLOW_PROXY_CERTS`, optional depth, `X509_verify_cert()`, then extract the leaf subject DN via `X509_NAME_oneline` into `res->dn_buf`. Owns nothing it is passed. |
| `gsi_verify.h` | Declares `brix_gsi_verify_chain()` and the `brix_gsi_verify_result_t` output struct (`char dn_buf[1024]`). |
| `pki_build.c` | `brix_build_ca_store()` — construct an `X509_STORE` from a CA dir and/or CA bundle file (falls back to system default paths if both NULL), apply caller `extra_flags`, then scan a CRL file or directory and, if any CRL loaded, enable `CRL_CHECK | CRL_CHECK_ALL | USE_DELTAS`. Internal `pki_load_crls*` scan `*.pem` and grid `*.r[0-9]` CRL files. |
| `pki_build.h` | Declares `brix_build_ca_store()`; documents NULL-path semantics and that CRL-path failure is fatal (returns NULL, store already freed). |
| `pki_load.c` | PEM loaders used by the startup audit: `brix_pki_load_certs_from_path()` / `brix_pki_load_crls_from_path()` accept a file or directory, allocate a `STACK_OF`, set `FD_CLOEXEC` on every opened file, filter CRL filenames, and free the stack if empty. Returns caller-owned stacks. |
| `pki_check.c` | Startup **consistency audit** (not online revocation): `brix_pki_check_paths()` loads CAs+CRLs, then `brix_pki_verify_crls()` matches each CRL's issuer DN to a CA subject DN **and** verifies the CRL signature with that CA's public key. Errors are logged but **non-fatal** (always returns NGX_OK). `brix_check_pki_and_crl()` is the thin module-facing wrapper. |
| `pki_check.h` | Declares the audit API; defines `UNIT_TEST` typedefs (`BRIX_LOG_T`, `BRIX_PKI_STATUS_T`) so `pki_check.c`/`pki_load.c` compile standalone for `tests/unit/` without nginx. |
| `ocsp.c` | OCSP client: `brix_ocsp_check_cert()` (online revocation of a client leaf via its AIA OCSP URL, with nonce + soft-fail policy) and `brix_ocsp_staple_fetch()` (fetch the server cert's OCSP response, DER-encode, cache in `xcf->ocsp_staple_data`). Synchronous HTTP/1.0 over OpenSSL BIO; HTTPS responders get a verifying TLS client with SNI. |
...
- **`X509_STORE`** — the trust anchor set returned by `brix_build_ca_store()`
...
1. **Proxy certs are required, and enabled in two places.**
   ...
   callers pass the same flag as `extra_flags` to `brix_build_ca_store()` so
...
2. **DN match alone never authorises a CRL.** `pki_check.c` deliberately does
   ...
   (`brix_pki_crl_signature_valid`). Two CAs can share a subject DN in a
   misconfigured store; the signature step is what closes that hole.
3. **The startup audit is fail-open by design.** `brix_pki_verify_crls` /
   `brix_pki_check_paths` **always return NGX_OK** and only log — a broken or
...
4. **OCSP soft-fail contract.** `brix_ocsp_check_cert` returns 0 (pass) or -1
...
8. **Known limitation, documented in code.** `brix_ocsp_check_cert` passes
   ...
   `xcf->gsi_store`.
9. **Ownership discipline.** `brix_gsi_verify_chain` and `brix_build_ca_store`
   ...
   `brix_pki_name_to_str` callers). Every file opened sets `FD_CLOEXEC` so a
...
- **Add a new CA/CRL source format:** extend the filename filter in
  `pki_load.c` (`brix_pki_crl_filename_matches`) and/or the directory scan in
...
- **Add a verification flag (e.g. policy constraints):** set it in
  `brix_gsi_verify_chain` (`gsi_verify.c:69`) so *both* protocols inherit it,
  and/or thread it through `extra_flags` of `brix_build_ca_store`. Never patch
  one caller only.
