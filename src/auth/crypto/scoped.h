...
/* Deliberately ngx-free: plain C99 `static inline` + OpenSSL headers only, so
 * this header also compiles into standalone TUs (e.g. the sd_s3 read unit
 * builds core/compat/crypto.c without the nginx tree on the include path). */
...
static inline void brix_bn_free(BIGNUM *p)
{ if (p) { BN_free(p); } }
...
static inline void brix_bn_clear_free(BIGNUM *p)
...
static inline void brix_x509_stack_free(STACK_OF(X509) *p)
...
static inline void brix_openssl_free(void *p)
{ if (p) { OPENSSL_free(p); } }
