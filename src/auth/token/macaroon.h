 ...
 * HOW: brix_token_is_macaroon() routes token type; brix_macaroon_secret_parse() converts hex secret to binary key material; brix_macaroon_validate_bundle() validates space-separated bundles with discharge verification; brix_macaroon_validate() thin wrapper for single-root tokens. */
 ...
 * HOW: Delegate to brix_macaroon_validate_bundle() which handles both single-root and multi-discharge cases uniformly — no local logic in this wrapper. */
