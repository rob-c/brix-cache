...
`brix_token_claims_t` describing the subject, issuer, audience, validity window,
VO groups, and a list of `permission:path` storage scopes. Every protocol that
accepts tokens funnels through one entry point, `brix_token_validate()` in
...
- **L1 — `worker_cache.c` (always on, per-worker, lockless).** A direct-mapped
  table keyed on SHA-256(token) → validated `brix_token_claims_t`, created
...
- **L2 — `token_cache.c` (optional, cross-worker SHM).** The pre-existing
  `brix_kv_t`-backed cache; consulted on an L1 miss and promoted into L1.
...
| File | Responsibility |
|---|---|
| `validate.c` | **Primary entry point.** `brix_token_validate()`: routes macaroon-vs-JWT, then for JWT does structure check (3 dot-segments) → alg check (RS256/ES256 only, rejects `alg:none`) → key selection by `kid` → signature verify → claim extraction (iss/sub/aud/exp/nbf/iat/scope/groups) → issuer/audience/time-window checks. Also `token_sanitize_for_log()` (anti log-injection) and `brix_token_extract_groups()`. |
| `scopes.c` | WLCG scope engine. `brix_token_parse_scopes()` splits the space-separated `permission:path` claim into `brix_token_scope_t[]`; `brix_token_check_read/_write()` test access with prefix matching guarded against the `/data` vs `/database` boundary attack. Maps `storage.read/write/create/modify/stage`. |
| `macaroon.c` | WLCG macaroon validator. `brix_macaroon_validate[_bundle]()` reconstructs the HMAC-SHA256 signature chain over binary packets, extracts `activity:`/`path:`/`before:` caveats, and verifies third-party discharge macaroons (AES-256-CBC vid decryption, caveat intersection). `brix_token_is_macaroon()` and `brix_macaroon_secret_parse()`. |
| `macaroon_issue.c` | Macaroon **minting** for the delegation endpoint. `brix_macaroon_issue()` builds the packet sequence (location/identifier/caveats/signature), computes the HMAC chain, base64url-encodes the result. |
| `jwks.c` | Loads a JWKS JSON file from disk (size-capped, `FD_CLOEXEC`), parses the `keys` array into `EVP_PKEY` handles via jansson. `brix_jwks_load/_free/_register_cleanup()`. |
| `keys.c` | JWK→OpenSSL bridge. `brix_token_rsa_pubkey_from_ne()` (n/e → RSA `EVP_PKEY`) and `brix_token_ec_pubkey_from_xy()` (x/y → EC P-256 `EVP_PKEY`) via `OSSL_PARAM_BLD` + `EVP_PKEY_fromdata`. |
| `signature.c` | JWT signature verification. `brix_token_verify_rs256()` (RSA+SHA-256 EVP chain) and `brix_token_verify_es256()` (converts IEEE P1363 raw `r||s` → DER ASN.1 before EVP). |
| `refresh.c` | JWKS hot-reload. Per-worker mtime-polling timer (`brix_token_jwks_schedule_refresh()` + handler) that reloads keys in-place only after a successful parse, preserving old keys on failure. |
| `config.c` | `brix_configure_token_auth()`: `nginx -t`-time validation that `token_jwks`/`token_issuer`/`token_audience` are set when auth is token/both, loads JWKS, records mtime, registers cleanup. |
| `token_cache.c` | L2: cross-worker validated-claims cache keyed on SHA-256(token), TTL = `min(exp-now, 5min)`. `brix_token_cache_lookup/_store()` and the `brix_token_cache` zone directive setter. |
| `worker_cache.c` | L1: always-on, per-worker, lockless direct-mapped validated-claims cache (`brix_token_l1_create/_lookup/_store()`). Front of the optional L2; created lazily per conf. Stops repeated token validation from stalling the event loop under load. |
| `oauth2.c` | `brix_oauth2_parse_access_token()`: extracts `access_token` from an OIDC/OAuth2 token-endpoint JSON response (jansson, duplicate-rejecting). |
| `file.c` | `brix_token_read_file()`: shared bounded reader for tokens/credentials stored on disk (size cap, whitespace strip, `FD_CLOEXEC`). |
...
| `token.h` | Public interface + core types (`brix_jwks_key_t`, `brix_token_scope_t`, `brix_token_claims_t`) and tunables (`BRIX_MAX_JWKS_KEYS`, `BRIX_MAX_TOKEN_SCOPES`, `BRIX_SCOPE_PATH_MAX`). |
...
| `macaroon.h` / `macaroon_issue.h` | Macaroon validate/issue prototypes and `BRIX_MACAROON_ISSUE_OUT_MAX`. |
...
- **`brix_token_claims_t`** (`token.h`) — the output of all validation. Fixed-size,
  ...
  `scopes[BRIX_MAX_TOKEN_SCOPES]` array with `scope_count`. This whole struct is
  what `token_cache.c` stores verbatim.
- **`brix_token_scope_t`** (`token.h`/`scopes.h`) — one `permission:path` grant:
  `path[BRIX_SCOPE_PATH_MAX]` plus `read/write/create/modify` bitfields. Empty path
  defaults to `/` (full-prefix) per WLCG convention; `storage.stage` maps to read.
- **`brix_jwks_key_t`** (`token.h`) — a loaded key: `kid[128]` + `EVP_PKEY *pkey`
  (RSA or EC). Held in the server-conf `jwks_keys[]` array (`config.c`/`refresh.c`).
- **`brix_macaroon_tp_t`** (`macaroon.c`, file-private) — a captured third-party
...
**Entry** is almost always `brix_token_validate()` (`validate.c`). It first calls
`brix_token_is_macaroon()`; if the token has no `.` it is treated as a macaroon and
...
parse the scope claim. Success populates a caller-supplied `brix_token_claims_t`.
...
- **Discharge depth is capped at 1.** Discharge macaroons may not themselves carry
  ...
  `BRIX_MACAROON_MAX_DISCHARGES`/`_TP_CAVEATS` (8 each). Recovered discharge keys
...
- **Add a new scope permission:** extend `token_scope_set_permission()` (`scopes.c`)
  with the exact permission string + length, set the right `brix_token_scope_t` bit,
  and (if write-like) honor it in `brix_token_check_write()`. Add a bit to
  `brix_token_scope_t` in both `token.h` and `scopes.h` if a new capability is needed.
...
- **Add a new JWT claim:** extract it in `brix_token_validate()` with
  `json_get_string/_int64()` into a new `brix_token_claims_t` field (remember the
