...
/* brix_gsi_parse_client_dh_public_key — decode the client's DH public value
 * (the "---BPUB---<hex>---EPUB--" kXRS_puk blob) into a caller-owned BIGNUM (free
 * with BN_free); NULL + WARN on a malformed blob. */
BIGNUM *
brix_gsi_parse_client_dh_public_key(ngx_connection_t *c, ngx_log_t *log,
...
/* brix_gsi_select_cipher_name — pick the session cipher from the kXRS_cipher_alg
...
void
brix_gsi_select_cipher_name(const u_char *payload, size_t payload_len,
...
/* brix_gsi_build_peer_dh_key — wrap the client's public BIGNUM as an EVP_PKEY peer
 * (against the server DH params) for EVP_PKEY_derive; NULL + WARN on failure. */
EVP_PKEY *
brix_gsi_build_peer_dh_key(ngx_log_t *log, EVP_PKEY *server_dh_key,
    BIGNUM *client_public_bn)
{
    /* The OpenSSL param pipeline now lives in the shared gsi_core.c. */
    EVP_PKEY *peer = brix_gsi_dh_build_peer(server_dh_key, client_public_bn);
