...
| File | Responsibility |
|------|----------------|
| `auth.c` | `brix_handle_auth()` — the public, rate-limited `kXR_auth` entry point and its inner dispatcher. Reads the 4-byte credtype, routes to the right handler, runs the full GSI round-2 path (parse → `brix_gsi_verify_chain` → optional OCSP → DN/VOMS extraction → identity + session registration → metrics), and enforces the `BRIX_MAX_AUTH_ATTEMPTS` brute-force limit. |
| `cert_response.c` | `brix_gsi_send_cert()` — GSI round 1. Pops/generates an ephemeral DH key, hex-encodes the public value as a `---BPUB---…---EPUB--` blob, optionally RSA-signs the client `rtag`, and assembles the `kXGS_cert` (`kXR_authmore`) wire response with `kXRS_puk`/`kXRS_cipher_alg`/`kXRS_md_alg`/`kXRS_x509`/`kXRS_main` buckets. |
| `parse_x509.c` | `brix_gsi_parse_x509()` — GSI round 2 top-level. Extracts client DH public + encrypted `kXRS_main`, derives the DH shared secret, sets `ctx->signing_key`/`ctx->signing_active`, AES-CBC-decrypts the chain, and returns the parsed `STACK_OF(X509)` (caller frees). |
| `parse_crypto_helpers.c` | OpenSSL building blocks for round 2: `brix_gsi_parse_client_dh_public_key()` (decode the BPUB/EPUB hex blob → `BIGNUM`), `brix_gsi_select_cipher_name()` (pick the cipher from `kXRS_cipher_alg`), `brix_gsi_build_peer_dh_key()` (build the client peer `EVP_PKEY` from merged server params + client pub). |
...
| `keypool.c` / `keypool.h` | Per-worker warm pool of ephemeral `ffdhe2048` DH keys: `brix_gsi_dh_keygen()` (one key), `brix_gsi_keypool_init()` (lazy seed at worker start — generates only `brix_gsi_keypool_seed` keys synchronously, then fills to `brix_gsi_keypool_size` **off the event thread** via the GSI server's thread pool; full synchronous warm-up only when no thread pool exists), `brix_gsi_keypool_pop()` (lock-free pop + off-thread refill at half-target low-water). Keeps keygen off the event thread — including at boot, which was the dominant per-worker startup cost. |
...
| `gsi_cipher.c` | Session-cipher negotiation + AES encrypt/decrypt (`brix_gsi_cipher_*`). *(Phase 38 split of `gsi_core.c`.)* |
...
- **`brix_gsi_verify_result_t`** (`../crypto/gsi_verify.h`) — carries the
  verified leaf's DN (`dn_buf`) back from chain verification.
- **`brix_token_claims_t`** (`../token/`) — JWT claims (`sub`, `groups`,
  `scopes[]`, `scope_count`) extracted on the `ztn` path.
- **`brix_gsi_refill_t`** (`keypool.c`) — task-local batch of freshly generated
...
- **Phase ordering is enforced.** `brix_handle_auth_inner()` rejects
  `kXR_auth` unless `ctx->logged_in` (`kXR_login` must precede it). With
  `BRIX_AUTH_NONE` it short-circuits to `auth_done = 1`.
...
- **Brute-force + amplification guard.** `brix_handle_auth()` stops after
  `BRIX_MAX_AUTH_ATTEMPTS` and explicitly does **not** count the `kXGC_certreq`
...
- **Hot CRL reload is non-disruptive.** `brix_rebuild_gsi_store()` builds the
...
- **Add a new credential type:** add the 4-byte credtype check in
  `brix_handle_auth_inner()` (`auth.c`), gate it against a new
  `BRIX_AUTH_*` value in `conf->auth`, and either implement the handler here or
