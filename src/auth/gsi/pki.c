 ...
 *       brix_pki_check_paths() which loads certificates, verifies cross-signatures between CA certs
 ...
 *       separate structure types (ngx_stream_brix_srv_conf_t vs ngx_http_brix_webdav_loc_conf_t) for
...
ngx_int_t
brix_check_pki_consistency_stream(ngx_log_t *log,
    ngx_stream_brix_srv_conf_t *xcf)
{
    const char         *ca_path;
    const char         *crl_path;

    ca_path = (xcf->common.trusted_ca.len > 0) ? (char *) xcf->common.trusted_ca.data : NULL;
    crl_path = (xcf->crl.len > 0) ? (char *) xcf->crl.data : NULL;

    return brix_pki_check_paths(log, ca_path, crl_path, "xrootd");
}
