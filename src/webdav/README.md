...
a `location` marked `xrootd_webdav on`, and projects the same on-disk export
...
| File | Responsibility |
|------|----------------|
| `dispatch.c` | Content-handler entry `ngx_http_xrootd_webdav_handler`: routes by `r->method`/`r->method_name`; pre-body lock check for PUT/MOVE; COPY → distinguishes TPC pull (`Source:`), TPC push (`Credential:`), local server-side copy; emits `Allow:` for unknown methods (405). |
| `access.c` | `ngx_http_xrootd_webdav_access_handler`: CORS, request metrics, IP-rate-limit shed, auth gate (cert→token), XrdHttp parse, global `allow_write` write-method gate, token write-scope check; skips re-auth for mirror subrequests. |
| `operation_table.c` | `xrootd_webdav_operations[]` descriptor table — one row per method with metric slot + capability flags (`XROOTD_PROTO_OP_READ/WRITE/LIST/LOCK/TPC`); drives Allow/CORS strings and write-method detection. |
| `path.c` | `ngx_http_xrootd_webdav_resolve_path` / `webdav_resolve_destination_path`: urldecode + strip trailing slashes + confine via `xrootd_http_resolve_path`; maps resolver codes to HTTP (404→409 Conflict for COPY/MOVE parents). |
...
| File | Responsibility |
|------|----------------|
| `get.c` | GET via VFS open (read-through cache aware) → `xrootd_http_serve_file_ranged` (shared ranged sendfile); routes multi-range to XrdHttp multipart; If-Modified-Since; checksum/XrdHttp headers; range + byte-transfer metrics. |
...
| File | Responsibility |
|------|----------------|
| `auth_cert.c` | `webdav_verify_proxy_cert` — GSI/x509 proxy chain verification with a two-tier per-TLS-connection + per-SSL_SESSION auth cache (OpenSSL ex_data); optional VOMS VO extraction; populates `xrootd_identity_t`. |
...
| `proxy_config.c` | Parse the proxy-URL list into resolved `xrootd_webdav_backend_t[]` (multi-backend, round-robin + passive health); legacy single-backend aliased to `backend[0]`. |
...
| `proxy_internal.h` | Proxy-mode-only header: `webdav_proxy_ctx_t`, `xrootd_webdav_backend_t`, and the six upstream lifecycle prototypes. |
...
- **`ngx_http_xrootd_webdav_loc_conf_t`** (`webdav.h`) — the per-location
  configuration. Embeds the shared `ngx_http_xrootd_shared_conf_t common`
...
- **`ngx_http_xrootd_webdav_req_ctx_t`** (`webdav.h`) — per-request context
...
- **`xrootd_webdav_operations[]` / `xrootd_http_operation_t`**
...
- **`xrootd_webdav_backend_t` / `xrootd_proxy_be_table_t` / `webdav_proxy_ctx_t`**
...
2. **Precontent phase**: `../mirror/` (`xrootd_http_mirror_precontent_handler`)
...
3. **Content phase** (`dispatch.c`): routes to the per-method handler. Each
   handler calls `ngx_http_xrootd_webdav_resolve_path` (`path.c`) to confine the
...
- **Kernel confinement is mandatory.** Every client URI and `Destination:` is
  resolved through `xrootd_http_resolve_path` / the `../path/` `RESOLVE_BENEATH`
...
- **Add a WebDAV method:** implement `webdav_handle_<m>` in a `.c` here, declare
  ...
  length+`ngx_strncmp`), add a row to `xrootd_webdav_operations[]` in
...
- **Add a config directive:** add the field to
  `ngx_http_xrootd_webdav_loc_conf_t` (`webdav.h`, sentinel `NGX_CONF_UNSET*`),
...
- **Add a metric:** follow `../metrics/README.md` (enum + field + export) then
  `XROOTD_WEBDAV_METRIC_INC(slot)` at the call site.
