 ...
 *
 * Enabled per-location with `xrootd_webdav_proxy_dynamic on`.  Unlike the
 ...
 */
#ifndef XROOTD_WEBDAV_PROXY_POOL_H
#define XROOTD_WEBDAV_PROXY_POOL_H
...
typedef enum {
    ...
    XROOTD_PROXY_BE_DRAINING,   /* no new selects; in-flight requests finish */
    XROOTD_PROXY_BE_DEAD,       /* health-check failure; no new selects */
...
typedef struct {
    ...
    ngx_uint_t               capacity;
    xrootd_proxy_be_entry_t  slots[];       /* capacity entries */
...
int xrootd_proxy_pool_remove(uint32_t id);
int xrootd_proxy_pool_drain(uint32_t id);
int xrootd_proxy_pool_undrain(uint32_t id);

/* Current in_flight count for id (or -1 if not found). */
long xrootd_proxy_pool_in_flight(uint32_t id);
