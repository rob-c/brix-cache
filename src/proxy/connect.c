/*
 * connect.c — upstream TCP connect, XRootD bootstrap, and cleanup.
 */
 ...
 *   24  kXR_login   request (username "xrd", anonymous)
 */
static size_t
xrootd_proxy_build_bootstrap(u_char *buf)
{
    u_char *cursor = buf;
    ...
    /* kXR_login (anonymous, username "xrd") */
    {
                     ...
                     ngx_stream_xrootd_srv_conf_t *conf)
{
    ...
    bslen = xrootd_proxy_build_bootstrap(bsbuf);
...
xrootd_proxy_cleanup(xrootd_proxy_ctx_t *proxy)
{
    ...
    if (proxy->conn != NULL) {
        ...
        if (!has_open && proxy->state == XRD_PX_IDLE) {
