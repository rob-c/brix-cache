...
| `forward_relay.c` | Aggregator TU that includes the three `forward_relay_*` fragments. The build compiles `forward_relay.c` as a single translation unit combining all three fragments. Safe to keep as-is or restructure later if needed. |
