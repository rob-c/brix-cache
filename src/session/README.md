...
| `lifecycle.c` | `kXR_ping` (no-op liveness reply via `XROOTD_RETURN_OK`) and `kXR_endsess` (graceful teardown). `endsess` fire-and-forgets an `kXR_endsess` frame to the proxy upstream when one is attached and idle, runs `xrootd_on_disconnect` + `xrootd_close_all_files`, then **clears `logged_in`/`auth_done`** (`:142-143`) so the dispatcher rejects any post-endsess ops (enforces GSI proxy-expiry / session-end semantics). |
...
- **Fail-closed auth boundary.** Downstream handlers require `logged_in &&
  auth_done`. `kXR_endsess` deliberately resets both to 0 (`lifecycle.c:142-143`)
  so a client cannot keep operating after session end or GSI proxy expiry.
