...
require both `ctx->logged_in` **and** `ctx->auth_done` set on `xrootd_ctx_t`.
When `xrootd_auth=none`, `auth_done` is set immediately at login; otherwise it is
...
| `lifecycle.c` | `kXR_ping` (no-op liveness reply) and `kXR_endsess` (graceful teardown). **`endsess` is session-scoped**: it terminates the session NAMED by `req->sessid`, not merely the connection it arrives on. If the named sessid is NOT this connection's own (e.g. the official client's reconnect recovery releasing its previous, dropped session on a freshly re-logged-in connection), it only `xrootd_session_unregister()`s that session and leaves THIS connection authenticated. Only an endsess for the connection's OWN session forwards the upstream frame (proxy), runs `xrootd_on_disconnect` + `xrootd_close_all_files`, and **clears `logged_in`/`auth_done`** (enforces GSI proxy-expiry / session-end). Clearing auth on a *foreign*-session endsess (the old behavior) made the official client's recovery `kXR_open` fail with `kXR_NotAuthorized` on a lossy link. Guarded by `tests/test_endsess_session_scope.py`. |
| `bind.c` | `kXR_bind` handler — attaches a read-only secondary data channel. Looks up the primary's sessid via `xrootd_session_lookup`, inherits its identity (DN/VO/token_auth → `ctx->identity` via `xrootd_identity_set_*`), assigns a `pathid` from a static counter cycling 1–253 (0 reserved for the primary), sets `is_bound/logged_in/auth_done=1`, and replies with the single-byte pathid. Fails closed (`kXR_NotAuthorized`) if the sessid is unknown. |
...
| `tls_config.c` | `xrootd_configure_tls()` — postconfig builder for the in-protocol TLS `SSL_CTX` (TLSv1.2/1.3, server cert+key from `conf->certificate`/`certificate_key`), optional kTLS send-offload (`SSL_OP_ENABLE_KTLS`, gated on `tls_ktls`, enables sendfile over TLS) and the OCSP-stapling status callback `xrootd_ocsp_stapling_cb`. Runs once per TLS-enabled server block; emerg-fails if cert/key are missing. |
| `registry.c` | Shared-memory **session registry**: zone creation/sizing (`xrootd_configure_session_registry`, sized by runtime `slots`), `*_shm_init_zone`, and the `register`/`lookup`/`unregister` lifecycle. Includes the Phase-27 LRU reap-on-full (evict the global-LRU slot older than `XROOTD_SESSION_REAP_MIN_AGE_MS` to defeat slot-exhaustion DoS) and emits `session_registry_full_total`/`session_evict_total` metrics. |
| `handles.c` | Shared-memory **published-handle table**: `publish`/`lookup`/`lookup_hint`/`unpublish`/`unpublish_all` plus the `xrootd_shared_handle_same_key` key matcher and zone init. Publishes only *readable* primary handles (write-only/`path==NULL` → entry removal); carries `device`/`inode`/`cached_size`/`path` for secondary reopen + identity re-validation. `lookup_hint` adds a caller-cached slot index for O(1) per-read revalidation on hot bound streams (Phase 33 C2). |
| `registry.h` | Types/prototypes/capacities for both shared tables: `xrootd_session_entry_t`, `xrootd_shared_handle_entry_t`, table structs (shmtx `lock` first), the sizing factors, and the full registry/handle API. |
| `session.h` | Documents the login state machine and declares the session opcode handlers + the GSI bucket/x509 helpers (`gsi_find_bucket`, `xrootd_gsi_parse_x509`, `xrootd_handle_auth` — **implemented in `../gsi/`**, declared here for the dispatcher). |

## Key types & data structures

- **`xrootd_session_entry_t`** (`registry.h`) — one registry slot: `sessid[16]`,
...
- **`xrootd_shared_handle_entry_t`** (`registry.h`) — one published-handle slot,
  ...
  canonical confined `path[XROOTD_MAX_PATH+1]` for reopen. Write-only handles are
  never published.
- **`xrootd_session_table_t` / `xrootd_shared_handle_table_t`** (`registry.h`) —
  ...
  `XROOTD_SESSION_HANDLE_SLOTS` (`XROOTD_SESSION_HANDLE_SESSIONS 512 ×
  XROOTD_SESSION_HANDLES_PER_SESSION 8 = 4096`, deliberately decoupled from the
  per-connection `XROOTD_MAX_FILES=16` to bound idle RAM — see the ~50 MB sizing
  note in `registry.h`).
- **Session state on `xrootd_ctx_t`** (`../types/context.h`) — set here and read
...
`kXR_protocol → xrootd_handle_protocol`, `kXR_login → xrootd_handle_login`,
`kXR_auth → xrootd_handle_auth` (in `../gsi/`), `kXR_ping → xrootd_handle_ping`,
`kXR_endsess → xrootd_handle_endsess`, `kXR_bind → xrootd_handle_bind`.
...
- `../config/postconfiguration.c` — invokes `xrootd_configure_tls()` and
  `xrootd_configure_session_registry()` once at startup (the latter sized by the
  max `registry_slots` across server blocks).
- `../metrics/` — `op_ok[XROOTD_OP_LOGIN]`, `op_ok[XROOTD_OP_PING]`,
  `session_registry_full_total`, `session_evict_total`.
- `../proxy/` — `lifecycle.c` forwards `kXR_endsess` to the upstream; proxy and
  bound paths call `xrootd_session_lookup()`/`xrootd_session_handle_lookup()`.
...
- Secondaries revalidate via `../connection/fd_table.c`
  (`xrootd_session_handle_lookup_hint`) and the proxy forwarding helpers in
  `../proxy/`.
- `xrootd_session_unregister()` is driven by `kXR_endsess` and the disconnect
...
- **Shared-memory discipline.** Every table access is serialised by its zone
  shmtx (`xrootd_session_mutex` / `xrootd_handle_mutex`); the `lock` field **must
...
- **Reap-on-full, not reject-only.** A full session table evicts the global-LRU
  slot if it is older than `XROOTD_SESSION_REAP_MIN_AGE_MS` (60 s) so an attacker
  ...
  `xrootd_session_unregister`) to avoid deadlock (`registry.c:281-289`).
...
- **New session opcode:** add `xrootd_handle_<op>()` here, declare it in
  `session.h` (and `../ngx_xrootd_module.h` if the dispatcher needs it), then
...
- **New TLS knob:** add the directive/field in `../config/`, then apply it inside
  `xrootd_configure_tls()` (guard new OpenSSL options with `#ifdef`).
...
- `../gsi/README.md`, `../token/README.md` — `kXR_auth` credential verification
  that feeds `xrootd_session_register()`.
