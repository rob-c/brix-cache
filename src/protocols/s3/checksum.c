    ...
    const char *alg_name, ngx_flag_t cache_only, char *out, size_t outsz)
{
    ...
    /* phase-105: caching the computed digest writes an xattr on the export, so
     * it needs this endpoint's write posture — a read-only bucket recomputes. */
    iopts.mutation_policy      =
