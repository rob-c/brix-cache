    ...
    ngx_stream_brix_srv_conf_t *conf, int idx, ngx_int_t *rc)
{
     ...
     * brix_durable_commit off relaxes the pre-publish data fsync to stock
     * close semantics (see staged_file.h). */
    if (brix_commit_staged(ctx->files[idx].fd, temp_path, final_path,
                             0,
                             (conf == NULL || conf->common.durable_commit)
                                 ? 0 : BRIX_COMMIT_RELAXED,
                             c->log) != NGX_OK) {
        int err = errno;
