...
| `pgwrite_helpers.c` / `.h` | Private pgwrite request state and synchronous completion helpers: per-handle accounting, dirty-range tracking, ordered error reporting, retry/Fob cleanup, recovery-journal recording, and status/CSE routing with the original request offset. |
