 ...
 *       write replies via brix_send_pgwrite_status (info = request offset).
 */
ngx_int_t
brix_handle_pgwrite(brix_ctx_t *ctx, ngx_connection_t *c)
{
...

	{
		    ...
		    brix_wrts_is_replay(&ctx->files[idx], offset, (uint32_t) flat_sz))
		{
			...
			return brix_send_pgwrite_status(ctx, c, offset);
		}
		...
		if (bad_count > 0) {
			...
			brix_pgw_fob_open(&ctx->files[idx]);
			for (bi = 0; bi < bad_count; bi++) {
				if (!brix_pgw_fob_add(&ctx->files[idx], bad_pages[bi].off,
				                        bad_pages[bi].dlen)) {
					snprintf(write_detail, sizeof(write_detail), "%lld+%zu",
							 (long long) offset, dlen);
					BRIX_RETURN_ERR(ctx, c, BRIX_OP_WRITE, "WRITE",
		 ...
		 * Fob correction (brix_pgw_fob_del) commits in-line right after the
		...
		(void) write_offset;
		if (bad_count > 0) {
			return brix_send_pgwrite_cse(ctx, c, offset,
			                               bad_pages, bad_count);
		}
		return brix_send_pgwrite_status(ctx, c, offset);
	}
