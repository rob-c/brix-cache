 ...
 *       (brix_resolve_op_path / brix_path_resolve_beneath), run the three-tier
 ...
 */
ngx_int_t
brix_handle_setattr(brix_ctx_t *ctx, ngx_connection_t *c,
    ngx_stream_brix_srv_conf_t *conf)
{
    ...
    if (brix_path_resolve_beneath(conf, c->log, reqpath, BRIX_PATH_EXISTING,
                                    resolved, sizeof(resolved)) != NGX_OK) {
        BRIX_RETURN_ERR(ctx, c, BRIX_OP_CHMOD, "SETATTR", reqpath, "-",
                          kXR_NotFound, "not found");
    }

    if (brix_auth_gate(ctx, c, BRIX_OP_CHMOD, "SETATTR",
...

    {
        ...
        if (brix_vfs_setattr(&vctx, &attr) != NGX_OK) {
            BRIX_RETURN_ERR(ctx, c, BRIX_OP_CHMOD, "SETATTR", resolved, "-",
                              brix_kxr_from_errno(errno), strerror(errno));
        }
    }

    BRIX_RETURN_OK(ctx, c, BRIX_OP_CHMOD, "SETATTR", resolved, "-", 0);
}
 ...
 */
ngx_int_t
brix_handle_symlink(brix_ctx_t *ctx, ngx_connection_t *c,
    ngx_stream_brix_srv_conf_t *conf)
{
    xrdw_twopath_req_t req;
    char     target[PATH_MAX];
    char     link_buf[BRIX_MAX_PATH + 1];
    ...
    if (brix_path_resolve_beneath(conf, c->log, link_buf, BRIX_PATH_WRITE,
                                    link_resolved, sizeof(link_resolved)) != NGX_OK) {
        BRIX_RETURN_ERR(ctx, c, BRIX_OP_MKDIR, "SYMLINK", link_buf, "-",
                          kXR_NotFound, "invalid link path");
    }
    if (brix_auth_gate(ctx, c, BRIX_OP_MKDIR, "SYMLINK",
                         link_buf, link_resolved, conf,
                         BRIX_AUTH_UPDATE, 1) != NGX_OK) {
        return ctx->write_rc;
    }

    if (brix_symlink_confined_canon(c->log, conf->common.root_canon,
                                      target, link_resolved) != 0) {
        BRIX_RETURN_ERR(ctx, c, BRIX_OP_MKDIR, "SYMLINK", link_resolved, "-",
                          brix_kxr_from_errno(errno), strerror(errno));
    }

    BRIX_RETURN_OK(ctx, c, BRIX_OP_MKDIR, "SYMLINK", link_resolved, "-", 0);
}
 ...
 */
ngx_int_t
brix_handle_link(brix_ctx_t *ctx, ngx_connection_t *c,
    ngx_stream_brix_srv_conf_t *conf)
{
    xrdw_twopath_req_t req;
    ...
    if (brix_path_resolve_beneath(conf, c->log, src_buf, BRIX_PATH_EXISTING,
        ...
        BRIX_RETURN_ERR(ctx, c, BRIX_OP_MKDIR, "LINK", src_buf, "-",
                          kXR_NotFound, "source not found");
    }
    if (brix_auth_gate(ctx, c, BRIX_OP_MKDIR, "LINK",
    ...
    if (brix_path_resolve_beneath(conf, c->log, dst_buf, BRIX_PATH_WRITE,
        ...
        BRIX_RETURN_ERR(ctx, c, BRIX_OP_MKDIR, "LINK", dst_buf, "-",
    ...
    if (brix_link_confined_canon(c->log, conf->common.root_canon,
                                   src_resolved, dst_resolved) != 0) {
        BRIX_RETURN_ERR(ctx, c, BRIX_OP_MKDIR, "LINK", dst_resolved, "-",
                          brix_kxr_from_errno(errno), strerror(errno));
    }

    BRIX_RETURN_OK(ctx, c, BRIX_OP_MKDIR, "LINK", dst_resolved, "-", 0);
}
 ...
 */
ngx_int_t
brix_handle_readlink(brix_ctx_t *ctx, ngx_connection_t *c,
    ngx_stream_brix_srv_conf_t *conf)
{
    char    reqpath[BRIX_MAX_PATH + 1];
    char    resolved[PATH_MAX];
    ...
    if (brix_resolve_op_path(ctx, c, BRIX_OP_STAT, "READLINK", conf,
                               BRIX_PATH_NOEXIST,
                               reqpath, sizeof(reqpath),
                               resolved, sizeof(resolved)) != NGX_OK) {
        return ctx->write_rc;
    }
    if (brix_auth_gate(ctx, c, BRIX_OP_STAT, "READLINK",
                         reqpath, resolved, conf,
                         BRIX_AUTH_READ, 0) != NGX_OK) {
        return ctx->write_rc;
    }

    n = brix_readlink_confined_canon(c->log, conf->common.root_canon,
                                       resolved, target, sizeof(target) - 1);
    if (n < 0) {
        BRIX_RETURN_ERR(ctx, c, BRIX_OP_STAT, "READLINK", resolved, "-",
                          brix_kxr_from_errno(errno), strerror(errno));
    }

    /* Body = the link target. Log + count, then send (own body → not RETURN_OK). */
    brix_log_access(ctx, c, "READLINK", resolved, "-", 1, kXR_ok, NULL, (size_t) n);
    BRIX_OP_OK(ctx, BRIX_OP_STAT);
    return brix_send_ok(ctx, c, target, (uint32_t) n);
}
