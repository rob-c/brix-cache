#ifndef NGX_BRIX_RELAY_H
#define NGX_BRIX_RELAY_H

/*
 * relay.h — transparent pass-through relay for the stream xrootd listener.
 *
 * WHAT: when brix_transparent_proxy host:port is configured, every connection
 ...
 *   plus a per-direction brix_tap_stream that is fed each freshly-recv'd chunk.
...
/* Directive handler: brix_transparent_proxy host:port (SRV_CONF|TAKE1).
 * Resolves the target into conf->relay_addr at config time. */
char *brix_conf_set_transparent_proxy(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
...
ngx_int_t brix_relay_start(ngx_stream_session_t *s, ngx_connection_t *c,
    void *srv_conf);

#endif /* NGX_BRIX_RELAY_H */
