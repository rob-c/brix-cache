#ifndef BRIX_CONN_FD_H
#define BRIX_CONN_FD_H
 ...
 *
 * Slot lifecycle:
 *   alloc  → brix_alloc_fhandle   (returns slot index, sets fd = -1)
 *   open   → caller sets ctx->files[idx].fd and other fields
 *   use    → brix_validate_{file,read,write}_handle before each I/O
 *   close  → brix_free_fhandle    (closes fd, frees path, clears slot)
 *   cleanup→ brix_close_all_files (called on disconnect)
 */
...
/*
 * brix_set_fhandle_path — store the canonical filesystem path for a slot.
 *
 ...
 */
ngx_int_t brix_set_fhandle_path(brix_ctx_t *ctx, ngx_connection_t *c,
    int handle_index, const char *path);

/*
 * brix_free_fhandle — close the fd and release all resources for a slot.
 *
 ...
 */
void brix_free_fhandle(brix_ctx_t *ctx, int handle_index);

/*
 * brix_close_all_files — close and free every slot in the handle table.
 *
 * Called on TCP disconnect via brix_on_disconnect().  After this call the
 * table is empty and all kernel fds are closed.
 */
void brix_close_all_files(brix_ctx_t *ctx);
 ...
 */
int brix_ctx_has_open_file(const brix_ctx_t *ctx);

/*
 * brix_ensure_read_handle — prepare a handle for read-side I/O.
 *
 ...
 * resource failure.
 */
ngx_int_t brix_ensure_read_handle(brix_ctx_t *ctx, ngx_connection_t *c,
    int handle_index);

/*
 * brix_validate_file_handle — verify that handle_index refers to an open
 ...
 */
ngx_flag_t brix_validate_file_handle(brix_ctx_t *ctx,
    ngx_connection_t *c, int handle_index, const char *verb, ngx_uint_t op,
    ngx_int_t *rc);

/*
 * brix_validate_read_handle — validate handle and check readable flag.
 * Returns: 1 if handle is open and readable, 0 otherwise.
 */
ngx_flag_t brix_validate_read_handle(brix_ctx_t *ctx,
    ngx_connection_t *c, int handle_index, const char *verb,
    ngx_uint_t op, ngx_int_t *rc);

/*
 * brix_validate_write_handle — validate handle and check writable flag.
 * Returns: 1 if handle is open and writable, 0 otherwise.
 */
ngx_flag_t brix_validate_write_handle(brix_ctx_t *ctx,
    ngx_connection_t *c, int handle_index, const char *verb,
    ngx_uint_t op, ngx_int_t *rc);
