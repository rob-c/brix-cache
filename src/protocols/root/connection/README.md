...
listener, `ngx_stream_brix_handler()` (`handler.c`) is the per-connection entry
point. It allocates the per-connection `brix_ctx_t`, marks all file-handle
...
| `write_helpers.c` | Response queue/flush engine: `brix_queue_response[_base]` (flat buffer), `brix_queue_response_chain` (sendfile/chain), `brix_flush_pending` (resume parked sends). Owns the `out_ring` FIFO so pipelined responses never interleave on the wire; spins up to `BRIX_SEND_CHAIN_SPIN_MAX`; clears `TCP_CORK` + sets `TCP_NODELAY` after final segment; accounts `wire_bytes_tx_total`. |
...
| `tls.c` | In-protocol `kXR_ableTLS` upgrade: `brix_start_tls()` (clears stale OpenSSL error queue, creates SSL connection, runs handshake) and `brix_tls_handshake_done()` (success → resume at `REQ_HEADER`; failure → disconnect). |
| `tls.h` | Prototypes + preconditions for the TLS upgrade pair. |
| `fd_table.c` | Per-session open-file slot table. Alloc/free/validate handles; heap-owned path storage; **bound-secondary** shared-handle revalidation and lazy reopen (`brix_ensure_read_handle`); per-op capability re-checks (`brix_validate_{file,read,write}_handle`); teardown of checkpoint/POSC/TPC/write-through state on free. |
...
| `disconnect.c` | `brix_on_disconnect()` — the single close path. Sets `destroyed=1` (AIO guard), releases budget + concurrency slot, cleans upstream/proxy/mirror, frees heap buffers + crypto state, finalizes metrics, emits "interrupted"/`DISCONNECT` access-log lines, unregisters the session, flushes the access log. |
| `disconnect.h` | Documents the 8-step teardown contract; note that it does **not** close fds (caller pairs it with `brix_close_all_files`). |
| `event_sched.c` | `brix_schedule_read_resume` / `brix_schedule_write_resume` — re-arm and post read/write events to `ngx_posted_events` so the loop resumes immediately after AIO/upstream completion instead of waiting for the next epoll cycle. |
| `event_sched.h` | Prototypes + duplicate-post-guard semantics for the resume helpers. |
| `chain_helpers.c` | `brix_chain_pending_bytes()` — sum `ngx_buf_size()` over a chain (NULL-safe); the single source of truth for "unsent bytes" used by all send/flush accounting. |
...
- **`brix_ctx_t`** (`../types/context.h`) — the per-TCP-connection session
  ...
  `files[BRIX_MAX_FILES]` table, the `out_ring` send FIFO, the reusable scratch
...
- **`ngx_brix_srv_metrics_t`** (`../metrics/`) — the SHM per-server metrics
  block `ctx->metrics` points at; `handler.c` binds it and `disconnect.c`
  finalizes it.

## Control & data flow

**Entry.** A new TCP connection enters at `handler.c::ngx_stream_brix_handler`,
installed by postconfiguration. It sets `state = XRD_ST_HANDSHAKE` and calls
`ngx_stream_brix_recv(c->read)` to start framing.

**Inbound (recv loop).** `recv.c` accumulates bytes per the current state:
- `HANDSHAKE` (20 bytes) → `brix_process_handshake()` in
...
- Dispatch goes to `brix_dispatch()`
...
it on the next write-ready event via `brix_flush_pending`.
...
via `brix_open_beneath(conf->rootfd, rel, ...)` (RESOLVE_BENEATH) for exactly
that reason.

**Teardown.** Every close path (read EOF, write error, timeout, handshake
rejection, fatal dispatch error) funnels through `disconnect.c::brix_on_disconnect`
+ `fd_table.c::brix_close_all_files`, then `ngx_stream_finalize_session`.

## Invariants, security & gotchas

1. **Payload cap before allocation.** `recv.c::brix_max_payload_for_request`
   ...
   `brix_ensure_payload_buffer` allocates anything. Per-opcode limits:
   write/pgwrite/writev/chkpoint → `BRIX_MAX_WRITE_PAYLOAD`; readv → segs ×
...
3. **Heap (not pool) for cross-request buffers.** `payload_buf`, `prepare_paths`,
   ...
   (`brix_release_disconnect_owned_buffers`) and on slot retire — missing one
...
4. **Response frames must not interleave.** Only the **head** `out_ring` slot ever
   touches the socket (`brix_flush_pending` drains strictly head-first; queue
...
7. **SHM-global transfer-heap budget.** `budget.h` charging is **idempotent
   reconciliation**, never paired inc/dec: `brix_budget_sync` (called after any
   scratch grow/trim in `recv.c`) applies only the delta vs. `budget_charged`, so
   it can't drift negative or double-count; `brix_budget_release` zeroes it on
   disconnect. Admission (`brix_budget_admit`) returns 0 → caller sends
   `kXR_wait` rather than suspending server-side, and never deadlocks a lone
   transfer when the pool is otherwise empty.
8. **fhandle = slot index, one wire byte.** `brix_alloc_fhandle` returns a slot
   in `[0, BRIX_MAX_FILES)` (16); the value is sent verbatim as the wire handle.
   Validate before every I/O — `brix_validate_{read,write}_handle` re-check the
...
9. **Bound secondaries are read-only and revocable.** A `kXR_bind` secondary may
   ...
   `brix_ensure_read_handle` re-checks the primary's shared-memory slot on every
   ...
   via `brix_open_beneath` with `device`/`inode` revalidation — a changed
...
10. **Disconnect is the only place fd-less cleanup happens.** POSC temp files,
    ...
    `brix_free_fhandle` when a session ends without a clean `kXR_close`. The
    `destroyed` guard plus the access-log flush (`brix_access_log_flush`) at the
    end of `brix_on_disconnect` make interrupted-transfer records durable
    immediately.
...
- **New response shape:** queue it through `write_helpers.c`
  (`brix_queue_response` for a flat buffer, `brix_queue_response_chain` for
...
- [../types/README.md](../types/README.md) — `brix_ctx_t`, `brix_state_t`, `brix_file_t`, `brix_resp_slot_t`, tunables.
