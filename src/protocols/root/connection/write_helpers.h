#ifndef BRIX_CONN_WRITE_HELPERS_H
#define BRIX_CONN_WRITE_HELPERS_H

#include "core/ngx_brix_module.h"
#include "event_sched.h"

/*
 * brix_queue_response_base — attempt to send buffer immediately; if the
 ...
 *
 * owned_base: if non-NULL, this heap buffer is freed (via brix_release_read_buffer)
 ...
 */
ngx_int_t brix_queue_response_base(brix_ctx_t *ctx, ngx_connection_t *c,
    u_char *buffer, size_t buffer_len, u_char *owned_base);

/*
 * brix_queue_response — convenience wrapper around brix_queue_response_base
...
/*
 * brix_queue_response_chain — attempt to drain chain via c->send_chain().
 *
 * Spins up to BRIX_SEND_CHAIN_SPIN_MAX times while progress is made; parks
 ...
 *
 * Called from the write event handler (ngx_stream_brix_send) after the
