...
ngx_int_t brix_send_pgwrite_status(brix_ctx_t *ctx,
 ...
 * kXR_pgRetry. Falls back to brix_send_pgwrite_status when n == 0. */
...
uint32_t brix_crc32c(const void *buf, size_t len);
...
uint32_t brix_crc32c_copy(const u_char *src, u_char *dst, size_t len);

/* CRC32c of an open file descriptor; returns (uint32_t)-1 on read error. */
uint32_t brix_crc32c_file(int fd, const char *path, ngx_log_t *log);

#endif /* BRIX_RESPONSE_H */
