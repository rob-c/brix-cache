...
/*
 *
 * WHAT: Sends XRootD kXR_ok (opcode 1) success response with optional payload body. Calculates total size = header length + bodylen, allocates buffer from connection pool via ngx_palloc(). Builds response header using brix_build_resp_hdr() with ctx->cur_streamid and status=kXR_ok, dlen=bodylen. If bodylen > 0 AND body != NULL, copies body payload to buffer after header via ngx_memcpy(); if bodylen == 0 or body is NULL skips copy (zero-length success responses valid). Queues complete response for wire delivery via brix_queue_response() with total size including header + body. Returns NGX_ERROR only on allocation failure; always succeeds when memory available. Per AGENTS.md INVARIANT: all wire paths → brix_queue_response() before sending — this function delegates to the queueing infrastructure for partial write/EAGAIN handling.
 *
    ...
    const void *body, uint32_t bodylen)
{
    ...
    brix_build_resp_hdr(ctx->cur_streamid, kXR_ok, bodylen,
    ...
    uint16_t errcode, const char *msg)
{
    ...
    brix_build_resp_hdr(ctx->cur_streamid, kXR_error, (uint32_t) bodylen,
        (ServerResponseHdr *) buf);
