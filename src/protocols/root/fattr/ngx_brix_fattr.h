#ifndef NGX_BRIX_FATTR_H
#define NGX_BRIX_FATTR_H

#include "core/ngx_brix_module.h"
#include "fs/vfs/vfs.h"   /* brix_vfs_ctx_t + VFS xattr seam */
 ...
 *
 * Key prefixes:
 *   BRIX_FATTR_XKEY_PFX     "user.U." — filesystem xattr key prefix
 *   BRIX_FATTR_XKEY_PFX_LEN 7         — byte length stripped to recover the
 *                                         bare client-visible name
 *   BRIX_FATTR_MAX_VBUF     65536     — maximum xattr value buffer
 */
#define BRIX_FATTR_XKEY_PFX   "user.U."
#define BRIX_FATTR_XKEY_PFX_LEN  7
#define BRIX_FATTR_MAX_VBUF   65536
...
void fattr_set_rc(brix_fattr_entry_t *attr, uint16_t rc);
 ...
 */
ngx_int_t fattr_send_vector_status(brix_ctx_t *ctx, ngx_connection_t *c,
...
ngx_int_t fattr_get(brix_ctx_t *ctx, ngx_connection_t *c,
 ...
 */
ngx_int_t fattr_set(brix_ctx_t *ctx, ngx_connection_t *c,
    brix_vfs_ctx_t *vctx, const char *path, int fd, int options,
    u_char *nvec_copy, size_t nvec_len, u_char *vvec_buf, size_t vvec_len,
    int numattr, brix_fattr_entry_t *attrs);

/* Remove xattr entries via the VFS xattr seam. */
ngx_int_t fattr_del(brix_ctx_t *ctx, ngx_connection_t *c,
    brix_vfs_ctx_t *vctx, const char *path, int fd,
    u_char *nvec_copy, size_t nvec_len,
    int numattr, brix_fattr_entry_t *attrs);
