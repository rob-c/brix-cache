 ...
 *       xattr names come from brix_vfs_listxattr into a stack buffer that is
...
fattr_recurse_dir(fattr_recurse_ctx_t *rctx, const char *dir_path, int depth)
{
    ...
    brix_vfs_ctx_init(&dvctx, rctx->pool, rctx->log, BRIX_PROTO_ROOT,
    ...
    while ((rc = brix_vfs_readdir(dir, &name, &vst)) == NGX_OK) {
         ...
         * none/error. A transient per-file ctx re-verifies confinement. */
        {
            ...
            brix_vfs_ctx_init(&fvctx, rctx->pool, rctx->log,
                BRIX_PROTO_ROOT, rctx->root_canon, NULL, 0, 0, NULL, fpath);
...
ngx_int_t fattr_list(brix_ctx_t *ctx, ngx_connection_t *c,
    ...
    if (list_sz < 0) {
        ...
        return brix_send_error(ctx, c, kXR_FSError, "listxattr failed");
    }
    if (list_sz == 0) {
        BRIX_OP_OK(ctx, BRIX_OP_FATTR);
        return brix_send_ok(ctx, c, NULL, 0);
    ...
    ssize_t actual = path ? brix_vfs_listxattr(vctx, raw, list_sz + 4096)
        ...
        if (strncmp(lp, BRIX_FATTR_XKEY_PFX, BRIX_FATTR_XKEY_PFX_LEN) == 0
            ...
            *wp++ = '\0';
            if (aData) {
                ...
                char    val[4096];
                ssize_t vlen = path
                    ? brix_vfs_getxattr(vctx, lp, val, sizeof(val))
                    : brix_vfs_fgetxattr(vctx, fd, lp, val, sizeof(val));
