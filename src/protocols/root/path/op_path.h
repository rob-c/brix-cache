#ifndef BRIX_PATH_OP_PATH_H
#define BRIX_PATH_OP_PATH_H

/*
 * Path resolution strategy for namespace operations (Phase 8: existence is
 * verified with brix_stat_beneath, not realpath; confinement is the kernel's
 ...
 * the op_table.h / ngx_brix_module.h include cycle (op_table embeds a mode).
 */
typedef enum {
    BRIX_PATH_EXISTING,
    BRIX_PATH_WRITE,
...
/*
 * brix_resolve_op_path — extract, depth-check, and resolve a path from the
 ...
 *   mode        — path resolution strategy (see brix_path_mode_t above)
 *   reqpath     — caller buffer, BRIX_MAX_PATH+1 bytes; filled on NGX_OK
 ...
 *   }
 */
ngx_int_t brix_resolve_op_path(brix_ctx_t *ctx, ngx_connection_t *c,
...
/*
 * brix_op_path_forbidden_component — returns 1 if reqpath contains a "." or
 ...
 * brix_resolve_op_path.  RESOLVE_BENEATH still blocks an escaping "..", but
 ...
 * NOT pass through brix_path_resolve_beneath's "."/".." rejection. If `reqpath`
 * contains a ".." component, this logs the traversal warning + access line and
 * sends a kXR_ArgInvalid error (exactly as brix_resolve_op_path does for the
 ...
 * read/metadata ops.
 */
int brix_reject_dotdot_path(brix_ctx_t *ctx, ngx_connection_t *c,
                              ngx_uint_t op_id, const char *op_name,
                              const char *reqpath);

/*
 * brix_path_resolve_beneath — realpath-free path validation + per-mode
 * existence gate, filling `resolved` with the confined root_canon+reqpath join.
 * The shared core of brix_resolve_op_path(); also called directly by kXR_mv
 ...
 */
ngx_int_t brix_path_resolve_beneath(ngx_stream_brix_srv_conf_t *conf,
                                      ngx_log_t *log,
                                      const char *reqpath,
                                      brix_path_mode_t mode,
                                      char *resolved, size_t resolved_sz);
