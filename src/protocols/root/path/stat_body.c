...
#include "protocols/root/protocol/stat_line.h"    /* shared stat-line grammar (encode side) */
#include "protocols/root/protocol/stat_flags.h"   /* shared stat `flags` semantics (encode side) */
...
void
brix_make_stat_body(const struct stat *st, ngx_flag_t is_vfs,
                      int extra_flags, char *out, size_t outsz)
{
    ...
    if (is_vfs) {
        brix_statline_format(out, outsz, 0ULL,
    ...
    flags = brix_stat_flags_from_stat(st, geteuid(), getegid(), extra_flags);
    ...
    brix_statline_format(out, outsz,
