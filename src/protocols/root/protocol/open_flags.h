 ...
 *       decoder:
 *         - brix_open_options_build:    intent  -> kXR_open options (the request)
 *         - brix_open_options_to_posix: options -> POSIX open(2) flags (the server)
 *         - brix_open_options_is_write: options -> is this a write-mode open?
 *         - brix_open_force_for_open/create: POSIX flags -> the `force` tri-state
 ...
 */
#ifndef BRIX_PROTOCOL_OPEN_FLAGS_H
#define BRIX_PROTOCOL_OPEN_FLAGS_H
...
#define BRIX_OPEN_WRITE_BITS \
    (kXR_new | kXR_delete | kXR_open_updt | kXR_open_wrto | kXR_open_apnd)

/* Is this kXR_open options word a write-mode open? */
static inline int
brix_open_options_is_write(uint16_t options)
{
    return (options & BRIX_OPEN_WRITE_BITS) ? 1 : 0;
}
...
brix_open_options_build(int write, int force, int posc, int mkpath)
{
    uint16_t options;
 ...
 * `options` word — the exact inverse of brix_open_options_build and the single
 ...
 * flag (see brix_open_options_is_write). Either out-pointer may be NULL.
 */
static inline void
brix_open_options_to_posix(uint16_t options, int is_write,
 ...
 */
static inline int
brix_open_force_for_open(int o_flags)
{
    return (o_flags & O_TRUNC) ? 1 : 2;
}

static inline int
brix_open_force_for_create(int o_flags)
{
    return (o_flags & O_EXCL) ? 0 : 1;
}

#endif /* BRIX_PROTOCOL_OPEN_FLAGS_H */
