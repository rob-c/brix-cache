 ...
 *         [ fhandle[4] ][ rlen[4 BE] ][ offset[8 BE] ]   (XROOTD_READV_SEGSIZE)
 *       This header owns that layout for everyone who assembles or reads it:
 *         - xrootd_readv_seg_pack: fields -> 16-byte header
 *         - xrootd_readv_seg_rlen: 16-byte header -> rlen (the field a reader needs)
 ...
 */
#ifndef XROOTD_PROTOCOL_READV_SEG_H
#define XROOTD_PROTOCOL_READV_SEG_H

#include <stdint.h>
#include <string.h>

#include "frame_hdr.h"   /* xrd_put_u32_be / xrd_put_u64_be / xrd_get_u32_be */
#include "flags.h"       /* XROOTD_READV_SEGSIZE */

/*
 * Pack one readv segment header into `out` (XROOTD_READV_SEGSIZE bytes): the
 * 4-byte file handle verbatim, then rlen and offset in big-endian. `offset` and
 * `rlen` are host-order; the encode is the exact inverse of xrootd_readv_seg_rlen
 * (and of the typed readahead_list the module parses on the request side).
 */
static inline void
xrootd_readv_seg_pack(uint8_t out[XROOTD_READV_SEGSIZE], const uint8_t fhandle[4],
...
xrootd_readv_seg_rlen(const uint8_t in[XROOTD_READV_SEGSIZE])
{
    return xrd_get_u32_be(in + 4);
}

#endif /* XROOTD_PROTOCOL_READV_SEG_H */
