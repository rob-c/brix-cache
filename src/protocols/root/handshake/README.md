...
| `policy.c` | The access gates used by every sub-dispatcher: `brix_dispatch_require_login`, `brix_dispatch_require_auth`, `brix_dispatch_require_write` (auth + not-bound + `allow_write`). Also `brix_check_token_scope` — applies WLCG token read/write scope checks to a *logical* path (only when `ctx->token_auth`). |
