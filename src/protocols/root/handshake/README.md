...
*ClientInitHandShake* (handled by `brix_process_handshake`), and thereafter,
for every request header + payload, through the central router
`brix_dispatch`. Nothing in this directory touches the filesystem or builds an
...
`brix_dispatch` is a layered fall-through router. It runs a fixed sequence of
...
`BRIX_DISPATCH_CONTINUE` (= `NGX_DECLINED`) so routing continues to the next
...
| File | Responsibility |
|---|---|
| `handshake.h` | Internal prototypes shared across the dispatcher files: the four `brix_dispatch_*_opcode` sub-dispatchers, the three `brix_dispatch_require_*` gates, and the two signing helpers. Defines `BRIX_DISPATCH_CONTINUE` (= `NGX_DECLINED`), the "not my opcode / keep routing" sentinel. |
| `client_hello.c` | `brix_process_handshake` — validates the initial 20-byte *ClientInitHandShake* (only the two magic fields `fourth==4` and `fifth==ROOTD_PQ` are checked), then replies with an 8-byte v5 body (`kXR_PROTOCOLVERSION` + `kXR_DataServer`) framed in a `ServerResponseHdr` with `streamid={0,0}`. Runs once per connection before any opcode. |
| `dispatch.c` | `brix_dispatch` — the central router. Sets `ctx->req_start`, runs sigver verify → security-level enforce → session dispatcher → (proxy short-circuit) → rate-limit gate → read dispatcher → write dispatcher → signing dispatcher, with mirror/wmirror hooks fired after read/write. Unclaimed opcode → `kXR_Unsupported`. |
| `dispatch_session.c` | `brix_dispatch_session_opcode` — routes the pre-auth / session-lifecycle opcodes: `kXR_protocol`, `kXR_login`, `kXR_auth`, `kXR_ping`, `kXR_set` (login-gated), `kXR_endsess`, `kXR_bind`. Handlers live in [`../session/`](../session/README.md). |
| `dispatch_read.c` | `brix_dispatch_read_opcode` — routes non-mutating / metadata opcodes (`kXR_stat`, `kXR_open` read-open, `kXR_read`, `kXR_close`, `kXR_dirlist`, `kXR_readv`, `kXR_query`, `kXR_prepare`, `kXR_pgread`, `kXR_locate`, `kXR_statx`, `kXR_fattr`, `kXR_clone`). Each case runs `require_auth` (via the `DISPATCH_RD`/`DISPATCH_RD_BOUND` macros) first; bound (secondary) streams are restricted to plain reads. |
| `dispatch_write.c` | `brix_dispatch_write_opcode` — routes mutating / namespace opcodes (`kXR_write`, `kXR_pgwrite`, `kXR_sync`, `kXR_truncate`, `kXR_mkdir`, `kXR_rm`, `kXR_writev`, `kXR_rmdir`, `kXR_mv`, `kXR_chmod`, `kXR_chkpoint`). Each case runs `require_write` (the `DISPATCH_WR` macro) — a stricter gate than `require_auth`. |
| `dispatch_signing.c` | `brix_dispatch_signing_opcode` — the final routing phase; claims only `kXR_sigver`, login-gates it, and calls `brix_handle_sigver` (in [`../session/`](../session/README.md)) which records the pending envelope for the *next* request. |
| `policy.c` | The access gates used by every sub-dispatcher: `brix_dispatch_require_login`, `brix_dispatch_require_auth`, `brix_dispatch_require_write` (auth + not-bound + `allow_write`, with one F16 exemption: the arm/fire `kXR_sync` of a native TPC **push** handle passes on a read-only export — a push source writes nothing locally — see `brix_write_gate_tpc_push_sync`). Also `brix_check_token_scope` — applies WLCG token read/write scope checks to a *logical* path (only when `ctx->token_auth`). |
| `sigver.c` | Request-signing verification: `brix_verify_pending_sigver` (checks the HMAC-SHA256 envelope queued by the previous `kXR_sigver` against the current header+payload) and `brix_signing_enforce_level` (rejects unsigned opcodes that the configured `security_level` 0–4 requires to be signed). |
...
`BRIX_DISPATCH_CONTINUE` (a `#define` for `NGX_DECLINED`) is the routing
...
return value of `brix_send_error` or `NGX_ERROR`).

## Control & data flow

**Entry.** [`../connection/recv.c`](../connection/README.md) is the only caller.
On the first frame of a connection it calls `brix_process_handshake`; on every
subsequent fully-framed request it calls `brix_dispatch(ctx, c, conf)`.

**`brix_dispatch` phase order (`dispatch.c`):**

```
connection/recv.c: ngx_stream_brix_recv()
  └─ handshake/dispatch.c: brix_dispatch()
       ...
       9. fall-through → brix_send_error(kXR_Unsupported)
```

Each numbered step returns `BRIX_DISPATCH_CONTINUE` to advance; any other
...
- session opcodes → [`../session/`](../session/README.md) (login, auth, bind,
  ping, set, endsess, and `brix_handle_sigver`).
...
- **Bound (secondary) streams are read-only.** `kXR_bind` channels exist for
  ...
  rejected with `kXR_NotAuthorized` (`dispatch_read.c:brix_reject_bound_nonread_file_op`,
...
- **`security_level` 0–4 escalation** (`brix_sigver_opcode_requires`,
...
- **Token scope uses the *logical* path.** `brix_check_token_scope` must be
...
2. Add the opcode constant to `../protocol/opcodes.h` and implement the
   `brix_handle_<op>` handler in the relevant sibling subsystem (declare its
...
3. If the opcode should require a signature at some `security_level`, add it to
   the appropriate tier in `brix_sigver_opcode_requires` (`sigver.c`).
...
requests): add a phase in `brix_dispatch` (`dispatch.c`) returning
`BRIX_DISPATCH_CONTINUE` to proceed; keep the signing dispatcher last.
