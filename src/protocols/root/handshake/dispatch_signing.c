 ...
 *
 * HOW: Single opcode check (ctx->recv.cur_reqid == kXR_sigver) → login gate via brix_dispatch_require_login() → call brix_handle_sigver() to verify HMAC-SHA256 envelope; returns BRIX_DISPATCH_CONTINUE for all other opcodes.
 */
ngx_int_t
brix_dispatch_signing_opcode(brix_ctx_t *ctx, ngx_connection_t *c)
{
    ...
    if (ctx->recv.cur_reqid != kXR_sigver) {
