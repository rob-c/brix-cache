/*
 * directives_cache.h — read-through cache directives (cache mode, cache_root/store, admission, transfer-heap budget)
...
#pragma once
    ...
    /* Age-based purge of CLEAN read-through fills (upstream pfc.purgecoldfiles):
     * independent of occupancy, so a cache that never reaches its high watermark
     * still releases objects nobody reads. 0 (default) = off. */
    { ngx_string("brix_cache_cold_max_age"),
      NGX_STREAM_SRV_CONF | NGX_CONF_TAKE1,
      ngx_conf_set_sec_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_brix_srv_conf_t, cache_cold_max_age),
      NULL },
    ...
    { ngx_string("brix_cache_high_watermark"),
      ...
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_brix_srv_conf_t, reaper.high_watermark),
      NULL },
    ...
    { ngx_string("brix_cache_low_watermark"),
      ...
      brix_conf_set_cache_watermark,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_brix_srv_conf_t, reaper.low_watermark),
      NULL },
    ...
    /* Checksum-on-fill policy for the read-through cache: off (trust the
     * transfer), best-effort (verify when the origin advertises a digest, fail
     * closed on mismatch), require (a usable origin digest is mandatory — an
     * unverifiable fill does not publish). Writes common.cache_verify_mode, which
     * runtime_server_backend maps into the tier's brix_cache_policy_t.verify; the
     * root-plane fill spine carries the xroot kXR_Qcksum origin-digest hook that
     * gives best-effort/require teeth. */
    { ngx_string("brix_cache_verify"),
      NGX_STREAM_SRV_CONF | NGX_CONF_TAKE1,
      ngx_conf_set_enum_slot,
      NGX_STREAM_SRV_CONF_OFFSET,
      offsetof(ngx_stream_brix_srv_conf_t, common.cache_verify_mode),
      &brix_cache_verify_modes },
