#ifndef BRIX_SESSION_REGISTRY_H
#define BRIX_SESSION_REGISTRY_H
 ...
 *
 * Capacity: BRIX_SESSION_REGISTRY_SLOTS entries (default 1024).  When the
...
#define BRIX_SESSION_REGISTRY_SLOTS  1024
 ...
 * registry_slots (1024) x BRIX_MAX_FILES (16) = 16384 slots cost ~68 MB of
 ...
 * independently of the per-connection open-file limit (BRIX_MAX_FILES, which
 ...
 * secondaries fall back to single-stream I/O) — see brix_session_handle_*.
 *
 ...
 */
#define BRIX_SESSION_HANDLE_SESSIONS      512
#define BRIX_SESSION_HANDLES_PER_SESSION  8
#define BRIX_SESSION_HANDLE_SLOTS \
    (BRIX_SESSION_HANDLE_SESSIONS * BRIX_SESSION_HANDLES_PER_SESSION)
...
#define BRIX_SESSION_REAP_MIN_AGE_MS  60000
...
typedef struct {
    u_char     sessid[BRIX_SESSION_ID_LEN]; /* primary session identifier — key component */
    ...
    char       path[BRIX_MAX_PATH + 1];     /* canonical confined path for reopen by secondary */
} brix_shared_handle_entry_t;
