 ...
 *
 * HOW: Call xrootd_fs_usage_stat(conf->common.root.data, &fsu) → on failure return
 *      kXR_IOError with statvfs error message → on success format response per
 *      opcode variant and send via xrootd_send_ok(). Both log access events at
 *      INFO level. Always returns NGX_OK from the handler (kXR status encoded in
 *      wire response).
 */

/*
 * xrootd_query_space — handle kXR_Qspace (3015) filesystem capacity query.
 *
 ...
 *
 * HOW: Call xrootd_fs_usage_stat(root, &fsu) → if NGX_OK fails, log access event
 ...
 *      increment OK metric, send via xrootd_send_ok().
 *
 * Parameters:
 ...
 * HOW: Call xrootd_fs_usage_stat(root, &fsu) → if fails increment error metric and
