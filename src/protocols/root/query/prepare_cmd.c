 ...
 *
 * HOW:  brix_plat_close_from(3): the PAL closes every descriptor >= 3 by
 *       the host's exact means (close_range(2), a /proc/self/fd scan, or a
 *       brute-force loop up to the sysconf limit) — see src/platform/.
 */
static void
brix_prepare_close_inherited_fds(void)
{
    brix_plat_close_from(3);
}
