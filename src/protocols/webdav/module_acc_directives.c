 ...
 *
 * Moved out of module.c: these setters back the brix_acc_* HTTP directives,
 ...
 */
#include "webdav.h"
#include "auth/authz/acc/acc.h"   /* XrdAcc enum tables + brix_acc_http_t */
 ...
 */
/*
 * brix_ktls on|off — registered once (WebDAV) but sets common.ktls on BOTH the
 * WebDAV and S3 loc-confs, so an S3-only location honours the flag too. Default
 * ON is applied by the shared merge; this is the explicit per-location override.
 */
char *
brix_http_set_ktls(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
...
static char *
brix_acc_http_set_enum(ngx_conf_t *cf, ngx_conf_enum_t *e, ngx_uint_t *wp,
    ngx_uint_t *sp)
{
    ngx_str_t *value = cf->args->elts;
    ngx_uint_t i;
...
char *
brix_acc_http_set_format(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_http_brix_webdav_loc_conf_t *wc =
        ngx_http_conf_get_module_loc_conf(cf, ngx_http_brix_webdav_module);
    ngx_http_s3_loc_conf_t            *sc =
        ngx_http_conf_get_module_loc_conf(cf, ngx_http_brix_s3_module);
    return brix_acc_http_set_enum(cf, brix_acc_format_modes,
                                    &wc->acc.format, &sc->acc.format);
}

char *
brix_acc_http_set_audit(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ngx_http_brix_webdav_loc_conf_t *wc =
        ngx_http_conf_get_module_loc_conf(cf, ngx_http_brix_webdav_module);
    ngx_http_s3_loc_conf_t            *sc =
        ngx_http_conf_get_module_loc_conf(cf, ngx_http_brix_s3_module);
    return brix_acc_http_set_enum(cf, brix_acc_audit_modes,
 ...
 */
static char *
brix_acc_http_both(ngx_conf_t *cf, brix_acc_http_t **wc, brix_acc_http_t **sc)
{
...
char *
brix_acc_http_set_nisdomain(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    brix_acc_http_t *wc, *sc;
    ngx_str_t         *value = cf->args->elts;
...
static char *
brix_acc_http_set_flag(ngx_conf_t *cf, ngx_flag_t *wp, ngx_flag_t *sp,
    ngx_str_t *val)
{
...
char *
brix_acc_http_set_spacechar(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    brix_acc_http_t *wc, *sc;
    ngx_str_t         *value = cf->args->elts;
...
char *
brix_acc_http_set_gidretran(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    brix_acc_http_t *wc, *sc;
    ngx_str_t         *value = cf->args->elts;
