 ...
 * stat(2) on the result — combines ngx_http_brix_webdav_resolve_path() and
 * stat(2) into a single helper for handlers needing both path and metadata.
 *
 * Calls the primary path resolver (ngx_http_brix_webdav_resolve_path()) to convert
 ...
 * ngx_http_brix_webdav_resolve_path() first — if resolution fails (invalid URI, outside
    ...
    struct stat *sb)
{
    ...
    /* Hand-rolled ctx (not brix_vfs_ctx_init): resolve the storage backend so
