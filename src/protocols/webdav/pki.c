 ...
 *
 * HOW: Select CA source from conf->cafile (single file path) or conf->cadir (directory containing multiple certificates). Extract CRL path from conf->crl if configured, otherwise pass NULL for no CRL requirement. Delegate validation to brix_pki_check_paths() helper from ../crypto/pki_check.h — this is the centralized PKI check utility that validates file existence, format correctness, and OpenSSL store loadability. Returns NGX_OK on valid configuration or NGX_ERROR with log error message when paths are missing/invalid.
 */
