 ...
 *         2. headers      — brix_http_set_file_headers() sets status,
 *            content-length, Content-Range, ETag/Last-Modified.
 *         3. dashboard    — brix_dashboard_http_start_identity() opens a READ
 *            transfer record for live monitoring.
 *         4. send         — the fd is dup()'d so brix_http_send_file_range()
 ...
 *            cache-backed files, brix_cache_record_access() updates LRU/hit
    ...
    brix_http_serve_result_t *result)
{
    ...
    backend_name = brix_vfs_file_backend_name(fh);
     ...
     * (brix_tcp_congestion) is owned by the always-present webdav http module;
    ...
    if (opts->compress) {
        ...
        enc = brix_http_compress_negotiate(r, vst->size, rng.present);
        if (enc != BRIX_CODEC_IDENTITY) {
            ...
            rc = brix_http_send_file_compressed(r, csend, fs_path, vst->size,
                                                  enc, vst->mtime,
            ...
            result->range_result = BRIX_SERVE_RANGE_FULL;
            result->bytes_sent   = bytes_out;          /* compressed bytes on wire */
            brix_metric_backend_bytes(backend_name, BRIX_METRIC_OP_READ,
                                        (size_t) bytes_out);
            brix_dashboard_http_add(r, (ngx_atomic_int_t) bytes_out);
            if (from_cache && vst->size > 0) {
                (void) brix_cache_record_access(cache_path, (size_t) vst->size,
                                                  r->connection->log);
            }
            return rc;
        }
    ...
    if (brix_http_set_file_headers(r, vst->mtime, vst->size, send_len,
    ...
    if (fd == NGX_INVALID_FILE) {
        rc = brix_serve_memory_backed(r, fh, range_start, send_len);
        brix_vfs_close(fh, r->connection->log);
        if (rc == NGX_ERROR) {
        ...
        if (!r->header_only) {
            ...
            brix_metric_backend_bytes(backend_name, BRIX_METRIC_OP_READ,
                                        (size_t) send_len);
            brix_dashboard_http_add(r, (ngx_atomic_int_t) send_len);
            if (from_cache && send_len > 0) {
                (void) brix_cache_record_access(cache_path, (size_t) send_len,
                                                  r->connection->log);
            }
        }
        brix_dashboard_http_finish(r);
    ...
    rc = brix_http_send_file_range(r, send_fd, fs_path,
