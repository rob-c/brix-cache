#ifndef NGX_BRIX_SSI_H
#define NGX_BRIX_SSI_H
 ...
 * HOW:  brix_handle_open binds a virtual handle whose ->ssi points at an
 *       brix_ssi_req_t. write accumulates the request and, once complete, runs
...
int brix_ssi_match(ngx_stream_brix_srv_conf_t *conf, const char *path,
...
ngx_int_t brix_ssi_open(brix_ctx_t *ctx, ngx_connection_t *c,
                          const char *service, size_t service_len,
                          uint16_t options);
...
ngx_int_t brix_ssi_write(brix_ctx_t *ctx, ngx_connection_t *c, int idx,
...
ngx_int_t brix_ssi_query(brix_ctx_t *ctx, ngx_connection_t *c, int idx,
                           const unsigned char *body, size_t body_len);

/* kXR_read on an SSI handle: serve streamed response bytes from the cursor. */
ngx_int_t brix_ssi_read(brix_ctx_t *ctx, ngx_connection_t *c, int idx,
                          uint64_t offset, uint32_t rlen);

/* Teardown hook for an SSI handle (an brix_ssi_session_t*). Cancels any armed
 * async-deferral timers and unregisters the session. Called from
 * brix_free_fhandle before the .ssi slot is cleared (on close + disconnect).
 * NULL-safe; a no-op for non-SSI handles. */
void brix_ssi_handle_cleanup(void *ssi_session);

#endif /* NGX_BRIX_SSI_H */
