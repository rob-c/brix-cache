 ...
 * brix_ssi_service_lookup (the reference "echo"). New native services (e.g.
...
int
brix_ssi_provider_lookup(const char *name, brix_ssi_provider_t *out)
{
    ...
    if (strcmp(name, "cta") == 0) {
        out->name    = "cta";
        out->process = brix_ssi_cta_process;
        return 1;
    }
    fn = brix_ssi_service_lookup(name);
    if (fn == NULL) {
        return 0;
    }
    ...
    out->name    = brix_ssi_service_canon_name(name);
    out->process = fn;
    return 1;
}
