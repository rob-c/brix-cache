...
static void
ssi_dispatch(brix_ssi_req_t *rq, brix_ssi_process_fn process)
{
                ...
                const char *service, size_t service_len, uint16_t options)
{
     ...
     * `brix_ssi_service cta;`. When enabled, push the config down to the
    ...
    if (rq->req_expected > 0 && rq->req_len >= rq->req_expected) {
        ...
        if (rq->deferred && !rq->responded) {
            ...
            if (ssi_defer_arm(ctx, c, sess, rq) != NGX_OK) {
                return brix_send_error(ctx, c, kXR_NoMemory, "ssi defer");
            }
            return brix_send_waitresp(ctx, c);
        }
    }
