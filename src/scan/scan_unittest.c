...
test_json_escape(void)
{
    ...
    CHECK(xrootd_scan_json_escape("a/b", 3, out, sizeof(out)) == 3, "plain len");
    CHECK(STREQ(out, "a/b"), "plain passthrough");

    CHECK(xrootd_scan_json_escape("a\"b", 3, out, sizeof(out)) == 4, "quote len");
    CHECK(STREQ(out, "a\\\"b"), "quote escaped");

    CHECK(xrootd_scan_json_escape("a\\b", 3, out, sizeof(out)) == 4, "bslash len");
    CHECK(STREQ(out, "a\\\\b"), "backslash escaped");

    CHECK(xrootd_scan_json_escape("a\nb", 3, out, sizeof(out)) == 4, "nl len");
    CHECK(STREQ(out, "a\\nb"), "newline short form");

    {
        char in[3] = { 'a', 0x01, 'b' };
        CHECK(xrootd_scan_json_escape(in, 3, out, sizeof(out)) == 8, "ctrl len");
        CHECK(STREQ(out, "a\\u0001b"), "control \\u escaped");
    }

    /* overflow → -1, never truncate */
    CHECK(xrootd_scan_json_escape("abcdef", 6, out, 4) == -1, "overflow rejects");
}
...
test_record_file(void)
{
    ...
    CHECK(xrootd_scan_record_file(buf, sizeof(buf), "/x", 12, 34, "adler32",
    ...
    /* path gets escaped */
    CHECK(xrootd_scan_record_file(buf, sizeof(buf), "/a\"b", 0, 0, "crc32c",
...
{
    char buf[256];
    xrootd_scan_summary_t s;

    CHECK(xrootd_scan_record_cursor(buf, sizeof(buf), "/a/b") > 0, "cursor rc");
    ...
    s.elapsed_s = 812.4;
    CHECK(xrootd_scan_record_summary(buf, sizeof(buf), &s) > 0, "summary rc");
    CHECK(STREQ(buf,
...
test_record_inspect_health(void)
{
    ...
    CHECK(xrootd_scan_record_inspect(buf, sizeof(buf), "/x", "posix", 12, 34,
    ...
    CHECK(xrootd_scan_record_inspect(buf, sizeof(buf), "/y", "posix", 0, 0,
                                     "none", 0) > 0, "inspect rc2");
    CHECK(strstr(buf, "\"namespace_consistent\":false") != NULL,
          "inspect ns false");

    CHECK(xrootd_scan_record_health(buf, sizeof(buf), "posix",
...
test_token_bucket(void)
{
    ...
    xrootd_scan_tb_init(&tb, 1.0e6, 1.0e6, 0);
    CHECK(xrootd_scan_tb_wait_ns(&tb, 5.0e5, 0) == 0, "full bucket: no wait");

    xrootd_scan_tb_consume(&tb, 1.0e6);   /* drain */
    /* need 1 MB, empty, 1 MB/s -> 1.0 s */
    CHECK(CLOSE(xrootd_scan_tb_wait_ns(&tb, 1.0e6, 0), 1.0e9, 1.0e6),
          "empty bucket: wait 1 s for 1 MB");

    /* after 0.5 s, 0.5 MB accrued -> need 1 MB still 0.5 s away */
    CHECK(CLOSE(xrootd_scan_tb_wait_ns(&tb, 1.0e6, 500000000ull), 5.0e8, 1.0e6),
          "half-refilled bucket: 0.5 s wait");

    /* unlimited rate -> never wait */
    xrootd_scan_tb_init(&tb, 0.0, 0.0, 0);
    CHECK(xrootd_scan_tb_wait_ns(&tb, 1.0e9, 12345) == 0, "rate 0 = unlimited");
}

static void
test_budget(void)
{
    xrootd_scan_budget_t b;

    b.max_bytes = 1000; b.max_seconds = 0;
    CHECK(!xrootd_scan_budget_hit(&b, 999, 1e9), "under byte budget");
    CHECK(xrootd_scan_budget_hit(&b, 1000, 0), "at byte budget");
    CHECK(xrootd_scan_budget_hit(&b, 1001, 0), "over byte budget");

    b.max_bytes = 0; b.max_seconds = 10.0;
    CHECK(!xrootd_scan_budget_hit(&b, 1ull << 40, 9.9), "under time budget");
    CHECK(xrootd_scan_budget_hit(&b, 0, 10.0), "at time budget");

    b.max_bytes = 0; b.max_seconds = 0;
    CHECK(!xrootd_scan_budget_hit(&b, 1ull << 40, 1e9), "no budget = never hit");
}
...
test_adapt(void)
{
    ...
    CHECK(CLOSE(xrootd_scan_adapt(0, 10.0, 10.0), 1.0, 1e-9), "idle/nominal = 1.0");
    /* 4x latency shrinks */
    CHECK(xrootd_scan_adapt(0, 40.0, 10.0) < 1.0, "high latency shrinks");
    CHECK(CLOSE(xrootd_scan_adapt(0, 40.0, 10.0), 0.25, 1e-9), "4x latency = 0.25");
    /* foreground pressure shrinks, monotonic */
    CHECK(xrootd_scan_adapt(10, 10.0, 10.0) < xrootd_scan_adapt(2, 10.0, 10.0),
          "more pressure = smaller multiplier");
    CHECK(xrootd_scan_adapt(2, 10.0, 10.0) < xrootd_scan_adapt(0, 10.0, 10.0),
          "any pressure < idle");
    /* floor clamp */
    CHECK(CLOSE(xrootd_scan_adapt(100000, 1e9, 10.0), 0.1, 1e-9), "clamped to 0.1");
    CHECK(xrootd_scan_adapt(100000, 1e9, 10.0) >= 0.1, "never below floor");
}
...
{
    xrootd_scan_emitq_t q;
    void          *slots[4];
    unsigned char  used[4];
    int            r;

    g_emit_n = 0;
    xrootd_scan_emitq_init(&q, slots, used, 4);

    /* out-of-order arrivals: 2 then 0 then 1 -> emitted strictly 0,1,2 */
    r = xrootd_scan_emitq_submit(&q, 2, PAY(2), emit_collect, NULL);
    CHECK(r == 0, "seq 2 buffered, nothing emitted yet");
    r = xrootd_scan_emitq_submit(&q, 0, PAY(0), emit_collect, NULL);
    CHECK(r == 1, "seq 0 releases just 0");
    r = xrootd_scan_emitq_submit(&q, 1, PAY(1), emit_collect, NULL);
...
test_emitq_errors(void)
{
    ...
    g_emit_n = 0;
    xrootd_scan_emitq_init(&q, slots, used, 2);

    /* beyond the window (next=0, window=2 -> max seq 1) */
    CHECK(xrootd_scan_emitq_submit(&q, 2, PAY(2), emit_collect, NULL) == -1,
          "seq beyond window rejected");

    /* emit 0, then a late duplicate 0 is rejected */
    CHECK(xrootd_scan_emitq_submit(&q, 0, PAY(0), emit_collect, NULL) == 1,
          "seq 0 emits");
    CHECK(xrootd_scan_emitq_submit(&q, 0, PAY(0), emit_collect, NULL) == -1,
...
test_drift(void)
{
    xrootd_scan_driftset_t *s = xrootd_scan_driftset_create(4);
    int64_t cs = -1;

    CHECK(s != NULL, "driftset created");
    CHECK(xrootd_scan_driftset_add(s, "a", 10) == 0, "add a");
    CHECK(xrootd_scan_driftset_add(s, "b", 20) == 0, "add b");
    CHECK(xrootd_scan_driftset_add(s, "orphan", 5) == 0, "add orphan");

    /* namespace stream vs catalog */
    CHECK(xrootd_scan_driftset_match(s, "a", 10, &cs) == XROOTD_DRIFT_IN_BOTH,
          "a in both"); CHECK(cs == 10, "a cat size");
    CHECK(xrootd_scan_driftset_match(s, "b", 99, &cs) == XROOTD_DRIFT_SIZE_MISMATCH,
          "b size mismatch"); CHECK(cs == 20, "b cat size");
    CHECK(xrootd_scan_driftset_match(s, "c", 1, NULL) == XROOTD_DRIFT_NAMESPACE_ONLY,
          "c namespace-only");

    /* growth past initial capacity must preserve entries */
    CHECK(xrootd_scan_driftset_add(s, "d", 40) == 0, "add d (grow)");
    CHECK(xrootd_scan_driftset_add(s, "e", 50) == 0, "add e (grow)");
    CHECK(xrootd_scan_driftset_match(s, "d", 40, NULL) == XROOTD_DRIFT_IN_BOTH,
          "d survives grow");

    /* unmatched catalog keys are orphans: "orphan" and "e" */
    g_orphan_n = 0;
    xrootd_scan_driftset_orphans(s, orphan_collect, NULL);
    ...
    xrootd_scan_driftset_add(s, "a", 11);
    CHECK(xrootd_scan_driftset_match(s, "a", 11, &cs) == XROOTD_DRIFT_IN_BOTH
          && cs == 11, "duplicate add updates size");

    xrootd_scan_driftset_free(s);
}
