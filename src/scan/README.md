...
because every filesystem touch goes through the VFS seam (`xrootd_vfs_*`) — no
...
| `scan_engine.{c,h}` | VFS-driven walk (`xrootd_vfs_walk`) + per-file action dispatch (`dump`/`verify`/`fill`/`compare` via `xrootd_integrity_get_fd`); appends NDJSON via `scan_record` | `tests/test_scan.py` (HTTP) |
...
`xrootd_scan_root`. `dump`/`verify`/`fill`/`compare`/`inspect` emit one record per
object + a final `summary`; `health` emits a single capacity record. Config:

```nginx
location /xrootd {
    xrootd_dashboard on; xrootd_dashboard_password "...";
    xrootd_scan_root      /export;   # empty (default) ⇒ endpoint 404 (disabled)
    xrootd_scan_max_files 100000;    # operator cap per request
}
```
...
- **Phase 2b (done):** `scan_engine.c` + `scan_http.c` + the `xrootd_scan_root`/
  ...
  **synchronously** in the request (bounded by `xrootd_scan_max_files`, opt-in,
...
- **Phase 3 (done):** `inspect` (A2 — per-file backend/stat/checksum-source +
  ...
  **POSIX view** of `xrootd_scan_root`: the engine walks the export via `openat2`,
...
- **Phase 4b (next, needs a backend + driver binding):** the SD-driver
  ...
  for pblock) + a driver-agnostic `xrootd_vfs_enumerate_catalog()` wrapper
