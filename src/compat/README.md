...
| `integrity_info.c` / `integrity_info.h` | Checksum **cache + policy** layer over `checksum.c`: reads/writes a `user.XrdCks.<alg>` xattr keyed by mtime+size (stale on change) — the fd-keyed cache routes through the VFS xattr seam (`xrootd_vfs_f{get,set,remove}xattr`); the `.cks` sidecar fallback is a separate below-seam store. Formats RFC-3230 HTTP `Digest` values, invalidates by fd (`xrootd_integrity_invalidate_fd`) or path (`xrootd_integrity_invalidate_path(log, root_canon, path)`) after writes; a `no_compute` (cache-only) opt declines on a miss for latency-sensitive read paths (S3 GET/HEAD echo). Shared by kXR_Qcksum, XrdHttp Want-Digest, dirlist dcksm, S3 ETag + crc64nvme. |
...
| `staged_file.c` / `staged_file.h` | Crash-safe atomic write lifecycle: `open` a unique `O_EXCL` temp under the confined root → write → `commit` or `abort` (close+unlink). `xrootd_commit_staged` is **backend-aware**: a POSIX final takes `rename(2)` (same-FS) or copy-then-rename (cross-device EXDEV); a **non-POSIX final export** (e.g. pblock) is published by streaming the partial INTO the backend via its driver `staged_*` state machine (`commit_staged_to_backend`, resolved by `xrootd_vfs_backend_resolve_for_path`) — the cross-filesystem atomic commit behind `upload_resume`/POSC on a pblock export. Used by S3 PUT, WebDAV PUT/COPY, and root:// close. |
