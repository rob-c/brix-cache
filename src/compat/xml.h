#ifndef XROOTD_COMPAT_XML_H
#define XROOTD_COMPAT_XML_H
 ...
 *
 * HOW: When libxml2 is available (XROOTD_HAVE_LIBXML2), xml_parse_lockinfo uses
...
/*
 * Flags for xrootd_xml_escape / xrootd_xml_write_text_element.
 *
 * Combine with bitwise OR; each flag controls one escape behaviour.
 */
#define XROOTD_XML_ESCAPE_APOS_ENTITY      0x0001u  /* encode ' as &apos; instead of &#39; */
#define XROOTD_XML_ESCAPE_CONTROL_PERCENT 0x0002u  /* encode control chars (<0x20, 0x7f) as %XX */

/*
 * xrootd_xml_escaped_len — compute the byte length of src after XML escaping.
 *
 * WHAT: Scans src[0..len] and returns the total number of bytes that would be
 *       produced by xrootd_xml_escape(), including space for a NUL terminator.
 *
 * WHY: Callers need to know the output size before allocating a buffer —
 *      avoids double-pass allocation or overflow. Used by
 *      xrootd_xml_write_text_element() and WebDAV lock response building.
 *
 ...
 */
size_t xrootd_xml_escaped_len(const unsigned char *src, size_t len,
    unsigned flags);

/*
 * xrootd_xml_escape — escape XML-special characters in a byte buffer.
 *
 ...
 *       are encoded as %XX when XROOTD_XML_ESCAPE_CONTROL_PERCENT is set.
 *
 ...
 *      control chars → emit %XX via xrootd_hex_nibble(); default → copy raw.
 ...
 *   flags     — combination of XROOTD_XML_ESCAPE_* flags (0 = default behaviour)
 ...
 */
int xrootd_xml_escape(const unsigned char *src, size_t len, unsigned flags,
    unsigned char *dst, size_t dst_size, size_t *written);

/*
 * xrootd_xml_text_element_len — compute byte length of a <name>text</name> element.
 *
 ...
 *
 * WHY: Pre-sizes buffers before calling xrootd_xml_write_text_element(). Used by
 ...
 */
size_t xrootd_xml_text_element_len(const char *name,
    const unsigned char *text, size_t text_len, unsigned flags);

/*
 * xrootd_xml_write_text_element — write an XML <name>escaped-text</name> element.
 *
 ...
 *
 * HOW: Validates name via xrootd_xml_element_name_valid(). Computes escaped_len()
 *      to check capacity. Writes <name>, calls xrootd_xml_escape() on text body,
 ...
 *   flags      — escaping flags for xrootd_xml_escape()
 ...
 */
int xrootd_xml_write_text_element(const char *name,
    const unsigned char *text, size_t text_len, unsigned flags,
    unsigned char *dst, size_t dst_size, size_t *written);

/*
 * xrootd_xml_parse_lockinfo — parse WebDAV <lockinfo> XML body.
 *
 ...
 *
 * Parameters:
 *   xml        — XML body bytes (may not exceed XROOTD_XML_MAX_LOCKINFO_BODY)
 ...
 */
int xrootd_xml_parse_lockinfo(const char *xml, size_t xml_len,
    char *owner, size_t owner_len, int *exclusive);

/*
 * xrootd_xml_backend_name — return the name of the active XML parsing backend.
 *
 ...
 */
const char *xrootd_xml_backend_name(void);
#endif /* XROOTD_COMPAT_XML_H */
