/*
 * codec_zstd.c — Zstandard backend for the codec abstraction (XROOTD_CODEC_ZSTD).
 *
 ...
 *       (compile-gated on -DXROOTD_HAVE_ZSTD).
...
static int
zstd_init(void **state, xrootd_codec_id_t id, xrootd_codec_dir_t dir, int level)
{
          ...
          uint8_t *out, size_t out_size, size_t *out_pos, int finish)
{
    ...
    if (st->dir == XROOTD_CODEC_DIR_DECOMPRESS) {
        ...
        *out_pos = ob.pos;
        ...
        return (r == 0) ? XROOTD_CODEC_END : XROOTD_CODEC_OK;
    }
    ...
    *out_pos = ob.pos;
    ...
    return (finish && r == 0) ? XROOTD_CODEC_END : XROOTD_CODEC_OK;
}
...
static const xrootd_codec_backend_t zstd_backend = {
    zstd_init, zstd_step, zstd_end
};

const xrootd_codec_desc_t xrootd_codec_zstd_desc = {
    XROOTD_CODEC_ZSTD, "zstd", "zstd", 1, 1, 19, 3, &zstd_backend
};

#else

const xrootd_codec_desc_t xrootd_codec_zstd_desc = {
    XROOTD_CODEC_ZSTD, "zstd", "zstd", 0, 0, 0, 0, NULL
};
