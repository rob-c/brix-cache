#ifndef XROOTD_TOKEN_OAUTH2_H
#define XROOTD_TOKEN_OAUTH2_H
...
* HOW: xrootd_oauth2_parse_access_token() validates input bounds, loads JSON via json_loads with duplicate rejection, extracts "access_token" string field, checks length against output capacity, copies with NUL termination. Returns NGX_OK on success or NGX_ERROR with error message populated in err buffer.
*/
...
ngx_int_t xrootd_oauth2_parse_access_token(const char *json, char *out,
    size_t out_sz, char *err, size_t err_sz);

#endif /* XROOTD_TOKEN_OAUTH2_H */
