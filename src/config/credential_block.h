#ifndef XROOTD_CONFIG_CREDENTIAL_BLOCK_H
#define XROOTD_CONFIG_CREDENTIAL_BLOCK_H

/*
 * credential_block.h — the reusable `xrootd_credential <name> { … }` config block
 * (phase-63 §14).
 *
 ...
 *       by `credential=<name>` (or the sibling `xrootd_storage_credential <name>`)
 ...
 *
 * HOW:  `xrootd_conf_credential_block()` is an nginx block handler (the ngx_map /
 ...
 *       interned in a small per-process table. `xrootd_credential_lookup()` finds a
 *       block by name; `xrootd_credential_bearer()` resolves its bearer token
...
/* Parsed `xrootd_credential` block (§14.4 shape). Empty fields ⇒ unset. */
typedef struct {
...
char *xrootd_conf_credential_block(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);
...
const xrootd_credential_t *xrootd_credential_lookup(const char *name);
...
ngx_int_t xrootd_credential_bearer(const xrootd_credential_t *cred, char *out,
    size_t cap, ngx_log_t *log);

#endif /* XROOTD_CONFIG_CREDENTIAL_BLOCK_H */
