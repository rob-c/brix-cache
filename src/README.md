...
| [protocol](protocols/root/protocol/README.md) | Header-only single source of truth for the `root://` binary wire protocol: opcodes, status/error codes, option bitmasks, and packed on-wire request/response structs mirrored from the published wire spec. |
