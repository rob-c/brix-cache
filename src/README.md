...
| [pmark](pmark/README.md) | SciTags network packet marking — tags each data flow with an `(experiment, activity)` pair via out-of-band firefly UDP reports + in-band IPv6 flow-label stamping, compatible with XRootD's `pmark`/SciTags collectors; shared across root://, WebDAV, and S3. |
