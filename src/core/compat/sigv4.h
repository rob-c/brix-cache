 ...
 *       out=HMAC(k3,"aws4_request"), using the shared xrootd_hmac_sha256 kernel.
 *       Pure ptr+len; no ngx, no allocation. (libxrdproto)
 */
#ifndef XROOTD_COMPAT_SIGV4_H
#define XROOTD_COMPAT_SIGV4_H
 ...
 */
int xrootd_sigv4_signing_key(const uint8_t *secret, size_t secret_len,
                             const char *date, const char *region,
                             const char *service, uint8_t out[32]);

#endif /* XROOTD_COMPAT_SIGV4_H */
