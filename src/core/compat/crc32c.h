#ifndef BRIX_COMPAT_CRC32C_H
#define BRIX_COMPAT_CRC32C_H

#include <stddef.h>
#include <stdint.h>

/*
 * brix_crc32c_value - compute CRC-32c checksum of a buffer.
 *
 ...
 *
 * HOW: Delegates to brix_crc32c_extend(0, buf, len).
 */
uint32_t brix_crc32c_value(const void *buf, size_t len);

/* Contract: CRC-32c (Castagnoli, refin/refout, init 0) of buf[0..len), returned
 * as the seed-0 case of brix_crc32c_extend. buf is read-only and unowned.
 * buf==NULL with len!=0 is a no-op that returns 0 (the seed); len==0 returns 0. */
uint32_t brix_crc32c_value(const void *buf, size_t len);
/*
 * brix_crc32c_copy_value - compute CRC-32c while copying buffer contents.
 *
 ...
 *
 * HOW: Delegates to brix_crc32c_copy_hw() or brix_crc32c_copy_sw() depending on
 *      SSE4.2 availability.
 */
uint32_t brix_crc32c_copy_value(const unsigned char *src,
...
uint32_t brix_crc32c_copy_value(const unsigned char *src,
    unsigned char *dst, size_t len);
/*
 * brix_crc32c_extend - incrementally extend a CRC-32c checksum with new data.
 *
 ...
 *      brix_checksum_u32_fd() and the pgread/pgwrite CRC paths.
 *
 * HOW: Delegates to brix_crc32c_hw_extend() on SSE4.2 (_mm_crc32_u64/u32/u8 intrinsics)
 *      or brix_crc32c_sw_extend() (bit-by-bit Castagnoli polynomial) otherwise.
 */
uint32_t brix_crc32c_extend(uint32_t crc, const void *buf, size_t len);
