 ...
 */
#ifndef BRIX_COMPAT_SSS_BF_H
#define BRIX_COMPAT_SSS_BF_H
 ...
 */
int brix_sss_bf_crypt(int encrypt, const uint8_t *key, size_t key_len,
                        const uint8_t *src, size_t src_len,
                        uint8_t *dst, size_t dst_max, size_t *out_len);

/*
 * brix_sss_build_credential — assemble a complete SSS kXR_auth credential blob
 ...
 * and gen_time (seconds since BRIX_SSS_BASE_TIME) so the RNG and clock stay at
 * the edges. username NULL/empty defaults to "xrd" (NAME TLV capped at 64 bytes).
 *
 * out_max must be >= BRIX_SSS_HDR_LEN + BRIX_SSS_DATA_HDR_LEN + 3 + 64 + 1 + 4
 ...
 */
int brix_sss_build_credential(const uint8_t *key, size_t key_len,
                                uint64_t key_id, const char *username,
                                const uint8_t nonce32[32], uint32_t gen_time,
                                uint8_t *out, size_t out_max, size_t *out_len);

#endif /* BRIX_COMPAT_SSS_BF_H */
