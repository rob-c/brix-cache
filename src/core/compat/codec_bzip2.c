/*
 * codec_bzip2.c — bzip2 backend for the codec abstraction (BRIX_CODEC_BZIP2).
 *
 ...
 *       -DBRIX_HAVE_BZIP2). Lowest priority of the modern set.
...
static int
bzip2_init(void **state, brix_codec_id_t id, brix_codec_dir_t dir, int level)
{
    ...
    if (r != BZ_OK) {
        free(st);
        return (r == BZ_MEM_ERROR) ? BRIX_CODEC_ERR_MEM : BRIX_CODEC_ERR_PARAM;
    }
    st->inited = 1;
    *state = st;
    return 0;
...
static const brix_codec_backend_t bzip2_backend = {
    bzip2_init, bzip2_step, bzip2_end
};

const brix_codec_desc_t brix_codec_bzip2_desc = {
    BRIX_CODEC_BZIP2, "bzip2", "bzip2", 1, 1, 9, 9, &bzip2_backend
};

#else

const brix_codec_desc_t brix_codec_bzip2_desc = {
    BRIX_CODEC_BZIP2, "bzip2", "bzip2", 0, 0, 0, 0, NULL
};
