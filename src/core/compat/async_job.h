#ifndef BRIX_ASYNC_JOB_H
#define BRIX_ASYNC_JOB_H
 ...
 *   brix_async_job_t  — per-job lifecycle state
 *   brix_async_job_init()        — zero and link the job to its owner
 *   brix_async_job_set_cleanup() — register a cleanup callback
 *   brix_async_job_cleanup_once() — idempotent cleanup (safe to call twice)
 *
...
typedef void (*brix_job_cleanup_pt)(void *data);

/*
 * brix_async_job_t — lifecycle state for a single background operation.
 *
 *   log          logger for cleanup-path messages
 *   owner        opaque pointer to the owning request/session context
 *   finalized    non-zero after brix_async_job_cleanup_once() runs
...
typedef struct {
    ...
    brix_job_cleanup_pt  cleanup;
    void                  *cleanup_data;
} brix_async_job_t;

/*
 * brix_async_job_init — initialise a job struct.
 *
 * Zeroes all fields, then sets log and owner.  cleanup and cleanup_data
 * remain NULL until set via brix_async_job_set_cleanup().
 */
void brix_async_job_init(brix_async_job_t *job, ngx_log_t *log,
    void *owner);
 ...
 */
void brix_async_job_set_cleanup(brix_async_job_t *job,
    brix_job_cleanup_pt cleanup, void *data);

/*
 * brix_async_job_cleanup_once — run the cleanup callback exactly once.
 *
 ...
 */
void brix_async_job_cleanup_once(brix_async_job_t *job);

#endif /* BRIX_ASYNC_JOB_H */
