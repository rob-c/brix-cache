 ...
 *
 * WHAT: Streaming (de)compression for BRIX_CODEC_GZIP and BRIX_CODEC_DEFLATE
...
typedef struct {
    z_stream  zs;
    int       dir;        /* brix_codec_dir_t */
    int       inited;
} zlib_state;

static int
zlib_window_bits(brix_codec_id_t id)
{
    /* gzip = raw deflate + gzip header/trailer (15+16); deflate = zlib wrapper (15). */
    return (id == BRIX_CODEC_GZIP) ? (15 + 16) : 15;
}

static int
zlib_init(void **state, brix_codec_id_t id, brix_codec_dir_t dir, int level)
{
    ...
    if (zr != Z_OK) {
        free(st);
        return (zr == Z_MEM_ERROR) ? BRIX_CODEC_ERR_MEM : BRIX_CODEC_ERR_PARAM;
    }
    st->inited = 1;
    *state = st;
    return 0;
          ...
          uint8_t *out, size_t out_size, size_t *out_pos, int finish)
{
    ...
    *out_pos += (size_t) (out_avail0 - st->zs.avail_out);
    ...
    return BRIX_CODEC_ERR_DATA;   /* Z_DATA_ERROR / Z_NEED_DICT / Z_STREAM_ERROR */
}
...
static const brix_codec_backend_t zlib_backend = {
    zlib_init, zlib_step, zlib_end
};

const brix_codec_desc_t brix_codec_gzip_desc = {
    BRIX_CODEC_GZIP, "gzip", "gzip", 1, 1, 9, 6, &zlib_backend
};
const brix_codec_desc_t brix_codec_deflate_desc = {
    BRIX_CODEC_DEFLATE, "deflate", "deflate", 1, 1, 9, 6, &zlib_backend
};

#else  /* !BRIX_HAVE_ZLIB — should never happen (zlib is mandatory) */

const brix_codec_desc_t brix_codec_gzip_desc = {
    BRIX_CODEC_GZIP, "gzip", "gzip", 0, 0, 0, 0, NULL
};
const brix_codec_desc_t brix_codec_deflate_desc = {
    BRIX_CODEC_DEFLATE, "deflate", "deflate", 0, 0, 0, 0, NULL
};
