 ...
 * HOW:  xrootd_async_job_cleanup_once() checks and sets job->finalized under
...
void
xrootd_async_job_init(xrootd_async_job_t *job, ngx_log_t *log, void *owner)
{
...
void
xrootd_async_job_set_cleanup(xrootd_async_job_t *job,
...
void
xrootd_async_job_cleanup_once(xrootd_async_job_t *job)
{
    if (job->finalized) {
        return;
    }
