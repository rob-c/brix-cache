#ifndef BRIX_COMPAT_HEX_H
#define BRIX_COMPAT_HEX_H

/*
 * ngx-free: uses uint8_t (== unsigned char, identical to nginx's u_char) so this
 * header compiles into both the nginx module and the standalone libxrdproto core.
 */
#include <stddef.h>
#include <stdint.h>

/*
 * brix_hex_nibble — convert nibble value to uppercase ASCII hex character.
 *
 * WHAT: Returns '0'-'9' for values 0-9, 'A'-'F' for values 10-15. WHY: Produces individual hex
 *      output characters from 4-bit input during byte-array encoding. */

uint8_t brix_hex_nibble(uint8_t v);

/*
 * brix_hex_from_char — parse ASCII hex character to nibble value, case-insensitive.
 *
 * WHAT: Returns 0-9 for '0'-'9', 10-15 for 'a'-'f'/'A'-'F', -1 for invalid input. WHY: Converts
 *      hex string characters back to byte values during checksum parsing and decoding. */

int brix_hex_from_char(unsigned char c);

/*
 * brix_hex_encode — convert byte array to null-terminated lowercase ASCII hex string.
 *
 * WHAT: Writes each input byte as two hex characters into out, null-terminating at end. WHY:
 *      Checksum computation results, ETag generation, metadata display need hex representation
 *      of arbitrary binary data. Caller must allocate 2*len+1 bytes for output buffer. */

void brix_hex_encode(const uint8_t *in, size_t len, char *out);

#endif /* BRIX_COMPAT_HEX_H */
