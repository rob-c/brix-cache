#ifndef BRIX_CONFIG_MERGE_MACROS_H
#define BRIX_CONFIG_MERGE_MACROS_H
/*
 ...
 *
 *   BRIX_MERGE_PTR      — NULL-sentinel pointer (no default; just inherit)
 *   BRIX_MERGE_HOSTPORT — paired ngx_str_t host + port (inherit together)
 *   BRIX_MERGE_ENUM     — custom enum with an explicit UNSET sentinel
 *
 * Naming mirrors nginx's ngx_conf_merge_* convention.
 */

/* NULL-sentinel pointer: copy from parent if child is unset (NULL). */
#define BRIX_MERGE_PTR(conf, prev, field)                         \
 ...
 */
#define BRIX_MERGE_HOSTPORT(conf, prev, host, port)               \
 ...
 */
#define BRIX_MERGE_ENUM(conf, prev, field, unset, def)            \
    if ((conf)->field == (unset)) {                                  \
        (conf)->field = ((prev)->field != (unset))                   \
                        ? (prev)->field : (def);                     \
    }

#endif /* BRIX_CONFIG_MERGE_MACROS_H */
