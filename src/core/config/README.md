...
| `server_conf_merge_cluster.c` | Owns brix_merge_srv_tpc() (TPC allowances, key TTL, transfer caps + reaper age, F7 hop budget + stream cap with their range checks, SSI/CNS, outbound credentials) and brix_merge_srv_cluster() (manager/redirector mode, staged uploads, pipeline/registry/session sizing, heal. |
