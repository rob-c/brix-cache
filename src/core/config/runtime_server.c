...
#include "runtime_server_backend_internal.h" /* brix_server_setup_tls (runtime_server_tls.c) */
...
/*
 * Outbound-TLS context setup (proxy-leg + redirector-leg SSL_CTXs with
 * fail-closed peer verification, brix_server_setup_tls / the file-local
 * brix_tls_ctx_enable_verify helper) lives in the sibling
 * runtime_server_tls.c; its prototype is in runtime_server_backend_internal.h.
 */
