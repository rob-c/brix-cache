...
`brix_kv_zone` directive and then resolved by name by feature directives. Every
configured zone is recorded in a module-wide registry so consumer directives can
look one up (`brix_kv_find`) and the Prometheus exporter can iterate them all.
...
| File | Responsibility |
|---|---|
| `kv.h` | Public API + the process-local `brix_kv_t` handle and the `brix_kv_stats_t` snapshot type. Declares `brix_kv_configure/get/set/delete/stats`, the zone-registry accessors (`brix_kv_find`, `brix_kv_zone_count`, `brix_kv_zone_get`), and the `brix_kv_zone_directive` setter. Intentionally depends only on `<ngx_config.h>`/`<ngx_core.h>` so it can be embedded in per-module config structs without an `ngx_brix_module.h` include cycle. |
...
- **`brix_kv_t`** (`kv.h`) — the *process-local* handle. Allocated from the
...
- **`brix_kv_header_t`** (`kv.c`, shared) — the first object in the zone's
...
- **`brix_kv_entry_t`** (`kv.c`, shared) — one bucket. Stores the 64-bit FNV-1a
...
- **`brix_kv_stats_t`** (`kv.h`) — a snapshot of `hits`/`misses`/`evictions`/
  `count`/`capacity` taken under the lock and returned to the (lock-free) caller
  for Prometheus export.
- **`brix_rate_limit_conf_t`** (`rate_limit.h`) — per-location/per-server
  limiter settings: the resolved `brix_kv_t *kv` (NULL = disabled), `rate`
  (sustained req/s), `burst` (bucket capacity), and `key_ip` (0 = key by DN,
  1 = key by client IP).
- **`brix_rl_val_t`** (`rate_limit.c`, the KV value) — `{ uint32_t tokens;
  ngx_msec_t last_refill; }`, 16 bytes, stored under a `"rl:" + id` key.

## Control & data flow

**Configuration time.** `brix_kv_zone <name> <size> key=<bytes> val=<bytes>;`
...
registry (cap `BRIX_KV_MAX_ZONES` = 16; size clamped up to `BRIX_KV_MIN_SIZE`
= 64 KiB). Feature directives such as `brix_rate_limit`, `brix_token_cache`,
and `brix_auth_cache` then run *after* and resolve their zone by name with
`brix_kv_find`, each validating that the zone's `val_max` (and where relevant
`key_max >= 32`) is large enough for their value struct — failing config if not.

**Zone init / reload.** `brix_kv_init_zone` runs when nginx maps the segment.
...
**Runtime data path.** Consumers call `brix_kv_get`/`brix_kv_set`/
`brix_kv_delete` directly. Each op takes the zone spinlock, performs a single
...
- **Lock-first layout is load-bearing.** `brix_kv_header_t.lock`
  ...
  locking.
- **0.5 load-factor cap, by design.** `brix_kv_set` refuses to insert a *new*
...
- **Tombstone-free deletion.** Removal uses Knuth Algorithm R backward-shift
  (`brix_kv_remove_at`, `kv.c:119`) so probe chains stay contiguous — there are
  no tombstones to degrade lookups over time. `brix_kv_should_shift` encodes the
...
- **Best-effort across-worker counting.** `brix_rate_limit_check` does a
...
- **Truncation, not error, on oversized reads.** `brix_kv_get` copies
...
- **Add a new KV-backed feature (a typed wrapper):** define your value struct,
  ...
  zone with `brix_kv_find` and validates `kv->val_max >= sizeof(your_val)`
  (see `brix_token_cache_directive` / `brix_auth_cache_directive` / this
  subsystem's `brix_rate_limit_directive` as templates), then use
  `brix_kv_get`/`brix_kv_set` on the hot path. Store the resolved
  `brix_kv_t *` in your module conf.
