#ifndef BRIX_FS_LIST_H
#define BRIX_FS_LIST_H
 ...
 *       brix_sd_driver_t the module ships, plus the store-URL scheme
 ...
 *               brix_sd_<sym>_driver in fs/backend/<dir>/sd_<sym>.c
 *         name  the driver's .name string — the config backend name,
 *               brix_sd_backend_name() (metric/log label: bounded,
 *               INVARIANT #8), and the tier scheme "driver" column
 *         kind  dispatch filter, consumed as BRIX_FS_ROW_<kind>:
 *                 BACKEND   name-resolvable primary store (registry row +
 *                           extern; selected by brix_storage_backend)
 ...
 *   1. write fs/backend/<dir>/sd_<sym>.c implementing brix_sd_driver_t
 ...
 *      probe for a library — cf. BRIX_HAVE_CEPH/BRIX_HAVE_SQLITE),
 ...
 *      a scheme alias row in BRIX_FS_SCHEME_LIST below [X: tier table]
...
#define BRIX_FS_DRIVER_LIST_CORE(X)                                         \
...
#if BRIX_HAVE_CEPH
#define BRIX_FS_DRIVER_LIST_CEPH(X)                                         \
...
#if BRIX_HAVE_SQLITE
#define BRIX_FS_DRIVER_LIST_SQLITE(X)                                       \
    X(PBLOCK,    pblock,    "pblock",   BACKEND)   /* packed-block sqlite  */
#else
#define BRIX_FS_DRIVER_LIST_SQLITE(X)
#endif

#define BRIX_FS_DRIVER_LIST(X)                                              \
    BRIX_FS_DRIVER_LIST_CORE(X)                                             \
    BRIX_FS_DRIVER_LIST_CEPH(X)                                             \
    BRIX_FS_DRIVER_LIST_SQLITE(X)

/* Kind-filter helper: consumers define BRIX_FS_ROW_BACKEND(...) etc. to a
 * row and the rest to nothing, then pass BRIX_FS_ROW as the X. */
#define BRIX_FS_ROW(ID, sym, name, kind) BRIX_FS_ROW_##kind(ID, sym, name)
 ...
 * without CEPH/SQLITE simply has a smaller BRIX_FS_ID_COUNT. Consumers:
 * the per-backend SHM byte counters (observability/metrics/metrics.h) index
 * by these ids; the exporters label by brix_fs_id_name(). The gate macros
 * are global -D CFLAGS (repo ./config), so every server TU agrees on
 * BRIX_FS_ID_COUNT and the SHM layout stays consistent within a build. */
typedef enum {
#define BRIX_FS_ROW_ENUM_ID(ID, sym, name, kind) BRIX_FS_ID_##ID,
    BRIX_FS_DRIVER_LIST(BRIX_FS_ROW_ENUM_ID)
#undef BRIX_FS_ROW_ENUM_ID
    BRIX_FS_ID_COUNT
} brix_fs_id_t;

/* Name <-> id lookups over the census (fs/backend/sd_fs_id.c — ngx-free,
 * no driver externs, unit-testable standalone).
 * brix_fs_id_name: bounded label for exporters; "?" for out-of-range.
 * brix_fs_id_from_name: exact-match scan; -1 for NULL/unknown. */
const char *brix_fs_id_name(int id);
int brix_fs_id_from_name(const char *name);

#endif /* BRIX_FS_LIST_H */
