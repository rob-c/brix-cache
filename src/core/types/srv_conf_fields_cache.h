/*
 * srv_conf_fields_cache.h: Pelican register, unified cache-state engine, LRU reaper, VFS cache storage, checksum-on-fill, io_uring restrictions — a field-declaration fragment of ngx_stream_brix_srv_conf_t, split
...
#pragma once
    ...
    time_t       cache_cold_max_age;      /* [brix_cache_cold_max_age] secs; 0=off.
                                             Audit §4.2 (upstream pfc.purgecoldfiles):
                                             a CLEAN read-through fill untouched for
                                             longer than this is purged regardless of
                                             occupancy — the watermark reaper only
                                             runs when the filesystem fills, so a
                                             roomy cache otherwise keeps cold objects
                                             forever. Runs on the same per-worker
                                             timer as the stale-dirty reaper. */
    ngx_event_t *cache_reap_timer;        /* per-worker stale-dirty reaper; NULL if off */
    ngx_event_t *csi_scrub_timer;         /* per-worker at-rest CSI scrub; NULL if off */
    ...
    ngx_flag_t  signing_required; /* [brix_signing_required on|off] (audit §5.2/§9.2)
                                     off (default): a session whose auth protocol
                                     established NO signing key (sss/ztn/krb5/unix/
                                     host — only GSI arms one today) passes
                                     brix_security_level unenforced. That silent
                                     bypass is the documented gap; it is now
                                     LOGGED either way. on: such a session is
                                     REFUSED (kXR_NotAuthorized) for any opcode the
                                     level requires signed, so the directive means
                                     what an operator reads it to mean. Off by
                                     default because turning it on rejects every
                                     stock non-GSI client, which is a deployment
                                     decision, not a default. */

    /* ---- D-1: session-posture floor (distinct axis from security_level) ---- */
    ngx_uint_t  min_sec_level;   /* [brix_min_sec_level none|compat|intense] BRIX_MIN_SEC_*;
                                     0 = no floor. compat requires a TLS session; intense also
                                     requires a non-anonymous identity. Enforced post-handshake
                                     in brix_min_sec_enforce (handshake/policy.c). */

    /* ---- D-2: opt-in opaque (CGI) schema enforcement ---- */
    ngx_flag_t  opaque_strict;   /* [brix_opaque_strict on|off] off = byte-hygiene only.
                                     on = additionally type-check oss.asize (unsigned int) and
                                     reject an opaque key outside the recognized namespace
                                     vocabulary, at brix_open_precheck (read/open_request.c). */

    /* ---- D-3: per-worker seccomp-BPF syscall filter ---- */
    ngx_uint_t  seccomp;  /* [brix_seccomp off|audit|enforce] BRIX_SECCOMP_*; process-
                             global — strictest across enabled blocks installed once per
                             worker at init_process (src/core/seccomp/seccomp.c). */

    /* ---- E-4: negative-path backoff (stat/locate-harvest throttle) ---- */
    brix_negcache_conf_t  negcache;  /* [brix_negcache_backoff off|<thr> <win_s> <wait_s>]
                                        threshold==0 → disabled. Cross-worker SHM
                                        state in src/core/negcache/negcache.c. */
    ...
    size_t      socket_sndbuf;      /* [brix_socket_sndbuf 0] Phase-33 P3-B3:
                                       SO_SNDBUF bytes at accept — pin the send
                                       buffer to the deployment BDP so a single
                                       download stream keeps a high-BDP pipe full
                                       instead of being capped by lagging send
                                       autotuning.  0 = leave the kernel default
                                       (autotuning). */
    size_t      socket_rcvbuf;      /* [brix_socket_rcvbuf 0] SO_RCVBUF bytes at
                                       accept — the symmetric knob for the
                                       upload/PUT direction.  0 = kernel default. */
