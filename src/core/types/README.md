...
context (`brix_ctx_t`), the per-server configuration block
(`ngx_stream_brix_srv_conf_t`), the per-open-file slot (`brix_file_t`), the
connection state-machine enum (`brix_state_t`), the compile-time tunables and metric
macros, and the one piece of behaviour that lives here: the protocol-agnostic
authenticated principal (`brix_identity_t`, with `identity.c`). Almost every other
...
`src/core/ngx_brix_module.h` so a contributor can read exactly one concept without wading
...
`src/core/ngx_brix_module.h:105`), each after the nginx, OpenSSL, `protocol/`, `metrics/`,
...
async I/O, and send phase until disconnect. The matching `ngx_stream_brix_srv_conf_t`
...
the tunables, the auth-mode constants, and the `brix_identity_t` principal defined here.
...
- **`brix_ctx_t` (`context.h`)** — one per TCP connection, allocated from
  ...
  *client* side, the server serialises responses). Notable sections:
  ...
  - *Session/auth:* `sessid`, `logged_in`/`auth_done`, `login_user[9]`/`login_pid`, capped
    ...
    (`brix_identity_t *`).
  - *Open-file table:* `files[BRIX_MAX_FILES]` (index = handle).
  - *Output pipelining (Phase 29+):* `out_ring[BRIX_PIPELINE_MAX]` of
    `brix_resp_slot_t` with `out_head`/`out_tail`/`out_count`, plus `recv_deferred`
    (drain barrier for non-pipelinable opcodes) and `resp_pipelinable` (only single-chunk
    sendfile reads may pipeline).
  - *Read pipeline / windowing (Phase 31/32):* `rd_pool[]` of `brix_read_slot_t`,
  ...
  - *Cluster/proxy/bind:* `tls_pending`, `upstream`, `proxy`, raw `bearer_token[4096]`
    (forwarded upstream when `brix_proxy_auth forward`), `is_bound`/`pathid`/`bound_sessid`
...
- **`brix_resp_slot_t` / `brix_read_slot_t` (`context.h`)** — per-in-flight-response
...
- **`brix_file_t` + `brix_wrts_entry_t` (`file.h`)** — handle bookkeeping; the
  `device`/`inode` captured at open are the validation key for bound-secondary reopens,
  and `wrts_journal[]` makes write replay idempotent (`brix_wrts_is_replay()`).
- **`ngx_stream_brix_srv_conf_t` + helpers (`config.h`)** — the merged, read-only
...
- **`brix_identity_t` (`identity.h`)** — DN/subject/issuer + VO/scope arrays + the
  `BRIX_AUTHN_*` method bitmask; the deliberate bridge that lets one policy/audit code
...
- **`brix_state_t` (`state.h`)** — the enum the connection event handlers switch on; the
...
5. `identity.c`'s `brix_identity_check_token_scope` delegates to `../token/`
...
- **Array index *is* the handle.** `ctx->files[i]` is addressed directly by the 4-byte
  ...
  `brix_free_fhandle()` (resets to `-1`). Never assume handles are dense or stable
  across reuse.
...
- **`identity.c` is fail-closed and pool-bounded.** Every setter rejects a NULL `id`,
  ...
  `BRIX_MAX_TOKEN_SCOPES` (`identity.c:323-324`). `brix_identity_check_token_scope`
...
- **Auth domains never share logic.** `BRIX_AUTHN_*` (`identity.h`, the *verified*
  principal bitmask) and the wire-auth `BRIX_AUTH_*` modes (`tunables.h`, the configured
  requirement) are distinct namespaces; S3 SigV4 and WLCG tokens are different auth surfaces
  and must not cross-pollinate.
- **The metric macros are conditional.** `BRIX_OP_OK/OP_ERR` and the `BRIX_RETURN_*`
...
- **`BRIX_RETURN_OK/REDIR` are for *no-body* responses only.** They call
  `brix_send_ok(ctx, c, NULL, 0)` / `brix_send_redirect`; handlers returning a body
  (read data, query results, pgwrite status) must keep the log/metric/send lines explicit
  (`tunables.h:218`). `BRIX_BAIL_ERR` is the variant for helper functions that report
  failure via an out-parameter and `return 0`.
- **Size caps are security backstops, not just sizing.** `BRIX_MAX_WALK_DEPTH` (32)
  rejects deep symlink chains *before* expensive `realpath`/`lstat`;
  `BRIX_MAX_CONN_POOL_BYTES` (64 MB) closes a connection with `kXR_NoMemory` rather than
  let a dirlist flood exhaust the worker heap; `BRIX_MAX_AUTH_ATTEMPTS` (10) caps
  GSI/token/SSS CPU-amplification. `BRIX_READ_WINDOW`/`_CONN_XFER_HEAP_MAX` bound the
  ...
  casually.
- **`destroyed` guard prevents use-after-free.** `brix_on_disconnect()` sets
...
- **Legacy/anchor naming.** Phase numbers in the comments (Phase 29/31/32/33) are
  ...
  `brix_resp_slot_t`, not as connection singletons. Read the struct, not the phase label.

## Entry points / extending

- **Add a config directive →** add the field to `ngx_stream_brix_srv_conf_t`
