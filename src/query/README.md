...
| `metadata.c` | `kXR_QStats` (`xrootd_query_stats`, XML server stats), `kXR_Qxattr` (`xrootd_query_xattr`, `oss.*` attrs + `user.U.*` xattrs — its stat goes through `xrootd_vfs_probe` and its list/get through the VFS xattr seam, never raw `stat`/`listxattr`), `kXR_QFinfo` (`xrootd_query_finfo`, placeholder `"0"`), and the `Qvisa`/`Qopaque`/`Qopaquf`/`Qopaqug` FSctl/fctl hooks that validate then return reference-compatible "unsupported". |
