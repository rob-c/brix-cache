 ...
 *      EVP-based digests support md5, sha1, and sha256 via the xrootd_checksum_alg directive. Two variants exist per algorithm — fd-based
 *      (for open handles during query) and file-based (path lookup + open + compute). INVARIANT: all path strings sanitized with
 *      xrootd_sanitize_log_string() before logging to prevent binary data corruption in access logs.
 ...
 *      with xrootd_open_confined() and close(). All error paths return 0xFFFFFFFF (adler32) or 0 (digest).
 */

/*
 * xrootd_query_adler32_fd — Adler-32 checksum from open file descriptor.
 ...
 * HOW: Delegates to xrootd_checksum_u32_fd() with XROOTD_CHECKSUM_ADLER32 → on
...
/*
 * xrootd_query_adler32_file — Adler-32 checksum from filesystem path.
 ...
 *      path string instead of a handle. Uses xrootd_open_confined for security
 *      confinement (prevents traversal outside configured root).

 * HOW: Sanitize path for logging → open via xrootd_open_confined(log, root, path,
 *      O_RDONLY) → call xrootd_query_adler32_fd(fd, path, log) → close(fd) → return.
...
/*
 * xrootd_query_digest_fd — EVP digest from open file descriptor.
 ...
 *      algorithms set via xrootd_checksum_alg directive.
...
/*
 * xrootd_query_digest_file — EVP digest from filesystem path.
 ...
 *      filesystem path string. Uses xrootd_open_confined for security confinement.

 * HOW: Open via xrootd_open_confined(log, root, path, O_RDONLY) → call
 *      xrootd_query_digest_fd(fd, path, md, out, outlen, log) → close(fd) → return.
...
xrootd_query_adler32_fd(int fd, const char *path, ngx_log_t *log)
{
    ...
    if (xrootd_checksum_u32_fd(XROOTD_CHECKSUM_ADLER32, fd, path, log, &out)
        != NGX_OK)
    {
        return 0xFFFFFFFF;
    }
...
ngx_flag_t
xrootd_query_digest_fd(int fd, const char *path,
    xrootd_checksum_alg_t alg,
    unsigned char *out, unsigned int *outlen, ngx_log_t *log)
{
    return xrootd_checksum_digest_fd(alg, fd, path, log, out, outlen)
           == NGX_OK ? 1 : 0;
}

ngx_flag_t
xrootd_query_digest_file(const ngx_str_t *root, const char *path,
    xrootd_checksum_alg_t alg, unsigned char *out, unsigned int *outlen,
    ngx_log_t *log)
{
    int        fd;
    char       safe[512];
    ...
    ok = xrootd_query_digest_fd(fd, path, alg, out, outlen, log);
    close(fd);
    return ok;
}

/*
 * xrootd_query_crc32_fd — ISO-3309 CRC-32 from open file descriptor.
 ...
 * HOW: Delegates to xrootd_checksum_u32_fd() with XROOTD_CHECKSUM_CRC32 → on NGX_OK
...
uint32_t
xrootd_query_crc32_fd(int fd, const char *path, ngx_log_t *log)
{
    uint32_t out;

    if (xrootd_checksum_u32_fd(XROOTD_CHECKSUM_CRC32, fd, path, log, &out)
        != NGX_OK)
    {
        return 0xFFFFFFFF;
    }
...
uint32_t
xrootd_query_crc32_file(const ngx_str_t *root, const char *path,
    ngx_log_t *log)
{
    int      fd;
    uint32_t cksum;
    ...
    cksum = xrootd_query_crc32_fd(fd, path, log);
    close(fd);
    return cksum;
}
/*
 ...
 */
/*
 * xrootd_query_crc32_file — ISO-3309 CRC-32 from filesystem path.
 ...
 *      distinct from CRC32c (Castagnoli). Uses xrootd_open_confined for security confinement.

 * HOW: Sanitize path for logging → open via xrootd_open_confined(log, root, path,
 *      O_RDONLY) → call xrootd_query_crc32_fd(fd, path, log) → close(fd) → return.
 *      Error sentinel 0xFFFFFFFF returned on open failure or checksum computation error.
