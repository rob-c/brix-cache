...
- **Heartbeat client** (`connect.c`, `recv.c`, `send.c`, `wire.c`, `space.c`,
  ...
  *up* to a parent CMS manager (`xrootd_cms_manager host:port`), announces
  ...
  `../config/process.c` (`ngx_xrootd_cms_start`).
- **Manager-side server** (`server_*.c`, `server.h`) — a separate stream
  module, `ngx_stream_xrootd_cms_srv_module`, enabled with
  `xrootd_cms_server on;`. It *accepts* CMS connections *down* from data nodes,
...
`kYR_locate` upstream via this subsystem's `ngx_xrootd_cms_send_locate`. When
...
| File | Responsibility |
|------|----------------|
| `config.c` | Directive parser for `xrootd_cms_manager host:port`: resolves and stores the manager `ngx_addr_t` in the stream server conf (`cms_addr`/`cms_manager`); rejects duplicates and missing ports. |
| `connect.c` | Connection lifecycle and timer engine: `ngx_xrootd_cms_start` (per-worker init), `_connect` (`ngx_event_connect_peer`), write handler (login → status → load), `_timer` (periodic heartbeat / reconnect), `_disconnect`, `_schedule`, and exponential-backoff `_schedule_retry`. |
...
| `cms_internal.h` | Client context `ngx_xrootd_cms_ctx_s`, all `kYR_*`/`CMS_*` wire constants, timing/sizing constants, and client-half prototypes. |

### Shared frame I/O

| File | Responsibility |
|------|----------------|
| `frame_io.c` / `frame_io.h` | Transport primitives used by both halves: `xrootd_cms_send_all` (loop `c->send` to completion) and `xrootd_cms_send_frame` (build the 8-byte header + dispatch payload). |

### Manager-side server (`ngx_stream_xrootd_cms_srv_module`)

| File | Responsibility |
|------|----------------|
| `server.h` | Server module API: per-connection `xrootd_cms_srv_ctx_t`, per-block `ngx_stream_xrootd_cms_srv_conf_t`, SSS handshake state enum `xrootd_cms_auth_state_t`, and all server-half prototypes. |
| `server_module.c` | Module descriptor, conf create/merge, and directives: `xrootd_cms_server` (installs `xrootd_cms_srv_handler` as the stream handler), `_interval`, `_allow` (CIDR list), `_sss_keytab`. |
| `server_handler.c` | Accept entry point `xrootd_cms_srv_handler`: allocates ctx, records peer host, runs the accept-time CIDR gate, sets initial SSS auth state, installs read/write handlers, arms first read. |
| `server_recv.c` | Inbound read loop + TLV/Pup parsers + frame dispatcher: `kYR_login`→parse + (optionally) SSS-challenge + `xrootd_srv_register`, `kYR_xauth`→verify + register, `kYR_load`/`kYR_avail`/`kYR_space`→`xrootd_srv_update_load`, `kYR_gone`→`xrootd_srv_unregister_path`, `kYR_pong`; ping timer; `xrootd_cms_srv_close` (blacklist + unregister on drop). |
| `server_send.c` | Server-side frame senders: `xrootd_cms_srv_send_ping` (liveness probe), `xrootd_cms_srv_send_xauth` (security challenge). |
| `server_auth.c` | Registration auth gates: `xrootd_cms_srv_check_peer` (CIDR allowlist, fail-open-with-warning when unset) and `xrootd_cms_srv_verify_xauth` (validate the data node's SSS credential via `../sss`). |

## Key types & data structures

- **`ngx_xrootd_cms_ctx_s`** (`cms_internal.h`) — per-manager client state, one
...
- **`xrootd_cms_srv_ctx_t`** (`server.h`) — per-accepted-connection state on the
...
payload bytes (`NGX_XROOTD_CMS_HDR_LEN` / `NGX_XROOTD_CMS_MAX_FRAME = 4096`).
...
`../config/process.c` calls `ngx_xrootd_cms_start` once per worker when
...
**Manager half (this node is a redirector).** With `xrootd_cms_server on;`,
the stream core dispatches accepted connections to `xrootd_cms_srv_handler`.
After the CIDR/SSS gates, `kYR_login` is parsed and the node is registered in
`../manager/registry.h` via `xrootd_srv_register`; subsequent heartbeats call
`xrootd_srv_update_load`; disconnect blacklists the node for 30s and
unregisters it.

**Client-locate redirect loop (manager mode).** Read-path handlers
(`../read/locate.c`, `../read/open_request.c`, `../read/stat.c`,
`../query/checksum_qcksum.c`) that cannot serve a path locally call
`ngx_xrootd_cms_next_streamid` + `xrootd_pending_insert` (`../manager/pending.c`),
set the client session to `XRD_ST_WAITING_CMS`, and emit `ngx_xrootd_cms_send_locate`
...
`xrootd_send_redirect` + `xrootd_schedule_read_resume`.
...
in manager mode by consulting `xrootd_srv_select` over the registry; on a data
node by a kernel-confined `xrootd_stat_beneath` probe (`../path/beneath.h`).
...
- **Kernel-confined existence probe for `kYR_state`.** A data node answering
  `kYR_have` must use `xrootd_stat_beneath` against the persistent export rootfd
...
- **SSS registration is deferred until DONE.** When a keytab is configured,
  ...
  (`&P=sss`) and registers only after `xrootd_cms_srv_verify_xauth` succeeds
...
- **The two halves are independent modules.** The client is part of the main
  module; the server is `ngx_stream_xrootd_cms_srv_module`. A node can run
  either, both, or neither; `manager_mode` makes the client aggregate
  cluster-wide space (`xrootd_srv_aggregate_space`) into its own load reports.

## Entry points / extending

- **Add a handled inbound opcode (client side):** add a `CMS_RR_*` constant in
  `cms_internal.h`, then a `case` in `ngx_xrootd_cms_process_frame`
...
- **Add a CMS server directive:** add the field to
  `ngx_stream_xrootd_cms_srv_conf_t` (`server.h`), an `ngx_command_t` entry +
  setter in `server_module.c`, and a `ngx_conf_merge_*` line in
  `xrootd_cms_srv_merge_conf`.
...
- [../manager/README.md](../manager/README.md) — SHM server registry, blacklist, `xrootd_srv_select`, and the pending-locate table this subsystem wakes.
- [../read/README.md](../read/README.md) — `locate`/`open`/`stat` handlers that originate `kYR_locate` and consume `kXR_redirect`.
- [../path/README.md](../path/README.md) — `xrootd_stat_beneath` / RESOLVE_BENEATH confinement used by the `kYR_state` probe.
