...
brix_upstream_tls_handshake_done(ngx_connection_t *uconn)
{
    ...
    if (!uconn->ssl->handshaked) {
        brix_upstream_abort(up, "upstream: TLS handshake failed");
        return;
    }
    ...
    if (req == NULL) {
        brix_upstream_abort(up, "upstream: TLS re-login OOM");
        return;
    }
    ...
    if (brix_upstream_flush(up) == NGX_ERROR) {
        brix_upstream_abort(up, "upstream: TLS re-login send failed");
        return;
    }
    ...
    if (up->wbuf_pos < up->wbuf_len) {
        ...
        if (ngx_handle_write_event(uconn->write, 0) != NGX_OK) {
            brix_upstream_abort(up, "upstream: TLS write event arm failed");
        }
        return;
    }
    ...
    if (ngx_handle_read_event(uconn->read, 0) != NGX_OK) {
        brix_upstream_abort(up, "upstream: TLS read event arm failed");
    }
}
