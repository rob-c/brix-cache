/*
 * events_bootstrap_auth.c — kXR_auth credential sourcing and send machinery
 * for the upstream bootstrap conversation.
 *
 * Split verbatim from events_bootstrap.c: the auth-frame builder, effective
 * upstream-auth resolution, the shared auth-send plumbing, and the per-mechanism
 * arms (kXR_authmore path + login-sec hint path). The bootstrap state machine
 * that drives these lives in events_bootstrap.c and reaches the cross-file arms
 * through events_bootstrap_internal.h.
 */
