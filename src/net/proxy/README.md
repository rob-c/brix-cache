...
| `events_splice.c` (Linux) | Zero-copy fast path for plaintext `kXR_read`/`kXR_pgread` bodies: `xrootd_proxy_try_splice()` (gating + header send), `xrootd_proxy_splice_pump()` (upstream→pipe→client via a 1 MiB kernel pipe), client-writable handler, and post-splice byte accounting. **Self-healing fallback:** if `splice(socket→pipe)` reports `EAGAIN` while body bytes remain, the pump hands the remainder to the buffered `recv` relay (`xrootd_proxy_splice_to_buffered` → `xrootd_proxy_splice_fallback_finish`, relaying RAW since the header is already on the wire). This keeps large reads reliable on kernels whose socket-splice under-drains (e.g. WSL2 returns ~64 KB then `EAGAIN` with data still queued, which would otherwise crawl a 5 MiB read past the client timeout); the zero-copy path is still used whole when the body is already buffered. |
