# proxy — Transparent XRootD reverse proxy (`brix_proxy`)

## Overview

This subsystem turns nginx-xrootd into a **transparent reverse proxy** for an
existing XRootD storage cluster. When `brix_proxy on` is set in a `server {}`
...
(`brix_proxy_read_handler` / `brix_proxy_write_handler`) while the client
...
file-based token bridge — selected by `brix_proxy_auth` and per-endpoint
overrides on `brix_proxy_upstream`.
...
| `events_bootstrap.c` | `brix_proxy_handle_bootstrap()` — the bootstrap phase machine; on `kXR_authmore` or a `P=ztn` login hint, builds and sends `kXR_auth` (`ztn` bearer-token, SSS credential, or file-token) per the effective auth policy; transitions to IDLE and dispatches any queued request. |
| `events_splice.c` (Linux) | Zero-copy fast path for plaintext `kXR_read`/`kXR_pgread` bodies: `brix_proxy_try_splice()` (gating + header send), `brix_proxy_splice_pump()` (upstream→pipe→client via a 1 MiB kernel pipe), client-writable handler, and post-splice byte accounting. **Self-healing fallback:** if `splice(socket→pipe)` reports `EAGAIN` while body bytes remain, the pump hands the remainder to the buffered `recv` relay (`brix_proxy_splice_to_buffered` → `brix_proxy_splice_fallback_finish`, relaying RAW since the header is already on the wire). This keeps large reads reliable on kernels whose socket-splice under-drains (e.g. WSL2 returns ~64 KB then `EAGAIN` with data still queued, which would otherwise crawl a 5 MiB read past the client timeout); the zero-copy path is still used whole when the body is already buffered. |
| `forward_request.c` | `brix_proxy_forward_request()` — copy client header+payload, then per-opcode: translate file handles (`open`/`read`/`write`/`close`/`stat`/`readv`/`writev`/`clone`/...), rewrite paths, capture audit paths, trigger bound-secondary lazy-open, save a `kXR_wait` retry copy, queue+flush to upstream. |
| `forward_relay_response.c` | `brix_proxy_relay_to_client()` — the response state machine: lazy-open completion, `kXR_wait` absorb+retry, `kXR_redirect` follow-through, path-op audit, upstream→local fhandle translation, byte metrics, streaming `kXR_oksofar`/`kXR_status` partials, and final relay back to the client. Compiled standalone. |
...
| `forward_session_helpers.c` | `proxy_write_audit()` (per-handle close audit), `brix_proxy_wait_handler()` (retry timer), `brix_proxy_alloc_local_fh()` (free-slot scan), `brix_proxy_lazy_open()` (synthetic anonymous `kXR_open` for a bound-secondary handle published by its primary). |
| `pool.c` | Worker-local idle-connection pool (`brix_proxy_pool_get/put/init`) keyed by upstream index + auth type + bearer-token MD5; `pool_get` does a *random-start* health-aware scan, `pool_put` evicts the oldest when full (`BRIX_PROXY_MAX_IDLE_CONNECTIONS`); idle-timeout keepalive timers (re-arm without ping — a sent-but-unread ping would leave a stray `kXR_ok` in the socket); a pool-safe read handler (`brix_proxy_pool_read_handler`) that evicts on a stray event; per-upstream health tracking (`up_status_init`/`up_mark_failed`/`up_mark_ok`). |
...
- **`brix_proxy_bs_t`** — bootstrap sub-phase:
  `HANDSHAKE → PROTOCOL → LOGIN → AUTH → DONE`.
- **`brix_proxy_pooled_conn_t`** — a parked idle connection: the conn, its
...
**Steady state (forward / relay).** `brix_proxy_forward_request()`
...
`brix_proxy_relay_to_client()` (`forward_relay_response.c`), which performs
...
- **File handles are namespaced and must be translated both ways.** The client
  ...
  `BRIX_PROXY_FH_FREE` (`-1`) = free, `255` = open in flight (treated as "open"
...
- **TLS vs splice are mutually exclusive.** `splice()` bypasses the TLS layer, so
  `brix_proxy_try_splice()` declines if either side is `ssl != NULL`. Splice is
...
- **Transparency features the client must not observe:** `kXR_wait` is absorbed
  and the request silently retried via a timer (max `BRIX_PROXY_MAX_WAIT_RETRIES`,
  capped at `BRIX_PROXY_MAX_WAIT_SECS`); `kXR_redirect` is followed in-process
...
- **Use-after-free guards.** Pooled connections set `conn->data = pc` (not the
  ...
  install `brix_proxy_pool_read_handler` so a stray upstream event evicts
...
- **Handle a new opcode that carries file handles:** add a `case` in the switch
  in `brix_proxy_forward_request()` (`forward_request.c`) to translate the
  handle byte(s) with `proxy_translate_fh()` at the correct payload offset, and a
  matching translation/metrics branch in `brix_proxy_relay_to_client()`
