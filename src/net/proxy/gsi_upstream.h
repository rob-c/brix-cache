#ifndef XROOTD_PROXY_GSI_UPSTREAM_H
#define XROOTD_PROXY_GSI_UPSTREAM_H

/*
 * gsi_upstream.h — Phase-4b GSI X.509 delegation for the terminating tap proxy.
 *
 * WHAT: when xrootd_tap_proxy_auth gsi is set, the proxy presents the client's
 ...
 */
int xrootd_proxy_gsi_write_pem_temp(const unsigned char *pem, size_t len,
    char *out, size_t cap);

/* The async GSI-login entry (xrootd_proxy_gsi_connect_async) is declared in
 * proxy_internal.h, where the proxy + nginx stream types are in scope. */

#endif /* XROOTD_PROXY_GSI_UPSTREAM_H */
