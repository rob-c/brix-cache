#ifndef XROOTD_CMS_FRAME_IO_H
#define XROOTD_CMS_FRAME_IO_H
 ...
 *
 * HOW: xrootd_cms_send_all() loops c->send() until len bytes are transmitted;
 *      returns NGX_AGAIN on partial write (caller retries), NGX_ERROR on failure,
 *      NGX_OK on full success. xrootd_cms_send_frame() constructs the header
 *      via ngx_xrootd_cms_put32/put16 from cms_internal.h then dispatches both
 *      header and payload through send_all(). */

ngx_int_t xrootd_cms_send_all(ngx_connection_t *c, const u_char *buf,
    size_t len);
ngx_int_t xrootd_cms_send_frame(ngx_connection_t *c, uint32_t streamid,
    u_char code, u_char modifier, const u_char *payload, size_t payload_len);

#endif /* XROOTD_CMS_FRAME_IO_H */
