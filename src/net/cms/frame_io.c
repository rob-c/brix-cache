...
/*
 * xrootd_cms_send_all — send a contiguous buffer over an nginx connection.
 *
 ...
 */
ngx_int_t
xrootd_cms_send_all(ngx_connection_t *c, const u_char *buf, size_t len)
{
    size_t sent;
...
/*
 * xrootd_cms_send_frame — build and transmit a CMS wire frame.
 *
 ...
 *      streamid via ngx_xrootd_cms_put32(), code/modifier as raw bytes,
 *      payload_len via ngx_xrootd_cms_put16(). Send header first; if
 *      payload_len > 0 send payload second. Returns NGX_OK on full success,
 *      NGX_ERROR on any xrootd_cms_send_all failure.
 */
ngx_int_t
xrootd_cms_send_frame(ngx_connection_t *c, uint32_t streamid, u_char code,
    u_char modifier, const u_char *payload, size_t payload_len)
{
    ...
    if (xrootd_cms_send_all(c, hdr, sizeof(hdr)) != NGX_OK) {
        return NGX_ERROR;
    }

    if (payload_len > 0
        && xrootd_cms_send_all(c, payload, payload_len) != NGX_OK)
    {
        return NGX_ERROR;
    }
