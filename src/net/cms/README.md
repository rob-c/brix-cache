...
| `cns.c` / `cns.h` | Composite Cluster Name Space inventory + event codec (§6): data servers report namespace mutations; the manager keeps a path→{size,mtime,server} inventory (v1 per-worker linear table). Ops: `ADD`/`DEL`/`MKDIR`/`RMDIR` plus the phase-97 two-path `MV`, whose payload appends `name2_len[2] + newpath` after the source path and is subtree-aware at apply time (`brix_cns_inv_rename`) — a rename carries every recorded child with it, which a `DEL`+`ADD` pair cannot. `CMS_RR_CNS` (40) is private to BriX peers, so a pre-MV manager ignores the tail and then rejects the unknown op as a no-op. |
| `cns_emit.c` / `cns_emit.h` | The single seam every plane reports a mutation through. `brix_cns_emit`/`brix_cns_emit_rename` take the `stream{}` server conf directly (the `root://` plane holds it); `brix_cns_emit_active`/`brix_cns_emit_at`/`brix_cns_emit_rename_at` are the phase-97 §5 entry points for WebDAV, S3 and gridftp, which have no path to that conf and so resolve the emitting block from `ngx_cycle` on demand — the same walk `core/config/process.c` does at worker init, caching nothing, so there is no global and nothing to invalidate on reload. `root_canon` is always the **caller's** export root, because an `http{}` location resolves against its own `brix_export`. `cns_emit_send()` fans one encoded event out to **every** logged-in manager link, deliberately not `ngx_brix_cms_pick_ctx()`: rotation is right for a locate (any manager can answer) and wrong for a mutation, where each manager keeps its own inventory. |
