...
/*
 * ngx_brix_cms_send_frame — thin wrapper around brix_cms_send_frame().
 *
...
static ngx_int_t
ngx_brix_cms_send_frame(ngx_brix_cms_ctx_t *ctx, uint32_t streamid,
    u_char code, u_char modifier, const u_char *payload, size_t payload_len)
{
    return brix_cms_send_frame(ctx->connection, streamid, code, modifier,
                                 payload, payload_len);
}
    ...
    uint32_t ecode, const char *text)
{
    ...
    ngx_brix_cms_put32(buf, ecode);
    if (tlen > 0) {
        ngx_memcpy(buf + 4, text, tlen);
    }
    buf[4 + tlen] = '\0';
    return ngx_brix_cms_send_frame(ctx, streamid, CMS_RSP_ERROR, 0,
                                     buf, 4 + tlen + 1);
}
...
ngx_brix_cms_send_login(ngx_brix_cms_ctx_t *ctx)
{
     ...
     * (e.g. "all.export / r/w" -> "w /").  brix_cms_paths may carry several
     * colon-separated namespace prefixes ("/data:/atlas").
     */
    paths = ngx_brix_cms_export_paths(ctx->conf);
    ...
    free_mb = 0;
    util_pct = 0;
    (void) ngx_brix_cms_stat_space(ctx->conf, &total_gb, &free_mb,
                                     &util_pct);

    if (ctx->conf->manager_mode) {
        uint32_t agg_free = 0, agg_util = 0;
        brix_srv_aggregate_space(&agg_free, &agg_util);
...
/*
 * ngx_brix_cms_send_status — announce traffic state after login.
 *
...
ngx_int_t
ngx_brix_cms_send_status(ngx_brix_cms_ctx_t *ctx)
{
    return ngx_brix_cms_send_frame(ctx, 0, CMS_RR_STATUS,
                                     CMS_ST_RESUME | CMS_ST_NOSTAGE, NULL, 0);
}

/*
 * ngx_brix_cms_send_load — periodic load heartbeat report.
 *
 ...
 *      here. HOW: Call stat_space() for free_mb (aggregate via brix_srv_aggregate_space
 *      if manager mode), pack payload as put_short(6) + 6 zero bytes + put_int(free_mb),
 *      send with CMS_RR_LOAD code. */

ngx_int_t
ngx_brix_cms_send_load(ngx_brix_cms_ctx_t *ctx)
{
    u_char    payload[32];
    u_char   *payload_cursor;
    uint32_t  free_mb;
    ...
    return ngx_brix_cms_send_frame(ctx, 0, CMS_RR_LOAD, 0, payload,
                                     (size_t) (payload_cursor - payload));
}

/*
 * ngx_brix_cms_send_avail — availability reply to CMS SPACE request.
 *
...
ngx_int_t
ngx_brix_cms_send_avail(ngx_brix_cms_ctx_t *ctx, uint32_t streamid)
{
    u_char    payload[16];
    u_char   *payload_cursor;
    uint32_t  free_mb;
    ...
    payload_cursor = payload;
    payload_cursor = ngx_brix_cms_put_int(payload_cursor, free_mb);
    payload_cursor = ngx_brix_cms_put_int(payload_cursor, util_pct);

    return ngx_brix_cms_send_frame(ctx, streamid, CMS_RR_AVAIL, 0, payload,
                                     (size_t) (payload_cursor - payload));
}

/*
 * ngx_brix_cms_send_pong — empty reply to CMS PING probe.
 *
...
ngx_int_t
ngx_brix_cms_send_pong(ngx_brix_cms_ctx_t *ctx, uint32_t streamid)
{
    return ngx_brix_cms_send_frame(ctx, streamid, CMS_RR_PONG, 0, NULL, 0);
}
...
/*
 * ngx_brix_cms_send_locate — client-side locate request to CMS manager.
 *
 ...
 * HOW: Copy path (NUL-terminated) into payload buffer bounded by BRIX_SRV_MAX_PATHS,
 *      send with CMS_RR_LOCATE code and the originating streamid. */

ngx_int_t
ngx_brix_cms_send_locate(ngx_brix_cms_ctx_t *ctx,
    uint32_t streamid, const char *path)
{
    u_char  payload[BRIX_SRV_MAX_PATHS];
    size_t  plen;
    ...
    return ngx_brix_cms_send_frame(ctx, streamid, CMS_RR_LOCATE, 0,
                                     payload, plen);
}

/*
 * ngx_brix_cms_send_have — tell the manager this node holds <path>.
 *
...
ngx_int_t
ngx_brix_cms_send_have(ngx_brix_cms_ctx_t *ctx, uint32_t streamid,
