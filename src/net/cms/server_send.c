...
/* xrootd_cms_srv_send_ping — send a header-only kYR_ping (streamid=0) to a
...
/* xrootd_cms_srv_send_xauth — send the kYR_xauth security challenge (the manager
...
/* xrootd_cms_srv_send_pong — answer a node's liveness kYR_ping with a header-only
 * kYR_pong (streamid=0), matching stock cmsd do_Ping's static CmsPongRequest. */
ngx_int_t
xrootd_cms_srv_send_pong(xrootd_cms_srv_ctx_t *ctx)
{
    return xrootd_cms_srv_send_frame(ctx, 0, CMS_RR_PONG, 0, NULL, 0);
}

/* xrootd_cms_srv_send_disc — echo a header-only kYR_disc back to a node that asked
 * to disconnect, before the manager closes the link (stock cmsd do_Disc). */
ngx_int_t
xrootd_cms_srv_send_disc(xrootd_cms_srv_ctx_t *ctx)
{
    return xrootd_cms_srv_send_frame(ctx, 0, CMS_RR_DISC, 0, NULL, 0);
}

/* xrootd_cms_srv_send_status — answer a kYR_update with a header-only kYR_status
...
ngx_int_t
xrootd_cms_srv_send_status(xrootd_cms_srv_ctx_t *ctx, u_char modifier)
{
    return xrootd_cms_srv_send_frame(ctx, 0, CMS_RR_STATUS, modifier, NULL, 0);
}

/* xrootd_cms_srv_send_data — reply CMS_RSP_DATA with an opaque payload, echoing the
 * request streamid; byte-exact with cmsd's do_StatFS/do_Stats data replies (Plane A
 * query path). */
ngx_int_t
xrootd_cms_srv_send_data(xrootd_cms_srv_ctx_t *ctx, uint32_t streamid,
    const u_char *payload, size_t len)
{
    return xrootd_cms_srv_send_frame(ctx, streamid, CMS_RSP_DATA, 0,
                                     payload, len);
}
