...
static ngx_int_t
rl_reject(ngx_http_request_t *r, xrootd_rl_rule_t *rule, uint32_t wait_sec,
    const char *key_str)
{
    XROOTD_RL_METRIC_INC(rl_throttled_http_total);

    if (!rule->nodelay && wait_sec > 0) {
        char buf[NGX_INT_T_LEN + 1];
        ngx_snprintf((u_char *) buf, sizeof(buf), "%ud%Z", wait_sec);
        (void) xrootd_http_set_header(r, "Retry-After", buf, NULL);
    }
...
ngx_int_t
xrootd_rl_http_access_handler(ngx_http_request_t *r)
{
     ...
     * crash, see xrootd_rl_http_log_handler).  Rate limiting does not apply to
    ...
    for (i = 0; i < lcf->rl_rules->nelts; i++) {
        ...
        if (rules[i].key_type == XROOTD_RL_KEY_VOLUME && !have_path) {
            if (ngx_http_xrootd_webdav_resolve_path(r, lcf->common.root_canon,
        ...
        if (rules[i].req_rate > 0) {
            if (xrootd_rl_check(&rules[i], key_str, &wait_sec) == NGX_AGAIN) {
        ...
        if (rules[i].bw_rate > 0) {
            if (xrootd_rl_bw_check(&rules[i], key_str, &wait_sec) == NGX_AGAIN) {
                return rl_reject(r, &rules[i], wait_sec, key_str);
            }
            ...
            if (wctx == NULL) {
                ...
                if (wctx != NULL) {
                    ngx_http_set_ctx(r, wctx, ngx_http_xrootd_webdav_module);
                }
            }
            if (wctx != NULL) {
        ...
        if (rules[i].req_conc > 0 && !conc_held) {
            if (xrootd_rl_conc_acquire(&rules[i], key_str) == NGX_AGAIN) {
            ...
            if (wctx != NULL) {
                ...
                xrootd_rl_conc_release(&rules[i], key_str);
            }
        }
    }
...
ngx_int_t
xrootd_rl_http_log_handler(ngx_http_request_t *r)
{
    ngx_http_xrootd_webdav_loc_conf_t *lcf;
    ngx_http_xrootd_webdav_req_ctx_t  *wctx;
    off_t                              nbytes;
     ...
     * garbage rule pointer (observed SIGSEGV in xrootd_rl_conc_release).  The
    ...
    if (wctx->rl_conc_rule != NULL && wctx->rl_conc_key[0] != '\0') {
        xrootd_rl_conc_release((xrootd_rl_rule_t *) wctx->rl_conc_rule,
                               wctx->rl_conc_key);
    ...
    if (nbytes > 0) {
        xrootd_rl_charge_bytes((xrootd_rl_rule_t *) wctx->rl_bw_rule,
