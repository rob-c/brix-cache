 ...
 *
 * Each `brix_rate_limit_zone zone=NAME:SIZE` directive declares one nginx
 * shared-memory slab zone holding an rbtree of brix_rl_node_t plus an LRU
...
static ngx_uint_t        brix_rl_zone_tag;
static brix_rl_zone_t *brix_rl_zones[BRIX_RL_MAX_ZONES];
static ngx_uint_t        brix_rl_nzones;
 ...
 * brix_rl_lookup_locked() walks the tree with the identical key-then-bytes
 * comparison, so the two MUST stay in lock-step.
 */
static void
brix_rl_rbtree_insert_value(ngx_rbtree_node_t *temp,
    ngx_rbtree_node_t *node, ngx_rbtree_node_t *sentinel)
{
    ngx_rbtree_node_t **p;
    brix_rl_node_t   *rln, *rlnt;
    ...
    for ( ;; ) {
        if (node->key < temp->key) {
            ...
            rln  = (brix_rl_node_t *) node;
            rlnt = (brix_rl_node_t *) temp;
 ...
 */
void
brix_rl_zone_reset_gauges(brix_rl_shctx_t *sh)
{
    ngx_queue_t      *q;
    brix_rl_node_t *rln;
...
static ngx_int_t
brix_rl_init_zone(ngx_shm_zone_t *shm_zone, void *data)
{
    brix_rl_zone_t *ozone = data;            /* previous (reload) */
    brix_rl_zone_t *zone  = shm_zone->data;
    ...
    if (ozone != NULL) {
        ...
        brix_rl_zone_reset_gauges(zone->sh);
        return NGX_OK;
    }
    ...
    zone->sh = ngx_slab_alloc(shpool, sizeof(brix_rl_shctx_t));
    if (zone->sh == NULL) {
        return NGX_ERROR;
    }
    shpool->data = zone->sh;
...
ngx_int_t
brix_rl_zone_add(ngx_conf_t *cf, ngx_str_t *name, size_t size,
    brix_rl_zone_t **out)
{
    brix_rl_zone_t *zone;
    ...
    zone = brix_rl_zone_get(name);
    if (zone != NULL) {
        if (out) { *out = zone; }
        return NGX_OK;
    }

    if (brix_rl_nzones >= BRIX_RL_MAX_ZONES) {
        ngx_conf_log_error(NGX_LOG_EMERG, cf, 0,
                           "too many brix_rate_limit_zone zones (max %d)",
                           BRIX_RL_MAX_ZONES);
        return NGX_ERROR;
    }
    if (size < BRIX_RL_MIN_SIZE) {
        size = BRIX_RL_MIN_SIZE;
    }

    BRIX_PCALLOC_OR_RETURN(zone, cf->pool, sizeof(*zone), NGX_ERROR);
    zone->name = *name;
    zone->size = size;
    ...
    shm_zone = ngx_shared_memory_add(cf, name, size, &brix_rl_zone_tag);
    if (shm_zone == NULL) {
        return NGX_ERROR;
    }
    if (shm_zone->data != NULL) {
        ...
        shm_zone->init = brix_rl_init_zone;
        shm_zone->data = zone;
    }
    zone->shm_zone = shm_zone;

    brix_rl_zones[brix_rl_nzones++] = zone;
    if (out) { *out = zone; }
    return NGX_OK;
}

brix_rl_zone_t *
brix_rl_zone_get(ngx_str_t *name)
{
    ngx_uint_t i;

    for (i = 0; i < brix_rl_nzones; i++) {
        if (brix_rl_zones[i]->name.len == name->len
            && ngx_strncmp(brix_rl_zones[i]->name.data, name->data,
                           name->len) == 0)
        {
            return brix_rl_zones[i];
        }
    }
    return NULL;
}
...
ngx_uint_t
brix_rl_zones_all(brix_rl_zone_t **out, ngx_uint_t max)
{
    ngx_uint_t i, n = 0;

    for (i = 0; i < brix_rl_nzones && n < max; i++) {
        out[n++] = brix_rl_zones[i];
    }
    return n;
}
...
brix_rl_node_t *
brix_rl_lookup_locked(brix_rl_zone_t *zone, uint32_t hash,
    const char *key_str, size_t len)
{
    ...
    /* Mirror of brix_rl_rbtree_insert_value's ordering: branch on hash, and
...
static int
brix_rl_evict_oldest_locked(brix_rl_zone_t *zone)
{
    ngx_queue_t      *q;
    brix_rl_node_t *rln;
    ...
    BRIX_RL_METRIC_INC(rl_eviction_total);
    return 1;
}
...
brix_rl_node_t *
brix_rl_create_locked(brix_rl_zone_t *zone, uint32_t hash,
    const char *key_str, size_t len)
{
    brix_rl_node_t *rln;
    ...
    size_t            sz = offsetof(brix_rl_node_t, key_str) + len;
    int               tries;
