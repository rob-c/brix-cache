...
token bucket of the Phase 20 `brix_rate_limit` directive. It enforces
...
(`brix_rate_limit_zone`). Each zone holds an rbtree of per-principal
...
note matters: this is the `brix_rate_limit_zone` / `_rule` / `_bandwidth` /
`_concurrency` family — unrelated to and unchanged by the Phase 20
`brix_rate_limit` directive.

## Files

| File | Responsibility |
|---|---|
| `ratelimit.h` | Public API + all data structures: `brix_rl_key_type_t` enum, `brix_rl_node_t` (per-principal bucket), `brix_rl_shctx_t` (shared rbtree+LRU), `brix_rl_zone_t` (zone handle), `brix_rl_rule_t` (one configured rule), `brix_rl_snapshot_entry_t` (dashboard row). Declares the zone/core/key/HTTP/stream entry points and documents the `× 1000` request-unit convention. |
| `ratelimit.c` | Leaky-bucket core. `brix_rl_check` (request rate), `brix_rl_bw_check` (bandwidth pre-check) + `brix_rl_charge_bytes` (post-send charge), `brix_rl_conc_acquire`/`_release` (W7 in-flight cap), `brix_rl_snapshot` (dashboard rows, sorted by throttle count). All take `shpool->mutex` internally. |
| `ratelimit_zone.c` | SHM zone lifecycle: FNV-1a32 `brix_rl_hash`, rbtree insert/lookup, LRU eviction, slab alloc. `brix_rl_zone_add`/`_get`/`_zones_all` (config-time registry of up to 16 zones), `brix_rl_lookup_locked`/`_create_locked` (node CRUD; caller holds `shpool->mutex`), `brix_rl_init_zone` (zone-init callback with reload inheritance). |
| `ratelimit_keys.c` | Identity → `"<type>:<value>"` key string for both planes (`brix_rl_key_stream`, `brix_rl_key_http`), DN hashing (`rl_key_dn_hash`), directive value parsers (`rl_parse_key`/`_req_rate`/`_bw_rate`/`_size`), and the directive setters (`brix_rl_zone_directive`, `_rule_directive`, `_bw_directive`, `_conc_directive`) shared by the HTTP and stream command tables via a single `rl_add_rule` builder. |
...
- **`brix_rl_node_t`** — one slab-allocated per-principal bucket. `node.key`
  ...
  inside the SHM zone.
- **`brix_rl_shctx_t`** — the shared `{rbtree, sentinel, queue}` at the head
  of the slab pool; `queue` head = most-recently-used.
- **`brix_rl_zone_t`** — runtime handle (resolved at config time): `sh`
  (shared ctx, set in the zone init callback), `shpool`, `shm_zone`, `name`,
  `size`. Up to `BRIX_RL_MAX_ZONES` (16) tracked in a static registry in
  `ratelimit_zone.c`.
- **`brix_rl_rule_t`** — one configured rule: `key_type` + `key_match`
...
- **`brix_rl_snapshot_entry_t`** — flat dashboard row copied out under the
...
brix_rate_limit_zone   zone=vo_limits:10m;

# 2. Request-rate rule (req/s). burst defaults to 1; nodelay = reject now
#    instead of kXR_wait/Retry-After.
brix_rate_limit_rule   zone=vo_limits key=vo rate=500r/s burst=800 [nodelay];

# 3. Bandwidth rule (bytes/s, k|m|g suffix). burst defaults to 1s of rate.
brix_bandwidth_limit   zone=vo_limits key=volume:/store/tape rate=50m/s burst=100m;
...
**Config time.** `brix_rate_limit_zone` (setter `brix_rl_zone_directive`)
declares an SHM slab zone via `brix_rl_zone_add` → `ngx_shared_memory_add`
under the shared tag. `brix_rate_limit_rule` / `brix_bandwidth_limit` /
`brix_concurrency_limit` push `brix_rl_rule_t`s into a rules array stored at
...
`ngx_http_brix_webdav_resolve_path`, see `../path/`), runs the three checks,
...
- **Low-cardinality keys only.** Keys are dimension prefixes
  ...
  whole key is capped at `BRIX_RL_KEY_LEN` (128 bytes).
...
- **W7 concurrency slot pairing differs by plane.** HTTP reserves **one slot
  ...
  released exactly once in `brix_on_disconnect` — so the stream cap is
...
- **Cross-plane zone sharing via the shared tag.** Same NAME in `http{}` and
  ...
  to `BRIX_RL_MIN_SIZE` (64 KiB); a zone-not-found at rule parse time is a
  hard config error. Reload inherits the already-initialised shared structure
  (`brix_rl_init_zone`), so live buckets survive `nginx -s reload`.
- **VOLUME rules are path-dependent and uncacheable.** Stream key caching skips
  VOLUME rules and rules beyond `BRIX_RL_RULE_CACHE_MAX`; the gate only copies
...
- **Add a key dimension:** extend `brix_rl_key_type_t` (`ratelimit.h`), add a
  `case` in both `brix_rl_key_stream` and `brix_rl_key_http`
