...
**Client redirect (read side):** a stream `kXR_open` reaches `../read/open_request.c`. In manager mode it (1) checks `brix_manager_tried_exhausted()` to honour the `tried/triedrc` retry protocol and answer `kXR_NotFound` once the client has visited every candidate; (2) consults `brix_redir_cache_lookup()`; (3) on a miss calls `brix_srv_select()` and, on success, caches the result via `brix_redir_cache_insert()` and emits a redirect; (4) when resolution must be delegated upward, `brix_pending_insert()` parks the client and a `kXR_locate` is forwarded to the upstream CMS via `../cms/send.c`. `../read/locate.c` follows the same cache → select → pending sequence and can return the full server set via `brix_srv_locate_all()` (lateral redirect). When `../cms/recv.c` receives the upstream `kXR_select`, it `brix_pending_lookup()`s the slot, writes the redirect host/port, wakes the waiting connection, and `brix_pending_remove()`s the slot.
...
**Calls out to:** `../cms/` (registration/heartbeat producers, `kXR_select` consumer), `../read/` (`open_request.c`, `locate.c`, `stat.c`), `../config/` (zone setup + timer start), `../upstream/` (`brix_upstream_build_bootstrap` reused by the probe), `../compat/net_target.h` (`brix_net_host_chars_valid`) and `../compat/shm_slots.h` (free-slot/expiry helpers), `../metrics/` (`registry_full_total`, `hc_*` counters), and is read by `../dashboard/` (`api.c`, `api_admin.c` via `brix_srv_snapshot`/`brix_srv_undrain`), `../metrics/cluster.c`, and `../cache/` (`evict_policy.c`, `thread.c` via `brix_srv_unregister_path`).
...
- **Add a registry field:** extend `brix_srv_entry_t` (and, if it must be exported, `brix_srv_snapshot_entry_t`) in `registry.h`, populate it under the spinlock in `brix_srv_register`/`_update_load`, copy it in `brix_srv_snapshot`, and surface it in `../dashboard/api.c` / `../metrics/cluster.c`. Bump nothing else — the zone size is derived from `sizeof(brix_srv_entry_t)`.
...
- `../read/README.md` — `open_request.c` / `locate.c` / `stat.c`: the stream handlers that call `brix_srv_select` / `_locate_all` / the redirect cache / pending table.
...
- `../dashboard/README.md` and `../metrics/README.md` — consumers of `brix_srv_snapshot` / `brix_srv_undrain` and the cluster/health counters.
