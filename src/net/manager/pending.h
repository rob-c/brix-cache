#ifndef BRIX_MANAGER_PENDING_H
#define BRIX_MANAGER_PENDING_H
...
#define BRIX_PENDING_LOCATE_SLOTS  32   /* concurrent in-flight locates */
 ...
 */
ngx_int_t brix_pending_insert(uint32_t streamid, ngx_pid_t worker_pid,
 ...
 * and redir_port.  The caller MUST call brix_pending_unlock() afterwards.
 * Returns NULL if not found.
 */
brix_pending_locate_t *brix_pending_lookup(uint32_t streamid,
    ngx_pid_t worker_pid);

/* Release the lock acquired by brix_pending_lookup(). */
void brix_pending_unlock(void);
 ...
 */
ngx_uint_t brix_pending_reap_expired(void);

/* A4: how often the worker-0 reaper sweeps (ms). */
#define BRIX_PENDING_REAP_INTERVAL_MS  5000

#endif /* BRIX_MANAGER_PENDING_H */
