 ...
 *
 * Off by default.  When `brix_mirror_url http[s]://host:port` is configured on
 * a WebDAV location, a qualifying READ request (GET/HEAD/PROPFIND/OPTIONS, per
 * brix_mirror_methods) spawns one fire-and-forget background subrequest per
 ...
 *   - brix_http_mirror_precontent_handler (NGX_HTTP_PRECONTENT_PHASE) does
 ...
 *   - brix_http_mirror_log_handler (NGX_HTTP_LOG_PHASE) records the primary's
 *     final status for the divergence comparison.
 * Both are registered from src/webdav/postconfig.c.
 */
#ifndef BRIX_MIRROR_HTTP_MIRROR_H
#define BRIX_MIRROR_HTTP_MIRROR_H
...
ngx_int_t brix_http_mirror_precontent_handler(ngx_http_request_t *r);
...
ngx_int_t brix_http_mirror_log_handler(ngx_http_request_t *r);
...
ngx_int_t brix_http_mirror_setup(ngx_conf_t *cf,
    ngx_http_brix_webdav_loc_conf_t *conf,
    ngx_http_brix_webdav_loc_conf_t *prev);

/* Directive setters (registered in src/webdav/module.c). */

/* `brix_mirror_url <http[s]://host[:port][/path]>` setter: parses and resolves
 ...
 * BRIX_MIRROR_MAX_TARGETS.  Returns NGX_CONF_OK, or NGX_CONF_ERROR on a bad
 * scheme, unresolvable host, the target cap, or allocation failure. */
char *brix_http_mirror_set_url(ngx_conf_t *cf, ngx_command_t *cmd, void *conf);

/* `brix_mirror_methods <METHOD>...` setter: OR-parses the listed HTTP/WebDAV
 * method names into conf->mirror.method_mask, REPLACING any prior value (not
 * additive).  Write methods still require brix_mirror_writes to actually
 * mirror.  Returns NGX_CONF_OK, or NGX_CONF_ERROR on an unrecognized method. */
char *brix_http_mirror_set_methods(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);

#endif /* BRIX_MIRROR_HTTP_MIRROR_H */
