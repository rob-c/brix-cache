...
#
#   1 a fill from the "RAL" mock attributes to xrootd_cvmfs_upstream_*{upstream=RAL}
...
#   4 xrootd_cvmfs_trace on: the client-op AND upstream-request lines appear at INFO
...
http { access_log off;
    server { listen 127.0.0.1:$CPORT;
        location /cvmfs/ {
            xrootd_cvmfs_storage_backend "$1";
            xrootd_cvmfs_cache_store posix:$PFX/cache;
            xrootd_cvmfs on;
${2:-}
        } }
    server { listen 127.0.0.1:$XPORT; location = /metrics { xrootd_metrics on; } }
}
EOF
}
restart() {
...
RAL="127.0.0.1:$MRAL"
R="$(mval "xrootd_cvmfs_upstream_requests_total{upstream=\"$RAL\"}")"
F="$(mval "xrootd_cvmfs_upstream_fills_total{upstream=\"$RAL\"}")"
B="$(mval "xrootd_cvmfs_upstream_origin_bytes_total{upstream=\"$RAL\"}")"
...
HC="$(mval "xrootd_cvmfs_upstream_fill_duration_seconds_count{upstream=\"$RAL\"}")"
HB="$(mval "xrootd_cvmfs_upstream_fill_duration_seconds_bucket{upstream=\"$RAL\",le=\"+Inf\"}")"
curl -s "http://127.0.0.1:$XPORT/metrics" \
    | grep -q "xrootd_cvmfs_upstream_fill_duration_seconds_sum{upstream=\"$RAL\"}" \
...
if curl -s "http://127.0.0.1:$XPORT/metrics" \
     | grep -E "xrootd_cvmfs_upstream_.*upstream=\"[^\"]*(/|data/|\.cvmfs)" >/dev/null; then
...
ALT="127.0.0.1:$MALT"
FO="$(mval "xrootd_cvmfs_upstream_failovers_total{upstream=\"$ALT\"}")"
FA="$(mval "xrootd_cvmfs_upstream_fills_total{upstream=\"$ALT\"}")"
...
mkconf "http://127.0.0.1:$MRAL" "            xrootd_cvmfs_trace on;" info
restart
curl -s "http://127.0.0.1:$CPORT$OBJ0" -o /dev/null
sleep 0.3
