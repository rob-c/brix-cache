...
def _guard_test_cache_s3_origin_flow_1():
    if not os.access(NGINX_BIN, os.X_OK):
        pytest.skip(f"nginx binary not executable: {NGINX_BIN}")

def _guard_test_cache_s3_origin_flow_2(results):
...
def test_cache_s3_origin_flow(tmp_path):
    _guard_test_cache_s3_origin_flow_1()

    results = run_checks(tmp_path, nginx_bin=NGINX_BIN)
    _guard_test_cache_s3_origin_flow_2(results)
