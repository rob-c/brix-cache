...
and then rebinds its own entry in :data:`sys.modules` to :mod:`brix_suite.catalogue.values`, so
``fleet_values`` and :mod:`brix_suite.catalogue.values` are ONE module object.
...
import brix_suite.catalogue.values as _canonical
