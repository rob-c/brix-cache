...
pytestmark = [pytest.mark.uses_lifecycle_harness,
              pytest.mark.xdist_group("lc-xrddiag-probe")]
...
@pytest.fixture()
def anon_server(lifecycle, _client_built, tmp_path):
    ...
    ep = lifecycle.start(NginxInstanceSpec(
        name="lc-xrddiag-probe",
        template="nginx_lc_stream_posix_anon.conf",
        protocol="root",
        template_values={"BIND_HOST": BIND_HOST, "DATA_DIR": str(data)},
        reason="xrddiag probe-robustness against a writable anon root server"))
    return ep.port, ep.config
