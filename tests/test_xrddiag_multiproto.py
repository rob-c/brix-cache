...
stream {{
    server {{ listen {BIND_HOST}:{pr}; xrootd on; xrootd_root {data}; xrootd_auth none; }}
}}
http {{
    access_log off;
    ...
    server {{ listen {BIND_HOST}:{ph};
             location / {{ xrootd_webdav on; xrootd_webdav_root {data}; xrootd_webdav_auth none; }} }}
    server {{ listen {BIND_HOST}:{ps} ssl;
             ssl_certificate {cert}; ssl_certificate_key {key};
             location / {{ xrootd_webdav on; xrootd_webdav_root {data}; xrootd_webdav_auth none; }} }}
    server {{ listen {BIND_HOST}:{p3};
             location / {{ xrootd_s3 on; xrootd_s3_root {data}; }} }}
}}
""")
