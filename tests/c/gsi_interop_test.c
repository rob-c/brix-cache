...
test_iv_is_load_bearing(void)
{
    ...
    if (!brix_gsi_cipher_lookup("aes-128-cbc", &c)) {
...
test_cipher_negotiation(void)
{
    ...
    CHECK(strncmp(brix_gsi_cipher_default_list(), "aes-128-cbc", 11) == 0,
          "default cipher list must start with aes-128-cbc");

    /* aes-128-cbc resolves to a 16/16 cipher. */
    CHECK(brix_gsi_cipher_lookup("aes-128-cbc", &c)
    ...
    CHECK(!brix_gsi_cipher_lookup("rc4", &c)
          && !brix_gsi_cipher_lookup("totally-bogus-cbc", &c),
          "unknown cipher names must be rejected by the allowlist");

    /* Pick skips an unknown leading token and selects a supported one. */
    name[0] = '\0';
    CHECK(brix_gsi_cipher_pick("bogus-cbc:aes-128-cbc", &c, name)
    ...
    name[0] = '\0';
    CHECK(brix_gsi_cipher_pick("aes-256-cbc:aes-128-cbc", &c, name)
          && c.key_len == 32,
...
test_bucket_terminator(void)
{
    ...
    brix_gbuf_init(&g);
    brix_gbuf_start(&g, (uint32_t) kXGC_cert);
    brix_gbuf_bucket(&g, (uint32_t) kXRS_x509, pem, strlen(pem));
    len_no_term = g.len;
    brix_gbuf_end(&g);
    ...
    CHECK(brix_gsi_find_bucket(g.p, g.len, (uint32_t) kXRS_x509,
                                 &found, &found_len) == 0
