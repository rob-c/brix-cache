...
static void
roundtrip_one(xrootd_codec_id_t id, const char *name, size_t n,
              size_t in_chunk, size_t out_chunk)
{
    ...
    rc = run_codec(id, XROOTD_CODEC_DIR_COMPRESS, orig, n, NULL,
...
static void
test_codec(xrootd_codec_id_t id, const char *name)
{
...
test_bomb_guard(void)
{
    ...
    if (!xrootd_codec_available(XROOTD_CODEC_GZIP)) { free(zeros); return; }
    ...
    CHECK(rc == XROOTD_CODEC_ERR_BOMB, "bomb out_cap guard rc=%d (want %d)",
    ...
    rc = run_codec(XROOTD_CODEC_GZIP, XROOTD_CODEC_DIR_DECOMPRESS, comp, complen, &g,
                   &plain, &plainlen, 1 << 16, 1 << 16);
    CHECK(rc == XROOTD_CODEC_ERR_BOMB, "bomb ratio guard rc=%d", rc);
    free(plain);
...
test_lookups(void)
{
    CHECK(xrootd_codec_by_http_token("br", 2) != NULL
          && xrootd_codec_by_http_token("br", 2)->id == XROOTD_CODEC_BROTLI, "token br");
    CHECK(xrootd_codec_by_http_token("gzip", 4)->id == XROOTD_CODEC_GZIP, "token gzip");
    CHECK(xrootd_codec_by_name("zstd", 4)->id == XROOTD_CODEC_ZSTD, "name zstd");
    CHECK(xrootd_codec_by_name("xz", 2)->id == XROOTD_CODEC_XZ, "name xz");
    CHECK(xrootd_codec_by_http_token("lzip", 4) == NULL, "unknown token -> NULL");
    CHECK(xrootd_codec_by_name("gz", 2) == NULL, "partial name no match");
    CHECK(xrootd_codec_by_id(XROOTD_CODEC_MAX) == NULL, "id bounds");
    printf("  ok   lookups\n");
}
...
test_identity(void)
{
    ...
    rc = run_codec(XROOTD_CODEC_IDENTITY, XROOTD_CODEC_DIR_COMPRESS, in, sizeof(in),
...
test_corrupt(void)
{
    ...
    if (!xrootd_codec_available(XROOTD_CODEC_XZ)) { return; }
    fill_pattern(in, sizeof(in), 7);
    rc = run_codec(XROOTD_CODEC_XZ, XROOTD_CODEC_DIR_COMPRESS, in, sizeof(in), NULL,
                   &comp, &complen, 1 << 16, 1 << 16);
    CHECK(rc == 0 && complen > 40, "corrupt setup rc=%d", rc);
    if (rc == 0) {
        comp[complen / 2] ^= 0xff;            /* flip a byte mid-stream */
        comp[complen / 2 + 1] ^= 0xa5;
        rc = run_codec(XROOTD_CODEC_XZ, XROOTD_CODEC_DIR_DECOMPRESS, comp, complen, NULL,
                       &plain, &plainlen, 1 << 16, 1 << 16);
        CHECK(rc == XROOTD_CODEC_ERR_DATA, "corrupt xz -> ERR_DATA rc=%d", rc);
        free(plain);
    }
    free(comp);
...
main(void)
{
    ...
    test_lookups();
    test_identity();
    test_codec(XROOTD_CODEC_GZIP,   "gzip");
    test_codec(XROOTD_CODEC_DEFLATE,"deflate");
    test_codec(XROOTD_CODEC_ZSTD,   "zstd");
    test_codec(XROOTD_CODEC_XZ,     "xz");
    test_codec(XROOTD_CODEC_BROTLI, "brotli");
    test_codec(XROOTD_CODEC_BZIP2,  "bzip2");
    test_codec(XROOTD_CODEC_LZ4,    "lz4");
    test_bomb_guard();
    test_corrupt();
