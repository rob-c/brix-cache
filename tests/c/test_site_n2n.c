...
int main(void) {
    ...
    /* 1. identity */
    memset(&c, 0, sizeof(c)); c.scheme = XROOTD_N2N_IDENTITY;
    assert(xrootd_n2n_lfn2pfn(&c, "/atlas/x", pfn, sizeof(pfn)) == 0);
    assert(strcmp(pfn, "/atlas/x") == 0);
    assert(xrootd_n2n_pfn2lfn(&c, "/atlas/x", lfn, sizeof(lfn)) == 0);
    assert(strcmp(lfn, "/atlas/x") == 0);

    /* 2. RAL: "<pool>:<lfn>" (no prefix) */
    memset(&c, 0, sizeof(c)); c.scheme = XROOTD_N2N_RAL;
    snprintf(c.pool, sizeof(c.pool), "atlas");
    assert(xrootd_n2n_lfn2pfn(&c, "/atlas/rucio/f1", pfn, sizeof(pfn)) == 0);
    assert(strcmp(pfn, "atlas:/atlas/rucio/f1") == 0);
    assert(xrootd_n2n_pfn2lfn(&c, pfn, lfn, sizeof(lfn)) == 0);
    ...
    memset(&c, 0, sizeof(c)); c.scheme = XROOTD_N2N_RAL;
    snprintf(c.pool, sizeof(c.pool), "atlas");
    assert(xrootd_n2n_lfn2pfn(&c, "/a/../../etc/passwd", pfn, sizeof(pfn)) != 0);
    assert(xrootd_n2n_lfn2pfn(&c, "../x", pfn, sizeof(pfn)) != 0);
    c.scheme = XROOTD_N2N_CEPHFS_PATH;
    assert(xrootd_n2n_lfn2pfn(&c, "/ok/../../bad", pfn, sizeof(pfn)) != 0);

    /* 7. truncation → -1 (no overflow) */
    memset(&c, 0, sizeof(c)); c.scheme = XROOTD_N2N_RAL;
    snprintf(c.pool, sizeof(c.pool), "atlas");
    assert(xrootd_n2n_lfn2pfn(&c, "/atlas/rucio/f1", pfn, 4) == -1);

    /* 8. pfn2lfn rejects a pfn not under the expected pool/prefix */
    memset(&c, 0, sizeof(c)); c.scheme = XROOTD_N2N_CEPHFS_PATH;
    snprintf(c.prefix, sizeof(c.prefix), "/mnt/cephfs/atlas");
    assert(xrootd_n2n_pfn2lfn(&c, "/elsewhere/f", lfn, sizeof(lfn)) != 0);
