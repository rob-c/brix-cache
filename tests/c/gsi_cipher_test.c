...
{
    EVP_PKEY *a = brix_gsi_cipher_keygen();
    EVP_PKEY *b = brix_gsi_cipher_keygen();
    size_t    la = 0, lb = 0;
    char     *pa, *pb;
    ...
    pa = brix_gsi_cipher_public(a, &la);
    pb = brix_gsi_cipher_public(b, &lb);
    ...
    a_peer = brix_gsi_cipher_parse_peer((uint8_t *) pa, la);
    b_peer = brix_gsi_cipher_parse_peer((uint8_t *) pb, lb);
    CHECK(a_peer && b_peer, "cipher_parse_peer");

    CHECK(brix_gsi_cipher_session_key(b, a_peer, 0, k1, 16), "session_key b·A");
    CHECK(brix_gsi_cipher_session_key(a, b_peer, 0, k2, 16), "session_key a·B");
...
test_encrypt_roundtrip(void)
{
    ...
    for (n = 0; n < sizeof(names) / sizeof(names[0]); n++) {
        brix_gsi_cipher_t c;
        uint8_t             key[BRIX_GSI_MAX_KEY];

        if (!brix_gsi_cipher_lookup(names[n], &c)) {
...
test_certreq(void)
{
    ...
    buf = brix_gsi_build_certreq("ssl", 10600, "638d0780.0", 0x80,
                                   rtag, sizeof(rtag), &len);
    CHECK(buf && len > 0, "build_certreq");
    /* The outer buffer must carry cryptomod + the main bucket. */
    CHECK(buf && brix_gsi_find_bucket(buf, len, kXRS_cryptomod, &rt, &rtlen) == 0
          && rtlen == 3 && memcmp(rt, "ssl", 3) == 0, "cryptomod bucket");
    CHECK(buf && brix_gsi_find_bucket(buf, len, kXRS_main, &main_buf, &main_len) == 0,
          "main bucket present");
    /* The nested main must carry the client rtag. */
    CHECK(main_buf && brix_gsi_find_bucket(main_buf, main_len, kXRS_rtag,
                                             &rt, &rtlen) == 0
...
test_parms(void)
{
    ...
    brix_gsi_parse_parms("v:10600,c:ssl,ca:638d0780.0|7a2dc0da.0",
...
test_rsa_sign_recover(void)
{
    ...
    slen = brix_gsi_rsa_encrypt_private(rsa, msg, sizeof(msg), sig, sizeof(sig));
    CHECK(slen > 0, "encrypt_private (chunked sign)");
    rlen = brix_gsi_rsa_decrypt_public(rsa, sig, slen, rec, sizeof(rec));
