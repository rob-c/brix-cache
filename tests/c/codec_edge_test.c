 ...
 * xrootd_codec_rc_t on error, -100 open failure, -200 budget exhausted).
 */
static int
run_codec(xrootd_codec_id_t id, xrootd_codec_dir_t dir,
          const uint8_t *in, size_t in_len, const xrootd_codec_guard_t *g,
          uint8_t **outp, size_t *outlenp, size_t in_chunk, size_t out_chunk)
{
    xrootd_codec_stream_t *s = xrootd_codec_open(id, dir, -1, g);
    uint8_t *out = NULL;
    ...
    while (!done) {
        ...
        for (;;) {
            ...
            rc = xrootd_codec_step(s, in, seg_end, &ip, tmp, out_chunk, &op, finish);
            ...
            if (rc == XROOTD_CODEC_END) { ret = 0; done = 1; break; }
...
typedef struct { xrootd_codec_id_t id; const char *name; } codec_row_t;
static const codec_row_t kCodecs[] = {
    { XROOTD_CODEC_GZIP,    "gzip"    },
    { XROOTD_CODEC_DEFLATE, "deflate" },
    { XROOTD_CODEC_ZSTD,    "zstd"    },
    { XROOTD_CODEC_BROTLI,  "brotli"  },
    { XROOTD_CODEC_XZ,      "xz"      },
    { XROOTD_CODEC_BZIP2,   "bzip2"   },
    { XROOTD_CODEC_LZ4,     "lz4"     },   /* phase-42 6th codec: in the edge matrix */
};
...
test_lookups(void)
{
    ...
    CHECK((d = xrootd_codec_by_id(XROOTD_CODEC_IDENTITY)) && d->id == XROOTD_CODEC_IDENTITY,
          "by_id identity");
    CHECK((d = xrootd_codec_by_id(XROOTD_CODEC_GZIP)) && d->id == XROOTD_CODEC_GZIP, "by_id gzip");
    CHECK((d = xrootd_codec_by_id(XROOTD_CODEC_BZIP2)) && d->id == XROOTD_CODEC_BZIP2, "by_id bzip2");

    /* canonical names are matched EXACTLY (config values); these are lowercase. */
    CHECK((d = xrootd_codec_by_name("gzip", 4)) && d->id == XROOTD_CODEC_GZIP, "name gzip");
    CHECK((d = xrootd_codec_by_name("deflate", 7)) && d->id == XROOTD_CODEC_DEFLATE, "name deflate");
    CHECK((d = xrootd_codec_by_name("zstd", 4)) && d->id == XROOTD_CODEC_ZSTD, "name zstd");
    CHECK((d = xrootd_codec_by_name("brotli", 6)) && d->id == XROOTD_CODEC_BROTLI, "name brotli");
    CHECK((d = xrootd_codec_by_name("xz", 2)) && d->id == XROOTD_CODEC_XZ, "name xz");
    CHECK((d = xrootd_codec_by_name("bzip2", 5)) && d->id == XROOTD_CODEC_BZIP2, "name bzip2");
    CHECK((d = xrootd_codec_by_name("identity", 8)) && d->id == XROOTD_CODEC_IDENTITY, "name identity");

    /* HTTP tokens are case-INSENSITIVE: mixed-case must still resolve. */
    CHECK((d = xrootd_codec_by_http_token("gzip", 4)) && d->id == XROOTD_CODEC_GZIP, "token gzip");
    CHECK((d = xrootd_codec_by_http_token("GZiP", 4)) && d->id == XROOTD_CODEC_GZIP, "token GZiP");
    CHECK((d = xrootd_codec_by_http_token("GZIP", 4)) && d->id == XROOTD_CODEC_GZIP, "token GZIP");
    CHECK((d = xrootd_codec_by_http_token("br", 2)) && d->id == XROOTD_CODEC_BROTLI, "token br");
    CHECK((d = xrootd_codec_by_http_token("Br", 2)) && d->id == XROOTD_CODEC_BROTLI, "token Br");
    CHECK((d = xrootd_codec_by_http_token("BR", 2)) && d->id == XROOTD_CODEC_BROTLI, "token BR");
    CHECK((d = xrootd_codec_by_http_token("xz", 2)) && d->id == XROOTD_CODEC_XZ, "token xz");
    CHECK((d = xrootd_codec_by_http_token("XZ", 2)) && d->id == XROOTD_CODEC_XZ, "token XZ");
    CHECK((d = xrootd_codec_by_http_token("bzip2", 5)) && d->id == XROOTD_CODEC_BZIP2, "token bzip2");
    CHECK((d = xrootd_codec_by_http_token("BZip2", 5)) && d->id == XROOTD_CODEC_BZIP2, "token BZip2");
    CHECK((d = xrootd_codec_by_http_token("zstd", 4)) && d->id == XROOTD_CODEC_ZSTD, "token zstd");
    CHECK((d = xrootd_codec_by_http_token("ZSTD", 4)) && d->id == XROOTD_CODEC_ZSTD, "token ZSTD");

    /* canonical-name lookup is case-SENSITIVE: uppercase must NOT match. */
    CHECK(xrootd_codec_by_name("GZIP", 4) == NULL, "name GZIP exact -> NULL");
    CHECK(xrootd_codec_by_name("Br", 2) == NULL, "name Br exact -> NULL");

    /* unknown tokens/names -> NULL. */
    CHECK(xrootd_codec_by_http_token("lzip", 4) == NULL, "token lzip -> NULL");
    CHECK(xrootd_codec_by_http_token("snappy", 6) == NULL, "token snappy -> NULL");
    CHECK(xrootd_codec_by_name("gz", 2) == NULL, "name gz partial -> NULL");
    CHECK(xrootd_codec_by_name("gzipx", 5) == NULL, "name gzipx (len mismatch) -> NULL");
    CHECK(xrootd_codec_by_http_token("g", 1) == NULL, "token g (prefix) -> NULL");

    /* degenerate args -> NULL (no crash). */
    CHECK(xrootd_codec_by_name(NULL, 0) == NULL, "name NULL -> NULL");
    CHECK(xrootd_codec_by_name("gzip", 0) == NULL, "name len0 -> NULL");
    CHECK(xrootd_codec_by_http_token(NULL, 4) == NULL, "token NULL -> NULL");
    CHECK(xrootd_codec_by_http_token("br", 0) == NULL, "token len0 -> NULL");
...
test_table_dense(void)
{
    ...
    for (i = 0; i < XROOTD_CODEC_MAX; i++) {
        const xrootd_codec_desc_t *d = xrootd_codec_by_id((xrootd_codec_id_t) i);
        CHECK(d != NULL, "table hole at id=%d (by_id returned NULL)", i);
        if (d != NULL) {
            CHECK(d->id == (xrootd_codec_id_t) i, "id mismatch slot=%d desc.id=%d", i, d->id);
            CHECK(d->name != NULL, "id=%d has NULL name", i);
            /* available() must agree with the descriptor's own flags. */
            int avail = xrootd_codec_available((xrootd_codec_id_t) i);
    ...
    CHECK(xrootd_codec_available(XROOTD_CODEC_IDENTITY), "identity must be available");

    /* out-of-range ids -> NULL / not available (no OOB read). */
    CHECK(xrootd_codec_by_id(XROOTD_CODEC_MAX) == NULL, "by_id(MAX) -> NULL");
    CHECK(xrootd_codec_by_id((xrootd_codec_id_t) -1) == NULL, "by_id(-1) -> NULL");
    CHECK(xrootd_codec_by_id((xrootd_codec_id_t) 9999) == NULL, "by_id(9999) -> NULL");
    CHECK(!xrootd_codec_available((xrootd_codec_id_t) -1), "available(-1) -> 0");
    CHECK(!xrootd_codec_available(XROOTD_CODEC_MAX), "available(MAX) -> 0");
    CHECK(!xrootd_codec_available((xrootd_codec_id_t) 9999), "available(9999) -> 0");
...
test_open_guards(void)
{
    ...
    CHECK(xrootd_codec_open((xrootd_codec_id_t) -1, XROOTD_CODEC_DIR_DECOMPRESS, -1, NULL) == NULL,
          "open(-1) -> NULL");
    CHECK(xrootd_codec_open(XROOTD_CODEC_MAX, XROOTD_CODEC_DIR_DECOMPRESS, -1, NULL) == NULL,
          "open(MAX) -> NULL");
    CHECK(xrootd_codec_open((xrootd_codec_id_t) 9999, XROOTD_CODEC_DIR_COMPRESS, -1, NULL) == NULL,
          "open(9999) -> NULL");
    ...
    size_t k;
    for (k = 0; k < NCODECS; k++) {
        if (!xrootd_codec_available(kCodecs[k].id)) {
            CHECK(xrootd_codec_open(kCodecs[k].id, XROOTD_CODEC_DIR_DECOMPRESS, -1, NULL) == NULL,
                  "%s unavailable -> open NULL", kCodecs[k].name);
        }
    }

    /* NULL-guard decode is fine: a tiny gzip round-trip with guard==NULL. */
    if (xrootd_codec_available(XROOTD_CODEC_GZIP)) {
        uint8_t  src[256], *comp = NULL, *plain = NULL;
        size_t   complen = 0, plainlen = 0;
        int      rc;
        fill_pattern(src, sizeof(src), 3);
        rc = run_codec(XROOTD_CODEC_GZIP, XROOTD_CODEC_DIR_COMPRESS, src, sizeof(src), NULL,
                       &comp, &complen, 64, 64);
        CHECK(rc == 0, "null-guard setup compress rc=%d", rc);
        if (rc == 0) {
            rc = run_codec(XROOTD_CODEC_GZIP, XROOTD_CODEC_DIR_DECOMPRESS, comp, complen,
...
/* xrootd_codec_close() does free(s) and does not null the caller's   */
...
{
    xrootd_codec_close(NULL);   /* documented no-op; must not crash */
...
test_empty_and_tiny(void)
{
    ...
    for (k = 0; k < NCODECS; k++) {
        xrootd_codec_id_t id = kCodecs[k].id;
        const char       *nm = kCodecs[k].name;
        uint8_t          *comp = NULL, *plain = NULL;
        size_t            complen = 0, plainlen = 0;
        int               rc;

        if (!xrootd_codec_available(id)) {
        ...
        rc = run_codec(id, XROOTD_CODEC_DIR_COMPRESS, (const uint8_t *) "", 0, NULL,
                       &comp, &complen, 1, 1);
        CHECK(rc == 0, "%s empty compress rc=%d", nm, rc);
        if (rc == 0) {
            rc = run_codec(id, XROOTD_CODEC_DIR_DECOMPRESS, comp, complen, NULL,
                           &plain, &plainlen, 1, 1);
         ...
         * loop step() draining a single byte at a time, byte-exact recovery. */
        {
            ...
            rc = run_codec(id, XROOTD_CODEC_DIR_COMPRESS, &one, 1, NULL,
                           &comp, &complen, 1, 1);
...
/* stream that expands BELOW XROOTD_CODEC_RATIO_FLOOR must NOT trip.    */
...
test_bomb_ratio(void)
{
     ...
     * total_out >= XROOTD_CODEC_RATIO_FLOOR. */
    {
        size_t   n = 32u * 1024u;                  /* below the 64 KiB floor */
        uint8_t *zeros = calloc(n, 1);
        uint8_t *comp = NULL, *plain = NULL;
        size_t   complen = 0, plainlen = 0;
        xrootd_codec_guard_t g;
        int      rc;

        rc = run_codec(XROOTD_CODEC_GZIP, XROOTD_CODEC_DIR_COMPRESS, zeros, n, NULL,
                       &comp, &complen, 1 << 12, 1 << 12);
        CHECK(rc == 0, "floor setup compress rc=%d", rc);
        if (rc == 0) {
            memset(&g, 0, sizeof(g));
            g.max_ratio = 2;          /* would trip if floor were ignored */
            rc = run_codec(XROOTD_CODEC_GZIP, XROOTD_CODEC_DIR_DECOMPRESS, comp, complen, &g,
     ...
     * output well past the floor MUST return ERR_BOMB. */
    {
        ...
        if (rc == 0) {
            ...
            CHECK(rc == XROOTD_CODEC_ERR_BOMB, "ratio guard must trip rc=%d (want %d)",
                  rc, XROOTD_CODEC_ERR_BOMB);
            free(plain);
        }
        free(comp); free(zeros);
    }
...
test_bomb_outcap(void)
{
    ...
    }
    {
        ...
        if (rc == 0) {
            ...
            rc = run_codec(XROOTD_CODEC_GZIP, XROOTD_CODEC_DIR_DECOMPRESS, comp, complen, &g,
                           &plain, &plainlen, 1 << 16, 1 << 16);
            CHECK(rc == XROOTD_CODEC_ERR_BOMB, "out_cap guard must trip rc=%d (want %d)",
                  rc, XROOTD_CODEC_ERR_BOMB);
            free(plain); plain = NULL;
...
test_truncated(void)
{
    ...
    for (k = 0; k < NCODECS; k++) {
        ...
        rc = run_codec(id, XROOTD_CODEC_DIR_COMPRESS, src, sizeof(src), NULL,
                       &comp, &complen, 1 << 16, 1 << 16);
        ...
        rc = run_codec(id, XROOTD_CODEC_DIR_DECOMPRESS, comp, truncated, NULL,
                       &plain, &plainlen, 1 << 16, 1 << 16);
...
test_trailing_bytes(void)
{
    ...
    for (k = 0; k < NCODECS; k++) {
        ...
        if (run_codec(id, XROOTD_CODEC_DIR_COMPRESS, src, sizeof(src), NULL,
...

        {
            xrootd_codec_stream_t *s =
                xrootd_codec_open(id, XROOTD_CODEC_DIR_DECOMPRESS, -1, NULL);
            uint8_t           out[40000];
            size_t            in_pos = 0, out_pos = 0;
            xrootd_codec_rc_t r = XROOTD_CODEC_OK;

            CHECK(s != NULL, "%s trailing open", nm);
            if (s != NULL) {
                /* feed the whole buffer once; loop step() until END (or error). */
                while (r == XROOTD_CODEC_OK && out_pos < sizeof(out)) {
                    r = xrootd_codec_step(s, buf, total, &in_pos,
                                          out, sizeof(out), &out_pos, 1);
                    if (r == XROOTD_CODEC_END) { break; }
                    if (r != XROOTD_CODEC_OK) { break; }
                    if (in_pos >= total) { break; }   /* ran out without END */
                }
                /* MUST finish the first frame cleanly... */
                CHECK(r == XROOTD_CODEC_END,
...
test_level_clamp(void)
{
    ...
    for (k = 0; k < NCODECS; k++) {
        ...
        if (!xrootd_codec_available(id) || d == NULL) {
        ...
        for (li = 0; li < 4; li++) {
            xrootd_codec_stream_t *s =
                xrootd_codec_open(id, XROOTD_CODEC_DIR_COMPRESS, levels[li], NULL);
            CHECK(s != NULL,
                  "%s open at out-of-range level %d must succeed (clamped)",
                  nm, levels[li]);
            if (s != NULL) { xrootd_codec_close(s); }
        }
         ...
         * compress manually at level_max+999, decode via the standard driver. */
        {
            ...
            s = xrootd_codec_open(id, XROOTD_CODEC_DIR_COMPRESS,
                                  d->level_max + 999, NULL);
            CHECK(s != NULL, "%s clamp round-trip open", nm);
            if (s != NULL) {
                while (r == XROOTD_CODEC_OK && out_pos < sizeof(comp)) {
                    r = xrootd_codec_step(s, src, sizeof(src), &in_pos,
                                          comp, sizeof(comp), &out_pos, 1);
                    if (r != XROOTD_CODEC_OK) { break; }
                }
                CHECK(r == XROOTD_CODEC_END, "%s clamp compress rc=%d", nm, (int) r);
                xrootd_codec_close(s);
                if (r == XROOTD_CODEC_END) {
                    rc = run_codec(id, XROOTD_CODEC_DIR_DECOMPRESS, comp, out_pos,
