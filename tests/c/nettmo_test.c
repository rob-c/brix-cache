 ...
 *
 * WHAT: Verifies xrdc_backoff_delay_ms is exponential, capped, and jittered
...
main(void)
{
    ...
    for (unsigned a = 0; a <= 9; a++) {
        ...
        unsigned d = xrdc_backoff_delay_ms(a, &seed);
        char msg[96];
    ...
    {
        unsigned d = xrdc_backoff_delay_ms(20, &seed);
        check(d >= 5000 && d <= 7500, "delay capped region for attempt 20");
    }

    fprintf(stderr, "[timeouts] setter overrides default\n");
    check(xrdc_tmo_connect_ms() == 15000, "default connect timeout 15000");
    xrdc_tmo_set_connect_ms(2500);
    check(xrdc_tmo_connect_ms() == 2500, "setter overrides connect timeout");
    xrdc_tmo_set_connect_ms(-1);   /* ignored */
    check(xrdc_tmo_connect_ms() == 2500, "non-positive setter ignored");
