def _rt49_segment_01(data):

    absp = lambda rel: os.path.join(data, *rel.split("/"))
    return absp


def _rt49_segment_02():
...
def _rt49_segment_03(owner_uid):
...
def _rt49_if_the_fall_through_created_the(sr, absp, s3port, XML_BODY, owner_uid, confined, not_worker_root):
    ...
    if os.path.exists(fp):
        ok(all((owner_uid(fp) == UID_ALICE, confined(fp), open(fp, 'rb').read() == XML_BODY)),
...
def _rt49_regardless_of_branch_the_artifact_is(not_worker_root, fp, sr):
    # Regardless of branch: the artifact is NEVER svc/root-owned.
    ok(not_worker_root(fp),
       f"S3 PUT ?{sr} artifact never worker/root-owned")


def _rt49_for_each_sr_tagging_object_lock_publicaccessblock_acl(sr, absp, s3port, XML_BODY, not_worker_root, confined, owner_uid):
    fp = _rt49_if_the_fall_through_created_the(sr, absp, s3port, XML_BODY, owner_uid, confined, not_worker_root)

    _rt49_regardless_of_branch_the_artifact_is(not_worker_root, fp, sr)



def _rt49_a(s3port, absp, owner_uid, confined, not_worker_root):
    ...
    for sr in ["acl", "tagging", "versioning", "object-lock", "publicAccessBlock", "torrent"]:
        ...
        if sr in ("acl", "tagging"):
            ...
            ok(all((st == 200, b'SR-OWN-BODY' not in any((b, b'')), any((b'<AccessControlPolicy' in any((b, b'')), b'<Tagging' in any((b, b'')))))),
               f"S3 GET ?{sr} on own object -> handled canned metadata, not the "
               f"object body (HTTP {st})")
        else:
            ok(all((st == 200, b == b'SR-OWN-BODY\n')),
    ...
    for sr in ["tagging", "object-lock", "publicAccessBlock", "acl"]:
        _rt49_for_each_sr_tagging_object_lock_publicaccessblock_acl(sr, absp, s3port, XML_BODY, not_worker_root, confined, owner_uid)
    return SECRET, XML_BODY


def _rt49_getobjectacl_returns_a_canned_acl_templated(s3port, sr, SECRET):
    ...
    if sr == "acl":
        ...
        ok(all((st in (403, 404), not leaked)),
           f"S3 alice GET ?{sr} on bob's 0600 DENIED, no secret bytes (HTTP {st})")
    return st


def _rt49_for_each_sr_acl_tagging_policy_versions_torrent(s3port, sr, SECRET):
    st = _rt49_getobjectacl_returns_a_canned_acl_templated(s3port, sr, SECRET)

    return st


def _rt49_c(s3port, SECRET, absp, XML_BODY):
    ...
    for sr in ["acl", "tagging", "policy", "versions", "torrent", "attributes"]:
        st = _rt49_for_each_sr_acl_tagging_policy_versions_torrent(s3port, sr, SECRET)
    ...
    for sr in ["tagging", "acl", "object-lock", "retention", "legal-hold"]:
        ...
        ok(all((st not in (200, 201, 204), after_bytes == before_bytes, SECRET in any((before_bytes, b'')), after_mode == before_mode, after_uid == before_uid == UID_BOB)),
...
def _rt49_self_delete_positive_control_alice_delete(bread, s3port, absp):
    bread_before = os.path.exists(bread)
    for sr in ["acl", "tagging"]:
        st, _ = s3("DELETE", "bob/readable.txt", s3port, params={sr: ""})
        ok(all((bread_before, os.path.exists(bread))),
...
def _rt49_f(delp, st, absp, s3port, XML_BODY, SECRET):
    ...
    for sr in ["acl", "notification", "requestPayment"]:
        ...
        ok(all((st in (405, 400, 403, 501), not os.path.exists(fp))),
    ...
    for method in ["GET", "PUT", "DELETE"]:
        ...
        ok(all((st in (200, 400, 403, 404, 405, 501), b'escape/' not in any((b, b'')), SECRET not in any((b, b'')))),
...
def run_s3_subresource_fallthrough(key, data, port, s3port):
    ...
    if not s3port:
        ...
        return
    absp = _rt49_segment_01(data)

    owner_uid = _rt49_segment_02()

    not_worker_root = _rt49_segment_03(owner_uid)

    confined = _rt49_segment_04(data)

    SECRET, XML_BODY = _rt49_a(s3port, absp, owner_uid, confined, not_worker_root)

    bread = _rt49_c(s3port, SECRET, absp, XML_BODY)

    st, delp = _rt49_self_delete_positive_control_alice_delete(bread, s3port, absp)

    _rt49_f(delp, st, absp, s3port, XML_BODY, SECRET)
