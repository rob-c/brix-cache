/*
 * creds_guard_test.c — exhaustive unit test of xrootd_imp_creds_privileged()
 ...
 * broker calls this with floor = XROOTD_IMP_HARD_MIN_ID before any credential
...
mk(uid_t uid, gid_t gid, int ngroups, const gid_t *groups)
{
    ...
    for (i = 0; groups != NULL && i < ngroups && i < XROOTD_IDMAP_MAXGROUPS; i++) {
        c.groups[i] = groups[i];
    }
    return c;
}
...
main(void)
{
    const uid_t FLOOR = XROOTD_IMP_HARD_MIN_ID;   /* 1000 */
    uint32_t    id;
    char        kind;
    int         r;

    /* The hard floor must be exactly 1000 as documented/requested. */
    CK(XROOTD_IMP_HARD_MIN_ID == 1000, "hard floor == 1000");

    /* ---- clean creds pass ---- */
    {
        gid_t g[] = { 1001, 1500 };
        xrootd_idmap_creds_t c = mk(1001, 1001, 2, g);
        r = xrootd_imp_creds_privileged(&c, FLOOR, &id, &kind);
        CK(r == 0, "clean uid/gid/supp >= floor -> allowed");
    }
    /* uid exactly at the floor is allowed (floor is inclusive lower bound). */
    {
        gid_t g[] = { 1000 };
        xrootd_idmap_creds_t c = mk(1000, 1000, 1, g);
        CK(xrootd_imp_creds_privileged(&c, FLOOR, NULL, NULL) == 0,
    ...
    {
        gid_t g[] = { 1001 };
        xrootd_idmap_creds_t c = mk(0, 1001, 1, g);
        r = xrootd_imp_creds_privileged(&c, FLOOR, &id, &kind);
        CK(r == 1 && kind == 'u' && id == 0, "uid 0 -> privileged (u,0)");
    }
    {
        gid_t g[] = { 1001 };
        xrootd_idmap_creds_t c = mk(999, 1001, 1, g);
        r = xrootd_imp_creds_privileged(&c, FLOOR, &id, &kind);
        CK(r == 1 && kind == 'u' && id == 999, "uid 999 -> privileged (u,999)");
    }
    {
        gid_t g[] = { 1001 };
        xrootd_idmap_creds_t c = mk(1, 1001, 1, g);
        CK(xrootd_imp_creds_privileged(&c, FLOOR, NULL, NULL) == 1,
           "uid 1 -> privileged");
    }
    ...
    {
        gid_t g[] = { 0 };
        xrootd_idmap_creds_t c = mk(1001, 0, 1, g);
        r = xrootd_imp_creds_privileged(&c, FLOOR, &id, &kind);
        CK(r == 1 && kind == 'g' && id == 0, "gid 0 -> privileged (g,0)");
    }
    {
        gid_t g[] = { 50 };
        xrootd_idmap_creds_t c = mk(1001, 50, 1, g);
        r = xrootd_imp_creds_privileged(&c, FLOOR, &id, &kind);
    ...
    {
        gid_t g[] = { 1001, 1500, 0 };
        xrootd_idmap_creds_t c = mk(1001, 1001, 3, g);
        r = xrootd_imp_creds_privileged(&c, FLOOR, &id, &kind);
    ...
    /* ---- ngroups bounds + NULL ---- */
    {
        xrootd_idmap_creds_t c = mk(1001, 1001, 0, NULL);
        CK(xrootd_imp_creds_privileged(&c, FLOOR, NULL, NULL) == 0,
           "ngroups 0 with safe uid/gid -> allowed");
    }
    {
        xrootd_idmap_creds_t c = mk(1001, 1001, -1, NULL);
        r = xrootd_imp_creds_privileged(&c, FLOOR, &id, &kind);
        CK(r == 1 && kind == 'n', "ngroups -1 -> privileged (n)");
    }
    {
        xrootd_idmap_creds_t c = mk(1001, 1001, XROOTD_IDMAP_MAXGROUPS + 1, NULL);
        r = xrootd_imp_creds_privileged(&c, FLOOR, &id, &kind);
        CK(r == 1 && kind == 'n', "ngroups > MAXGROUPS -> privileged (n)");
    }
    CK(xrootd_imp_creds_privileged(NULL, FLOOR, &id, &kind) == 1
           && kind == 'n', "NULL creds -> privileged (n)");

    /* ---- a higher configured floor refuses ids that are fine at 1000 ---- */
    {
        gid_t g[] = { 1500 };
        xrootd_idmap_creds_t c = mk(1500, 1500, 1, g);
        CK(xrootd_imp_creds_privileged(&c, FLOOR, NULL, NULL) == 0,
           "uid/gid 1500 allowed at floor 1000");
        CK(xrootd_imp_creds_privileged(&c, 2000, NULL, NULL) == 1,
    ...
    /* ---- exhaustive sweep: every id 0..1000 below floor must be refused ---- */
    {
        ...
        for (u = 0; u < FLOOR; u++) {
            gid_t g1[] = { 1001 };
            xrootd_idmap_creds_t cu = mk(u, 1001, 1, g1);   /* sub-floor uid */
            gid_t g2[] = { (gid_t) u };
            xrootd_idmap_creds_t cg = mk(1001, (gid_t) u, 1, g2); /* sub-floor gid */
            gid_t g3[] = { 1001, (gid_t) u };
            xrootd_idmap_creds_t cs = mk(1001, 1001, 2, g3); /* sub-floor supp */
            if (xrootd_imp_creds_privileged(&cu, FLOOR, NULL, NULL) != 1) bad++;
            if (xrootd_imp_creds_privileged(&cg, FLOOR, NULL, NULL) != 1) bad++;
            if (xrootd_imp_creds_privileged(&cs, FLOOR, NULL, NULL) != 1) bad++;
        }
