...
def _guard_test_user_backend_cred_scenario_1():
...
def _guard_test_user_backend_cred_scenario_2(nginx):
    if not nginx.exists():
        pytest.skip(f"nginx binary not found: {nginx}")

def _guard_test_user_backend_cred_scenario_3(scenario):
...
def test_user_backend_cred_scenario(scenario: str):
    _guard_test_user_backend_cred_scenario_1()
    nginx = Path(os.environ.get("NGINX_BIN", "/tmp/nginx-1.28.3/objs/nginx"))
    _guard_test_user_backend_cred_scenario_2(nginx)
    _guard_test_user_backend_cred_scenario_3(scenario)
    rc = user_backend_cred.SCENARIOS[scenario](nginx)
    _guard_test_user_backend_cred_scenario_4(rc)
    _check_test_user_backend_cred_scenario_1(rc)
