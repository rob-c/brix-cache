...
stream { server { listen 127.0.0.1:${OPORT}; xrootd on; xrootd_root $PFX/o/root; xrootd_auth none; xrootd_allow_write on; } }
E2
...
http { client_body_temp_path $PFX/b/tmp; server { listen 127.0.0.1:${BPORT};
  location / { dav_methods PUT DELETE;
    xrootd_webdav on; xrootd_webdav_root $PFX/b/export; xrootd_webdav_auth none; xrootd_webdav_allow_write on;
    xrootd_webdav_storage_backend root://127.0.0.1:${OPORT};
    xrootd_webdav_stage on; xrootd_webdav_stage_store posix:$PFX/b/stage; xrootd_webdav_stage_flush async; } } }
E2
