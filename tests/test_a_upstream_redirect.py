...
class TestUpstreamRedirect:
    ...
    def test_locate_wait_then_redirect(self, upstream_wait_nginx):
        ...
        class TwoRequestMock:
            ...
            def _serve(self):
                try:
                    ...
                    # Second request normally arrives after nginx waits 1 s.
                    # Under full-suite load, the retry can race with this
                    # Python mock; still send the final redirect instead of
                    # closing the upstream connection and turning the test into
                    # a mock scheduling failure.
                    conn.settimeout(3)
                    try:
                        hdr = _recv_exact(conn, 24)
                        req_sid = hdr[:2]
        ...
        mock = TwoRequestMock(UPSTREAM_WAIT_BACKEND_PORT)
