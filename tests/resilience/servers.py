...
def ensure_pki():
    ...
    if os.path.isfile(CA_CERT) and os.path.isfile(SERVER_CERT) and os.path.isfile(USER_PROXY):
        return
    env = dict(os.environ)
...
class FaultProxy:
    ...
    of severing the stream — application-visible wire loss)."""
    ...
    def set_loss(self, pct):
        return self.ctl(f"lossy {int(pct)}") if pct > 0 else self.ctl("clear")
