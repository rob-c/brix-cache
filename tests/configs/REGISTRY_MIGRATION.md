...
Because every test now declares its servers, the fleet can boot only the
**declared union** — the backbone plus the closure of every spec the collected
tests declare — instead of all ~120 instances. This is opt-in via
`REGISTRY_SUBSET_BOOT=1` (`_specs_to_boot`); a single-file run then starts only
what that file needs. The default still boots the full fleet, unchanged.
