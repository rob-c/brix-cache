...
def _check_test_cache_http_source_flow_1(results):
    assert all(ok for ok, _ in results), "\n".join(
        f"{'ok' if ok else 'FAIL'} {message}" for ok, message in results
    )

def _check_test_cache_http_source_flow_2(messages):
...
def test_cache_http_source_flow(tmp_path):
    ...
    _check_test_cache_http_source_flow_1(results)
    messages = [message for _, message in results]
    def _assert_test_cache_http_source_flow_1():
        assert os.access(XRDFS, os.X_OK)
        assert "byte-exact serve (filled from HTTP)" in messages

    _assert_test_cache_http_source_flow_1()
    def _assert_test_cache_http_source_flow_2():
        assert "object landed in the local cache (fill stored)" in messages
        assert "warm hit byte-exact" in messages

    _assert_test_cache_http_source_flow_2()
    _check_test_cache_http_source_flow_2(messages)
