...
stream {{ server {{ listen {BIND_HOST}:{port}; brix_root on; brix_auth none; brix_storage_backend posix:{root}; brix_allow_write on; }} }}
""",
        encoding="utf-8",
    )
    return conf
...
def run_checks(base: Path, nginx_bin: str = NGINX_BIN, xrdfs: Path = XRDFS) -> list[tuple[bool, str]]:
    ...
    origin_port, cache_port = cmdscript_ports("cache_backend_source")
    origin = base / "o"
    node = base / "b"
    ...
    try:
        ...
        results.append(
            (
                exact_transfer(xrdfs_cat, cache_port, "/small.bin", cold_got,
...
def entry(argv: list[str]) -> int:
    ...
    return print_results(results, "run_cache_backend_source")
