...
def set_fault(run: LiveRun, mode: str, count: int) -> None:
    run.call(
        ["curl", "-sS", "-o", "/dev/null", "-X", "POST", "-d", json.dumps({"mode": mode, "count": count}), f"http://127.0.0.1:{MOCK_PORT}/ctl/fault"]
    )
...
def run_port(nginx: Path | None = None) -> int:
    ...
    with LiveRun("cvmfs_verify", nginx) as run:
        ...
        check(run.curl_status(f"http://127.0.0.1:{CACHE_PORT}{obj}") == 502, "corrupt fill -> 502, not admitted")
        set_fault(run, "none", 0)
        check(any(quarantine.iterdir()), "corrupt part quarantined")
        metrics = run.call(["curl", "-sS", f"http://127.0.0.1:{CACHE_PORT}/metrics"]).stdout
        ...
        poison_one = run.curl_bytes(f"http://127.0.0.1:{CACHE_PORT}{obj}")
        poison_two = run.curl_bytes(f"http://127.0.0.1:{CACHE_PORT}{obj}")
        ...
        set_fault(run, "corrupt", 8)
        check(run.curl_status(f"http://127.0.0.1:{CACHE_PORT}{obj}") == 502, "default rejects corrupt fill")
        set_fault(run, "none", 0)
