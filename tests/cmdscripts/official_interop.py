...
The suites exercise a RUNNING nginx+xrootd anonymous listener (NGINX_ANON_PORT,
default 11094 — the managed fleet's anon port) with the INSTALLED stock XRootD
client tools. Execution is opt-in via the phase-81 live knob; import stays cheap.

What they no longer need is the XRootD project's source tree.  ``host()`` used
to copy ``tests/XRootD/host.cfg`` out of a ``$BRIX_SRC`` checkout, so the suite
silently SKIPPED wherever that checkout was absent — which is everywhere the
repo is built from itself.  This is a clean-room implementation and the suite
must run from this repository alone, so the payload is generated here: what the
round-trip proves is that the bytes survive upload, download and cat unchanged,
and that holds for any payload.  The stock CLIENTS are still the oracle; only
the dependency on upstream SOURCES is gone.
"""
...
#: The `host` suite's round-trip payload.  Shaped like the config file the
#: original shell test copied (comments, key=value lines, a blank line and a
#: trailing newline) so the transfer still carries text with the byte patterns
#: — CR-free newlines, '#', '=' — that a mangling transfer would disturb.
_HOST_PAYLOAD = b"""# host.cfg - round-trip payload for the host interop suite.
# Generated by the suite; no upstream source tree is consulted.
all.export /
xrootd.async off
xrd.port any
...
# --------------------------------------------------------------------------- #
# host: simple copy + diff (adapted from the stock host.sh interop scenario)
# --------------------------------------------------------------------------- #
def host() -> int:
    missing = _missing_tools()
    if missing:
    ...
    with LiveRun("official_host") as run:
        suite = _Suite(run)
        local = run.mkdir(tag)
        # Written into the run's own tree, not read out of an upstream checkout:
        # the assertion is byte-equality after the round trip, so the suite owns
        # its payload and can no longer skip itself for a missing source tree.
        srcfile = local / "host.cfg.src"
        srcfile.write_bytes(_HOST_PAYLOAD)
