...
def _guard_test_cache_stage_throttle_flow_2(results):
    if results and results[0][1].startswith("SKIP "):
        pytest.skip(results[0][1])

def _check_test_cache_stage_throttle_flow_1(results):
    assert all(ok for ok, _ in results), "\n".join(
        f"{'ok' if ok else 'FAIL'} {message}" for ok, message in results
    )

def _guard_test_cache_stage_throttle_flow_1(tool):
...
def test_cache_stage_throttle_flow(tmp_path):
    for tool in (NGINX_BIN, str(XRDCP), str(XRDFS)):
        _guard_test_cache_stage_throttle_flow_1(tool)

    results = run_checks(tmp_path, nginx_bin=NGINX_BIN)
    _guard_test_cache_stage_throttle_flow_2(results)

    _check_test_cache_stage_throttle_flow_1(results)
    messages = [message for _, message in results]
    def _assert_test_cache_stage_throttle_flow_1():
        assert "reject: root:// write failed (staging full)" in messages
        assert "reject: no file created (shed before any write)" in messages

    _assert_test_cache_stage_throttle_flow_1()
    def _assert_test_cache_stage_throttle_flow_2():
        assert "reject: READ still works (reads never throttled)" in messages
        assert "reject: throttled_total{reject} > 0" in messages

    _assert_test_cache_stage_throttle_flow_2()
    def _assert_test_cache_stage_throttle_flow_3():
    ...
    _assert_test_cache_stage_throttle_flow_3()
