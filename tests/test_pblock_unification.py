"""test_pblock_unification.py — phase-101 W4: brix_webdav_pblock_block_size →
brix_pblock_block_size (pblock stripe size). The backing field was already in the
shared preamble, so only the registration moved to the common module.

  * success     — bare brix_pblock_block_size parses at a webdav location.
  * hard-rename — brix_webdav_pblock_block_size is now "unknown directive".
"""
import os
import subprocess
import tempfile
...
def test_bare_pblock_block_size_parses():
    rc, out = _nginx_t(
...
def test_old_webdav_pblock_is_unknown():
    rc, out = _nginx_t(
