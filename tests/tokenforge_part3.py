"""Conformance artifacts and manifest rows for the WLCG token forge.
...
own entry in :data:`sys.modules` to the manifest module, so ``tokenforge_part3`` and
:mod:`brix_suite.security.tokens.manifest` are ONE module object rather than two copies.

Part 3 reached ``TokenForge`` and ``write_jwks`` through the globals it was
``exec``-ed into.  Both are real imports in the moved module.
...
import brix_suite.security.tokens.manifest as _canonical
