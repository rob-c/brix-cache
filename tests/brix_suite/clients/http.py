"""HTTP driver for the phase-65 guard suites (was ``tests/guard_http_lib.py``).

One deviation from the grown body, and it is the whole reason this file is not
a copy: the settings import names ``brix_suite.settings`` instead of the flat
``settings``.  The flat spelling resolves only with ``tests/`` on ``sys.path``;
inside the package that is nobody's guarantee, and the two spellings are ONE
module object anyway (the §10.2 shim), so no value changes.
"""
...
from brix_suite.settings import HOST, GUARD_STUB_PORT
