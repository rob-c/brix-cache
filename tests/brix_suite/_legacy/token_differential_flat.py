"""ARCHIVE — the flat ``tests/token_differential.py`` as it stood before TS-5.

Kept verbatim so the move can be checked body-by-body (AST hash) rather than
taken on trust.  Not imported by anything: the ``from tokenforge import ...``
and ``from settings import ...`` lines resolve only with ``tests/`` on
``sys.path`` (which the deleted self-locate used to arrange), and ``FINDINGS``
walks one ``..`` from the *flat* location, so from ``_legacy/`` it would name
``tests/brix_suite/docs/``.  The live module is
``brix_suite.security.tokens_vectors``.
"""
