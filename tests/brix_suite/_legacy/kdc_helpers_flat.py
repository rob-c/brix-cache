"""ARCHIVE — the flat ``tests/kdc_helpers.py`` as it stood before TS-5.

Kept verbatim so the move can be checked body-by-body (AST hash) rather than
taken on trust.  Not imported by anything: the self-locate below inserts this
file's own directory, which from ``_legacy/`` is the wrong tree.  The live
module is ``brix_suite.security.kdc``.
"""
