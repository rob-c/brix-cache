"""ARCHIVE — the flat ``tests/_xrdcl_worker.py`` as it stood before TS-5.

Kept verbatim so the move can be checked body-by-body (AST hash) rather than
taken on trust.  Nothing imports it; the live module is ``brix_suite.clients.xrdcl.worker``.

Started as a *script* by absolute path, never imported: hence the
``__main__`` guard at its foot, which guard #11 now pins to the live head.
"""
