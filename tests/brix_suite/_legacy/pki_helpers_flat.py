"""ARCHIVE — the flat ``tests/pki_helpers.py`` as it stood before TS-5.

Kept verbatim so the move can be checked body-by-body (AST hash) rather than
taken on trust.  Not imported by anything: the ``from settings import ...``
line resolves only with ``tests/`` on ``sys.path``, and ``ROOT_DIR`` here
computes ``parents[1]`` from the *flat* location, so from ``_legacy/`` it would
point one directory too shallow.  The live module is
``brix_suite.security.pki``.
"""
