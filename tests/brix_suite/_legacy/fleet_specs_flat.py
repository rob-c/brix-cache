# ARCHIVE — the pre-TS-4 flat body of ``tests/fleet_specs.py``, kept byte-identical
# so the "verbatim move" claim in the TS-4 decision note is checkable on disk
# rather than only in git history.  Nothing imports this; the live catalogue is
# ``brix_suite.catalogue``.  ``test_ci_ts4_catalogue_merge.py`` diffs every
# moved function against this text.
...
# which is exactly the mechanism TS-4 item 7 dissolved.  The shard it
# pulled in is archived beside this file as ``fleet_specs_part2_flat.py``.
