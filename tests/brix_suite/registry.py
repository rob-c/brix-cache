"""Registry model for pytest-owned test servers (TS-4).

The verbatim grown body of ``tests/server_registry.py``, which is now a
§10.2 self-replacement shim onto this module — so ``import
server_registry`` and ``import brix_suite.registry`` name ONE module
object and the ``_SPECS`` / ``_COMMAND_SPECS`` / ``_REGISTRATION_SITES``
singletons stay single, as the 354 dependent modules require.

Three deviations from the byte-identical body, each forced by the move:
the ``settings`` import takes its canonical spelling; ``InstanceSpec`` is
the class name with ``NginxInstanceSpec`` preserved as an alias (the spec
has covered xrootd, haproxy, proc and external kinds for a long time);
and ``_caller_site`` identifies this module by path instead of by the
old basename.
"""
...
#: Historical name, kept because 879 call sites spell it.  A plain alias,
#: not a subclass: ``isinstance`` and equality stay exact, and the two
#: names cannot drift into two dataclasses with two ``__hash__``.
NginxInstanceSpec = InstanceSpec
...
def register_nginx(spec: InstanceSpec) -> InstanceSpec:
...
def register_xrootd(spec: InstanceSpec) -> InstanceSpec:
...
def replace_spec(spec: InstanceSpec) -> InstanceSpec:
...
def registered_specs() -> list[InstanceSpec]:
    return [_SPECS[name] for name in dependency_order()]


def declared_ports(spec: InstanceSpec) -> set[int]:
...
def port_conflicts(
    specs: list[InstanceSpec] | tuple[InstanceSpec, ...] | None = None
) -> dict[int, list[str]]:
...
def _caller_site() -> str:
    # Raw f_back walk, NOT inspect.stack(): stack() eagerly builds source
    # context for EVERY frame, which inside pytest's deep hook stack costs
    # ~80ms per call — ~10s per session across the 126 fleet registrations.
    # Only the filename:lineno of the first frame outside this module matters.
    #
    # Compared against this module's own path rather than a hard-coded
    # basename: the basename form named the pre-TS-4 file, so after the
    # move every registration would have reported this file's own line
    # instead of the caller's — a wrong answer, not an error.
    frame = inspect.currentframe()
    while frame is not None:
        filename = frame.f_code.co_filename
        if os.path.abspath(filename) != _MODULE_FILE:
