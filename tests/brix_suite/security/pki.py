...
Moved verbatim from ``tests/pki_helpers.py`` at TS-5 (archive:
``brix_suite/_legacy/pki_helpers_flat.py``).  Two deliberate deviations, both
forced by the new location and both pinned by ``test_ci_ts5_pki_move.py``:

* ``ROOT_DIR`` walks three parents instead of one.  It is the repo root, and
  the module is now three directories deeper; left alone it would have made
  ``MAKE_PROXY`` point at ``tests/utils/make_proxy.py``, which does not exist.
  Nothing would have raised at import — ``blitz_test_pki`` would simply have
  stopped minting proxies at its last line, after the certificates it *did*
  write made the tree look complete.
* the settings import names ``brix_suite.settings`` rather than the flat
  ``settings``.  The §10.2 shim makes those one module object, so this is a
  spelling change and not a behaviour change; it keeps the package importable
  without ``tests/`` on ``sys.path``.

Six call sites reach this module by *string* — ``python -c "from pki_helpers
import blitz_test_pki; blitz_test_pki()"`` in ``lib_py/pki``,
``resilience/servers`` and four ``cmdscripts`` — which no import rewrite can
see.  The flat spelling therefore stays a live shim, not a deprecation.
"""
...
from brix_suite.settings import CA_CERT, CA_DIR, CA_KEY, PKI_DIR, SERVER_CERT, SERVER_KEY, USER_CERT, USER_KEY

#: repo root: tests/brix_suite/security/pki.py -> security -> brix_suite
#: -> tests -> repo.  Was ``parents[1]`` when this file sat in tests/.
