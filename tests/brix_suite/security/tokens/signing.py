...
Moved verbatim out of `tests/_tokenforge_part2_mixinb.py` by TS-5.  The move
fixes a live `NameError`: `header_jwk_injection` calls `_rsa_jwk`, which the
flat slice never defined — it resolved only in `tokenforge.py`'s globals, and
the slice was a real import, so the method raised for every caller.  Two
security tests exercise it (`test_wlcg_token_conformance_headers.py` HDR-11
and `test_wlcg_token_conformance_parity.py`), asserting the server refuses a
token whose key is embedded in the `jwk` header (RFC 7515 §4.1.3, rules
29/150).
"""
import base64
import datetime
...
from brix_suite.security.tokens.jose import _b64url, _rsa_jwk, _seg


class _TokenForgeSigning:
    """Real signature algorithms, raw signing, and `crit`/`typ` header cases."""
