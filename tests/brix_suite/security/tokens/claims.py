"""Claim type/value conformance cases for the token forge.

Moved verbatim out of `tests/_tokenforge_part2_mixinc.py` and
`tests/_tokenforge_part2_mixind.py` by TS-5.  Mixin D held one method,
`aud_any`, and existed only because the slices were cut by line count; it
joins `aud_wildcard` here.  No method name was defined by more than one slice,
so the four-way MRO carried no resolution order and merging changes nothing.
"""
import base64
import json
import time
...
class _TokenForgeClaims:
    """Claim types, NumericDate edges, base64 variants, and scope grammar."""
