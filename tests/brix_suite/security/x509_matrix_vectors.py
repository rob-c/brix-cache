...
Moved verbatim from ``tests/x509_matrix_differential.py`` at TS-5 (archive:
``brix_suite/_legacy/x509_matrix_differential_flat.py``).  It stays a module of
its own rather than merging into :mod:`brix_suite.security.x509_vectors` as the
plan's mapping table suggested: the two drivers share a shape and nothing else.
This one replays the *clause matrix* against a single ``ConformanceFleet`` and
one stock server; the other replays *named hostile scenarios*, standing up a
fresh ``WlcgInstance`` and a fresh stock server per credential.  Merging them
would have produced one module with two disjoint halves and two FINDINGS paths.

Deviations from the archived body, all pinned by
``test_ci_ts5_differentials_move.py``:

* ``FINDINGS`` walks ``parents[3]`` instead of ``parents[1]`` — see the sibling
  module; the failure mode is the same silent one, a findings table written
  under ``tests/brix_suite/docs/`` and never seen again.
* a ``sys.path`` entry for ``tests/``: ``ephemeral_port``, ``clauses`` and
  ``wlcg_conformance_fleet`` have not moved yet.
* ``x509forge`` is imported canonically and bound to the old spelling, keeping
  the ``x509forge.build_all`` call in :func:`run` byte-identical.
* ``import sys`` moves to the top, out of the retired ``__main__`` guard.
"""
...
def main(argv=None):
    ...
    Behaviourally identical: one optional positional argument (the corpus root,
    defaulting to ``/tmp/x509matrixdiff``) and ``run``'s return value straight
    through as the exit code.
    """
    argv = list(sys.argv[1:] if argv is None else argv)
    out = Path(argv[0] if argv else "/tmp/x509matrixdiff")
    return run(out)
