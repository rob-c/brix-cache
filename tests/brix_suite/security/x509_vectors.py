"""x509_vectors — replay forge scenarios against our module and stock XRootD.
...
Moved verbatim from ``tests/x509_differential.py`` at TS-5 (archive:
``brix_suite/_legacy/x509_differential_flat.py``).  Deliberate deviations, all
forced by the new location and all pinned by
``test_ci_ts5_differentials_move.py``:

* ``FINDINGS`` walks ``parents[3]`` instead of ``parents[1]``.  It names a file
  in the *repo's* ``docs/10-reference/``, and the module is now three
  directories deeper; left alone ``_write_findings`` would have created
  ``tests/brix_suite/docs/10-reference/`` and written there without raising,
  so the published findings table would silently have stopped being updated.
* a ``sys.path`` entry for ``tests/`` is added: ``ephemeral_port`` and
  ``wlcg_fleet`` have not moved yet and are unimportable from here without it.
* ``x509forge`` is imported under its canonical name and bound to the old
  spelling, so every ``x509forge.*`` call in the bodies below stays byte-identical.
* ``import sys`` moves to the top: the flat file imported it inside its
  ``__main__`` guard, which is now :func:`main`.

That guard carried real logic — argv parsing and the process exit code — and
``cmdscripts/x509_differential.py`` starts this driver by *path*, behind a
pytest wrapper that SKIPs unless ``TEST_X509_DIFF=1``.  A stranded entry point
would therefore have exited 0 without running a single scenario, and been
reported as a pass.
"""
...
FINDINGS = (Path(__file__).resolve().parents[3]
...
def main(argv=None):
    ...
    Behaviourally identical: one optional positional argument (the work root,
    defaulting to ``/tmp/x509diff``) and ``run``'s return value straight
    through as the exit code.
    """
    argv = list(sys.argv[1:] if argv is None else argv)
    out = Path(argv[0] if argv else "/tmp/x509diff")
    return run(out)
