"""brix_suite.security.kdc — a self-contained MIT Kerberos KDC for the krb5 tier.
...
Mirrors ``brix_suite.security.pki``: a side-effecting "blitz" provisioner (wipe
+ rebuild) that communicates state purely through files under TEST_ROOT, with a
``_run`` wrapper that raises ``RuntimeError`` (stdout+stderr embedded) on a
non-zero exit.

Moved verbatim from ``tests/kdc_helpers.py`` at TS-5 (archive:
``brix_suite/_legacy/kdc_helpers_flat.py``).  Two deviations, both forced by the
new location and both pinned by ``test_ci_ts5_kdc_move.py``:

* the self-locate walks two parents.  It exists because the spec catalogue
  starts this module as a *script by absolute path*
  (``[py, tests/kdc_helpers.py, "up"]``), where ``sys.path[0]`` is the script's
  own directory and nothing else is on the path.  Left at ``.parent`` it would
  put ``brix_suite/security`` there, and the import below would fail at KDC
  start — after the shell had already gated the tier as available.
* the settings import names ``brix_suite.settings``; the §10.2 shim makes those
  one module object, so this is a spelling change only.
"""
...
# Self-locate so the import below works regardless of the caller's cwd: the
# catalogue invokes us by absolute path, not via PYTHONPATH=tests.  Two parents
# up from brix_suite/security/ is tests/, which is where ``brix_suite`` lives.
sys.path.insert(0, str(Path(__file__).resolve().parents[2]))

from brix_suite.settings import (  # noqa: E402  (path insert precedes import)
    HOST,
    KRB5_CCACHE,
    KRB5_CLIENT_KEYTAB,
