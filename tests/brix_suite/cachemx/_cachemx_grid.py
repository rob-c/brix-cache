...
# Sibling in this package.  The flat spelling still resolves (it is a §10.2
# shim that self-replaces with this very module object), but naming the
# package directly keeps the cluster's internal edges independent of
# whether ``tests/`` happens to be on sys.path for a given entry point.
from brix_suite.cachemx import _cachemx as cx
