"""Fleet start-up: dependency levels, per-kind spawn, quiescence.

Moved verbatim out of ``tests/_server_launcher_part2_mixina.py`` by TS-4 item 4.  Bodies are
...
from brix_suite.settings import (
    BRIX_BIN,
    PKI_DIR,
    REGISTRY_STRICT_TEMPLATES,
    TESTS_DIR,
)
from config_templates import render_config_to_path

class _LauncherStart:
    def __init__(self, tests_dir: str | None = None):
        # ``TESTS_DIR``, not ``dirname(__file__)``: this class used to live in
        # the flat tree where those were the same string, and after the move
        # the default would have silently become ``brix_suite/launcher`` — a
        # directory that exists, so nothing would have raised.  No caller
        # reads ``self.tests_dir`` today, which is why the move hazard is
        # latent here rather than a live bug; it is fixed anyway, because the
        # next caller to read it would have had no way to notice.
