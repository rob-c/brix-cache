...
from brix_suite.settings import NGINX_BIN, HOST, BIND_HOST, TESTS_DIR
from brix_suite.mesh.mesh_config import render
...
# Anchored on the suite root the settings module *searches* for, not on this
# file's parents: two parents from ``tests/`` was the repo, two parents from
# ``tests/brix_suite/mesh/`` is ``tests/`` — a directory that exists, so the
# hop would have kept resolving and simply named the wrong tree.  The SSS
# admin below would then be looked for at ``tests/client/`` and quietly not
# found, and every sss topology would skip instead of fail.
