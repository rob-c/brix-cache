"""Wildcard-audience conformance case for the token forge.
...
own entry in :data:`sys.modules` to the package facade, so ``_tokenforge_part2_mixind`` and
...
This slice held exactly one method, ``aud_any``; it was merged into
:mod:`brix_suite.security.tokens.claims` alongside ``aud_wildcard``.
