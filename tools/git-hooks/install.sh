...
echo "  pre-push now runs the tools/ci guards (seconds; see tools/ci/guard_set.py --explain)"
echo "  then 'python3 -m cmdscripts.operator_runtime suite --fast' (~4min). Bypass: git push --no-verify"
