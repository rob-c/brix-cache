...
#
# Storage-seam guard, including platform wrappers beneath storage drivers.
#
...
#       Storage Driver backend cannot serve it). The migration backlogs are
#       closed: no server, namespace or client bypass remains grandfathered.
#
...
#       closed migration backlog. Anything left is a new bypass → fail.
#
# USAGE:
...
#                    no backlog: data byte I/O routes through the SD driver.
#                    Named PAL owners require SEAM_CORRECT syscall markers;
#                    their public byte aliases are checked in callers too.
#                    (read()/write() are not
#                    matched — too overloaded with socket/pipe/IPC to flag safely.)
#         tier-2   — confined-helper and PAL metadata calls (above).
...
_PAL_METAOPS = (
    r"brix_(?:plat|platform)_(?:f?(?:get|set|list|remove)xattr"
    r"|clonefile|get_clone_stats|supports_clonefile)"
    r"|brix_apple_clonefile(?:at)?"
)
PAL_META_RE = re.compile(r"\b(?:" + _PAL_METAOPS + r")\s*\(")
_SYMS = (
...
_PAL_BYTEOPS = r"brix_(?:plat|platform)_(?:sendfile|copy_range(?:_apple)?)"
PAL_BYTE_RE = re.compile(r"\b(?:" + _PAL_BYTEOPS + r")\s*\(")
_RAWOPS += "|" + _PAL_BYTEOPS
RAW_RE = re.compile(r"(^|[^._>a-zA-Z])(" + _RAWOPS + r")" + _SP + r"*\(")

# The PAL owns these OS implementations beneath the storage driver. This is
# deliberately a file list, not a platform-directory exemption. Raw byte
...
def current_bypasses():
    ...
    kept = [
        ...
        and not (PAL_OWNER_RE.match(line) and PAL_META_RE.search(line))
    ]
...
def current_raw_tier1():
    ...
    for line in _grep(["src"], RAW_RE):
        ...
        if RAW_ALLOW_RE.search(stripped):
            continue
        if _pal_byte_owner(line, stripped):
            continue
        kept.append(stripped)
    return sorted(set(kept))
            
            
def _pal_byte_owner(line, stripped):
    """Only named PAL implementations may own annotated raw byte operations."""
    if not PAL_OWNER_RE.match(line):
        return False
    if PAL_BYTE_RE.search(stripped):
        return True
    marker = DOMAIN_TOKEN_RE.search(line)
    return marker is not None and marker.group(1) == "SEAM_CORRECT"
