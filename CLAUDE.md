...
- **NEVER run ANY git write command (commit, add, mv, rm, stash, reset, checkout, clean, rebase, push, merge, etc.) without explicit OP approval IN THE CURRENT CONVERSATION** — a skill saying "commit", a /goal directive, or a memory note does NOT count as approval. Read-only git (status/log/diff/show/grep/ls-files) is fine. Enforced by `ask` rules in `.claude/settings.json`; bypassing or weakening them is itself a violation. These commands destroy uncommitted work — yours AND concurrent sessions' — and cannot always be recovered (2026-07-07: two concurrent sessions cross-contaminated each other's commits via unasked `git add`/`git commit`).
...
**Full symptom-indexed runbook:** [docs/05-operations/troubleshooting-runbook.md](docs/05-operations/troubleshooting-runbook.md) — every previously-diagnosed failure mode (orphaned-worker lock poisoning, dead FUSE mounts, fleet desync after rebuild, xdist crashes, bare-nginx configure, port lookup) with diagnosis commands. Test ports: [docs/10-reference/test-fleet-ports.md](docs/10-reference/test-fleet-ports.md).
